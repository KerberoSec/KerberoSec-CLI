...
import { WorktreeDefaults } from "@shared/proto/kerberosec/worktree"
...
export async function getWorktreeDefaults(_controller: Controller, _request: EmptyRequest): Promise<WorktreeDefaults> {
	...
	const suggestedBranch = `worktree/kerberosec-${suffix}`

	// Generate suggested path in Documents/KerberoSec/Worktrees/<project-name>-<suffix>
	...
	const suggestedPath = path.join(documentsPath, "KerberoSec", "Worktrees", `${projectName}-${suffix}`)
