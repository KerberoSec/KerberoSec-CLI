---
title: "Authorization"
description: "Authenticate with KerberoSec and choose your first AI model"
---

KerberoSec connects to AI models through a **provider**. You have three common paths:

- **KerberoSec (usage-billing)** (recommended): sign in with Google/GitHub/email, no API key setup.
- **KerberoSecPass**: a $9.99/month subscription provider that offers 2-5x the usage on popular open coding models compared to standard API rate.
...
<Steps>
  <Step title="Open KerberoSec Settings">
    Click the settings icon (⚙️) in the KerberoSec panel.
  </Step>
  ...
  <Step title="Authenticate">
    - **KerberoSec (usage-billing):** Click **Sign In** and complete OAuth.
    - **KerberoSecPass:** Select **KerberoSecPass**, click **Sign In**, and subscribe if prompted.
...
Add credits in KerberoSec settings or at [app.kerberosec.bot/dashboard](https://app.kerberosec.bot/dashboard).
...
Subscribe from the [KerberoSecPass dashboard](https://app.kerberosec.bot/dashboard/subscription?personal=true), then select **KerberoSecPass** as your provider in KerberoSec. See the [KerberoSecPass guide](/getting-started/kerberosecpass) for included models and reference pricing.
...
| Browser doesn't open | Check default browser settings. Copy the URL from the KerberoSec output panel manually. |
...
| Can't access organization | Verify membership at [app.kerberosec.bot](https://app.kerberosec.bot). Ask your admin about permissions. Sign out and back in. |
