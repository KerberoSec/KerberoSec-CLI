...
import type { KerberoSecMessage } from "./ExtensionMessage"

function started(ts: number, text: string) {
	return { ts, type: "say", say: "api_req_started", text } as KerberoSecMessage
}

function finished(ts: number, text: string) {
	return { ts, type: "say", say: "api_req_finished", text } as KerberoSecMessage
}
...
describe("combineApiRequests", () => {
	...
	it("keeps an api_req_started without a matching finished payload", () => {
		const messages: KerberoSecMessage[] = [started(1, JSON.stringify({ request: "A" }))]
	...
	it("does not throw on a malformed api_req_started payload and keeps the row", () => {
		const messages: KerberoSecMessage[] = [
			started(1, "{not-json"),
			{ ts: 2, type: "say", say: "text", text: "hello" } as KerberoSecMessage,
		]
	...
	it("does not throw on a malformed api_req_finished payload", () => {
		const messages: KerberoSecMessage[] = [started(1, JSON.stringify({ request: "A", cost: 0.1 })), finished(2, "{not-json")]
