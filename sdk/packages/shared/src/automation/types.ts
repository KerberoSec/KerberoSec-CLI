 ...
 *
 * - `one_off`: a single run materialized from a `.kerberosec/cron/*.md` spec.
 * - `schedule`: recurring runs materialized from a `.kerberosec/cron/*.cron.md` spec.
 * - `event`: reactive runs materialized from a `.kerberosec/cron/events/*.event.md`
...
export interface AutomationSpecSource {
	/**
	 * POSIX-normalized path of the source file relative to `.kerberosec/cron/`.
