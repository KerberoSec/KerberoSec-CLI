...
): Pick<KerberoSecCoreStartInput["config"], "reasoningEffort" | "thinking"> {
...
export function resolveLaunchContext(
	...
	const providerId =
		...
		process.env.KERBEROSEC_PROVIDER?.trim() ??
		"";
	const modelId =
		override?.model ??
		override?.modelId ??
		...
		process.env.KERBEROSEC_MODEL?.trim() ??
		"";
	const root =
	...
	if (!providerId || !modelId) {
		throw new Error(
			"No provider/model available. Start a session in another KerberoSec client first, or set KERBEROSEC_PROVIDER and KERBEROSEC_MODEL.",
		);
	}
	return {
		workspaceRoot: root,
...
function buildSessionStartInput(
	...
	return {
		...
		config: {
			...
			teamName: options?.teamName ?? "kerberosec-hub",
...
async function loadHistoryFor(
	...
	if (!ctx.kerberosec) return [];
	try {
		return (await ctx.kerberosec.readMessages(sessionId)) as unknown[];
	} catch (error) {
...
export async function createSession(
	...
	const result = await ctx.kerberosec.start(
...
export async function deleteSession(
	...
	if (!ctx.kerberosec) throw new Error("Hub is not connected.");
	const deleted = await ctx.kerberosec.delete(sessionId);
	if (!deleted) {
		ctx.send(peer, {
			type: "status",
...
export async function abortPeerTurn(
	...
	if (!ctx.kerberosec || !peer.selectedSessionId) return;
	rejectPendingApprovalsForSession(
		ctx,
		peer.selectedSessionId,
		"Turn aborted before approval was resolved.",
	);
	await ctx.kerberosec.abort(peer.selectedSessionId);
...
export async function forkPeerSession(
	...
	try {
		const rawMessages = (await ctx.kerberosec.readMessages(
			forkedFromSessionId,
		...
		const sourceSession = await ctx.kerberosec.get(forkedFromSessionId);
		if (!sourceSession) {
			ctx.send(peer, {
				type: "fork_error",
		...
		peer.selectedSessionId = result.sessionId;
		const newSession = await ctx.kerberosec.get(result.sessionId);
...
export async function restorePeerSession(
	...
	const sourceSession = await ctx.kerberosec.get(sourceSessionId);
	if (!sourceSession) {
		ctx.send(peer, {
			type: "error",
	...
	const result = await ctx.kerberosec.restore({
	...
	peer.selectedSessionId = result.sessionId;
	const restoredSession = await ctx.kerberosec.get(result.sessionId);
...
export async function initializePeer(
	...
	await syncHubClientsAndSessions();
	ctx.send(peer, { type: "status", text: "KerberoSec Hub is ready." });
