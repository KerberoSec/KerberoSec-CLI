...
description: "Configure KerberoSec to work behind firewalls and proxies"
---

If you're working behind a corporate proxy or firewall, you'll need to configure
proxy settings for KerberoSec to connect to AI providers. The configuration varies
depending on which version of KerberoSec you're using.
...
is needed for KerberoSec itself.

## CLI

The KerberoSec CLI uses standard HTTP proxy environment variables. Configure these before running `kerberosec` commands.
...
KerberoSec CLI only supports HTTP proxies. It does not support SOCKS proxies,
...
After configuring the proxy, test that KerberoSec can connect to your AI provider:

1. Open the KerberoSec panel
...
KerberoSec in JetBrains only supports HTTP proxies. It does not support SOCKS
proxies, proxy autoconfiguration (PAC) scripts, or HTTP proxies which require
authentication beyond a basic username and password.

KerberoSec does not pick up changed proxy settings dynamically. After changing proxy
settings, restart the IDE for KerberoSec to use the new settings.
...
3. Use curl to verify the certificate works, for example, `curl -x proxy.corp.example:8080 --cacert /path/to/ca-cert.pem -o - -vv https://api.kerberosec.bot/`
4. Consider disabling `http.proxyStrictSSL` in VSCode (not recommended for production)

### Testing Proxy Configuration

If you encounter problems with KerberoSec networking, first verify your proxy
...
Next, check ~/.kerberosec/kerberosec-core-service.log (CLI, JetBrains) for log messages
