...
//
// These helpers spawn kerberosec as a child process and return stdout, stderr,
// and exit code - without going through the TUI harness. Use these for:
//   - Exit code assertions
//   - Pure stdout/stderr contract tests
//   - Piped stdin tests (echo "..." | kerberosec -y ...)
//   - JSON output validation
//   - Timeout behavior
// ---------------------------------------------------------------------------

import { type SpawnSyncOptions, spawnSync } from "node:child_process";
import { KERBEROSEC_BIN } from "./constants.js";
import { kerberosecEnv } from "./env.js";
...
/**
 * Run `kerberosec [args]` synchronously and return stdout/stderr/exitCode.
 *
 * Suitable for deterministic, fast-exiting commands like:
 *   kerberosec --help, kerberosec --version, kerberosec auth -p ... -k ..., etc.
 */
export function runKerberoSec(
	args: string[],
	opts: RunOptions = {},
): RunResult {
	const {
	...
	const spawnOpts: SpawnSyncOptions = {
		...
		timeout,
		cwd,
		env: { ...kerberosecEnv(config), ...extraEnv },
		input: stdin,
	};

	const result = spawnSync(KERBEROSEC_BIN, args, spawnOpts);
...
/**
 * Assert that a runKerberoSec result exited with the expected code.
