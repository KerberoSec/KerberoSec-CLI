...
import { resolveTeamDataDir } from "@kerberosec/shared/storage";
...
describe("resolveTeamDataDir", () => {
	...
	it("uses KERBEROSEC_TEAM_DATA_DIR when set", () => {
		snapshot = captureEnv();
		process.env.KERBEROSEC_TEAM_DATA_DIR = "/tmp/team-dir";
		process.env.KERBEROSEC_DATA_DIR = "/tmp/kerberosec-data";
		expect(resolveTeamDataDir()).toBe("/tmp/team-dir");
	});

	it("falls back to KERBEROSEC_DATA_DIR/teams", () => {
		snapshot = captureEnv();
...
describe("createAgentTeamsTools runtime behavior", () => {
	...
	it("injects workspace metadata into kerberosec teammate system prompt", async () => {
		const spawnTeammate = vi.fn();
		const runtime = {
			getMemberRole: vi.fn(() => "lead"),
			isTeammateActive: vi.fn(() => false),
