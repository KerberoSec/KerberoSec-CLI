...
A KerberoSec plugin is a TypeScript module that extends any agent built on the KerberoSec SDK. The same plugin runs in the KerberoSec CLI, VS Code and JetBrains extensions, and any custom app built on `@kerberosec/core`.
...
2. Plugin package -- a directory with `package.json`, npm dependencies, and optionally bundled assets. Installable via `kerberosec plugin install`.
...
Plugins can declare normalized event types and emit them into KerberoSec automation. Hosts that don't have automation enabled simply ignore both -- feature-detect `ctx.automation`.
...
- `<workspace>/.kerberosec/plugins/` -- project-scoped plugins.
...
Save as `my-plugin.ts`, drop in `.kerberosec/plugins/`:
...
- `kerberosec.plugins` -- the discovery contract. Array of entries, each with `paths` (entry files) and `capabilities` (pre-declared, validated before importing).
- `peerDependencies` for `@kerberosec/core` -- the host already provides it. Marking it optional lets you typecheck in isolation.
...
Add a `runDemo()` in your plugin file (see the single-file template above) that boots a real `KerberoSecCore` session:
...
- `ctx.workspaceInfo` is undefined in SDK tests -- the host didn't pass `extensionContext.workspace`. In SDK code, set it explicitly (see the KerberoSecCore loading example above).
...
- `../kerberoseccore/REFERENCE.md` - Using plugins with KerberoSecCore
