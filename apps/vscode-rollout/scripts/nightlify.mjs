 ...
 *   - textual rewrites: "claude-dev" -> "kerberosec-nightly" everywhere, and every
 *     `"kerberosec.` ID prefix -> `"kerberosec-nightly.` (commands, settings, view IDs,
 *     when-clauses that START with the key — mid-string references like
 *     `config.kerberosec.x` are NOT rewritten, same as the standalone nightly)
...
export const NIGHTLY_NAME = "kerberosec-nightly";
export const NIGHTLY_DISPLAY_NAME = "KerberoSec (Nightly)";
...
export function nightlifyPackageJson(rawContent, version) {
	...
	const content = rawContent
		...
		.replaceAll('"kerberosec.', `"${NIGHTLY_NAME}.`);
