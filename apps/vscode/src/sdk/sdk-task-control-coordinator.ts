...
export class SdkTaskControlCoordinator {
	...
	async cancelKerberoSecTaskOnSignOut(isKerberoSecManagedProvider: boolean): Promise<void> {
		const activeSession = this.options.sessions.getActiveSession()
		if (!isKerberoSecManagedProvider || !activeSession?.isRunning) {
			return
		}
	...
	async showTaskWithId(taskId: string): Promise<HistoryItem | undefined> {
		...
		try {
			...
			const rawMessages = await this.options.taskHistory.getKerberoSecMessages(taskId)
			if (isSuperseded()) {
				return historyItem
			}
			...
			// The final state update below includes the loaded kerberosecMessages. Avoid pushing
	...
	private appendFreshResumeMessage(messages: KerberoSecMessage[], sessionStatus?: string): KerberoSecMessage[] {
	...
	private appendLegacyTaskWarningAndResumeMessage(messages: KerberoSecMessage[]): KerberoSecMessage[] {
		const cleanedMessages = messages.filter((m) => m.ask !== "resume_task" && m.ask !== "resume_completed_task")
		const now = Date.now()
		cleanedMessages.push(
			{
