import { syncStoredProviderRegistration } from "@kerberosec/core"
...
describe("createProviderConfigStore", () => {
	...
	// bypassing the @kerberosec/core mock above): the store's converters must pass
	...
	it("round-trips every SDK model capability and a full override set under the real stored-entry schema", async () => {
		const { ModelCapabilitySchema } = await import("@kerberosec/shared")
		// vi.importActual bypasses the @kerberosec/core mock above and resolves via
		// the vitest alias to the stub, which re-exports the real schema.
		const { StoredModelEntrySchema } = (await vi.importActual("@kerberosec/core")) as {
