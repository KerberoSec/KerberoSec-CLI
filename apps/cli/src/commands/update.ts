...
import { resolveKerberoSecBuildEnv } from "@kerberosec/shared";
...
const DEFAULT_PACKAGE_NAME = "kerberosec";
...
export function getInstallationInfo(currentVersion: string): InstallationInfo {
	...
	try {
		const scriptPath = realpathSync(
			process.env.KERBEROSEC_WRAPPER_PATH || process.argv[1] || "",
		).replace(/\\/g, "/");
 ...
 * kerberosec processes are running swaps the binary under them — their respawn
 ...
 *
 * Skipped for npx, dev, unknown installs. Disable with KERBEROSEC_NO_AUTO_UPDATE=1.
 */
export function autoUpdateOnStartup(): void {
	if (process.env.IS_DEV === "true") return;
	if (process.env.KERBEROSEC_NO_AUTO_UPDATE === "1") return;
...
async function otherCliClientsAttached(): Promise<boolean> {
	...
	const client = new NodeHubClient({
		...
		displayName: "kerberosec update check",
	});
	try {
...
export async function checkForUpdates(
	...
	writeln(
		`${c.cyan}Checking for updates${includeKanban ? " to KerberoSec CLI and kanban" : ""}…${c.reset}`,
	);
	...
	try {
		...
		if (cliUpdateAvailable && latestVersion) {
			if (!updateCommand) {
				writeln(
					`${c.dim}Unable to determine KerberoSec update command. Please update manually with your package manager.${c.reset}`,
				);
				hadFailure = true;
			} else {
				...
				try {
					...
					if (exitCode === 0) {
						...
						writeln(
							`${c.dim}The update takes effect the next time kerberosec starts.${c.reset}`,
						);
					} else {
						writeErr(
							`KerberoSec update failed (exit code ${exitCode}). Try running: ${manualUpdateCommand.command}`,
						);
						hadFailure = true;
					}
				} catch (error) {
					...
					writeErr(
						`Failed to run KerberoSec update command ${manualUpdateCommand.command}: ${message}`,
					);
					hadFailure = true;
				}
			}
