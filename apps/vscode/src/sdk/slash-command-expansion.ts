 ...
 * in @kerberosec/core's user-instruction-config-loader). The SDK strips the
...
/**
 * Verbatim port of @kerberosec/shared's private `sanitizeSegment`
...
/** Matches files materialized from remote config (`.kerberosec/remote-config/…`). */
const REMOTE_CONFIG_PATH_REGEX = /[/\\]\.kerberosec[/\\]remote-config[/\\]/
 ...
 * extension, but the webview autocomplete (and legacy KerberoSec versions) surface
