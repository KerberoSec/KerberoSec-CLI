...
function createCliEnv(
	...
	return {
		...
		// Without this, the KerberoSecPass promo dialog renders over the chat view.
...
describe("cli tuistory e2e", () => {
	...
	it("launches config view directly with `kerberosec config`", async () => {
	...
	it("dismisses the KerberoSecPass promo with any key and marks it as shown", async () => {
		// Re-enable the promo dialog that the shared env suppresses.
		const env = createCliEnv({
			KERBEROSEC_DISABLE_KERBEROSEC_PASS_NOTICE: undefined,
		});
		const dataDir = env.KERBEROSEC_DATA_DIR as string;
		const session = await launchCli([], env);

		await session.waitForText("Try KerberoSecPass", {
			timeout: LAUNCH_TIMEOUT_MS,
		});
		...
		await session.text({
			waitFor: (text) => !text.includes("Try KerberoSecPass"),
			timeout: UI_TIMEOUT_MS,
		});

		const screen = await session.text();
		expect(screen).toContain("What can I do for you?");
		expect(screen).not.toContain("Open KerberoSecPass");
		...
		expect(readFileSync(markerPath, "utf8")).toContain(
			'"kerberosec-cli-kerberosec-pass-intro": true',
		);
	});
});
