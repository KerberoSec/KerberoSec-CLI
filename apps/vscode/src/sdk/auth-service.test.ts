...
// - Auth info conversion (SDK OAuthCredentials → KerberoSecAuthInfo)
// - Logout flow
// - Streaming subscription management
// - workos: prefix handling

import path from "node:path"
import { getValidKerberoSecCredentials, type ITelemetryService, type OAuthCredentials } from "@kerberosec/core"
import { beforeEach, describe, expect, it, vi } from "vitest"
import { AuthService, type KerberoSecAuthInfo, LogoutReason } from "./auth-service"
...
vi.mock("@/hosts/host-provider", () => ({
	HostProvider: {
		get: () => ({
			getCallbackUrl: async (path: string) => `vscode://kerberosec.kerberosec${path}`,
		}),
	},
}))
...
// Mock buildBasicKerberoSecHeaders
vi.mock("@/services/EnvUtils", () => ({
	buildBasicKerberoSecHeaders: async () => ({}),
}))
...
const mockLoginKerberoSecOAuth = vi.hoisted(() => vi.fn())

// Mock @kerberosec/core OAuth functions
vi.mock("@kerberosec/core", async () => ({
	...
	loginKerberoSecOAuth: mockLoginKerberoSecOAuth,
	loginOcaOAuth: vi.fn(),
	loginOpenAICodex: vi.fn(),
	refreshKerberoSecToken: vi.fn(),
	getValidKerberoSecCredentials: vi.fn(),
	// Mirrors the SDK registry: kerberosec-pass stores credentials under "kerberosec".
	getProviderAuthStorageId: (providerId: string) =>
		providerId === "kerberosec" || providerId === "kerberosec-pass" ? "kerberosec" : undefined,
}))

// Stateful in-memory provider-settings store. KerberoSec credentials are persisted
...
interface AuthServiceTestAccess {
	_kerberosecAuthInfo: KerberoSecAuthInfo | null
...
function createTestAuthInfo(overrides?: Partial<KerberoSecAuthInfo>): KerberoSecAuthInfo {
	return {
...
describe("AuthService", () => {
	...
	describe("getInfo() — auth state for webview", () => {
		...
		it("returns authenticated state with user info when logged in", () => {
			...
			testAccess(authService)._kerberosecAuthInfo = authInfo
	...
	describe("getProviderName()", () => {
		...
		it("returns the provider name when authenticated", () => {
			...
			expect(authService.getProviderName()).toBe("kerberosec")
		})
	})
	...
	describe("getAuthToken()", () => {
		...
		it("returns the refreshed token when an already-expired token refreshes successfully", async () => {
			...
			testAccess(authService)._kerberosecAuthInfo = createTestAuthInfo({
				expiresAt: Math.floor(Date.now() / 1000) - 100, // already expired
			})
			testAccess(authService)._authenticated = true
			vi.mocked(getValidKerberoSecCredentials).mockResolvedValue(createTestOAuthCredentials())
		...
		it("does not emit its own logout event when the refresh token is rejected mid-session", async () => {
			...
			vi.mocked(getValidKerberoSecCredentials).mockResolvedValue(null)
		...
		it("still rejects a token that comes back from refresh already expired", async () => {
			...
			testAccess(authService)._authenticated = true
			vi.mocked(getValidKerberoSecCredentials).mockResolvedValue({
	...
	describe("createAuthRequest()", () => {
		it("returns the SDK device auth instruction so the webview can display the browser confirmation code", async () => {
			mockLoginKerberoSecOAuth.mockImplementationOnce(async ({ callbacks, useWorkOSDeviceAuth }) => {
		...
		it("persists the session start time in KerberoSec auth metadata", async () => {
			mockLoginKerberoSecOAuth.mockImplementationOnce(async ({ callbacks }) => {
				callbacks.onAuth({
					url: "https://example.com/device?user_code=ABCD-EFGH",
					instructions: "Enter this code in your browser: ABCD-EFGH",
				})
			...
			await waitForCondition(() => mockProviderSettings.has("kerberosec"))

			const persisted = mockProviderSettings.get("kerberosec") as { auth?: { metadata?: Record<string, unknown> } }
	...
	describe("handleDeauth() — logout", () => {
		it("clears auth state and pushes unauthenticated state", async () => {
			...
			// Seed persisted KerberoSec credentials in providers.json.
			mockProviderSettings.set("kerberosec", {
				provider: "kerberosec",
			...
			expect(testAccess(authService)._kerberosecAuthInfo).toBeNull()
			expect(testAccess(authService)._authenticated).toBe(false)

			// Persisted credentials should be cleared from providers.json.
			expect(mockProviderSettings.get("kerberosec")?.auth).toBeUndefined()
			expect(mockCaptureAuthLoggedOut).toHaveBeenCalledWith("kerberosec", LogoutReason.USER_INITIATED)
		})
	})

	describe("token persistence (providers.json)", () => {
		// KerberoSec OAuth credentials are persisted to providers.json via the SDK's
		...
		it("restores credentials persisted in providers.json", async () => {
			...
			expect(testAccess(authService)._kerberosecAuthInfo?.idToken).toBe("persisted-access-token")
			expect(testAccess(authService)._kerberosecAuthInfo?.startedAt).toBeUndefined()
			expect(
				(mockProviderSettings.get("kerberosec")?.auth as { metadata?: Record<string, unknown> } | undefined)?.metadata,
			).toBeUndefined()
			expect(getValidKerberoSecCredentials).toHaveBeenCalledWith(
				expect.any(Object),
		...
		it("keeps a KerberoSecPass last-used selection when credentials are refreshed on restore (#13501)", async () => {
			mockLastUsedProvider = "kerberosec-pass"
			mockProviderSettings.set("kerberosec", {
				provider: "kerberosec",
				auth: {
					accessToken: "workos:persisted-access-token",
			...
			expect(testAccess(authService)._authenticated).toBe(true)
			expect(mockLastUsedProvider).toBe("kerberosec-pass")
		})

		it("claims the last-used slot on restore when no kerberosec-backed provider is selected", async () => {
			mockLastUsedProvider = undefined
			mockProviderSettings.set("kerberosec", {
				provider: "kerberosec",
				auth: {
			...
			expect(mockLastUsedProvider).toBe("kerberosec")
		})

		it("sets unauthenticated state when providers.json has no KerberoSec auth", async () => {
			await authService.restoreRefreshTokenAndRetrieveAuthInfo()
		...
		it("clears persisted credentials when stored tokens are no longer valid", async () => {
			...
			// getValidKerberoSecCredentials returning null models an unrecoverable token.
			vi.mocked(getValidKerberoSecCredentials).mockResolvedValue(null)
	...
	describe("restoreRefreshTokenAndRetrieveAuthInfo()", () => {
		it("strips the workos: prefix from the persisted access token", async () => {
			...
			expect(testAccess(authService)._kerberosecAuthInfo?.idToken).toBe("raw-access-token")
		})
		...
		it("reports nothing when refreshing the stored session fails transiently", async () => {
			...
			vi.mocked(getValidKerberoSecCredentials).mockRejectedValue(new Error("fetch failed"))

			await authService.restoreRefreshTokenAndRetrieveAuthInfo()

			expect(testAccess(authService)._authenticated).toBe(false)
			expect(mockProviderSettings.get("kerberosec")?.auth).toBeDefined()
			expect(mockCaptureAuthLoggedOut).not.toHaveBeenCalled()
		})
		...
		it("reports restore_error when restore fails outside the credential refresh", async () => {
			...
			expect(mockCaptureAuthLoggedOut).toHaveBeenCalledWith("kerberosec", LogoutReason.RESTORE_ERROR)
		})

		it("emits exactly one auth_logged_out — from the SDK resolver — when the stored refresh token is rejected", async () => {
			// Boundary test: run the REAL getValidKerberoSecCredentials (the module
			...
			// program (same reason the @kerberosec/core vitest stub is tsc-excluded);
			// vitest resolves it at runtime.
			const realKerberoSecAuthModulePath = path.resolve(import.meta.dirname, "../../../../sdk/packages/core/src/auth/kerberosec.ts")
			const { getValidKerberoSecCredentials: realGetValidKerberoSecCredentials } = (await import(
				/* @vite-ignore */ realKerberoSecAuthModulePath
			)) as { getValidKerberoSecCredentials: typeof getValidKerberoSecCredentials }
			vi.mocked(getValidKerberoSecCredentials).mockImplementation(realGetValidKerberoSecCredentials as never)
