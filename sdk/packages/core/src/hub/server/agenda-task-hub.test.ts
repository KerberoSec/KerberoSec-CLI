...
describe("Hub agenda task vertical slice", () => {
	it("creates an approved user task, starts it, and completes it", async () => {
		const root = mkdtempSync(join(tmpdir(), "kerberosec-hub-agenda-"));
		...
		// Agent-created schedules anchor in the ~/.kerberosec/schedules home; point
		// the KerberoSec dir at the test root so that home stays inside this test.
		...
		const kerberosecHome = join(root, "kerberosec-home");
		mkdirSync(kerberosecHome);
		const previousKerberoSecDir = process.env.KERBEROSEC_DIR;
		process.env.KERBEROSEC_DIR = kerberosecHome;
		onTestFinished(() => {
	...
	it("keeps agenda spec watchers off while the todo tool is disabled", async () => {
		const root = mkdtempSync(join(tmpdir(), "kerberosec-hub-agenda-watch-"));
