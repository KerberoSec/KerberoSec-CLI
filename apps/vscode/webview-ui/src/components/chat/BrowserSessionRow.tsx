...
import { BrowserAction, BrowserActionResult, KerberoSecMessage, KerberoSecSayBrowserAction } from "@shared/ExtensionMessage"
	...
	const pages = useMemo(() => {
		const result: {
			currentState: {
				...
				consoleLogs?: string
				messages: KerberoSecMessage[] // messages up to and including the result
			}
			nextAction?: {
				messages: KerberoSecMessage[] // messages leading to next result
			}
		}[] = []

		let currentStateMessages: KerberoSecMessage[] = []
		let nextActionMessages: KerberoSecMessage[] = []
		...
		<BrowserSessionRowContainer style={{ marginBottom: -10 }}>
			<div style={browserSessionRowContainerInnerStyle}>
				...
				<span style={approveTextStyle}>
					{isAutoApproved ? "KerberoSec is using the browser:" : "KerberoSec wants to use the browser:"}
				</span>
			</div>
			<div
				style={{
