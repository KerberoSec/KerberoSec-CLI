...
describe("Selection Auto-Scroll Acceleration", () => {
	it("should define a calibrated selection auto-scroll multiplier", () => {
		expect(SELECTION_AUTO_SCROLL_MULTIPLIER).toBe(1.875);
	});
	...
	it("configureFastAutoScroll should scale instance autoScroll speeds to calibrated levels", () => {
		...
		expect(sb.autoScrollSpeedSlow).toBe(11.25);
		expect(sb.autoScrollSpeedMedium).toBe(67.5);
		expect(sb.autoScrollSpeedFast).toBeGreaterThanOrEqual(135);
	});

	it("getAutoScrollSpeed should scale by at least 135 when dragging below container", () => {
		applyAutoScrollSpeedPatch();
		...
		// Calibrated auto-scroll should be >= 135
		expect(speed).toBeGreaterThanOrEqual(135);
	});
});
