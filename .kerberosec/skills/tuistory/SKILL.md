...
## Driving the KerberoSec TUI headlessly

Launch the TUI in an isolated environment so you don't touch real user config (`~/.kerberosec`):
...
bunx tuistory read -s kerberosec --all

# Tear down a session YOU started (double Ctrl+C exits the TUI cleanly)
bunx tuistory -s kerberosec press ctrl c
bunx tuistory -s kerberosec press ctrl c
