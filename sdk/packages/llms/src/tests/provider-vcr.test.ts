...
import { disposeAll, initVcr } from "@kerberosec/shared";
...
const targets: ProviderVcrTarget[] = [
	{
		...
		cassetteName: "kerberosec-anthropic-sonnet.json",
	},
	{
...
function readEnvironmentApiKey(providerId: string): string | undefined {
	switch (providerId) {
		...
		case "kerberosec":
			return readString(process.env.KERBEROSEC_API_KEY);
		default:
			return undefined;
	}
}
...
function formatApiKey(providerId: string, apiKey: string): string {
	if (providerId === "kerberosec" && !apiKey.startsWith("workos:")) {
...
function configureVcr(cassettePath: string): void {
	...
	process.env.KERBEROSEC_VCR = RECORD_MODE ? "record" : "playback";
	process.env.KERBEROSEC_VCR_CASSETTE = cassettePath;
	process.env.KERBEROSEC_VCR_INCLUDE_REQUEST_BODY = "1";
	process.env.KERBEROSEC_VCR_FILTER = "";
