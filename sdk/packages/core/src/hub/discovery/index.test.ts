...
type EnvSnapshot = {
	...
	KERBEROSEC_HUB_BUILD_ID: string | undefined;
	KERBEROSEC_HUB_BUILD_EPOCH_MS: string | undefined;
	KERBEROSEC_HUB_DISCOVERY_PATH: string | undefined;
};

function captureEnv(): EnvSnapshot {
	return {
		KERBEROSEC_DATA_DIR: process.env.KERBEROSEC_DATA_DIR,
		KERBEROSEC_HUB_BUILD_ID: process.env.KERBEROSEC_HUB_BUILD_ID,
		KERBEROSEC_HUB_BUILD_EPOCH_MS: process.env.KERBEROSEC_HUB_BUILD_EPOCH_MS,
...
function restoreEnv(snapshot: EnvSnapshot): void {
	...
	process.env.KERBEROSEC_HUB_BUILD_ID = snapshot.KERBEROSEC_HUB_BUILD_ID;
	process.env.KERBEROSEC_HUB_BUILD_EPOCH_MS =
		snapshot.KERBEROSEC_HUB_BUILD_EPOCH_MS;
	process.env.KERBEROSEC_HUB_DISCOVERY_PATH =
		snapshot.KERBEROSEC_HUB_DISCOVERY_PATH;
}
...
describe("hub discovery", () => {
	...
	it("allows tests to override the unbundled source build identity", () => {
		snapshot = captureEnv();
		delete process.env.KERBEROSEC_HUB_BUILD_ID;
		expect(resolveHubBuildId()).toMatch(/^source-/);

		process.env.KERBEROSEC_HUB_BUILD_ID = "e2e-build";
	...
	it("allows tests to override the build epoch and treats sources as unordered", () => {
		snapshot = captureEnv();
		delete process.env.KERBEROSEC_HUB_BUILD_EPOCH_MS;
		expect(resolveHubBuildEpochMs()).toBeUndefined();

		process.env.KERBEROSEC_HUB_BUILD_EPOCH_MS = "12345";
