...
function createDaemonTelemetry(
	...
	const config = createKerberoSecTelemetryServiceConfig({
		metadata: createKerberoSecTelemetryServiceMetadata({
			...
			kerberosec_type: metadata.kerberosec_type,
...
void main().catch((error) => {
	process.stderr.write(
		`[kerberosec-vscode hub-daemon] ${error instanceof Error ? (error.stack ?? error.message) : String(error)}\n`,
	);
	process.exit(1);
});
