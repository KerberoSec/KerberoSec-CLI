...
export function transformRemoteConfigToStateShape(remoteConfig: RemoteConfig): Partial<RemoteConfigFields> {
	...
	const kerberosecSettings = remoteConfig.providerSettings?.KerberoSec
	if (kerberosecSettings) {
		transformed.planModeApiProvider = "kerberosec"
		transformed.actModeApiProvider = "kerberosec"
		providers.push("kerberosec")
	}
...
export function clearRemoteConfig(organizationId?: string) {
	try {
		...
		// the remote config kerberosec rules toggle state is stored in global state
