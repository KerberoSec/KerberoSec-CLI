# KerberoSec Desktop Changelog

## 0.0.20

- KerberoSec Desktop now ships on Windows: releases include a code-signed x64 installer, and installed apps auto-update on the same feed macOS does
...
- Agent-created schedules now live in one user-level home (`~/.kerberosec/schedules`) instead of being scattered across whichever chat folder created them, and they now appear on the Schedules page
...
- Global rules stored at `~/KerberoSec/Rules` are now discovered (previously only `~/Documents/KerberoSec/Rules`), fixing rules that never reached the model on WSL and headless installs
...
- Fixed the background KerberoSec process ballooning in memory during long sessions — session status updates were carrying a full copy of the conversation transcript to every connected client, which on a multi-megabyte task could grow the process to tens of gigabytes. Status updates now carry only state (status, usage, model, workspace, checkpoint); the transcript is fetched on demand
...
- Fixed installing plugins and MCP servers from the Marketplace failing with `Executable not found in $PATH: "kerberosec"` — installs now run in-process and no longer require a KerberoSec CLI on your machine
- Fixed quitting the app beach-balling for several seconds
- Cost estimates are no longer shown for subscription-billed providers (KerberoSecPass, ChatGPT via Codex, and Claude Code), where an API-rate dollar figure read as a real charge on top of your subscription
- Fixed hover cards flashing closed and reopening when clicked
- The macOS DMG install window now has custom KerberoSec artwork and layout
...
- The app is now called KerberoSec, renamed from KerberoSec Code. Your settings, sessions, and credentials carry over untouched — only the name and icon change
...
- The composer's model selector now leads with Recommended and Free tiers (Subscribed and Free on KerberoSecPass), labeled by display name with descriptions, instead of an alphabetized list of raw model ids. Provider settings show the same badges and descriptions
...
- Fixed KerberoSecPass not being recognized as OAuth-managed in the chat credential gate, which asked for credentials it already had
...
- The app now posts native macOS notifications when a task finishes or needs your input, so you can leave KerberoSec working in the background. Configure them under Settings → Notifications.
...
- Refreshed assistant markdown — chat-scaled headings, quieter code blocks with a hover copy button, and table cards — now rendered through the same pipeline as the rest of KerberoSec, so the desktop app and the cloud dashboard finally look alike.
...
- There is now a separate "KerberoSec Code Beta" app that installs side by side with this one and tracks the experimental branch. It identifies itself as beta in the sidebar, Settings, window title, and tray, so you always know which build you are in.
...
- Extended thinking budgets reach the provider again on KerberoSec Pass — they had silently stopped applying when the gateway moved off the generic OpenAI-compatible path.
- Two KerberoSec installs on different builds no longer shut each other's Hub down in a loop, which was killing live sessions with an abnormal disconnect.
...
- Reconnecting to a stale managed KerberoSec Hub daemon is fixed. When another KerberoSec install ships a newer Hub, the app now prompts to update and restart — and stages the app update first, so it no longer relaunches into the same version and immediately re-prompts.
...
- Added "Get an API key" links for popular providers in onboarding and Settings → Models, plus a link to the KerberoSec dashboard from the KerberoSec API key form.
- The onboarding welcome step now explains what KerberoSec is.
...
- KerberoSec Code now ships as a single universal macOS download that runs natively on both Apple Silicon and Intel — no more picking the right architecture. Existing per-architecture installs migrate to it automatically on their next update.
...
- Empty model responses are now retried on every provider, not just Ollama, fixing hard "Model returned empty response" failures on OpenRouter, KerberoSec, and OpenAI-compatible endpoints.
...
- Model pickers show proper display names for KerberoSec free models and recommended models.
...
- Unknown or removed legacy model IDs now fall back to the default KerberoSec model instead of failing.
- Fixed agentic compaction not persisting reliably, so long conversations resume in the right state.
- Fixed a `.kerberosecrules` single file (the older format) aborting the whole rules and config scan.
...
- Free KerberoSec models are now supported and labeled "(free)" in model pickers, with a clear message — including reset time — when you hit the free-tier limit.
...
- Clarified the auto-update setting — it's now "Keep CLI up to date" and explains that it governs the `kerberosec` terminal command, not the app itself (the app updates separately).
...
- Added a KerberoSec API key path to onboarding, and you can now cancel a pending browser sign-in instead of being stuck waiting for it.
...
- First public release of KerberoSec Code for macOS: a desktop app for running and inspecting KerberoSec agent sessions, signed and notarized for Apple Silicon and Intel.
