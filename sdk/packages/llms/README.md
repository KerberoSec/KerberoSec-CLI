# [experimental] @kerberosec/llms

`@kerberosec/llms` is the model and provider layer for the KerberoSec SDK. It gives
...
- `@kerberosec/llms/runtime` for declarative config and runtime registry creation
- `@kerberosec/llms/providers` for handler creation and provider settings/types
- `@kerberosec/llms/models` for model catalogs and query helpers
- `@kerberosec/llms` root exports for the gateway registry and shared llm contracts

## Installation

```bash
npm install @kerberosec/llms zod
```

## Quick Start

```ts
import { createHandler } from "@kerberosec/llms";
...
```ts
import { createLlmsRuntime, defineLlmsConfig } from "@kerberosec/llms/runtime";
```

### Providers

Use `@kerberosec/llms/providers` for:
...
`@kerberosec/llms` and `@kerberosec/shared`.

### Models

Use `@kerberosec/llms/models` when you need generated provider/model metadata for
...
```ts
import { transcribeAudio } from "@kerberosec/llms";
...
```ts
import { createStreamingAudioTranscriptionSession } from "@kerberosec/llms";
...
- `@kerberosec/llms`: runtime-focused convenience entrypoint
- `@kerberosec/llms/node`: explicit Node/runtime entrypoint
- `@kerberosec/llms/browser`: browser-safe bundle
- `@kerberosec/llms/runtime`: focused runtime entrypoint
- `@kerberosec/llms/models`: model catalog/query entrypoint
- `@kerberosec/llms/providers`: provider handler/settings entrypoint

## Related Packages

- `@kerberosec/agents`: agent loop and tool execution
- `@kerberosec/core`: stateful runtime assembly and provider settings storage
...
1. Ensure provider keys are present in your environment (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY`, `KERBEROSEC_API_KEY`, etc.).
...
bun -F @kerberosec/llms run test:live
```
...
- When recording provider cassettes for committed replay tests, set `KERBEROSEC_VCR=record` and `KERBEROSEC_VCR_INCLUDE_REQUEST_BODY=1` so playback also verifies the sanitized request body contract.

OpenAI Codex subscription live runs use the saved OAuth credentials from `~/.kerberosec/data/settings/providers.json` after `kerberosec auth --provider openai-codex`. Point the plain or reasoning suite at `packages/llms/src/tests/live-providers.openai-codex.example.json` or `packages/llms/src/tests/live-providers.openai-codex.reasoning.example.json`.
...
To check that disabling reasoning actually suppresses reasoning output across models, use `packages/llms/src/tests/live-providers.reasoning-disabled.example.json`; it covers direct and routed provider paths across `kerberosec`, `openai`, `openrouter`, `anthropic`, `gemini`, `vercel-ai-gateway`, `zai`, and `deepseek` where model support exists, with `reasoning.enabled: false` and the strongest available no-reasoning expectation for each provider.
