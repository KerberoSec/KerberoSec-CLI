...
test.describe("kerberosec --json persisted messages contract - authenticated @live", () => {
	const sessionDataDir = mkdtempSync(
		join(tmpdir(), "kerberosec-headless-messages-contract-"),
	);
	test.use({
		...
		env: kerberosecEnv("default", {
			...
			KERBEROSEC_SESSION_DATA_DIR: sessionDataDir,
		}),
	});
