...
const MCP_SETTINGS_FILE_NAME = "kerberosec_mcp_settings.json"
...
export async function getLegacyMcpSettingsSources(vscodeContext: vscode.ExtensionContext): Promise<LegacyMcpSource[]> {
	...
	sources.push({
		id: "documentsKerberoSecMcp",
		path: path.join(documentsDir, "KerberoSec", "MCP", MCP_SETTINGS_FILE_NAME),
	})
	return sources
}
...
export function getSharedMcpSettingsPath(storage: StorageContext): string {
	...
	const explicitDataDir = process.env.KERBEROSEC_DATA_DIR?.trim()
	if (explicitDataDir) {
	...
	return path.join(kerberosecDir, "data", "settings", MCP_SETTINGS_FILE_NAME)
}
