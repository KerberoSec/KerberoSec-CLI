...
const originalSessionDataDir = process.env.KERBEROSEC_SESSION_DATA_DIR;
...
async function writeMessagesFile(
	...
	if (!tempSessionDataDir) {
		tempSessionDataDir = await mkdtemp(
			join(tmpdir(), "kerberosec-core-history-"),
		);
	}
...
describe("session history", () => {
	afterEach(async () => {
		...
		if (originalSessionDataDir === undefined) {
			...
			process.env.KERBEROSEC_SESSION_DATA_DIR = originalSessionDataDir;
		}
	});
	...
	it("falls back to nested metadata provider/model ids before reading messages", async () => {
		...
		const [row] = await hydrateSessionHistory({ readSessionMessages }, [
			createRow({
				...
				metadata: {
					title: "hello",
					provider: { id: "kerberosec" },
					model: { id: "anthropic/claude-haiku-4.5" },
				},
			}),
		]);

		expect(row.provider).toBe("kerberosec");
	...
	it("merges manifest fallback rows when the backend list is short", async () => {
		...
		process.env.KERBEROSEC_SESSION_DATA_DIR = tempSessionDataDir;
