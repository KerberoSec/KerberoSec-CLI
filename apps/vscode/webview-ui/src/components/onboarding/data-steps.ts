export enum NEW_USER_TYPE {
	KERBEROSEC_PASS = "kerberosec-pass",
	FREE = "free",
	POWER = "power",
	BYOK = "byok",
}
...
const BASE_USER_TYPE_SELECTIONS: UserTypeSelection[] = [
	...
	{ title: "Bring my own API key", description: "Use KerberoSec with your provider of choice", type: NEW_USER_TYPE.BYOK },
]

/** Free leads (and is the default); KerberoSecPass is inserted second when its models are available. */
export function getUserTypeSelections(hasKerberoSecPassModels: boolean): UserTypeSelection[] {
	if (!hasKerberoSecPassModels) {
		return BASE_USER_TYPE_SELECTIONS
	}
	const [free, ...rest] = BASE_USER_TYPE_SELECTIONS
	return [free, KERBEROSEC_PASS_USER_TYPE_SELECTION, ...rest]
}
