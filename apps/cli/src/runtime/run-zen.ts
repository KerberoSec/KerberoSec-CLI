...
import { HubSessionClient } from "@kerberosec/core";
import type { ChatStartSessionRequest } from "@kerberosec/shared";
 ...
 * running, users can still find the result later via `kerberosec history`.
 *
...
export async function runZen(
	...
	if (
		process.env.KERBEROSEC_SESSION_BACKEND_MODE?.trim().toLowerCase() ===
		"local"
	) {
		writeErr(
			"--zen requires the hub backend but KERBEROSEC_SESSION_BACKEND_MODE=local is set.",
		);
		process.exitCode = 1;
		return;
	}
	...
	const sessionClient = new HubSessionClient({
		...
		clientType: "cli-zen",
		displayName: "KerberoSec CLI (zen)",
		workspaceRoot,
		cwd: config.cwd,
	});
