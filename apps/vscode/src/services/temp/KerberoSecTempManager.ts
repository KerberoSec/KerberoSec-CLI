/**
 * KerberoSecTempManager - Manages temporary files for KerberoSec with automatic cleanup.
 *
 * Simple approach:
 * - Uses a "kerberosec" subdirectory inside the system temp dir (falls back to system temp if creation fails)
...
/**
 * Singleton manager for KerberoSec's temporary files.
 */
class KerberoSecTempManagerImpl {
	private readonly tempDir: string
	private cleanupIntervalId: NodeJS.Timeout | null = null

	constructor() {
		// Uses system temp directory with a dedicated "kerberosec" subdirectory when possible:
		// macOS: /var/folders/xx/.../T/kerberosec
		// Windows: C:\Users\{user}\AppData\Local\Temp\kerberosec
		// Linux: /tmp/kerberosec
		const baseTempDir = os.tmpdir()
		const kerberosecTempDir = path.join(baseTempDir, "kerberosec")

		try {
			fs.mkdirSync(kerberosecTempDir, { recursive: true })
			this.tempDir = kerberosecTempDir
		} catch {
	...
	/**
	 * Clean up old KerberoSec temp files based on age and total size constraints.
	 * Called on extension activation.
	 *
	 * Strategy:
	 * 1. Scan the KerberoSec temp directory
	...
	async cleanup(): Promise<{ deletedCount: number; freedBytes: number }> {
		...
		try {
			...
			if (deletedCount > 0) {
				Logger.info(`KerberoSec temp cleanup: deleted ${deletedCount} files, freed ${Math.round(freedBytes / 1024 / 1024)}MB`)
			}
		} catch (error) {
			Logger.error("Error during KerberoSec temp cleanup", error)
		}
...
export const KerberoSecTempManager = new KerberoSecTempManagerImpl()
