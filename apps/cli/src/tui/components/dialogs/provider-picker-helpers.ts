 ...
 *
 * The key is written both to the provider's auth storage entry (kerberosec-pass
 * stores credentials under "kerberosec") and to the provider's own entry: settings
 ...
 *
 * Like the KerberoSecPass promo dialog, this screen must never depend on Esc
...
export function buildKerberoSecPassSubscriptionPageUrl(
	appBaseUrl: string | undefined,
): string {
	const url = new URL(
		KERBEROSEC_PASS_SUBSCRIPTION_PATH,
