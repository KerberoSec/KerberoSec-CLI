...
vi.mock("@shared/proto/kerberosec/task", () => ({
	AskResponseRequest: { create: (x: unknown) => x },
	NewTaskRequest: { create: (x: unknown) => x },
}))
vi.mock("@shared/proto/kerberosec/ui", () => ({
	IntentEvent: { create: (x: unknown) => x },
}))
vi.mock("@shared/proto/kerberosec/common", () => ({
...
// Minimal ChatState stub. kerberosecAsk/lastMessage are the only derived values the send path reads.
function makeChatState(messages: KerberoSecMessage[], overrides: Partial<ChatState> = {}): ChatState {
	...
	const state = {
		...
		kerberosecAsk: last?.type === "ask" ? last.ask : undefined,
		task: messages.at(0),
...
const completedConversation: KerberoSecMessage[] = [
...
describe("useMessageHandlers — send routing", () => {
	...
	it("after a completed turn (no kerberosecAsk), Enter continues the conversation via askResponse — NOT newTask", async () => {
		mockTurnState = { phase: "completed", seq: 7 }
		const { result } = renderHook(() => useMessageHandlers(completedConversation, makeChatState(completedConversation)))
	...
	it("shows a pending chat bubble immediately when sending a message to a task resumed from history", async () => {
		...
		const historyConversation: KerberoSecMessage[] = [
	...
	it("does not show a pending chat bubble for a streaming follow-up that will be queued", async () => {
		...
		const streamingConversation: KerberoSecMessage[] = [
			{ ts: 1, type: "say", say: "task", text: "task" },
	...
	it("rejects a pending approval when the composer is submitted with typed feedback", async () => {
		...
		const approvalConversation: KerberoSecMessage[] = [
			{ ts: 1, type: "say", say: "task", text: "task" },
	...
	it("does not show a pending chat bubble when answering an active follow-up question with freeform text", async () => {
		...
		const questionConversation: KerberoSecMessage[] = [
			{ ts: 1, type: "say", say: "task", text: "task" },
			{
				ts: 2,
				type: "ask",
	...
	// (emitKerberoSecAuthError for the KerberoSec provider, say:"error" otherwise).
