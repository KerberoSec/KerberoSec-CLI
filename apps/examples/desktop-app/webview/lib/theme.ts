export const HUB_THEME_STORAGE_KEY = "kerberosec-hub-theme";
...
export const HUB_THEME_BOOTSTRAP_SCRIPT = `(() => {
	...
	root.dataset.kerberosecHubTheme = theme;
})();`;
...
export function applyHubTheme(theme: HubTheme): HubTheme {
	...
	document.documentElement.dataset.kerberosecHubTheme = theme;
	return theme;
}
...
export const HUB_ACCENT_STORAGE_KEY = "kerberosec.code.accent.v1";

/**
 * Accent palettes selectable in Settings. "violet" is the built-in brand
 * accent from @kerberosec/ui tokens; the others override the interactive tokens
 * via `[data-kerberosec-accent]` blocks in globals.css.
 */
...
export function applyHubAccent(accent: HubAccent): HubAccent {
	if (accent === DEFAULT_HUB_ACCENT) {
		...
		document.documentElement.dataset.kerberosecAccent = accent;
	}
	return accent;
}
