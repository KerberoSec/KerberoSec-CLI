...
/**
 * All valid hook types that can be created and executed by KerberoSec.
...
export async function resolveHooksDirectory(
	...
	if (isGlobal) {
		return globalHooksDirOverride || path.join(os.homedir(), "Documents", "KerberoSec", "Hooks")
	}
	...
	if (workspaceName) {
		...
		return path.join(targetWorkspace, ".kerberosecrules", "hooks")
	}

	// Single workspace: use getCwd
	const cwd = await getCwd(getDesktopDir())
	return path.join(cwd, ".kerberosecrules", "hooks")
}
