...
/**
 * Slack delivers `@kerberosec hi` as `<@U0B8E8H3U1F> hi`, and the chat SDK
...
> {
	...
	protected override async runWithOptions(
		...
		const backgroundExitCode = await this.maybeRunInBackground({
			...
			childEnvVar: "KERBEROSEC_SLACK_CONNECT_CHILD",
			statePath,
			readState: (path) => this.readConnectorState(path),
			...
			foregroundHint:
				"[slack] use `kerberosec connect slack -i ...` to run in the foreground",
