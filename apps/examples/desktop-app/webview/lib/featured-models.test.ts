...
describe("buildModelPickerData", () => {
	it("builds recommended / free / all sections for the kerberosec provider", () => {
		const { options, sections } = buildModelPickerData("kerberosec", [
	...
	it("orders featured tiers by feed rank, not list order", () => {
		const { options } = buildModelPickerData("kerberosec", [
	...
	it("builds subscribed / free sections for kerberosec-pass and hides stale catalog leftovers", () => {
		const { options, sections } = buildModelPickerData("kerberosec-pass", [
			model("kerberosec-pass/kimi-k3", "Kimi K3", {
				tier: "subscribed",
				rank: 0,
				tags: [],
			}),
	...
	it("falls back to the full kerberosec-pass catalog when the subscribed tier is empty", () => {
		const { options, sections } = buildModelPickerData("kerberosec-pass", [
			model("deepseek/deepseek-v4-flash", "DeepSeek V4 Flash", {
				tier: "free",
				rank: 0,
