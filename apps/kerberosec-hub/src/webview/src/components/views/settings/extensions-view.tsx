...
const sectionDescriptions: Record<CustomizationSection, string> = {
	Rules: "Review project and global rule files that shape KerberoSec behavior.",
...
const sectionCommands: Record<CustomizationSection, string> = {
	Rules: "kerberosec config rules",
	Hooks: "kerberosec config hooks",
	MCP: "kerberosec mcp install",
	Skills: "kerberosec skill",
	Agents: "kerberosec config agents",
	Plugins: "kerberosec plugin",
	Tools: "kerberosec config tools",
};
	...
	const { projectRules, globalRules } = useMemo(() => {
		...
		for (const rule of rules) {
			...
			if (
				...
				normalized.includes("/.kerberosecrules/")
			) {
				project.push(rule);
			} else {
				global.push(rule);
	...
	const { projectHooks, globalHooks } = useMemo(() => {
		...
		for (const hook of hooks) {
			...
			if (
				normalizedRoot &&
				normalized.startsWith(`${normalizedRoot}/`) &&
				normalized.includes("/.kerberosecrules/hooks")
			) {
				project.push(hook);
			} else {
				global.push(hook);
	...
	const { projectPlugins, globalPlugins } = useMemo(() => {
		...
		for (const plugin of plugins) {
			...
			if (
				normalizedRoot &&
				normalized.startsWith(`${normalizedRoot}/`) &&
				normalized.includes("/.kerberosec/plugins")
			) {
				project.push(plugin);
			} else {
				global.push(plugin);
