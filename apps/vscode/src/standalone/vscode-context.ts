...
log("Running standalone kerberosec", ExtensionRegistryInfo.version)
log(`KERBEROSEC_ENVIRONMENT: ${process.env.KERBEROSEC_ENVIRONMENT}`)

// WE WILL HAVE TO MIGRATE THIS FROM DATA TO v1 LATER
const SETTINGS_SUBFOLDER = "data"

export function initializeContext(kerberosecDir?: string) {
	const KERBEROSEC_DIR = kerberosecDir || process.env.KERBEROSEC_DIR || `${os.homedir()}/.kerberosec`
	const DATA_DIR = path.join(KERBEROSEC_DIR, SETTINGS_SUBFOLDER)
	...
	const extensionContext: ExtensionContext = {
		...
		storagePath: WORKSPACE_STORAGE_DIR, // Deprecated, not used in kerberosec.
		globalStorageUri: URI.file(DATA_DIR),
		globalStoragePath: DATA_DIR, // Deprecated, not used in kerberosec.

		// Logs are global per extension, not per workspace.
		logUri: URI.file(DATA_DIR),
		logPath: DATA_DIR, // Deprecated, not used in kerberosec.

		extensionUri: URI.file(EXTENSION_DIR),
		extensionPath: EXTENSION_DIR, // Deprecated, not used in kerberosec.
