...
describe("resolveModelInfo", () => {
	...
	it("resolves a free model id from the kerberosec-pass catalog without coercing to the default", async () => {
		const { resolveModelInfo } = await import("../resolveModelInfo")
		const store = makeStore({ providerId: parseProviderId("kerberosec-pass") })
		const catalog = makeCatalog()
		// The kerberosec-pass catalog carries the endpoint's kerberosecPass bucket plus the
		// KerberoSec free models (zero-priced, OpenRouter-style ids without the
		// kerberosec-pass/ prefix). Selecting a free model must not be replaced by the
		// default pass model.
		vi.mocked(catalog.peekModels).mockReturnValue(
			peekResult(
				"kerberosec-pass",
				[
					["kerberosec-pass/glm-5.1", { name: "GLM 5.1", supportsPromptCache: false, contextWindow: 200_000 }],
