...
import { resolveKerberoSecDir } from "@kerberosec/shared/storage";
...
const MARKETPLACE_CATALOG_URL =
	process.env.KERBEROSEC_MARKETPLACE_CATALOG_URL?.trim() ||
...
function resolveKerberoSecInvocation(): {
	command: string;
	argsPrefix: string[];
} {
	...
	return { command: "kerberosec", argsPrefix: [] };
}
...
function getGlobalSkillPaths(skillName: string): string[] {
	return [
		join(resolveKerberoSecDir(), "skills", skillName, "SKILL.md"),
...
function ensureGlobalSkillsDirWritable(): void {
	...
	try {
		...
		const probePath = join(
			skillsDir,
			`.kerberosec-marketplace-write-test-${process.pid}-${Date.now()}`,
		);
...
async function installSkill(
	...
	if (!isGlobalSkillInstalled(entry)) {
		throw new Error(
			`Skill install completed, but ${entry.name ?? entry.id} was not found in KerberoSec's global skills directories.`,
		);
	}
	return {
		id: entry.id,
		type: entry.type,
		status: "installed",
		message: `Installed ${entry.name ?? entry.id} globally for KerberoSec.`,
		output,
	};
}
...
async function installPlugin(
	...
	const { command, argsPrefix } = resolveKerberoSecInvocation();
