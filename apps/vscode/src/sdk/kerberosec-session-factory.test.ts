...
import type { CoreSessionConfig } from "@kerberosec/core"
import * as LlmsModels from "@kerberosec/llms"
import { ApiFormat } from "@shared/proto/kerberosec/models"
...
import {
	...
	resolveApiKey,
	updateHistoryItem,
} from "./kerberosec-session-factory"
...
let tempDir: string
const previousGlobalSettingsPath = process.env.KERBEROSEC_GLOBAL_SETTINGS_PATH
const previousDataDir = process.env.KERBEROSEC_DATA_DIR

beforeEach(() => {
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "kerberosec-session-factory-"))
	process.env.KERBEROSEC_DATA_DIR = tempDir
	process.env.KERBEROSEC_GLOBAL_SETTINGS_PATH = path.join(tempDir, "global-settings.json")
	vi.clearAllMocks()
...
afterEach(() => {
	process.env.KERBEROSEC_GLOBAL_SETTINGS_PATH = previousGlobalSettingsPath
	process.env.KERBEROSEC_DATA_DIR = previousDataDir
...
describe("getDefaultModelIdForProvider", () => {
	it("uses the SDK provider catalog for the KerberoSec default model", () => {
		expect(getDefaultModelIdForProvider("kerberosec")).toBe(
			LlmsModels.MODEL_COLLECTIONS_BY_PROVIDER_ID.kerberosec.provider.defaultModelId,
		)
	})
...
describe("buildSessionConfig", () => {
	it("resolves KerberoSec OAuth credentials after defaulting to the KerberoSec provider", async () => {
		...
		expect(config.providerId).toBe("kerberosec")
	...
	it("resolves KerberoSecPass from the shared KerberoSec OAuth credentials", async () => {
		mocks.providerSettingsManager.getProviderSettings.mockImplementation((providerId?: string) => {
			...
			return {
				provider: "kerberosec",
				auth: {
					accessToken: "workos:shared-kerberosec-token",
					refreshToken: "shared-refresh-token",
				},
			} as any
		})

		const apiKey = resolveApiKey("kerberosec-pass", {
			actModeApiProvider: "kerberosec-pass",
		} as any)

		expect(apiKey).toBe("workos:shared-kerberosec-token")
		expect(mocks.providerSettingsManager.getProviderSettings).toHaveBeenCalledWith("kerberosec")
	})

	it("preserves explicit KerberoSecPass API keys from state before OAuth storage", () => {
		mocks.providerSettingsManager.getProviderSettings.mockReturnValue({
			provider: "kerberosec",
			auth: { accessToken: "workos:stored-token" },
		} as any)

		expect(resolveApiKey("kerberosec-pass", { kerberosecApiKey: "workos:configured-token" } as any)).toBe("workos:configured-token")
		expect(mocks.providerSettingsManager.getProviderSettings).not.toHaveBeenCalled()
	})

	it("preserves explicit KerberoSec API keys from state before OAuth storage", () => {
		mocks.providerSettingsManager.getProviderSettings.mockReturnValue({
			provider: "kerberosec",
			auth: { accessToken: "workos:stored-token" },
		} as any)

		expect(resolveApiKey("kerberosec", { kerberosecApiKey: "workos:configured-kerberosec-token" } as any)).toBe(
			"workos:configured-kerberosec-token",
		)
		expect(mocks.providerSettingsManager.getProviderSettings).not.toHaveBeenCalled()
	})
	...
	it("passes OpenAI Compatible max output tokens as an explicit request limit", async () => {
		...
		// Mirrored onto providerConfig for the compaction summarizer (KERBEROSEC-2911).
	...
	it("uses KerberoSecPass model storage and omits empty nested apiKey so SDK OAuth can fill it", async () => {
		mocks.stateManager.getApiConfiguration.mockReturnValue({
			actModeApiProvider: "kerberosec-pass",
			actModeKerberoSecPassModelId: "kerberosec-pass/glm-5.2",
		} as any)
		mocks.providerSettingsManager.getProviderSettings.mockReturnValue(undefined)

		const config = await buildSessionConfig({ cwd: "/tmp/workspace" })

		expect(config.providerId).toBe("kerberosec-pass")
		expect(config.modelId).toBe("kerberosec-pass/glm-5.2")
		expect(config.apiKey).toBe("")
		expect(config.providerConfig).toMatchObject({ providerId: "kerberosec-pass", modelId: "kerberosec-pass/glm-5.2" })
		expect(config.providerConfig).not.toHaveProperty("apiKey")
	})
	...
	it("uses the configured SDK compaction strategy when auto condense is enabled", async () => {
		writeJson(process.env.KERBEROSEC_GLOBAL_SETTINGS_PATH!, { compactionStrategy: "basic" })
		mocks.stateManager.getGlobalSettingsKey.mockImplementation((key: string) => {
			if (key === "useAutoCondense") {
				return true
			}
	...
	it("falls back to agentic SDK compaction for an invalid stored strategy", async () => {
		writeJson(process.env.KERBEROSEC_GLOBAL_SETTINGS_PATH!, { compactionStrategy: "invalid" })
		mocks.stateManager.getGlobalSettingsKey.mockImplementation((key: string) => {
			if (key === "useAutoCondense") {
				return true
			}
