...
import { KerberoSecAccountUserInfo } from "@/services/auth/AuthService"
...
export type TelemetryMetadata = {
	/**
	 * The extension or kerberosec-core version. JetBrains and CLI have different
	 * versioning than the VSCode Extension, but on those platforms this will be the _kerberosec-core version_
	 * which uses the same as the versioning as the VSCode extension.
	 */
	extension_version: string
	/**
	 * The type of kerberosec distribution, e.g VSCode Extension, JetBrains Plugin or CLI. This
	 ...
	 */
	kerberosec_type: string
	/**
	 * The version of the host-side KerberoSec distribution package: the JetBrains plugin version
...
/**
 * TelemetryService handles telemetry event tracking for the KerberoSec extension
...
export class TelemetryService {
	...
	public static readonly METRICS = {
		TASK: {
			TURNS_TOTAL: "kerberosec.turns.total",
			TURNS_PER_TASK: "kerberosec.turns.per_task",
			TOKENS_INPUT_TOTAL: "kerberosec.tokens.input.total",
			TOKENS_INPUT_PER_RESPONSE: "kerberosec.tokens.input.per_response",
			TOKENS_OUTPUT_TOTAL: "kerberosec.tokens.output.total",
			TOKENS_OUTPUT_PER_RESPONSE: "kerberosec.tokens.output.per_response",
			COST_TOTAL: "kerberosec.cost.total",
			COST_PER_EVENT: "kerberosec.cost.per_event",
		},
		CACHE: {
			WRITE_TOTAL: "kerberosec.cache.write.tokens.total",
			WRITE_PER_EVENT: "kerberosec.cache.write.tokens.per_event",
			READ_TOTAL: "kerberosec.cache.read.tokens.total",
			READ_PER_EVENT: "kerberosec.cache.read.tokens.per_event",
			HITS_TOTAL: "kerberosec.cache.hits.total",
		},
		TOOLS: {
			CALLS_TOTAL: "kerberosec.tool.calls.total",
			CALLS_PER_TASK: "kerberosec.tool.calls.per_task",
		},
		ERRORS: {
			TOTAL: "kerberosec.errors.total",
			PER_TASK: "kerberosec.errors.per_task",
		},
		API: {
			TTFT_SECONDS: "kerberosec.api.ttft.seconds",
			DURATION_SECONDS: "kerberosec.api.duration.seconds",
			THROUGHPUT_TOKENS_PER_SECOND: "kerberosec.api.throughput.tokens_per_second",
		},
		HOOKS: {
			EXECUTIONS_TOTAL: "kerberosec.hooks.executions.total",
			DURATION_SECONDS: "kerberosec.hooks.duration.seconds",
			FAILURES_TOTAL: "kerberosec.hooks.failures.total",
			CANCELLATIONS_TOTAL: "kerberosec.hooks.cancellations.total",
			CONTEXT_MODIFICATIONS_TOTAL: "kerberosec.hooks.context_modifications.total",
			CACHE_ACCESSES_TOTAL: "kerberosec.hooks.cache.accesses.total",
		},
		AI_OUTPUT: {
			ACCEPTED_LINES_ADDED: "kerberosec.ai_output.accepted.lines_added.total",
			ACCEPTED_LINES_DELETED: "kerberosec.ai_output.accepted.lines_deleted.total",
			ACCEPTED_LINES_CHANGED: "kerberosec.ai_output.accepted.lines_changed.total",
			ACCEPTED_FILES_CREATED: "kerberosec.ai_output.accepted.files_created.total",
			ACCEPTED_FILES_DELETED: "kerberosec.ai_output.accepted.files_deleted.total",
			ACCEPTED_FILES_MOVED: "kerberosec.ai_output.accepted.files_moved.total",
			REJECTED_LINES_ADDED: "kerberosec.ai_output.rejected.lines_added.total",
			REJECTED_LINES_DELETED: "kerberosec.ai_output.rejected.lines_deleted.total",
			REJECTED_LINES_CHANGED: "kerberosec.ai_output.rejected.lines_changed.total",
			REJECTED_FILES_CREATED: "kerberosec.ai_output.rejected.files_created.total",
			REJECTED_FILES_DELETED: "kerberosec.ai_output.rejected.files_deleted.total",
			REJECTED_FILES_MOVED: "kerberosec.ai_output.rejected.files_moved.total",
		},
		GRPC: {
			RESPONSE_SIZE_BYTES: "kerberosec.grpc.response.size_bytes",
		},
		MIGRATION: {
			// Fires whenever KerberoSec checks an old pre-SDK task and decides whether/how to migrate it.
			LEGACY_TASK_ATTEMPTS_TOTAL: "kerberosec.migration.legacy_task.attempts.total",
			// Fires when the user opens an old task and KerberoSec successfully copies it into SDK session storage.
			LEGACY_TASK_SUCCESS_TOTAL: "kerberosec.migration.legacy_task.success.total",
			// Fires when KerberoSec tried to migrate an old task but failed while building or writing the SDK session.
			LEGACY_TASK_FAILURES_TOTAL: "kerberosec.migration.legacy_task.failures.total",
			// Fires when no migration happens because it is unnecessary or impossible, e.g. already migrated or missing old messages.
			LEGACY_TASK_SKIPPED_TOTAL: "kerberosec.migration.legacy_task.skipped.total",
			// Fires for every migration decision; measures how long the check/migration took.
			LEGACY_TASK_DURATION_SECONDS: "kerberosec.migration.legacy_task.duration.seconds",
			// Fires when KerberoSec finds old conversation messages; records how many old messages were found.
			LEGACY_TASK_LEGACY_MESSAGES_COUNT: "kerberosec.migration.legacy_task.legacy_messages.count",
			// Fires after conversion; records how many messages made it into SDK-compatible form.
			LEGACY_TASK_CONVERTED_MESSAGES_COUNT: "kerberosec.migration.legacy_task.converted_messages.count",
			// Fires when history is listed; counts old pre-SDK tasks still waiting to be migrated.
			LEGACY_TASK_PENDING_COUNT: "kerberosec.migration.legacy_task.pending.count",
			// Fires when history is listed; counts old tasks that already made it safely into SDK session storage.
			LEGACY_TASK_MIGRATED_COUNT: "kerberosec.migration.legacy_task.migrated.count",
		},
	}
	...
	private static readonly EVENTS = {
		...
		TASK: {
			...
			// Tracks when individual KerberoSec rules are toggled on/off
		...
		UI: {
			...
			// Tracks when the KerberoSec panel becomes visible
	...
	public static async create(): Promise<TelemetryService> {
		...
		const metadata: TelemetryMetadata = {
			...
			...(hostVersion.kerberosecVersion ? { host_plugin_version: hostVersion.kerberosecVersion } : {}),
			platform: hostVersion.platform || "unknown",
			platform_version: hostVersion.version || "unknown",
			kerberosec_type: hostVersion.kerberosecType || "unknown",
			os_type: os.platform(),
	...
	public async updateTelemetryState(didUserOptIn: boolean): Promise<void> {
		...
		if (hostSetting.isEnabled === Setting.DISABLED) {
			// Only show warning if user has opted in to KerberoSec telemetry but host telemetry is disabled
			if (didUserOptIn) {
				void HostProvider.window
					.showMessage({
						type: ShowMessageType.WARNING,
						message:
							"Anonymous KerberoSec error and usage reporting is enabled, but IDE telemetry is disabled. To enable error and usage reporting for this extension, enable telemetry in IDE settings.",
						options: {
							items: ["Open Settings"],
						},
					})
	 ...
	 * checkpoints, focus chain, legacy-task migration), so @kerberosec/core cannot
	 * emit them and the missing piece is a call site here. Tracked in
	 * https://linear.app/kerberosec-bot/issue/ENG-2401
	 *
	 * Anything whose event @kerberosec/core already emits was deleted instead: a
	 * second capture path for a core-owned event is how the
	 * task.provider_api_error double-emission happened (kerberosec/kerberosec#12820).
	 *
	 ...
	 */
	public identifyAccount(userInfo: KerberoSecAccountUserInfo) {
		const propertiesWithMetadata: TelemetryProperties = {
			...this.telemetryMetadata,
		}
	...
	/**
	 * Records when individual KerberoSec rules are toggled on/off
	 ...
	 */
	public captureKerberoSecRuleToggled(ulid: string, ruleFileName: string, enabled: boolean, isGlobal: boolean) {
