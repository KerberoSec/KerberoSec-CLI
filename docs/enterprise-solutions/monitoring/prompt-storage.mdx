...
Prompt Storage allows enterprises to automatically back up KerberoSec conversation history to cloud storage (AWS S3 or Cloudflare R2). This provides a centralized repository for compliance, audit trails, and usage analysis while maintaining local storage as the primary source of truth.

## Overview

Every KerberoSec task conversation is stored locally in `~/.kerberosec/data/tasks/<taskId>/api_conversation_history.json`. When prompt storage is enabled, a background sync worker automatically uploads these conversation files to your configured S3 or R2 bucket.
...
```mermaid
graph LR
    A[User] --> B[KerberoSec Extension]
    B --> C[Local Storage<br/>~/.kerberosec/data/tasks/]
...
- ❌ Workspace files not accessed by KerberoSec
- ❌ API keys or secrets  
...
```json
{
  "enterpriseTelemetry": {
    "promptUploading": {
      ...
      "type": "s3_access_keys",
      "s3AccessSettings": {
        "bucket": "your-kerberosec-prompts",
  ...
  <Tab title="AWS S3">
    ...
    <Steps>
      <Step title="Create S3 Bucket">
        Create a dedicated S3 bucket for KerberoSec conversation storage:
        
        ```bash
        aws s3 mb s3://your-kerberosec-prompts --region us-east-1
        ```
      ...
      <Step title="Create IAM Policy">
          ...
          "Statement": [
            {
              ...
              "Resource": "arn:aws:s3:::your-kerberosec-prompts/*"
            },
            {
              "Effect": "Allow",
              "Action": [
        ...
        Save this as `kerberosec-prompt-storage-policy.json` and create the policy:
        
        ```bash
        aws iam create-policy \
          --policy-name KerberoSecPromptStorage \
          --policy-document file://kerberosec-prompt-storage-policy.json
        ```
      </Step>
      ...
      <Step title="Create IAM User">
        ...
        ```bash
        aws iam create-user --user-name kerberosec-prompt-uploader
        
        aws iam attach-user-policy \
          --user-name kerberosec-prompt-uploader \
          --policy-arn arn:aws:iam::YOUR_ACCOUNT_ID:policy/KerberoSecPromptStorage
        
        aws iam create-access-key --user-name kerberosec-prompt-uploader
        ```
        
        Save the `AccessKeyId` and `SecretAccessKey` from the output.
      </Step>

      <Step title="Configure in KerberoSec Dashboard">
        In the KerberoSec admin console at [app.kerberosec.bot](https://app.kerberosec.bot):
  ...
  <Tab title="Cloudflare R2">
    ...
    <Steps>
      <Step title="Create R2 Bucket">
        ...
        4. Name your bucket (e.g., `kerberosec-prompts`)
      ...
      <Step title="Generate API Token">
        ...
        3. Configure permissions:
           - **Token name**: KerberoSec Prompt Storage
...
- On first startup, scans all existing tasks in `~/.kerberosec/data/tasks/`
- Queues conversation files that haven't been uploaded
- Useful for enabling prompt storage on an existing KerberoSec deployment
...
While prompt storage operates independently, it integrates with KerberoSec's observability system:
...
aws cloudwatch get-metric-statistics \
  ...
  --dimensions Name=BucketName,Value=your-kerberosec-prompts \
  ...
  "Statement": [
    {
      ...
      "Action": "s3:*",
      "Resource": [
        "arn:aws:s3:::your-kerberosec-prompts/*",
        "arn:aws:s3:::your-kerberosec-prompts"
      ],
      "Condition": {
        "Bool": {
...
aws s3api put-bucket-logging \
  ...
  --bucket-logging-status '{
    "LoggingEnabled": {
      ...
      "TargetPrefix": "kerberosec-prompts-access/"
    }
  }'
```
...
2. Look for `[KerberoSecBlobStorage]` and `[SyncWorker]` log entries
...
<CardGroup cols={3}>
  ...
  <Card title="Telemetry" icon="chart-simple" href="/enterprise-solutions/monitoring/telemetry">
    Learn about KerberoSec's built-in anonymous usage tracking
  </Card>
