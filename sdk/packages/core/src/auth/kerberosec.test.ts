...
import type { KerberoSecOAuthCredentials } from "./kerberosec";
import {
...
function createCredentials(
	overrides: Partial<KerberoSecOAuthCredentials> = {},
...
describe("auth/kerberosec getValidKerberoSecCredentials", () => {
	afterEach(() => {
		vi.restoreAllMocks();
	...
	it("returns existing credentials when not expired", async () => {
		...
		const result = await getValidKerberoSecCredentials(
			current,
			PROVIDER_OPTIONS,
		);
	...
	it("returns null when refresh fails with invalid_grant", async () => {
		...
		const capture = vi.fn();
		const result = await getValidKerberoSecCredentials(current, {
			...PROVIDER_OPTIONS,
	...
	it("omits request_id when a failed response has no request ID header", async () => {
		...
		const capture = vi.fn();
		await getValidKerberoSecCredentials(current, {
			...PROVIDER_OPTIONS,
			telemetry: { capture } as never,
		});
		...
		const current = createCredentials({
			...
			metadata: {
				...
				userInfo: {
					...
					email: "user@example.com",
					name: "User",
					kerberosecUserId: "kerberosec-user-2",
					accounts: [],
				},
			},
		});
	...
	it("throws on transient refresh error when the token is already expired", async () => {
		...
		const capture = vi.fn();
		await expect(
			getValidKerberoSecCredentials(current, {
				...PROVIDER_OPTIONS,
				telemetry: { capture } as never,
			}),
...
describe("auth/kerberosec loginKerberoSecOAuth", () => {
	afterEach(() => {
		vi.restoreAllMocks();
		globalThis.fetch = ORIGINAL_FETCH;
	});
				...
				new Response(
					JSON.stringify({
						...
						data: {
							...
							refreshToken: "kerberosec-refresh",
							tokenType: "Bearer",
							expiresAt: "2030-01-01T00:00:00.000Z",
							userInfo: {
								subject: "sub-1",
...
describe("auth/kerberosec completeKerberoSecDeviceAuth", () => {
	afterEach(() => {
		vi.restoreAllMocks();
		globalThis.fetch = ORIGINAL_FETCH;
	});
