...
An HTTP-controlled debug server for the KerberoSec VSCode extension. Provides
...
- **Data isolation**: separate `~/.kerberosec2` profile so debugee doesn't interfere with debugger
...
Options:
  ...
  --workspace PATH    Workspace directory to open (default: /tmp/kerberosec-debug-workspace)
  --port PORT         Server port (default: 19229)
  --kerberosec-dir PATH    Override the debugee's KERBEROSEC_DIR (default: ~/.kerberosec2)
```
...
The debugee runs with `KERBEROSEC_DIR=~/.kerberosec2` by default, keeping its data
separate from your real `~/.kerberosec`. This prevents:
...
The isolated KERBEROSEC_DIR is reported in `status()` and `launch()` responses:

```bash
curl localhost:19229/api -d '{"method":"status"}'
# → { "kerberosecDir": "/Users/you/.kerberosec2", ... }
```

To use a different directory: `--kerberosec-dir /tmp/test-kerberosec-dir`

## Browser Capture & OAuth Testing

When the debug harness launches VSCode, it sets `KERBEROSEC_CAPTURE_BROWSER=1`
which intercepts all `openExternal()` calls in the debugee. Instead of
opening a real browser, URLs are:

1. **Logged to disk** at `$KERBEROSEC_DIR/data/debug-captured-urls.jsonl`
...
### Testing KerberoSec OAuth (login flow)

The KerberoSec OAuth flow uses the SDK's local callback server. When the user
...
curl localhost:19229/api -d '{"method":"ui.locator","params":{"text":"Login to KerberoSec","frame":"sidebar","action":"click"}}'

# 2. Check captured URLs to find the authorization URL
curl localhost:19229/api -d '{"method":"oauth.captured_urls"}'
# → { "urls": [{ "url": "https://api.kerberosec.bot/auth/authorize?callback_url=http://127.0.0.1:PORT/..." }] }
...
# → { "found": true, "hasAccountId": true, "keys": ["kerberosec:kerberosecAccountId"] }
...
#    handler module. Instead, call the __kerberosecHandleUri hook (see below):
curl localhost:19229/api -d '{
  "method": "ext.evaluate",
  "params": {
    "awaitPromise": true,
    "expression": "globalThis.__kerberosecHandleUri(\"vscode://saoudrizwan.claude-dev/mcp-auth/callback/HASH?code=REAL_CODE&state=SAVED_STATE\")"
  }
}'
...
> **`globalThis.__kerberosecHandleUri(url)` — debug-only URI delivery hook.**
> Registered in `src/extension.ts` during activation, **only** when
> `KERBEROSEC_CAPTURE_BROWSER` is set (which the harness always sets), so it never
...
|---------|--------------|
| `kerberosec.accountButtonClicked` | Account / sign-in view |
| `kerberosec.historyButtonClicked` | Task history view |
| `kerberosec.settingsButtonClicked` | Settings view |
| `kerberosec.mcpButtonClicked` | MCP servers view |
| `kerberosec.plusButtonClicked` | New task (chat view) |
| `kerberosec.worktreesButtonClicked` | Worktrees view |

```bash
# Navigate to account view
curl localhost:19229/api -d '{"method":"ui.command_palette","params":{"command":"kerberosec.accountButtonClicked"}}'

# Navigate to history view
curl localhost:19229/api -d '{"method":"ui.command_palette","params":{"command":"kerberosec.historyButtonClicked"}}'

# Navigate to settings view
curl localhost:19229/api -d '{"method":"ui.command_palette","params":{"command":"kerberosec.settingsButtonClicked"}}'

# Navigate to MCP view
curl localhost:19229/api -d '{"method":"ui.command_palette","params":{"command":"kerberosec.mcpButtonClicked"}}'

# Start a new task (return to chat view)
curl localhost:19229/api -d '{"method":"ui.command_palette","params":{"command":"kerberosec.plusButtonClicked"}}'
```
...
# 3. Check status (verify KERBEROSEC_DIR, browser capture, etc.)
curl localhost:19229/api -d '{"method":"status"}'
...
| `ui.open_sidebar` | | Open the KerberoSec sidebar |
...
`globalThis.__kerberosecHandleUri("vscode://saoudrizwan.claude-dev/...?code=...&state=...")`.
It invokes the same `SharedUriHandler.handleUri` as VSCode's real URI handler
and is registered only when `KERBEROSEC_CAPTURE_BROWSER` is set (never in prod). See
...
3. **Data Isolation**: Sets `KERBEROSEC_DIR=~/.kerberosec2` in the debugee's environment, ensuring
   the debugee uses a completely separate data directory from the user's real `~/.kerberosec`.
...
4. **Browser Capture**: Sets `KERBEROSEC_CAPTURE_BROWSER=1` and `KERBEROSEC_DEBUG_HARNESS_PORT=19229`
   in the debugee's environment. When `openExternal()` is called in `src/utils/env.ts`, it
   checks for `KERBEROSEC_CAPTURE_BROWSER` and, if set, logs the URL to a JSONL file and POSTs
...
**⚠️ Data Isolation**: The debugee uses `~/.kerberosec2` by default. If you need to test with
existing data from your real `~/.kerberosec`, copy it: `cp -r ~/.kerberosec ~/.kerberosec2`. Be aware that
secrets (API keys, auth tokens) will be shared if you do this.

**⚠️ "Introducing KerberoSec Kanban" overlay**: On fresh launches, a full-screen promo overlay may
...
**Screenshots**: `ui.screenshot` and `ui.sidebar_screenshot` save PNG files to `/tmp/kerberosec-debug/`
...
to open. For KerberoSec OAuth, the SDK's local callback server is still running and will accept
...
**KerberoSec OAuth with invalid codes**: If you simulate the OAuth callback with a fake code, the
...
**"Sidebar frame not found"**: The KerberoSec sidebar isn't open. Use `ui.open_sidebar` first.
...
**Screenshots directory**: Saved to `/tmp/kerberosec-debug/` (configurable via SCREENSHOT_DIR).

**Debugee still uses ~/.kerberosec**: Check that `KERBEROSEC_DIR` appears in the `status()` response.
