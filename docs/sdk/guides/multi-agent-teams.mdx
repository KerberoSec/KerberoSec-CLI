...
Teams are a KerberoSecCore feature. Enable them in the session config:

```typescript
import { KerberoSecCore } from "@kerberosec/sdk"

const kerberosec = await KerberoSecCore.create({ clientName: "team-app" })
...
```bash
kerberosec --team-name auth-sprint "Continue -- pick up incomplete tasks"
```
...
```bash
# Start a new team
kerberosec --team-name auth-sprint "Plan and implement user auth with tests"

# Resume work
kerberosec --team-name auth-sprint "What's the status? Continue with unfinished tasks."

# Different team for a different workstream
kerberosec --team-name perf-sprint "Profile the API endpoints and optimize the slowest 3"
```
