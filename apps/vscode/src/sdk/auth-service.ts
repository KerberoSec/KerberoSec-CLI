...
//
// SDK-backed authentication service. Uses @kerberosec/core OAuth functions
...
// disk — it's fetched from the KerberoSec API on startup and cached in memory.
// This matches the CLI's pattern (see apps/cli/src/runtime/interactive-welcome.ts).

import type { ITelemetryService, OAuthCredentials, ProviderSettings } from "@kerberosec/core"
import {
...
import { AuthState, UserInfo } from "@shared/proto/kerberosec/account"
import type { EmptyRequest, String } from "@shared/proto/kerberosec/common"
...
import { buildBasicKerberoSecHeaders } from "@/services/EnvUtils"
import { featureFlagsService } from "@/services/feature-flags"
import { telemetryService } from "@/services/telemetry"
import { KERBEROSEC_API_ENDPOINT } from "@/shared/kerberosec/api"
...
export interface KerberoSecAuthInfo {
	idToken: string
	refreshToken?: string
	expiresAt?: number // seconds since epoch
	userInfo: KerberoSecAccountUserInfo
	provider: string
	startedAt?: number
}

export interface KerberoSecAccountUserInfo {
	createdAt?: string
	displayName: string
	email: string
	id: string
...
/**
 * Read KerberoSec OAuth credentials from providers.json.
 * Returns { accessToken, refreshToken, expiresAt, accountId } or null.
 */
function readKerberoSecCredentials(): {
	accessToken: string
	refreshToken?: string
	...
	try {
		...
		if (!settings?.auth?.accessToken) {
			sdkDebug("[SdkAuthService] readKerberoSecCredentials: no auth.accessToken found")
			return null
		}
		...
		sdkDebug(
			`[SdkAuthService] readKerberoSecCredentials: found credentials (accessHash=${hashSecret(result.accessToken)}, refreshHash=${hashSecret(result.refreshToken)}, expiresAt=${result.expiresAt}, sessionStartedAtMs=${result.sessionStartedAtMs})`,
		)
		return result
	} catch (error) {
...
/**
 * Write KerberoSec OAuth credentials to providers.json.
 */
function writeKerberoSecCredentials(credentials: {
	accessToken: string
	refreshToken?: string
	...
	try {
		const manager = getProviderSettingsManager()
		const existing = manager.getProviderSettings("kerberosec")
		...
		// backed by this credential entry (kerberosec-pass stores under "kerberosec") —
		// otherwise every refresh resets a KerberoSecPass selection back to
		// usage-billing in the shared providers.json (#13501).
		const lastUsedProvider = manager.read().lastUsedProvider
		const setLastUsed = getProviderAuthStorageId(lastUsedProvider ?? "") !== "kerberosec"

		manager.saveProviderSettings(
			{
				...(existing ?? { provider: "kerberosec" }),
				provider: "kerberosec",
		...
		sdkDebug(
			`[SdkAuthService] writeKerberoSecCredentials: wrote (accessHash=${hashSecret(credentials.accessToken)}, refreshHash=${hashSecret(credentials.refreshToken)}, expiresAt=${credentials.expiresAt}, sessionStartedAtMs=${sessionStartedAtMs})`,
		)
	} catch (error) {
...
/**
 * Clear KerberoSec OAuth credentials from providers.json.
 */
function clearKerberoSecCredentials(): void {
	try {
		const manager = getProviderSettingsManager()
		const existing = manager.getProviderSettings("kerberosec")
		if (existing) {
			sdkDebug("[SdkAuthService] clearKerberoSecCredentials: clearing auth from providers.json")
			manager.saveProviderSettings(
				{
					...existing,
					provider: "kerberosec",
...
export class AuthService {
	...
	private _kerberosecAuthInfo: KerberoSecAuthInfo | null = null
	...
	// ---- SDK OAuth → KerberoSecAuthInfo conversion ----

	/**
	 * Convert SDK OAuthCredentials to our KerberoSecAuthInfo format.
	 * Also fetches full user info from the KerberoSec API.
	 */
	private async credentialsToAuthInfo(credentials: OAuthCredentials, provider: string): Promise<KerberoSecAuthInfo> {
	...
	/**
	 * Fetch user info from the KerberoSec API using an access token.
	 */
	private async fetchUserInfoFromApi(accessToken: string): Promise<KerberoSecAccountUserInfo | null> {
		try {
			const apiBaseUrl = KerberoSecEnv.config().apiBaseUrl
			...
			const response = await axios.get(`${apiBaseUrl}/api/v1/users/me`, {
				headers: {
					...
					...(await buildBasicKerberoSecHeaders()),
				},
				...getAxiosSettings(),
			})
	...
	private toOAuthCredentials(authInfo: KerberoSecAuthInfo): OAuthCredentials {
		return {
	...
	private async resolveValidKerberoSecCredentials(
		authInfo: KerberoSecAuthInfo,
		...
		return getValidKerberoSecCredentials(
			this.toOAuthCredentials(authInfo),
			{ apiBaseUrl: KerberoSecEnv.config().apiBaseUrl, telemetry: this._telemetry },
	...
	async getAuthToken(): Promise<string | null> {
		if (!this._kerberosecAuthInfo?.idToken) {
			return null
		}

		// Check if we need to refresh
		const expiresAt = this._kerberosecAuthInfo.expiresAt
		if (expiresAt) {
		...
		// a successful refresh above replaces _kerberosecAuthInfo, and the expiry
		// captured before it belongs to the old token — which has already passed
		// whenever the process sat idle beyond token lifetime.
		const currentExpiresAt = this._kerberosecAuthInfo.expiresAt
		if (currentExpiresAt && Date.now() / 1000 >= currentExpiresAt) {
			return null
		}

		return `${WORKOS_TOKEN_PREFIX}${this._kerberosecAuthInfo.idToken}`
	}

	/**
	 * Refresh the access token using the SDK's shared KerberoSec credential validator.
	 * Persists refreshed credentials to providers.json when credentials change.
	 */
	private async refreshAccessToken(): Promise<boolean> {
		if (this._refreshPromise) {
			await this._refreshPromise
			return this._kerberosecAuthInfo?.idToken !== undefined
		}

		if (!this._kerberosecAuthInfo?.refreshToken) {
			sdkDebug("[SdkAuthService] refreshAccessToken: no refresh token available")
			return false
		}

		sdkDebug(`[SdkAuthService] refreshAccessToken: starting (currentTokenHash=${hashSecret(this._kerberosecAuthInfo.idToken)})`)
		this._refreshPromise = (async () => {
			try {
				const currentInfo = this._kerberosecAuthInfo
				if (!currentInfo) {
					return undefined
				}
				const newCredentials = await this.resolveValidKerberoSecCredentials(currentInfo, { forceRefresh: true })
				if (!newCredentials) {
					...
					this._kerberosecAuthInfo = null
					this._authenticated = false
					clearKerberoSecCredentials()
					setImmediate(() => {
				...
				if (credentialsChanged) {
					...
					writeKerberoSecCredentials({
						accessToken: newCredentials.access,
						refreshToken: newCredentials.refresh,
						expiresAt: newCredentials.expires,
						accountId: this._kerberosecAuthInfo.userInfo.id,
				...
				return this._kerberosecAuthInfo.idToken
			} catch (error) {
	...
	getActiveOrganizationId(): string | null {
		if (!this._kerberosecAuthInfo?.userInfo?.organizations) return null
		const activeOrg = this._kerberosecAuthInfo.userInfo.organizations.find((org) => org.active)
	 ...
	 */
	getUserOrganizations(): KerberoSecAccountOrganization[] | undefined {
		return this._kerberosecAuthInfo?.userInfo?.organizations
	}
	...
	getProviderName(): string | null {
		return this._kerberosecAuthInfo?.provider ?? null
	}
	 ...
	 */
	getInfo(): AuthState {
		if (this._kerberosecAuthInfo && this._authenticated) {
			const userInfo = this._kerberosecAuthInfo.userInfo
			userInfo.appBaseUrl = KerberoSecEnv.config().appBaseUrl
	...
	/**
	 * Initiate KerberoSec OAuth login.
	 * Uses SDK's loginKerberoSecOAuth() which spawns a local callback server.
	...
	async createAuthRequest(strict = false): Promise<String> {
		...
		if (strict && this._authenticated) {
			...
			const { String: ProtoString } = await import("@shared/proto/kerberosec/common")
		...
		// of loginKerberoSecOAuth(), which opens a real browser window the tests can't
		...
		void (async () => {
			try {
				...
				const credentials = await loginKerberoSecOAuth({
					...
					headers: await buildBasicKerberoSecHeaders(),
				...
				const authInfo = await this.credentialsToAuthInfo(credentials, "kerberosec")
				this._kerberosecAuthInfo = authInfo
				this._authenticated = true
				...
				Logger.error("[SdkAuthService] KerberoSec OAuth login failed:", error)
			}
		})()
	...
	private async createMockAuthRequest(): Promise<String> {
		if (KerberoSecEnv.config().environment !== "local") {
			throw new Error("E2E mock auth is only available when KERBEROSEC_ENVIRONMENT=local")
		}

		const apiBaseUrl = KerberoSecEnv.config().apiBaseUrl
		const tokenUrl = new URL(KERBEROSEC_API_ENDPOINT.TOKEN_EXCHANGE, apiBaseUrl)
		...
		await this.sendAuthStatusUpdate()
		Logger.log(`[SdkAuthService] E2E mock login completed as ${this._kerberosecAuthInfo.userInfo.email}`)
	...
	async handleDeauth(reason: LogoutReason = LogoutReason.UNKNOWN): Promise<void> {
		try {
			telemetryService.captureAuthLoggedOut("kerberosec", reason)
			this._kerberosecAuthInfo = null
			this._authenticated = false
			clearKerberoSecCredentials()
			await this.sendAuthStatusUpdate()
	...
	async handleAuthCallback(authorizationCode: string, provider: string): Promise<void> {
		try {
			// Exchange the authorization code for tokens using the KerberoSec API
			const apiBaseUrl = KerberoSecEnv.config().apiBaseUrl
			...
			const sessionStartedAtMs = Date.now()
			const authInfo: KerberoSecAuthInfo = {
				idToken: tokenData.accessToken,
				refreshToken: tokenData.refreshToken,
				userInfo: userInfo ?? {
					id: tokenData.userInfo.kerberosecUserId || "",
					email: tokenData.userInfo.email || "",
					displayName: tokenData.userInfo.name || "",
					createdAt: new Date().toISOString(),
					organizations: [],
	...
	async restoreRefreshTokenAndRetrieveAuthInfo(): Promise<void> {
		try {
			const creds = readKerberoSecCredentials()
			if (!creds) {
				this._authenticated = false
				this._kerberosecAuthInfo = null
				return
			...
			const restoredAuthInfo: KerberoSecAuthInfo = {
			...
			try {
				validCredentials = await this.resolveValidKerberoSecCredentials(restoredAuthInfo)
			} catch (error) {
			...
			if (this._authenticated && this._kerberosecAuthInfo) {
				const userInfo = await this.fetchUserInfoFromApi(this._kerberosecAuthInfo.idToken)
				if (userInfo) {
					this._kerberosecAuthInfo.userInfo = userInfo
				} else {
			...
			// Notify BannerService of auth change (mirrors classic AuthService)
			BannerService.onAuthUpdate(this._kerberosecAuthInfo?.userInfo?.id || null).catch((error) => {
			...
			telemetryService.captureAuthLoggedOut("kerberosec", LogoutReason.RESTORE_ERROR)
			this._authenticated = false
			this._kerberosecAuthInfo = null
		}
	}
	...
	async sendAuthStatusUpdate(): Promise<void> {
		...
		const authInfo = this._kerberosecAuthInfo
