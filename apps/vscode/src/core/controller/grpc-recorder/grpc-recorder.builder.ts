	...
	static getRecorder(controller: Controller): IRecorder {
		if (!GrpcRecorderBuilder.recorder) {
			GrpcRecorderBuilder.recorder = GrpcRecorder.builder()
				.enableIf(process.env.GRPC_RECORDER_ENABLED === "true" && process.env.KERBEROSEC_ENVIRONMENT === "local")
...
function testFilters(): GrpcRequestFilter[] {
	...
	return [
		...
		(req) => ["kerberosec.UiService", "kerberosec.McpService", "kerberosec.WebService"].includes(req.service),
		(req) =>
			[
