import type { AgentExtension, AgentTool } from "@kerberosec/shared";
import { createTool, zodToJsonSchema } from "@kerberosec/shared";
import { z } from "zod";
import {
...
- Use \`kind: "scheduled"\` only when the user explicitly asks KerberoSec to execute work once at a future time or on a recurrence. One-time schedules require an exact future ISO 8601 \`run_at\` with an offset or Z. Recurring schedules require a five-field \`cron_pattern\` and may include an IANA \`timezone\`.
...
Use the \`tasks\` tool only when the user explicitly asks KerberoSec to execute work once at a future time or on a recurrence. Always pass \`kind: "scheduled"\`.
