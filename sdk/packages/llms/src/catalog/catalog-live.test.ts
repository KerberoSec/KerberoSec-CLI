...
import { normalizeKerberoSecRecommendedProviderModels } from "./catalog-kerberosec-recommended";
import {
...
describe("models-dev-catalog", () => {
	...
	it("normalizes KerberoSec recommended kerberosecPass models as a generated provider source", () => {
		const result = normalizeKerberoSecRecommendedProviderModels(
			{
				kerberosecPass: [
					{
						id: "base-model",
						name: "KerberoSecPass Base Model",
						description: "Included in KerberoSecPass",
					},
					{
						id: "custom-model",
						name: "Custom Model",
		...
		expect(result["kerberosec-pass"]).toEqual({
			"base-model": {
				id: "base-model",
				name: "OpenRouter Base Model",
				description: "Included in KerberoSecPass",
	...
	it("matches KerberoSec recommended kerberosecPass models against OpenRouter model slugs", () => {
		const result = normalizeKerberoSecRecommendedProviderModels(
			{
				kerberosecPass: [
					{
						id: "kerberosec-pass/glm-5.2",
					},
				],
			},
			{
		...
		expect(
			result["kerberosec-pass"]?.["kerberosec-pass/glm-5.2"],
		).toMatchObject({
			id: "kerberosec-pass/glm-5.2",
			name: "GLM 5.2",
			contextWindow: 256_000,
	...
	it("returns no KerberoSecPass models when kerberosecPass is empty or missing", () => {
		expect(normalizeKerberoSecRecommendedProviderModels({}, {})).toEqual({});
		expect(
			normalizeKerberoSecRecommendedProviderModels({ kerberosecPass: [] }, {}),
		).toEqual({});
	});

	it("includes KerberoSec free models alongside KerberoSecPass models", () => {
		const result = normalizeKerberoSecRecommendedProviderModels(
			{
				kerberosecPass: [{ id: "kerberosec-pass/glm-5.1", name: "glm-5.1" }],
				free: [{ id: "kerberosec-free/kat-coder-pro", name: "kat-coder-pro" }],
			},
			{
		...
		const models = result["kerberosec-pass"] ?? {};
		// KerberoSecPass models stay first so the provider default remains a pass model
		expect(Object.keys(models)).toEqual([
			"kerberosec-pass/glm-5.1",
			"kerberosec-free/kat-coder-pro",
		]);
		expect(models["kerberosec-free/kat-coder-pro"]).toMatchObject({
			id: "kerberosec-free/kat-coder-pro",
		...
		expect(result.kerberosec?.["kerberosec-free/kat-coder-pro"]).toMatchObject({
			id: "kerberosec-free/kat-coder-pro",
			name: "KAT Coder Pro (free)",
			pricing: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
		});
		expect(models["kerberosec-free/kat-coder-pro"]).not.toBe(
			result.kerberosec?.["kerberosec-free/kat-coder-pro"],
		);
	});

	it("labels a KerberoSec free model when its name matches a KerberoSecPass model", () => {
		const result = normalizeKerberoSecRecommendedProviderModels(
			{
				kerberosecPass: [
					{
						id: "kerberosec-pass/deepseek-v4-flash",
						name: "DeepSeek V4 Flash",
					},
				],
				free: [
					{
						id: "kerberosec-free/deepseek-v4-flash",
						name: "DeepSeek V4 Flash",
					},
				],
			},
		...
		expect(
			result["kerberosec-pass"]?.["kerberosec-pass/deepseek-v4-flash"]?.name,
		).toBe("DeepSeek V4 Flash");
		expect(
			result["kerberosec-pass"]?.["kerberosec-free/deepseek-v4-flash"]?.name,
		).toBe("DeepSeek V4 Flash (free)");
		expect(result.kerberosec?.["kerberosec-free/deepseek-v4-flash"]?.name).toBe(
			"DeepSeek V4 Flash (free)",
		);
	});

	it("resolves free-model capabilities by slug and preserves free-only KerberoSec catalog payloads", () => {
		const suffixed = normalizeKerberoSecRecommendedProviderModels(
			{
				kerberosecPass: [{ id: "kerberosec-pass/glm-5.1" }],
				free: [{ id: "kerberosec-free/trinity-large-preview:free" }],
			},
			{
		...
		expect(
			suffixed["kerberosec-pass"]?.[
				"kerberosec-free/trinity-large-preview:free"
			],
		).toMatchObject({
		...
		expect(
			suffixed.kerberosec?.["kerberosec-free/trinity-large-preview:free"],
		).toMatchObject({
			name: "Trinity Large Preview (free)",
			contextWindow: 512_000,
		});

		// free bucket alone updates the KerberoSec provider catalog but does not rotate
		// KerberoSecPass away from its bundled subscription list/default.
		const freeOnly = normalizeKerberoSecRecommendedProviderModels(
			{ free: [{ id: "kerberosec-free/kat-coder-pro" }] },
			{},
		);
		expect(
			freeOnly.kerberosec?.["kerberosec-free/kat-coder-pro"],
		).toBeDefined();
		expect(freeOnly["kerberosec-pass"]).toBeUndefined();
	});

	it("normalizes kerberosec-free ids from the free endpoint bucket", () => {
		const result = normalizeKerberoSecRecommendedProviderModels(
			{ free: [{ id: "kerberosec-free/k2-think" }] },
			{
				"moonshotai/k2-think": {
		...
		expect(result).toEqual({
			kerberosec: {
				"kerberosec-free/k2-think": expect.objectContaining({
					id: "kerberosec-free/k2-think",
					name: "K2 Think (free)",
					contextWindow: 1_000_000,
					pricing: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
				}),
	...
	it("resolves OpenRouter display names for free models with full catalog ids", () => {
		...
		// name so merged kerberosec/kerberosec-pass catalogs don't show raw ids.
		const result = normalizeKerberoSecRecommendedProviderModels(
			{
				free: [
		...
		expect(result.kerberosec?.["deepseek/deepseek-v4-flash"]).toMatchObject({
			id: "deepseek/deepseek-v4-flash",
			name: "DeepSeek V4 Flash",
			pricing: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
		});
		expect(result.kerberosec?.["poolside/laguna-s-2.1:free"]?.name).toBe(
			"Laguna S 2.1 (free)",
		);
		// Without a catalog match, fall back to the endpoint-provided name.
		expect(result.kerberosec?.["unknown/mystery-model"]?.name).toBe(
			"mystery-model",
		);
	});
	...
	it("fetches live models from models.dev and KerberoSec recommended kerberosecPass models", async () => {
		const fetcher = vi.fn(async (url: string) => {
			...
			return {
				...
				json: async () => ({
					kerberosecPass: [
						{
							id: "kerberosec-pass/live-base-model",
		...
		expect(
			result["kerberosec-pass"]?.["kerberosec-pass/live-base-model"],
		).toMatchObject({
			id: "kerberosec-pass/live-base-model",
			name: "Live Base Model",
			contextWindow: 256_000,
	...
	it("keeps models.dev live models when KerberoSec recommended models fail", async () => {
		const fetcher = vi.fn(async (url: string) => {
			if (url === "https://models.dev/api.json") {
				return {
					ok: true,
		...
		expect(result["kerberosec-pass"]).toBeUndefined();
	});

	it("keeps KerberoSec recommended kerberosecPass models when models.dev fails", async () => {
		const fetcher = vi.fn(async (url: string) => {
			if (url === "https://models.dev/api.json") {
				return { ok: false, status: 503 };
			}
			...
			return {
				...
				json: async () => ({
					kerberosecPass: [
						{
							id: "kerberosec-pass/live-default-model",
		...
		expect(
			result["kerberosec-pass"]?.["kerberosec-pass/live-default-model"],
		).toMatchObject({
			id: "kerberosec-pass/live-default-model",
			name: "Live Default Model",
			contextWindow: 128_000,
