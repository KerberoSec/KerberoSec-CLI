...
function isBundledDaemonEntryInvocation(): boolean {
	...
	return (
		process.argv.includes("--kerberosec-hub-daemon") ||
		isBunEmbeddedEntry ||
...
function formatClientLabel(clientType: string | undefined): string {
	...
	if (normalized.includes("kerberosec")) {
		return "KerberoSec";
	}
	return normalized
		.split(/[-_\s]+/)
...
function summarizeClient(client: TrackedClient): {
	...
	if (
		...
		return {
			key: "code-app",
			label: "KerberoSec Desktop",
			name: "KerberoSec Desktop",
		};
	}
	return {
		key: client.clientId,
...
if (claimedDaemonSentinel || isBundledDaemonEntryInvocation()) {
	await import("@kerberosec/core/hub/daemon-entry");
} else {
	main().catch((err) => {
		const msg = err instanceof Error ? err.message : String(err);
