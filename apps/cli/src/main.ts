...
import type { ToolPolicy } from "@kerberosec/core";

import { registerDisposable } from "@kerberosec/shared";
...
async function createProviderSettingsManager() {
	const { ProviderSettingsManager } = await import("@kerberosec/core");
 ...
 * config directory before commander parses, because setKerberoSecDir() must run
...
function writePromptArgError(args: string[]): void {
	...
	writeErr(
		`Unknown command or unquoted prompt: ${renderedArgs}\nPrompt text must be passed as a single quoted argument, for example: kerberosec "fix the tests". Use "kerberosec --help" to see available commands and flags.`,
	);
}
...
export async function runCli(): Promise<void> {
	...
	const { setKerberoSecDir, setHomeDir } = await import(
		"@kerberosec/shared/storage"
	);
	if (configDir) {
		setKerberoSecDir(configDir);
	}
	setHomeDir(homedir());
	...
	const authCmd = program
		...
		.option(
			"--data-dir <dir>",
			"Use isolated local state at <dir> instead of ~/.kerberosec (enables sandbox mode)",
		)
		...
		.action(async (positionalProvider: string | undefined) => {
			...
			// The early pre-pass in runCli() also calls setKerberoSecDir(), but only
			...
			// settings manager is constructed against ~/.kerberosec.
			if (opts.config?.trim()) {
				const { setKerberoSecDir } = await import("@kerberosec/shared/storage");
				setKerberoSecDir(opts.config.trim());
			}
			// Honor --data-dir before constructing the provider settings manager
			// so writes land under the chosen data dir instead of ~/.kerberosec.
			configureSandboxEnvironment({
				enabled:
					!!opts.dataDir || process.env.KERBEROSEC_SANDBOX?.trim() === "1",
	...
	program
		.command("logout")
		.description("Sign out and clear stored authentication credentials")
		.option("--config <dir>", "configuration directory")
		.action(async (opts: { config?: string }) => {
			...
			const { logoutKerberoSecAccount } = await import(
				"./tui/kerberosec-account"
			);
			const providerSettingsManager = await createProviderSettingsManager();
			logoutKerberoSecAccount(providerSettingsManager);
	...
	const pluginCmd = program
		.command("plugin")
		.description("Manage KerberoSec Plugins")
		.action(() => {
			pluginCmd.help();
		});
	const pluginInstallCmd = pluginCmd
		...
		.alias("i")
		.description(
			"Install a KerberoSec Plugin from an official keyword, npm, git, URL, or a local path",
		)
		.argument(
			"<source>",
		...
		.option("--json", "Output as JSON")
		.option("--cwd <path>", "Install to <path>/.kerberosec/plugins")
	...
	const pluginUninstallCmd = pluginCmd
		...
		.alias("remove")
		.alias("rm")
		.description("Uninstall a KerberoSec Plugin by name or path")
		.argument("<name>", "plugin package name, installed slug, or plugin path")
		.option("--json", "Output as JSON")
		.option(
			"--cwd <path>",
			"Search <path>/.kerberosec/plugins before global plugins",
		)
	...
	const skillCmd = program
		.command("skill")
		.description(
			"Manage KerberoSec Skills via the open skills CLI (npx skills)",
		)
		.allowUnknownOption()
		.passThroughOptions()
		...
		.addHelpText(
			...
			"\nForwards to the open skills CLI via npx. Examples:\n" +
				"  kerberosec skill add <owner/repo>       Add a skill into KerberoSec\n" +
				"  kerberosec skill install <owner/repo>   Alias for add\n" +
				"  kerberosec skill list                   List installed skills\n" +
				"  kerberosec skill remove                 Remove installed skills\n" +
				"  kerberosec skill uninstall              Alias for remove\n" +
				"\nadd/install and remove/uninstall default to '--agent kerberosec' unless you pass your own --agent.\n" +
	...
	const connectCmd = program
		...
		.argument("[channel]", "Channel to connect KerberoSec CLI to")
	...
	const mcpCmd = program
		...
		.action(async () => {
			if (isFullTTY) {
				...
				writeln(
					"MCP wizard requires a TTY. Use kerberosec config mcp to list servers.",
				);
			}
		});
	...
	const dashboardCmd = program
		.command("dashboard")
		.description("Start the KerberoSec Hub dashboard and open it in a browser")
		.option("--config <dir>", "configuration directory")
	...
	program
		.command("version")
		.description("Show KerberoSec CLI version number")
		.action(async () => {
	...
	program
		...
		.passThroughOptions()
		.action(async (_opts: unknown, cmd: Command) => {
			const { createOllamaCommand } = await import("./commands/ollama");
			const providerSettingsManager = await createProviderSettingsManager();
			const ollamaCmd = createOllamaCommand(providerSettingsManager);
			await ollamaCmd.parseAsync(cmd.args, { from: "user" });
			ctx.exitCode =
	...
	if (args.id !== undefined) {
		...
		startupTarget = "chat";
		process.env.KERBEROSEC_HOOK_AGENT_RESUME = "1";
	} else {
		delete process.env.KERBEROSEC_HOOK_AGENT_RESUME;
	}
	if (startupTarget) {
		args = {
			...args,
	...
	if (args.hooksDir?.trim()) {
		process.env.KERBEROSEC_HOOKS_DIR = args.hooksDir.trim();
	}
	...
	// when KERBEROSEC_SANDBOX=1 is set in the environment (in which case the data
	// dir falls back to $KERBEROSEC_SANDBOX_DATA_DIR or /tmp/kerberosec-sandbox).
	const sandboxEnabled =
		!!args.dataDir || process.env.KERBEROSEC_SANDBOX?.trim() === "1";
	...
	try {
		const persistedKerberoSecAccountId = providerSettingsManager
			.getProviderSettings("kerberosec")
			?.auth?.accountId?.trim();
		if (persistedKerberoSecAccountId) {
			setCliFeatureFlagsAccountContext({ id: persistedKerberoSecAccountId });
		}
		refreshCliFeatureFlagsInBackground();
		...
		// Apply locally persisted KerberoSec account identity so subsequent events
		...
		// lightweight subcommand and pre-auth CLI flows. See KERBEROSEC-2406.
		...
		if (args.interactive || !args.prompt) {
			...
			const initialKerberoSecProviderSettings =
				provider === "kerberosec" ? selectedProviderSettings : undefined;
			let initialNotice:
			...
			if (
				...
				initialNotice = getKerberoSecCliMigrationNotice(
					undefined,
					process.env,
					{
			...
			await runInteractive(config, userInstructionService, resumeSessionId, {
				...
				kerberosecApiBaseUrl: initialKerberoSecProviderSettings?.baseUrl,
				kerberosecProviderSettings: initialKerberoSecProviderSettings,
				startupTarget,
				initialNotice,
