...
import type { UserInstructionConfigService } from "@kerberosec/core";
...
describe("interactive config data loader", () => {
	...
	afterEach(async () => {
		if (envSnapshot.KERBEROSEC_GLOBAL_SETTINGS_PATH === undefined) {
			delete process.env.KERBEROSEC_GLOBAL_SETTINGS_PATH;
		} else {
			process.env.KERBEROSEC_GLOBAL_SETTINGS_PATH =
				envSnapshot.KERBEROSEC_GLOBAL_SETTINGS_PATH;
		}
		if (envSnapshot.KERBEROSEC_MCP_SETTINGS_PATH === undefined) {
			delete process.env.KERBEROSEC_MCP_SETTINGS_PATH;
		} else {
			process.env.KERBEROSEC_MCP_SETTINGS_PATH =
				envSnapshot.KERBEROSEC_MCP_SETTINGS_PATH;
		}
		await Promise.all(
	...
	async function writeSettingsPlugin(tempRoot: string): Promise<string> {
		const pluginsDir = join(tempRoot, ".kerberosec", "plugins");
...
Find installable skills.`,
	...
	it("keeps plugin tool toggle behavior", async () => {
		...
		const settings = JSON.parse(
			await readFile(process.env.KERBEROSEC_GLOBAL_SETTINGS_PATH, "utf8"),
...
Erase stale plugin commands.`,
	...
	it("uses the package name for package-backed plugin entries", async () => {
		...
		await writeFile(
			...
			JSON.stringify(
				{
					name: "kerberosec-sdk-portable-agents",
					kerberosec: {
						plugins: [{ paths: ["./index.ts"] }],
					},
				},
		...
		expect(plugin?.name).toBe("kerberosec-sdk-portable-agents");
	});
	...
	it("marks bundled package skills with their plugin owner", async () => {
		...
		await writeFile(
			...
			JSON.stringify(
				{
					name: "kerberosec-installed-plugin-demo",
					kerberosec: {
...
Review with the bundled skill.`,
		...
		expect(skill).toMatchObject({
			name: "review",
			pluginName: "kerberosec-sdk-portable-agents",
			pluginPath,
			source: "workspace-plugin",
		});
	});
	...
	it.skipIf(process.platform === "win32")(
		...
		async () => {
			...
			process.env.KERBEROSEC_GLOBAL_SETTINGS_PATH = globalSettingsPath;
			process.env.KERBEROSEC_MCP_SETTINGS_PATH = settingsPath;
			const pluginPath = await writeMcpSettingsPlugin(tempRoot);
			await writeFile(
				settingsPath,
