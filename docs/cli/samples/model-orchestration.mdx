...
KerberoSec CLI's `--config` and `--thinking` flags enable sophisticated multi-model workflows. Instead of using a single model for all tasks, you can route different work to different models based on cost, capability, and specialization.
...
See our production GitHub Actions workflow that uses KerberoSec CLI for automated PR reviews: [kerberosec-pr-review.yml](https://github.com/kerberosec/kerberosec/blob/main/.github/workflows/kerberosec-pr-review.yml)
...
SUMMARY=$(echo "$ISSUE_CONTENT" | kerberosec --auto-approve true --config ~/.kerberosec-haiku \
  "summarize this issue in 2-3 sentences")

# Phase 2: Detailed plan with expensive model + thinking
PLAN=$(echo "$SUMMARY" | kerberosec --auto-approve true --thinking high --config ~/.kerberosec-opus \
  "create detailed implementation plan with edge cases")

# Phase 3: Execute with mid-tier model
echo "$PLAN" | kerberosec --auto-approve true --config ~/.kerberosec-sonnet \
  "implement the plan from above"
```

<Note>
Each `kerberosec` invocation needs to complete before passing output to the next phase. Use shell variables to store intermediate results rather than piping `kerberosec` commands directly.
</Note>
...
mkdir -p ~/.kerberosec-haiku ~/.kerberosec-sonnet ~/.kerberosec-opus

# Configure each with different models
kerberosec --config ~/.kerberosec-haiku auth anthropic --modelid claude-haiku-4-20250514
kerberosec --config ~/.kerberosec-sonnet auth anthropic --modelid claude-sonnet-4-20250514  
kerberosec --config ~/.kerberosec-opus auth anthropic --modelid claude-opus-4-5-20251101

# Or use different providers entirely
kerberosec --config ~/.kerberosec-gemini auth gemini --modelid gemini-2.0-flash-exp
kerberosec --config ~/.kerberosec-codex auth openai-codex --modelid gpt-5-latest
```

Now you can switch models per-task with `--config`:

```bash
kerberosec --config ~/.kerberosec-haiku "quick task"
kerberosec --config ~/.kerberosec-opus "complex reasoning task"
```
...
git show | kerberosec --auto-approve true --config ~/.kerberosec-gemini "review and save to gemini-review.md" &
git show | kerberosec --auto-approve true --config ~/.kerberosec-codex "review and save to codex-review.md" &
git show | kerberosec --auto-approve true --config ~/.kerberosec-opus "review and save to opus-review.md" &

# Wait for all to complete
wait

# Synthesize
cat *-review.md | kerberosec --auto-approve true "create consensus review"
```

<Note>
Parallel execution requires managing multiple KerberoSec instances. See [Multi-instance workflows](/usage/cli-overview#automation-patterns) for details.
</Note>

## Extended Thinking for Complex Tasks

Use the `--thinking` flag when KerberoSec needs to analyze multiple approaches:

```bash
# Without thinking: Fast but may miss nuances
kerberosec --auto-approve true "refactor this codebase"

# With thinking: Slower but more thorough
kerberosec --auto-approve true --thinking high \
...
gh pr view $PR | kerberosec --auto-approve true --config ~/.kerberosec-haiku \
    "list all issues to fix, output as JSON"

# Opus with thinking: Deep analysis only if issues found
if [ -s issues.json ]; then
    kerberosec --auto-approve true --thinking high --config ~/.kerberosec-opus \
...
git diff main | kerberosec --auto-approve true --config ~/.kerberosec-gemini "security review" > gemini-sec.md &
git diff main | kerberosec --auto-approve true --config ~/.kerberosec-opus "security review" > opus-sec.md &
git diff main | kerberosec --auto-approve true --config ~/.kerberosec-codex "security review" > codex-sec.md &
wait

# High-priority: Issues all 3 models found
cat *-sec.md | kerberosec --auto-approve true "find security issues all 3 reviews mentioned"
```
...
<Columns cols={2}>
  ...
  <Card title="Headless Mode" icon="robot" href="/usage/cli-overview#headless-mode">
    Run KerberoSec autonomously in scripts, CI/CD pipelines, and automated workflows.
  </Card>
  
  <Card title="KerberoSec Provider" icon="brain" href="/getting-started/kerberosec-provider">
  ...
  <Card title="CI/CD Integration" icon="github" href="/cli/samples/github-integration">
    Automate GitHub workflows with KerberoSec CLI
  </Card>
</Columns>
