...
describe("provider-ids", () => {
	...
	it("registers KerberoSecPass as a distinct KerberoSec-compatible built-in provider", async () => {
		expect(BUILT_IN_PROVIDER_IDS).toContain("kerberosec-pass");
		...
		for (const providerId of ["kerberosec", "kerberosec-pass"]) {
			const registration = BUILTIN_PROVIDER_REGISTRATIONS.find(
				(item) => item.manifest.id === providerId,
			);
			await expect(registration?.loadProvider?.()).resolves.toMatchObject({
				createProvider: createKerberoSecProvider,
			});
		}
	});
