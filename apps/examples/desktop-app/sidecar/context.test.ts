...
import type { RuntimeCapabilities } from "@kerberosec/core";
	...
	it("registers the desktop capability factory with core", async () => {
		...
		expect(nodeHubClientCtorMock).toHaveBeenCalledWith(
			expect.objectContaining({
				...
				displayName: "KerberoSec Desktop observer",
			}),
		);
	});
...
describe("disposeSidecarContext attachment cleanup", () => {
	...
	beforeEach(() => {
		previousSessionDataDir = process.env.KERBEROSEC_SESSION_DATA_DIR;
		testSessionDataDir = join(
			tmpdir(),
			`kerberosec-desktop-dispose-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
