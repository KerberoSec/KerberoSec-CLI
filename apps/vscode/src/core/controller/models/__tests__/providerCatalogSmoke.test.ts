...
describe("provider model catalog backend smoke", () => {
	...
	beforeAll(async () => {
		kerberosecDir = await fs.mkdtemp(path.join(os.tmpdir(), "kerberosec-provider-catalog-smoke-"))
		originalKerberoSecDir = process.env.KERBEROSEC_DIR
		process.env.KERBEROSEC_DIR = kerberosecDir
		await StateManager.initialize(createStorageContext({ kerberosecDir, workspacePath: kerberosecDir }))
	...
	afterAll(async () => {
		...
		await fs.rm(kerberosecDir, { recursive: true, force: true })
	})
