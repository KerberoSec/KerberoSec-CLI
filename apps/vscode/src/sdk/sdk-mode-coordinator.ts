import { getProviderAuthStorageId } from "@kerberosec/core"
import { createModeSwitchNoticeTracker, type ModeSwitchNotice, type ModeSwitchNoticeTracker } from "@kerberosec/shared"
import type { ChatContent } from "@shared/ChatContent"
import type { KerberoSecMessage, TurnPhase } from "@shared/ExtensionMessage"
...
export class SdkModeCoordinator {
	 ...
	 * CLI's round-trip-cancelling tracker (@kerberosec/shared), scoped to the session
	...
	async togglePlanActMode(modeToSwitchTo: Mode, chatContent?: ChatContent): Promise<boolean> {
		...
		if (activeSession) {
			...
			const latestAssistantResult = [...kerberosecMessages]
				.reverse()
				.find(
					(message) =>
						message.type === "say" &&
	...
	private async performRebuildSessionForMode(
		...
		try {
			...
			if (usesKerberoSecAccountAuth(config.providerId) && !config.apiKey) {
				Logger.warn(
					`[SdkController] Mode rebuild: new mode '${newMode}' provider is '${config.providerId}' but no KerberoSec auth token - emitting auth error`,
				)
				...
				this.options.emitKerberoSecAuthError()
				await this.options.postStateToWebview()
				return false
			}
			...
			if (options.autoContinue) {
				...
				if (userPrompt || userImages?.length || userFiles?.length) {
					const userMessage: KerberoSecMessage = {
						ts: Date.now(),
						type: "say",
						say: "user_feedback",
						text: userPrompt ?? "",
			...
			// messages (kerberosecMessages ride on the state post), and auto-continue
			...
			const errorMessage: KerberoSecMessage = {
				ts: Date.now(),
				type: "say",
				say: "error",
	...
	private async cancelRunningTurnForModeChange(oldManager: SdkSessionHost, oldSessionId: string): Promise<void> {
		...
		if (task?.messageStateHandler) {
			const current = task.messageStateHandler.getKerberoSecMessages()
		...
		const resumeMessage: KerberoSecMessage = {
			ts: Date.now(),
			type: "ask",
			ask: "resume_task",
			text: "",
