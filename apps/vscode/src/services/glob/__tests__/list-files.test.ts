...
describe("listFiles", () => {
	const tmpDir = path.join(os.tmpdir(), `kerberosec-list-files-test-${Math.random().toString(36).slice(2)}`)
...
describe("listFiles gitignore handling", () => {
	...
	const baseDir = path.join(os.tmpdir(), `kerberosec-gitignore-test-${Math.random().toString(36).slice(2)}`)
