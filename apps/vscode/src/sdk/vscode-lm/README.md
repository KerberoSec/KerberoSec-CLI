...
Routes KerberoSec inference through the [VS Code Language Model API](https://code.visualstudio.com/api/extension-guides/language-model)
(`vscode.lm`), letting KerberoSec use models contributed by any extension that
...
- **`vscode-lm-handler.ts`** — `VsCodeLmHandler`, a KerberoSec SDK `ApiHandler`
  (`@kerberosec/llms`) backed by `vscode.lm`. Selects a chat model, streams the
...
The SDK's `@kerberosec/llms` handler registry (`registerHandler`) exists for providers
