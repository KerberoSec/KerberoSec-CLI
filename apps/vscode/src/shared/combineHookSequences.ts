import { KerberoSecMessage } from "./ExtensionMessage"

/**
 * Hook metadata extracted from hook message text.
 * Mirrors the KerberoSecSayHook interface but represents parsed data.
 */
...
function getSay(msg: KerberoSecMessage): string | undefined {
 ...
 */
function isToolOrCommandMessage(msg: KerberoSecMessage): boolean {
 ...
 */
function parseHookMetadata(hookMessage: KerberoSecMessage): HookMetadata | null {
 ...
 */
function dedupeToolOrCommandMessagesByTimestamp(messages: KerberoSecMessage[]): KerberoSecMessage[] {
...
function combineHookWithOutputs(
	hookMessage: KerberoSecMessage,
	startIndex: number,
	messages: KerberoSecMessage[],
): { combined: KerberoSecMessage; nextIndex: number } {
 ...
 */
function combineAllHooks(messages: KerberoSecMessage[]): KerberoSecMessage[] {
	// Pass 1: Build map of combined hooks by timestamp
	const combinedHooksByTs = new Map<number, KerberoSecMessage>()
	...
	const result: KerberoSecMessage[] = []
 ...
 */
function findImmediateNextToolTimestamp(hookIndex: number, messages: KerberoSecMessage[]): number | null {
 ...
 */
function buildPreToolUseMap(processedMessages: KerberoSecMessage[], originalMessages: KerberoSecMessage[]): Map<number, KerberoSecMessage[]> {
	const map = new Map<number, KerberoSecMessage[]>()
 ...
 */
function reorderWithPreToolUseHooks(messages: KerberoSecMessage[], preToolUseMap: Map<number, KerberoSecMessage[]>): KerberoSecMessage[] {
	const result: KerberoSecMessage[] = []
 ...
 *
 * @param messages Array of KerberoSecMessage objects to process
 * @returns New array with hooks combined and PreToolUse hooks reordered
 */
export function combineHookSequences(messages: KerberoSecMessage[]): KerberoSecMessage[] {
