...
import type { BasicLogger, RuntimeLoggerConfig } from "@kerberosec/core";
import { resolveKerberoSecDataDir } from "@kerberosec/core";
...
function normalizeRuntimeConfig(input: {
	...
	const enabledEnv = process.env.KERBEROSEC_LOG_ENABLED?.trim();
	const enabled =
		base?.enabled ??
		!(enabledEnv === "0" || enabledEnv?.toLowerCase() === "false");
	const level = normalizeLogLevel(
		base?.level ?? process.env.KERBEROSEC_LOG_LEVEL,
	);
	const destination =
	...
	const name =
		base?.name?.trim() ||
		process.env.KERBEROSEC_LOG_NAME?.trim() ||
