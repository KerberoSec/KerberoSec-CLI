...
describe("ProviderSettingsManager", () => {
	...
	it("resolves auth storage settings for providers registered with a storage provider id", () => {
		...
		expect(manager.getProviderSettings("kerberosec-pass")).toEqual({
		...
		expect(manager.getProviderConfig("kerberosec-pass")).toMatchObject({
	...
	it("falls back to kerberosec when last-used provider is kerberosec-pass and the feature is disabled", () => {
		const tempDir = mkdtempSync(
			path.join(os.tmpdir(), "core-provider-settings-"),
		);
		tempDirs.push(tempDir);
		...
		expect(
			manager.getLastUsedProviderSettings({ isKerberoSecPassEnabled: false }),
		).toEqual({
			provider: "kerberosec",
			model: "anthropic/claude-sonnet-4.6",
			baseUrl: "https://api.example.test",
		...
		expect(
			manager.getLastUsedProviderConfig({ isKerberoSecPassEnabled: false }),
		).toMatchObject({
	...
	it("returns default kerberosec settings when kerberosec-pass is last-used and no kerberosec settings exist", () => {
		const tempDir = mkdtempSync(
			path.join(os.tmpdir(), "core-provider-settings-"),
		);
		tempDirs.push(tempDir);
