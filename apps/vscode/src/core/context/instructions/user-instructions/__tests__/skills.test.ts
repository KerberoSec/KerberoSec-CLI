...
describe("Skills Utility Functions", () => {
	...
	const GLOBAL_SKILLS_DIR = path.join("/home", "user", ".kerberosec", "skills")
	...
	beforeEach(() => {
		...
		getSkillsDirectoriesForScanStub.returns([
			{ path: path.join(TEST_CWD, ".kerberosecrules", "skills"), source: "project" },
			{ path: path.join(TEST_CWD, ".kerberosec", "skills"), source: "project" },
...
Instructions here`)
		...
		// Regression test for https://github.com/kerberosec/kerberosec/issues/12151:
...
This is a test skill.`)
		...
		it("should discover skills from project .kerberosecrules/skills directory", async () => {
			const projectSkillsDir = path.join(TEST_CWD, ".kerberosecrules", "skills")
...
Use analogies and ASCII diagrams when explaining code.`)
		...
		it("should discover skills from project .kerberosec/skills directory", async () => {
			const kerberosecSkillsDir = path.join(TEST_CWD, ".kerberosec", "skills")
			const skillDir = path.join(kerberosecSkillsDir, "debugging")
			const skillMdPath = path.join(skillDir, "SKILL.md")

			fileExistsStub.withArgs(kerberosecSkillsDir).resolves(true)
			fileExistsStub.withArgs(skillMdPath).resolves(true)
			isDirectoryStub.withArgs(kerberosecSkillsDir).resolves(true)
			readdirStub.withArgs(kerberosecSkillsDir).resolves(["debugging"])
			statStub.withArgs(skillDir).resolves({ isDirectory: () => true })
			readFileStub.withArgs(skillMdPath, "utf-8").resolves(`---
name: debugging
	...
	describe("Remote Skills", () => {
		...
		describe("Override resolution (remote > disk-global > project)", () => {
			...
			it("remote overrides project skill of same name", async () => {
				const entries = [makeEntry("coding", "Remote coding")]
				const projDir = path.join(TEST_CWD, ".kerberosecrules", "skills")
...
describe("setSkillDisabledInFrontmatter", () => {
	...
	it("writes disabled: true to the SKILL.md when disabling a disk skill", async () => {
		const skillPath = path.join("/home", "user", ".kerberosec", "skills", "s", "SKILL.md")
