...
const {
	...
	resolveKerberoSecDataDir: vi.fn(() => "/tmp/kerberosec-data"),
	...
	KERBEROSEC_RUN_AS_HUB_DAEMON_ENV: "KERBEROSEC_RUN_AS_HUB_DAEMON",
}));

const originalRunAsHubDaemon = process.env[KERBEROSEC_RUN_AS_HUB_DAEMON_ENV];
const originalConnectorCliLaunch = process.env.KERBEROSEC_CONNECTOR_CLI_LAUNCH;
...
vi.mock("@kerberosec/shared", () => ({
	KERBEROSEC_RUN_AS_HUB_DAEMON_ENV,
	KERBEROSEC_HUB_PORT: 25463,
	KERBEROSEC_HUB_DEV_PORT: 25466,
	...
	isHubDaemonProcess: (env: NodeJS.ProcessEnv = process.env) =>
		env[KERBEROSEC_RUN_AS_HUB_DAEMON_ENV] === "1",
	resolveKerberoSecBuildEnv: () => "production",
	withResolvedKerberoSecBuildEnv: (env: NodeJS.ProcessEnv) => env,
}));
...
describe("ensureDetachedHubServer", () => {
	...
	afterEach(() => {
		...
		if (originalRunAsHubDaemon === undefined) {
			...
			process.env[KERBEROSEC_RUN_AS_HUB_DAEMON_ENV] = originalRunAsHubDaemon;
		}
		if (originalConnectorCliLaunch === undefined) {
			delete process.env.KERBEROSEC_CONNECTOR_CLI_LAUNCH;
		} else {
			process.env.KERBEROSEC_CONNECTOR_CLI_LAUNCH = originalConnectorCliLaunch;
		}
	});

	it("does not use port 0 for default production startup", async () => {
		process.env.KERBEROSEC_CONNECTOR_CLI_LAUNCH = JSON.stringify({
			launcher: "bun",
		...
		expect(spawnOptions?.env?.[KERBEROSEC_RUN_AS_HUB_DAEMON_ENV]).toBe("1");
		expect(spawnOptions?.env?.KERBEROSEC_CONNECTOR_CLI_LAUNCH).toBe(
			process.env.KERBEROSEC_CONNECTOR_CLI_LAUNCH,
		);
	});
	...
	it("retries a transient ETXTBSY spawn failure while starting the detached daemon", async () => {
		...
		try {
			const textFileBusy = Object.assign(
				new Error(
					"ETXTBSY: text file is busy, posix_spawn '/usr/local/bin/kerberosec'",
				),
				{ code: "ETXTBSY" },
			);
			spawn
	...
	it("throws a targeted error when an incompatible hub cannot be retired", async () => {
		...
		try {
			...
			const pending = expect(
				...
				"An incompatible KerberoSec Hub is already running at ws://127.0.0.1:25463/hub and could not be retired automatically.",
			);
			await vi.runAllTimersAsync();
	...
	it("throws when a compatible expected hub has no discovery record", async () => {
		...
		await expect(ensureDetachedHubServer("/workspace")).rejects.toThrow(
			"A compatible KerberoSec Hub is already running at ws://127.0.0.1:25463/hub, but its discovery record is missing or unreadable and no usable auth token is available.",
		);
		expect(spawn).not.toHaveBeenCalled();
	});
