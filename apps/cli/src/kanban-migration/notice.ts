...
const NOTICE_ID = "kerberosec-cli-kerberosec-pass-intro";
const FORCE_NOTICE_ENV = "KERBEROSEC_FORCE_KERBEROSEC_PASS_NOTICE";
const DISABLE_NOTICE_ENV = "KERBEROSEC_DISABLE_KERBEROSEC_PASS_NOTICE";
...
export function shouldSuppressKerberoSecCliMigrationNoticeForActiveProvider(
	activeProviderId: string | undefined,
	env: NodeJS.ProcessEnv = process.env,
): boolean {
	return (
		activeProviderId?.trim() === "kerberosec-pass" && !isForceNoticeEnabled(env)
	);
}

export function resolveCliNoticeStatePath(
	dataDir = resolveKerberoSecDataDir(),
): string {
	return join(dataDir, "settings", "cli-notices.json");
}

export function getKerberoSecCliMigrationNotice(
	dataDir = resolveKerberoSecDataDir(),
	env: NodeJS.ProcessEnv = process.env,
...
export function markKerberoSecCliMigrationNoticeShown(
	dataDir = resolveKerberoSecDataDir(),
): void {
	const noticePath = resolveCliNoticeStatePath(dataDir);
	const noticeState = readNoticeState(noticePath);
