...
import type { KerberoSecCoreStartInput } from "./kerberosec-core/types";
import type {
	StartSessionInput,
	StartSessionResult,
...
import type { AgentResult } from "@kerberosec/shared";
import { KerberoSecCore } from "./KerberoSecCore";
import { NoOpFeatureFlagsProvider } from "./services/feature-flags";

function createStartInput(): KerberoSecCoreStartInput {
	return {
		config: {
...
describe("KerberoSecCore", () => {
	...
	it("keeps raw reads canonical and offers an explicit display projection", async () => {
		...
		const core = await KerberoSecCore.create();
	...
	it("compares a checkpoint to the current workspace through the public SDK API", async () => {
		const dir = mkdtempSync(join(tmpdir(), "kerberosec-core-compare-"));
		let core: KerberoSecCore | undefined;
		try {
			git(dir, ["init", "-b", "main"]);
			...
			core = await KerberoSecCore.create();
	...
	it("applies start-session bootstraps before delegating to the host", async () => {
		...
		const applyToStartSessionInput = vi.fn(
			async (input: KerberoSecCoreStartInput) => ({
				...input,
				config: {
					...input.config,
	...
	it("prefers the per-session telemetry service over the KerberoSecCore one", async () => {
		const host = {
			runtimeAddress: undefined,
	...
	it("can list sessions without hydrating message history", async () => {
		...
		// KerberoSecCore/listSessionHistory; the host list contract only receives the
		// numeric scan limit.
	...
	it("exposes event automation through KerberoSecCore instead of CronService", async () => {
		const root = mkdtempSync(join(tmpdir(), "kerberosec-core-automation-"));
		const cronDir = join(root, ".kerberosec", "cron");
		const reportsDir = join(cronDir, "reports");
		const dbPath = join(root, ".kerberosec", "data", "db", "cron.db");
