...
import { getCliSubscriptionUrl } from "../utils/kerberosec-pass-errors";
...
vi.mock(
	"@kerberosec/core",
	async (importActual: () => Promise<typeof import("@kerberosec/core")>) => ({
		...(await importActual()),
		getKerberoSecOrgIndividualInferenceSubscriptionMessage: () =>
			KERBEROSEC_ORG_INDIVIDUAL_INFERENCE_SUBSCRIPTION_MESSAGE,
		getKerberoSecPassSubscriptionUrl: () => KERBEROSEC_PASS_SUBSCRIPTION_URL,
		isKerberoSecNotSubscribedError: (error: unknown) =>
			error instanceof Error && error.name === "KerberoSecNotSubscribedError",
		isKerberoSecNotSubscribedMessage: (text: string) =>
			text
				.toLowerCase()
				.includes("the user is not subscribed to required model plan"),
		isKerberoSecOrgIndividualInferenceSubscriptionError: (error: unknown) =>
			error instanceof Error &&
			error.name === "KerberoSecOrgIndividualInferenceSubscriptionError",
		isKerberoSecOrgIndividualInferenceSubscriptionMessage: (text: string) =>
			text
				.toLowerCase()
				.includes(
					"organization accounts cannot use individual model inference subscriptions",
				),
		isKerberoSecPassLimitError: (error: unknown) =>
			error instanceof Error && error.name === "KerberoSecPassLimitError",
		isKerberoSecPassLimitMessage: (text: string) => {
			const normalized = text.toLowerCase();
			return (
				normalized.includes("you have reached your") &&
				normalized.includes("kerberosecpass limit") &&
...
vi.mock("./interactive-welcome", () => ({
	resolveKerberoSecWelcomeLine: vi.fn(async () => undefined),
}));
...
describe("runAgent", () => {
	...
	it("renders KerberoSecPass subscription errors with friendly copy when startup throws", async () => {
		const error = new Error(SDK_KERBEROSEC_PASS_SUBSCRIPTION_MESSAGE);
		...
		expect(outputMocks.writeErr).toHaveBeenCalledWith(
			CLI_KERBEROSEC_PASS_SUBSCRIPTION_MESSAGE,
		);
	});
	...
	it("emits JSON error lines for non-completed results", async () => {
		...
		sessionManagerMocks.start.mockResolvedValue({
			...
			result: {
				text: 'Missing API key for provider "kerberosec".',
				usage: {
					inputTokens: 0,
					outputTokens: 0,
					cacheReadTokens: 0,
		...
		expect(process.exitCode).toBe(1);
		expect(outputMocks.writeErr).toHaveBeenCalledWith(
			'Missing API key for provider "kerberosec".',
		);
	});

	it("renders KerberoSecPass subscription errors with friendly copy for failed results", async () => {
		const startedAt = new Date("2026-03-22T00:00:00.000Z");
		const endedAt = new Date("2026-03-22T00:00:01.000Z");
		sessionManagerMocks.start.mockResolvedValue({
			sessionId: "session-1",
			...
			result: {
				text: SDK_KERBEROSEC_PASS_SUBSCRIPTION_MESSAGE,
				usage: {
					inputTokens: 0,
					outputTokens: 0,
					cacheReadTokens: 0,
				...
				iterations: 1,
				finishReason: "error",
				model: { id: "premium-model", provider: "kerberosec-pass", info: {} },
				startedAt,
				endedAt,
				durationMs: 1000,
			},
	...
	it("does not duplicate KerberoSecPass subscription errors already displayed by agent events", async () => {
		const startedAt = new Date("2026-03-22T00:00:00.000Z");
		const endedAt = new Date("2026-03-22T00:00:01.000Z");
		sessionManagerMocks.start.mockImplementation(async () => {
			sessionEventsMocks.listener?.({
				type: "error",
				error: new Error(SDK_KERBEROSEC_PASS_SUBSCRIPTION_MESSAGE),
				recoverable: false,
			});
	...
	it("formats KerberoSecPass limit errors with usage-based billing guidance", async () => {
		const startedAt = new Date("2026-03-22T00:00:00.000Z");
		const endedAt = new Date("2026-03-22T00:00:01.000Z");
		sessionManagerMocks.start.mockResolvedValue({
			sessionId: "session-1",
			...
			manifest: { session_id: "session-1" },
			result: {
				text: `Error: ${KERBEROSEC_PASS_LIMIT_DETAIL_MESSAGE}`,
				usage: {
					inputTokens: 0,
					outputTokens: 0,
					cacheReadTokens: 0,
		...
		expect(process.exitCode).toBe(1);
		expect(outputMocks.writeErr).toHaveBeenCalledWith(
			CLI_KERBEROSEC_PASS_LIMIT_MESSAGE,
		);
	});

	it("does not duplicate KerberoSecPass limit errors already displayed by agent events", async () => {
		const startedAt = new Date("2026-03-22T00:00:00.000Z");
		const endedAt = new Date("2026-03-22T00:00:01.000Z");
		sessionManagerMocks.start.mockImplementation(async () => {
			sessionEventsMocks.listener?.({
				type: "error",
				error: new Error(`Error: ${KERBEROSEC_PASS_LIMIT_DETAIL_MESSAGE}`),
				recoverable: false,
			});
	...
	it("zeros KerberoSec free model costs in JSON results and agent events", async () => {
		const startedAt = new Date("2026-03-22T00:00:00.000Z");
		const endedAt = new Date("2026-03-22T00:00:01.000Z");
		vi.stubGlobal(
			"fetch",
