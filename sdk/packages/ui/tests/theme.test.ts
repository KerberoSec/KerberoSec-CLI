...
describe("@kerberosec/ui theme contract", () => {
	...
	it("maps readable roles onto the palettes and preserves shadcn names", () => {
		...
		expect(tokens).toContain('@import "@kerberosec/ui/theme/palette.css";');
		expect(tokens).not.toContain("--kerberosec-");
		for (const token of [
			...visualRoleTokens,
			...statusRoleTokens,
			...semanticTokens,
	...
	it("generates a fully scoped palette and semantic theme", () => {
		...
		expect(scoped).toContain(".kerberosec-ui-theme");
		expect(scoped).toContain(".dark .kerberosec-ui-theme");
		expect(scoped).toContain(".kerberosec-ui-theme.dark");
		for (const paletteName of paletteNames) {
	...
	it("provides Tailwind mappings without registering raw palette steps", () => {
		...
		for (const token of mappedColorTokens) {
			...
			expect(componentTheme).toMatch(
				new RegExp(
					`--color-kerberosec-ui-${token}:\\s*var\\(\\s*--${token}\\s*\\);`,
				),
			);
		}
		...
		expect(componentTheme).toContain("@custom-variant kerberosec-ui-dark");
		expect(componentTheme).not.toContain("@custom-variant dark ");
		expect(componentTheme).toContain(
			"--font-weight-kerberosec-ui-medium: var(--font-weight-medium);",
		);
		expect(componentTheme).toContain(
			"--text-kerberosec-ui-xs: var(--text-xs);",
		);
		expect(componentTheme).toContain(
			"--text-kerberosec-ui-md: var(--text-md);",
		);
		expect(componentTheme).toContain(
			"--radius-kerberosec-ui-lg: var(--radius-lg);",
		);
		expect(componentTheme).not.toMatch(
			/--color-kerberosec-ui-(?:neutral|accent|error|success|warning|info)-(?:a)?\d+:/,
		);
		expect(components).toContain('@import "./theme/component-theme.css";');
		expect(base).toContain(
			'@import "@kerberosec/ui/components/markdown.css" layer(components);',
		);
		expect(markdown).toContain(":is(.markdown, .kerberosec-markdown)");
	...
	it("keeps embedded component utility names host-safe", () => {
		...
		expect(componentSources).not.toMatch(/(?<!kerberosec-ui-)dark:/);
		expect(componentSources).toContain("kerberosec-ui-dark:");
	});
