...
import type { KerberoSecAccountOrganization } from "@kerberosec/core";
...
type AccountState =
	...
	| { status: "loaded"; snapshot: KerberoSecAccountSnapshot }
...
function userInitial(snapshot: KerberoSecAccountSnapshot): string {
	const candidate =
...
function organizationDescription(org: KerberoSecAccountOrganization): string {
...
export function AccountDialogContent(
	...
	const reload = useCallback(async () => {
		...
		try {
			...
			if (generation.current === currentGeneration) {
				setState({
					status: isKerberoSecAccountAuthErrorMessage(message)
						? "unauthenticated"
						: "error",
					message,
				});
	...
	return (
		<box flexDirection="column" paddingX={1} gap={1}>
			...
			<box flexDirection="column" border borderColor="gray" paddingX={1}>
				...
				<AccountField
					label="Credits"
					value={formatKerberoSecCredits(loaded.displayedBalance)}
				/>
				{loaded.activeOrganization && (
					<AccountField
						label="Personal"
						value={formatKerberoSecCredits(loaded.balance.balance)}
					/>
				)}
				<AccountField
					label="Organizations"
