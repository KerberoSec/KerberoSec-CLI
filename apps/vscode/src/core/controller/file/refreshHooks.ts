import { HookInfo, HooksToggles, WorkspaceHooks } from "@shared/proto/kerberosec/file"
...
export async function refreshHooks(
	...
	const globalHooksDir = globalHooksDirOverride || path.join(os.homedir(), "Documents", "KerberoSec", "Hooks")
	...
	for (const workspacePath of workspacePaths.paths) {
		const workspaceHooksDir = path.join(workspacePath, ".kerberosecrules", "hooks")
