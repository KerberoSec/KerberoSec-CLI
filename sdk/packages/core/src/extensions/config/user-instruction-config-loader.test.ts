	...
	it("resolves legacy-compatible default search paths", () => {
		...
		expect(resolveSkillsConfigSearchPaths(workspacePath)).toEqual(
			expect.arrayContaining([
				join(workspacePath, ".kerberosecrules", "skills"),
				join(workspacePath, ".kerberosec", "skills"),
		...
		expect(resolveRulesConfigSearchPaths(workspacePath)).toEqual(
			expect.arrayContaining([
				...
				join(workspacePath, ".kerberosecrules"),
				join(workspacePath, ".kerberosec", "rules"),
			]),
		);
		expect(
		...
		expect(paths).toContain(join(workspacePath, ".kerberosec", "workflows"));
		expect(
			paths.some(
				(p) =>
					p.includes("Documents") &&
					p.includes("KerberoSec") &&
					p.includes("Workflows"),
			),
		).toBe(true);
		expect(paths).not.toContain(
			join(process.env.HOME ?? "~", ".kerberosec", "data", "workflows"),
		);
	});

	it("discovers managed plugin instruction roots from workspace .kerberosec manifests", () => {
		const workspacePath = "/repo/demo";
		expect(
			createSkillsConfigDefinition({ workspacePath }).directories,
		).toContain(join(workspacePath, ".kerberosec"));
		expect(
...
Document rollout and rollback steps.`,
	...
	// Regression test for https://github.com/kerberosec/kerberosec/issues/12151: a leading UTF-8 BOM
...
Escalation runbook`,
	...
	it("still loads all rules when .kerberosecrules is a legacy single file", async () => {
		const tempRoot = await mkdtemp(
			join(tmpdir(), "core-user-instructions-kerberosecrules-file-"),
		);
		tempRoots.push(tempRoot);
		...
		const globalRulesDir = join(tempRoot, "home", ".kerberosec", "rules");
		await mkdir(workspaceRoot, { recursive: true });
		await mkdir(globalRulesDir, { recursive: true });
		// Legacy single-file ruleset: `.kerberosecrules/skills` and
		// `.kerberosecrules/workflows` now resolve through a file (ENOTDIR), which
		// must not abort scanning of the other config sources.
		await writeFile(
			join(workspaceRoot, ".kerberosecrules"),
	...
	it.skipIf(process.platform === "win32")(
		...
		async () => {
			...
			tempRoots.push(tempRoot);
			const skillsDir = join(tempRoot, ".kerberosec", "skills");
...
Use conventional commits.`,
	...
	it("loads enterprise-style managed rules, workflows, and skills through the default workspace watcher", async () => {
		...
		const pluginRoot = join(tempRoot, ".kerberosec", "enterprise");
...
Use the security review checklist.`,
	...
	it("lets workspace .kerberosec workflows override legacy .kerberosecrules workflows with the same name", async () => {
		const tempRoot = await mkdtemp(
			join(tmpdir(), "core-user-instructions-workflow-precedence-"),
		);
		tempRoots.push(tempRoot);

		await mkdir(join(tempRoot, ".kerberosecrules", "workflows"), {
			recursive: true,
		});
		await mkdir(join(tempRoot, ".kerberosec", "workflows"), {
			recursive: true,
		});
		await writeFile(
			join(tempRoot, ".kerberosecrules", "workflows", "release.md"),
			`---
name: release
---
Legacy release workflow.`,
		...
		await writeFile(
			join(tempRoot, ".kerberosec", "workflows", "release.md"),
			`---
name: release
---
