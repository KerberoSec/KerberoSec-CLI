...
describe("ProviderListContent", () => {
	...
	it("shows connection status and auth kind per row", async () => {
		...
		expect(rowFor("KerberoSec")?.textContent).toContain("Sign in");
...
describe("ProviderDetailContent auth flows", () => {
	...
	beforeEach(() => {
		...
		document.body.appendChild(container);
		root = createRoot(container);
		// "kerberosec" is a featured provider, so mounting its detail refreshes the
...
describe("ProviderDetailContent audio capabilities", () => {
	...
	it("scopes the fetched featured list to its provider and list revision", async () => {
		const kerberosecProvider: Provider = {
			id: "kerberosec",
			name: "KerberoSec",
			models: 1,
			color: "#000",
			...
			enabled: true,
			modelList: [
				{ id: "kerberosec/snapshot-model", name: "KerberoSec Snapshot" },
			],
		};
		const kerberosecPassProvider: Provider = {
			id: "kerberosec-pass",
			name: "KerberoSecPass",
			models: 1,
			color: "#000",
		...
		await render(kerberosecProvider);
		...
		await render(kerberosecPassProvider);
	...
	it("keeps the catalog snapshot when the refresh after a switch fails or is empty", async () => {
		...
		// earlier successful kerberosec fetch no longer applies).
		loadProviderModelsMock.mockReset().mockResolvedValue([]);
		await render({
			...kerberosecProvider,
			modelList: [
		...
		expect(container.textContent).toContain("KerberoSec Snapshot");
	});
