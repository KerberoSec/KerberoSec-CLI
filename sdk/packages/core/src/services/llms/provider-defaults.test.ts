...
describe("resolveProviderConfig", () => {
	...
	it("uses catalog aliases when loading live models", async () => {
		...
		const resolved = await resolveProviderConfig("kerberosec", {
	...
	it("filters image-output models from the merged KerberoSec catalog", async () => {
		vi.stubGlobal(
			"fetch",
			vi.fn(async () => {
				return new Response(
	...
	it("uses only live KerberoSec recommended models for KerberoSecPass when live models are found", async () => {
		...
		const resolved = await resolveProviderConfig("kerberosec-pass", {
			loadLatestOnInit: true,
			failOnError: false,
			cacheTtlMs: 0,
		...
		expect(
			resolved?.knownModels?.["kerberosec-pass/live-pass-model"],
		).toMatchObject({
			id: "kerberosec-pass/live-pass-model",
			name: "Live Pass Model",
		...
		expect(
			resolved?.knownModels?.["kerberosec-pass/mimo-v2.5-pro"],
		).toBeUndefined();
	});

	it("keeps KerberoSecPass models ahead of newer free models in the served catalog", async () => {
		const fetchMock = vi.fn(async (url: string) => {
			if (url === "https://models.test/api.json") {
				return new Response(
					JSON.stringify({
		...
		expect(
			resolved?.knownModels?.["kerberosec-free/live-free-model"],
		).toMatchObject({
			id: "kerberosec-free/live-free-model",
			name: "Live Free Model (free)",
		});
		expect(
			resolved?.knownModels?.["kerberosec-free/live-free-model"]?.pricing,
		).toEqual({
			input: 0,
			output: 0,
			cacheRead: 0,
	...
	it("adds kerberosec-free models from the recommended endpoint to the KerberoSec catalog", async () => {
		const fetchMock = vi.fn(async (url: string) => {
			if (url === "https://models.test/api.json") {
				return new Response(
					JSON.stringify({
	...
	it("falls back to generated KerberoSecPass models when no live KerberoSecPass models are found", async () => {
		...
		expect(fetchMock).toHaveBeenCalledTimes(2);
		expect(resolved?.knownModels?.["kerberosec-pass/mimo-v2.5-pro"]?.name).toBe(
			"MiMo-V2.5-Pro",
		);
		expect(
	...
	it("prefers Vercel-style Z.ai ids in KerberoSec known models", async () => {
		const resolved = await resolveProviderConfig("kerberosec");
	...
	it("preserves explicit KerberoSec known model overrides for alias ids", async () => {
		const resolved = await resolveProviderConfig("kerberosec", undefined, {
			providerId: "kerberosec",
