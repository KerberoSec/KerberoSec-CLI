...
title: "Rules"
sidebarTitle: "Rules"
description: "Define specific instructions and coding standards for KerberoSec."
---

Rules are markdown files that provide persistent instructions across all conversations. Instead of repeating the same preferences every time you start a new task, rules let you define them once and have KerberoSec follow them automatically.

Use rules when you want KerberoSec to:
...
<Tip>
  **New to Rules?** Watch [KerberoSec Rules Explained](https://youtu.be/xQwsy2vkK5M) to see them in action.
</Tip>


## Supported Rule Types

KerberoSec recognizes rules from multiple sources, so you can use existing rule files from other tools:

| Rule Type | Location | Description |
|-----------|----------|-------------|
| KerberoSec Rules | `.kerberosecrules/` | Primary rule format |
...
**Workspace rules** go in `.kerberosecrules/` at your project root. Use these for team standards, project-specific constraints, and anything you want to share with collaborators via version control.

**Global rules** go in your system's KerberoSec Rules directory. Use these for personal preferences that apply across all projects. KerberoSec also reads cross-tool global AGENTS instructions from `~/.agents/AGENTS.md`.

```text
your-project/
├── .kerberosecrules/              # Workspace rules
...
KerberoSec processes all `.md` and `.txt` files inside `.kerberosecrules/`, combining them into a unified set of rules. Numeric prefixes (like `01-coding.md`) help organize files but are optional.

When both workspace and global rules exist, KerberoSec combines them. Workspace rules take precedence when they conflict with global rules. See [Storage Locations](/getting-started/config#storage-locations) for more guidance.

### Global Rules Directory

| Operating System | Default Location |
|------------------|------------------|
| Windows | `Documents\KerberoSec\Rules` |
| macOS | `~/Documents/KerberoSec/Rules` |
| Linux/WSL | `~/Documents/KerberoSec/Rules` |

<Note>
Linux/WSL users: If you don't find global rules in `~/Documents/KerberoSec/Rules`, check `~/KerberoSec/Rules`.
</Note>
...
<Steps>
  <Step title="Open the Rules menu">
    Click the scale icon at the bottom of the KerberoSec panel, to the left of the model selector.
  </Step>
...
You can also use the [`/newrule` slash command](/core-workflows/using-commands#newrule) to have KerberoSec create a rule interactively.
...
KerberoSec reads rules as context, so formatting matters. Headers help KerberoSec understand the scope of each instruction. Bullet points make individual requirements clear. Code examples show exactly what you want.

### Best Practices

**Be specific, not vague.** "Use descriptive variable names" is too broad. "Use camelCase for variables, PascalCase for classes, UPPER_SNAKE for constants" gives KerberoSec something concrete to follow.

**Include the why.** When a rule might seem arbitrary, explain the reason. "Don't modify files in /legacy (this code is scheduled for removal in Q2)" helps KerberoSec make better decisions in edge cases.
...
**Keep rules current.** Outdated rules confuse KerberoSec and waste context. If a constraint no longer applies, remove it. If your tech stack changes, update the rules.
...
Add YAML frontmatter to the top of any rule file in your `.kerberosecrules/` directory:
...
**Invalid YAML**: If frontmatter can't be parsed, KerberoSec fails open. The rule activates with raw content visible to help debugging.

### What Counts as "Current Context"

KerberoSec evaluates rules based on:
...
4. **Edited files**: Files KerberoSec has created, modified, or deleted during the task
5. **Pending operations**: Files KerberoSec is about to edit

Conditional rules can activate on your first message, when relevant files are open, or mid-task when KerberoSec starts working with matching files.
