...
import { TestConnectionResult } from "@shared/proto/kerberosec/state"
import { Logger } from "@/shared/services/Logger"
import { blobStorage } from "@/shared/storage/KerberoSecBlobStorage"
...
export async function testPromptUploading(_controller: Controller, _: EmptyRequest): Promise<TestConnectionResult> {
	try {
		...
		const testKey = `kerberosec-test-${Date.now()}.json`
