import { buildInviteUrl, resolveKerberoSecHubServerOptions } from "./options";
...
const defaults = resolveKerberoSecHubServerOptions({});
...
const lan = resolveKerberoSecHubServerOptions({
	HOST: "0.0.0.0",
	KERBEROSEC_HUB_DASHBOARD_PORT: "9000",
...
const tailscale = resolveKerberoSecHubServerOptions({
	HOST: "0.0.0.0",
...
expectThrows(
	() => resolveKerberoSecHubServerOptions({ HOST: "0.0.0.0" }),
	"non-local bind without ROOM_SECRET",
);
expectThrows(
	() =>
		resolveKerberoSecHubServerOptions({
			KERBEROSEC_HUB_DASHBOARD_PORT: "70000",
		}),
	"invalid dashboard port",
);
expectThrows(
	() => resolveKerberoSecHubServerOptions({ PUBLIC_URL: "ftp://example.test" }),
	"invalid PUBLIC_URL protocol",
);

console.log("kerberosec-hub option validation passed");
