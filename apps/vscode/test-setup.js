...
Module.prototype.require = function (id) {
	...
	// extension host, so `require("@kerberosec/core")` fails before tests start.
	// Mock the small surface needed by legacy VS Code integration tests.
	if (id === "@kerberosec/core") {
		...
		const getProviderAuthStorageId = (providerId) => {
			const normalized = String(providerId || "")
				.trim()
				.toLowerCase()
			if (normalized === "kerberosec" || normalized === "kerberosec-pass") return "kerberosec"
			if (normalized === "oca" || normalized === "openai-codex") return normalized
			return undefined
		}
		const formatKerberoSecApiKey = (token) => {
		...
		const getProviderAuthHandler = (providerId) => {
			...
			return {
				...
				getApiKey(settings) {
					...
					if (accessToken) return storageProviderId === "kerberosec" ? formatKerberoSecApiKey(accessToken) : accessToken
		...
		return {
			createKerberoSecTelemetryServiceConfig: (config = {}) => ({
				enabled: false,
				metadata: {
			...
			fetchKerberoSecRecommendedModels: async () => ({ recommended: [], free: [] }),
			...
			getValidKerberoSecCredentials: async () => undefined,
			loginKerberoSecOAuth: async () => undefined,
			loginOcaOAuth: async () => undefined,
			loginOpenAICodex: async () => undefined,
		}
	}

	if (id === "@kerberosec/shared") {
		return {
