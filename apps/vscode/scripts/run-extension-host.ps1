...
param(
    ...
    [string]$Environment = $(if ($env:KERBEROSEC_ENVIRONMENT) { $env:KERBEROSEC_ENVIRONMENT } else { "production" })
)
...
$env:KERBEROSEC_ENVIRONMENT = $Environment
...
Get-Process -Name "powershell", "pwsh" -ErrorAction SilentlyContinue |
    Where-Object { $_.MainWindowTitle -like "kerberosec-dev:*" } |
...
function Start-WatcherWindow {
    ...
    $psi = Start-Process -FilePath "powershell.exe" -ArgumentList @(
        "-NoExit",
        "-Command",
        "`$Host.UI.RawUI.WindowTitle = 'kerberosec-dev: $Title'; Set-Location '$Workspace'; $Command"
    ) -PassThru
    return $psi
}
...
code --extensionDevelopmentPath="$Workspace" `
    ...
    --disable-extension saoudrizwan.kerberosec-nightly `
    "$Workspace"
