...
import { resolveProviderSettingsPath } from "@kerberosec/shared/storage";
...
export interface ResolveLastUsedProviderSettingsOptions {
	isKerberoSecPassEnabled?: boolean;
}
...
export class ProviderSettingsManager {
	...
	read(): StoredProviderSettings {
		...
		try {
			...
			if (result.success) {
				...
				const kerberosecAuth =
					result.data.providers["kerberosec"]?.settings?.auth;
				sdkDebug(
					`providers.read providers=[${Object.keys(result.data.providers).join(",")}] lastUsed=${result.data.lastUsedProvider ?? "none"} kerberosecAuthPresent=${!!kerberosecAuth?.accessToken} kerberosecAccessTokenHash=${hashSecret(kerberosecAuth?.accessToken)} kerberosecRefreshTokenHash=${hashSecret(kerberosecAuth?.refreshToken)}`,
				);
				return result.data;
			}
		} catch {
	...
	write(state: StoredProviderSettings): void {
		...
		// KerberoSec processes (CLI, extension, hub) share this file; a bare
	...
	saveProviderSettings(
		...
		this.write(next);
		const prevKerberoSecAuth = previous.providers["kerberosec"]?.settings?.auth;
		const nextKerberoSecAuth =
			validatedSettings.provider === "kerberosec"
				? validatedSettings.auth
				: next.providers["kerberosec"]?.settings?.auth;
		const authDropped =
			!!prevKerberoSecAuth?.accessToken && !nextKerberoSecAuth?.accessToken;
		sdkDebug(
			`providers.save providerId=${providerId} tokenSource=${tokenSource} kerberosecAuthWasPresent=${!!prevKerberoSecAuth?.accessToken} kerberosecAuthIsPresent=${!!nextKerberoSecAuth?.accessToken} authDropped=${authDropped}`,
		);
		return next;
	}
	...
	private resolveLastUsedProviderId(
		...
		if (
			...
			options.isKerberoSecPassEnabled === false
		) {
			return KERBEROSEC_PROVIDER_ID;
		}
