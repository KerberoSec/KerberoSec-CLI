...
function findKerberoSecSpec() {
	const spec = BUILTIN_SPECS.find((s) => s.id === "kerberosec");
	if (!spec) {
		throw new Error("kerberosec builtin spec not found");
	}
	return spec;
}

describe("kerberosec builtin spec defaults.baseUrl", () => {
	const originalEnvironment = process.env[KERBEROSEC_ENVIRONMENT_ENV];

	beforeEach(() => {
		delete process.env[KERBEROSEC_ENVIRONMENT_ENV];
	});
	...
	afterEach(() => {
		if (originalEnvironment === undefined) {
			...
			process.env[KERBEROSEC_ENVIRONMENT_ENV] = originalEnvironment;
		}
	});

	it("re-resolves baseUrl when KERBEROSEC_ENVIRONMENT changes between reads", () => {
		const spec = findKerberoSecSpec();

		expect(spec.defaults?.baseUrl).toBe(
			`${KERBEROSEC_ENVIRONMENTS.production.apiBaseUrl}/api/v1`,
		);

		process.env[KERBEROSEC_ENVIRONMENT_ENV] = "staging";
		expect(spec.defaults?.baseUrl).toBe(
			`${KERBEROSEC_ENVIRONMENTS.staging.apiBaseUrl}/api/v1`,
		);

		process.env[KERBEROSEC_ENVIRONMENT_ENV] = "local";
		expect(spec.defaults?.baseUrl).toBe(
			`${KERBEROSEC_ENVIRONMENTS.local.apiBaseUrl}/api/v1`,
		);
...
describe("kerberosec builtin models", () => {
	it("exposes its canonical default model ID", () => {
		expect(findKerberoSecSpec().defaultModelId).toBe(
			KERBEROSEC_DEFAULT_MODEL_ID,
		);
	});

	it("prefers Vercel-style Z.ai model ids over equivalent OpenRouter ids", async () => {
		const models = await getModelsForProvider("kerberosec");
	...
	it("excludes image-output models without changing upstream catalogs", async () => {
		...
		const [kerberosecModels, openRouterModels, vercelModels] =
			await Promise.all([
				getModelsForProvider("kerberosec"),
				getModelsForProvider("openrouter"),
				getModelsForProvider("vercel-ai-gateway"),
			]);

		expect(kerberosecModels[modelId]).toBeUndefined();
		expect(
			Object.values(kerberosecModels).some(
...
describe("kerberosec-pass builtin spec", () => {
	it("registers a distinct KerberoSec-compatible provider with a custom model list", async () => {
		const models = await getModelsForProvider("kerberosec-pass");
		const provider = await getProvider("kerberosec-pass");
...
describe("built-in provider metadata", () => {
	it("declares OpenRouter image transport for KerberoSec-compatible gateways", async () => {
		...
		await expect(getProvider("kerberosec-pass")).resolves.toMatchObject({
			metadata: {
				imageTransport: "openrouter",
				responseEnvelope: "success-data",
			},
	...
	it("uses generated specs directly when no runtime override is required", () => {
		// moonshot is intentionally absent: it carries a KerberoSec-specific
