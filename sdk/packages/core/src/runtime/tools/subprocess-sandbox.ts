...
export const KERBEROSEC_JS_RUNTIME_PATH_ENV = "KERBEROSEC_JS_RUNTIME_PATH";
...
export function resolveSubprocessRuntimeExecutable(
	...
	const explicit =
		...
		env[KERBEROSEC_JS_RUNTIME_PATH_ENV]?.trim();
	if (explicit) {
		return explicit;
	}
	...
	start(): void {
		...
		const child = spawn(
			...
			command.slice(1),
			{
				...
				env: withResolvedKerberoSecBuildEnv(process.env),
