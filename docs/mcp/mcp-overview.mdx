...
title: "MCP"
sidebarTitle: "MCP"
description: "Add, configure, and use MCP servers in KerberoSec."
---

MCP (Model Context Protocol) lets KerberoSec use external tools and data sources through MCP servers.

## What MCP gives you

- Connect KerberoSec to external APIs and services
- Add custom tools beyond built-in KerberoSec tools
- Use either local servers or remote hosted servers

## Quick start

1. Open **MCP Servers** in KerberoSec
...
- **CLI:** `~/.kerberosec/mcp.json`
- **IDE extensions:**
  1. In the KerberoSec panel, click the **MCP Servers** icon (stacked server icon in the top toolbar).
...
MCP also works in KerberoSec CLI. Configure servers in CLI MCP settings and use the same server definitions.

You can also list servers non-interactively:

```bash
kerberosec config mcp
kerberosec config mcp --json
```
