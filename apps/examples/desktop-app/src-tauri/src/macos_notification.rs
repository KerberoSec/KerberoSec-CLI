...
const DEV_BUNDLE_NAME: &str = "KerberoSec.app";
...
fn create_dev_application_bundle(
    ...
    let bundled_executable = macos.join("kerberosec-app");
...
mod tests {
    ...
    impl TestDirectory {
        fn new() -> Self {
            ...
            let path = std::env::temp_dir().join(format!(
                "kerberosec-notification-identity-test-{}-{unique}",
                std::process::id()
            ));
    ...
    fn creates_a_registered_application_shape_for_development() {
        ...
        let executable = test_directory.0.join("target/debug/kerberosec-app");
        ...
        let bundle =
            create_dev_application_bundle(&executable, &icon, "bot.kerberosec.app.dev", "KerberoSec Dev")
                .unwrap();
        let plist = fs::read_to_string(bundle.join("Contents/Info.plist")).unwrap();

        assert!(plist.contains("<string>bot.kerberosec.app.dev</string>"));
        assert!(plist.contains("<string>KerberoSec Dev</string>"));
        assert_eq!(
            fs::read_link(bundle.join("Contents/MacOS/kerberosec-app")).unwrap(),
            executable
        );
        assert_eq!(
