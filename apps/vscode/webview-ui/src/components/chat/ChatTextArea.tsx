...
import { FileSearchRequest, FileSearchType, RelativePathsRequest } from "@shared/proto/kerberosec/file"
import { PlanActMode, TogglePlanActModeRequest } from "@shared/proto/kerberosec/state"
...
import KerberoSecRulesToggleModal from "../kerberosec-rules/KerberoSecRulesToggleModal"
		...
		const modelDisplayName = useMemo(() => {
			...
			switch (selectedProvider) {
				...
				case "kerberosec-pass":
					// Free models selected on KerberoSecPass go through KerberoSec usage billing,
					// so label them the same way as the kerberosec provider
					return selectedModelId.startsWith("kerberosec-pass/")
						? `${selectedProvider}:${selectedModelId.replace(/^kerberosec-pass\//, "")}`
						: `kerberosec:${selectedModelId}`
				case "openai":
				...
				<div className="flex justify-between items-center -mt-[2px] px-3 pb-2">
					...
					<Tooltip>
						<TooltipContent
							...
							side="top">
							{`In ${shownTooltipMode === "act" ? "Act" : "Plan"}  mode, KerberoSec will ${shownTooltipMode === "act" ? "complete the task immediately" : "gather information to architect a plan"}`}
