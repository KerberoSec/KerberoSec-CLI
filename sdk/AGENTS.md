---
description: Development reference for the KerberoSec SDK workspace.
globs: "*.ts,*.tsx,*.js,*.jsx,*.json,*.md"
alwaysApply: true
---

# KerberoSec SDK — Development Reference
...
- `@kerberosec/shared`: shared contracts, schemas, path helpers, hook engine, extension registry, low-level utilities
- `@kerberosec/llms`: provider settings/config, model catalogs, provider manifests, gateway contracts, handler creation
- `@kerberosec/agents`: stateless agent loop, tool orchestration, hook/extension runtime, event streaming
- `@kerberosec/core`: stateful orchestration, session lifecycle, storage, config watching, plugin loading, default tools, telemetry. Exposes `@kerberosec/core/hub` for discovery, the detached daemon entry, WebSocket clients, and session/UI client adapters, plus `@kerberosec/core/hub/daemon-entry` for launching the shared daemon

### Dependency Direction

```mermaid
flowchart TD
  shared["@kerberosec/shared"] --> llms["@kerberosec/llms"] & agents["@kerberosec/agents"] & core["@kerberosec/core"]
  llms --> agents & core
  agents --> core
...
- model/provider schemas or handler behavior: `@kerberosec/llms`
- stateless loop, tool orchestration, streaming, hook/extension runtime: `@kerberosec/agents`
- session lifecycle, storage, config watching, default tools, plugin loading, telemetry, hub runtime services, hub discovery, hub daemon spawn, and session-oriented client helpers (`HubSessionClient`, `HubUIClient`, `connectToHub`): `@kerberosec/core` (hub pieces live under `src/hub/`)
- remote-config schemas, managed instruction materialization, blob upload metadata, and OpenTelemetry config normalization: `@kerberosec/shared/src/remote-config`
...
```sh
bun -F @kerberosec/shared test
bun -F @kerberosec/llms test
bun -F @kerberosec/agents test
bun -F @kerberosec/core test:unit
bun -F @kerberosec/cli test:unit
```

If a focused test command fails with a missing `@kerberosec/*` export or missing `dist/` file, build the relevant dependency package or run `bun run build:sdk`, then rerun the same test command. Treat that as a workspace setup issue, not as evidence of a source-code bug.
...
- For `@kerberosec/llms` provider/model routing rules, follow [packages/llms/AGENTS.md](./packages/llms/AGENTS.md).
