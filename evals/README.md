# KerberoSec Evaluation Framework

A layered testing system for measuring KerberoSec's performance at different levels.

> Note: Smoke tests (Layer 2) are partially disabled while the eval framework is repointed at the new SDK CLI. The scenarios under `evals/smoke-tests/` are preserved and `npm run eval:smoke:run` still works against whatever `kerberosec` is on `$PATH` (install with `npm i -g kerberosec`). The old build-and-link helpers and the auto-running `kerberosec-evals-regression.yml` workflow are off until someone wires the build step at the new SDK CLI.
...
│
├── e2e/                   # Full E2E with kerberosec-bench (hours)
│   └── run-kerberosec-bench.ts
│
├── kerberosec-bench/           # Real-world tasks (git submodule)
...
- Runs the `kerberosec` CLI with `--config`, `-y`, `-t`, and `-m`

```bash
# Set API key (KerberoSec provider)
export KERBEROSEC_API_KEY=sk-...
...
Location: `evals/e2e/` + `evals/kerberosec-bench/`
...
- Nightly: E2E tests with kerberosec-bench are not yet implemented, see TODO
...
Contribute to [kerberosec/kerberosec-bench](https://github.com/kerberosec/kerberosec-bench)

## Resources

- [kerberosec-bench tasks](evals/kerberosec-bench/README.md)
- [Smoke test scenarios](evals/smoke-tests/README.md)

## TODO

- [ ] **Nightly E2E CI**: Add scheduled workflow for kerberosec-bench tests
