...
import type { KerberoSecApiReqInfo, KerberoSecMessage } from "@shared/ExtensionMessage"
import { Logger } from "@/shared/services/Logger"
import type { MessageIdMinter } from "./message-id-minter"
import type { TaskProxy } from "./task-proxy"
import { pushMessageToWebview } from "./webview-grpc-bridge"

export type SessionEventListener = (messages: KerberoSecMessage[], event: CoreSessionEvent) => void
...
export class SdkMessageCoordinator {
	...
	private saveKerberoSecMessagesTimer: ReturnType<typeof setTimeout> | undefined
	 ...
	 */
	private stamp(messages: KerberoSecMessage[]): void {
	...
	cancelPendingSave(): void {
		if (this.saveKerberoSecMessagesTimer) {
			clearTimeout(this.saveKerberoSecMessagesTimer)
			this.saveKerberoSecMessagesTimer = undefined
		}
	}
	...
	emitSessionEvents(messages: KerberoSecMessage[], event: CoreSessionEvent): void {
	...
	appendMessages(messages: KerberoSecMessage[]): void {
	...
	appendAndEmit(messages: KerberoSecMessage[], event: CoreSessionEvent): void {
		this.appendMessages(messages)
		this.emitSessionEvents(messages, event)
	}

	emitHookMessage(message: KerberoSecMessage): void {
	 ...
	 */
	finalizeMessagesForSave(messages: KerberoSecMessage[]): KerberoSecMessage[] {
		return messages.map((msg, index) => {
			...
			if (updated.type === "say" && updated.say === "api_req_started") {
				try {
					const info: KerberoSecApiReqInfo = JSON.parse(updated.text || "{}")
