...
import { isChatWorkspacePath } from "@kerberosec/shared/storage";
...
export class HubServerTransport implements NativeHubTransport {
			...
			createTasksTool({
				...
				scheduled: {
					...
					resolveSessionDefaults: async (sessionId) => {
						...
						// ~/.kerberosec/schedules home instead of whichever chat
	...
	private async startAgendaTaskSession(
		...
		const providerId = task.modelSelection?.providerId?.trim() || "kerberosec";
		const modelId =
			task.modelSelection?.modelId?.trim() ||
			(providerId === "kerberosec" ? KERBEROSEC_DEFAULT_MODEL_ID : "");
