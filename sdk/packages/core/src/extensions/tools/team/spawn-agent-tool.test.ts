import type { AgentConfig } from "@kerberosec/shared";
...
describe("createSpawnAgentTool", () => {
	...
	it("appends workspace metadata for kerberosec sub-agents when missing", async () => {
		const { createSpawnAgentTool } = await import("./spawn-agent-tool.js");
		runMock.mockResolvedValue({
			text: "ok",
			iterations: 1,
		...
		const workspaceMetadata = `# Workspace Configuration
{
  "workspaces": {
	...
	it("does not duplicate workspace metadata for kerberosec sub-agents", async () => {
		const { createSpawnAgentTool } = await import("./spawn-agent-tool.js");
		runMock.mockResolvedValue({
			text: "ok",
			iterations: 1,
