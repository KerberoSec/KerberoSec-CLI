...
export function sharedSessionDataDir(): string {
	return (
		process.env.KERBEROSEC_SESSION_DATA_DIR?.trim() || resolveSessionDataDir()
	);
}
...
export function toolApprovalDir(): string {
	return (
		process.env.KERBEROSEC_TOOL_APPROVAL_DIR?.trim() ||
...
export function resolveMcpSettingsPath(): string {
	return (
		process.env.KERBEROSEC_MCP_SETTINGS_PATH?.trim() ||
		join(
			homedir(),
			".kerberosec",
			"data",
...
function kanbanDataRoot(): string {
	return (
		process.env.KERBEROSEC_KANBAN_DATA_DIR?.trim() ||
		join(homedir(), ".kerberosec", "apps", "kanban")
	);
}
