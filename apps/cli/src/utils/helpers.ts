...
import { ensureHookLogDir } from "@kerberosec/shared/storage";
	...
	switch (toolName) {
		...
		case "view_file":
			...
			if (
				...
				return truncate(
					String(obj.path ?? obj.file_path ?? obj.AbsolutePath),
					120,
				);
			}
			break;
		...
		case "write_to_file":
			if (
				...
				return truncate(
					String(obj.path ?? obj.file_path ?? obj.TargetFile),
					120,
				);
			}
			break;
		...
		case "find_by_name":
			...
			if (typeof obj.query === "string" || typeof obj.Query === "string") {
				return truncate(String(obj.query ?? obj.Query), 120);
			}
			break;
...
export async function appendHookAudit(event: HookEventPayload): Promise<void> {
	...
	const envPath = process.env.KERBEROSEC_HOOKS_LOG_PATH?.trim() || undefined;
...
export function resolveSandboxDataDir(
	...
	const envDir = process.env.KERBEROSEC_SANDBOX_DATA_DIR?.trim();
	const baseDir =
		explicitDir?.trim() || envDir || join(tmpdir(), "kerberosec-sandbox");
...
export function configureSandboxEnvironment(options: {
	...
	process.env.KERBEROSEC_SANDBOX = "1";
	process.env.KERBEROSEC_SANDBOX_DATA_DIR = dataDir;
	process.env.KERBEROSEC_DATA_DIR = dataDir;
	process.env.KERBEROSEC_DB_DATA_DIR = join(dataDir, "db");
	process.env.KERBEROSEC_SESSION_DATA_DIR = join(dataDir, "sessions");
	process.env.KERBEROSEC_TEAM_DATA_DIR = join(dataDir, "teams");
	process.env.KERBEROSEC_PROVIDER_SETTINGS_PATH = join(
		dataDir,
		"settings",
		"providers.json",
	);
	process.env.KERBEROSEC_HOOKS_LOG_PATH = join(dataDir, "logs", "hooks.jsonl");
	return dataDir;
}
