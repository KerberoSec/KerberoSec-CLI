...
export type KerberoSecTokenResolution = {
	forceRefresh?: boolean;
	refreshBufferMs?: number;
	retryableTokenGraceMs?: number;
};

interface KerberoSecAuthApiUser {
	subject: string | null;
	email: string;
	name: string;
...
interface KerberoSecAuthResponseData {
	accessToken: string;
	refreshToken?: string;
	tokenType: string;
	expiresAt: string;
	userInfo: KerberoSecAuthApiUser;
}

type KerberoSecTokenResponse = {
	success: boolean;
	data: KerberoSecAuthResponseData;
};
...
export interface KerberoSecOAuthProviderOptions {
	apiBaseUrl: string;
	headers?: HeaderInput;
...
export interface KerberoSecOAuthCredentials extends OAuthCredentials {
	metadata?: {
		provider?: string;
		tokenType?: string;
		userInfo?: KerberoSecAuthApiUser;
...
class KerberoSecOAuthTokenError extends Error {
	...
	constructor(
		...
		super(message);
		this.name = "KerberoSecOAuthTokenError";
...
function toKerberoSecCredentials(
	responseData: KerberoSecAuthResponseData,
...
function getAuthCredentialTelemetryProperties(
	credentials: KerberoSecOAuthCredentials,
): AuthCredentialTelemetryProperties {
...
function requireKerberoSecTokenResponse(
	payload: KerberoSecTokenResponse,
	message: string,
): KerberoSecAuthResponseData {
...
async function requestWorkOSDeviceAuthorization(
	...
	if (!response.ok) {
		throw new KerberoSecOAuthTokenError(
...
async function registerWorkOSTokens(
	...
	options: KerberoSecOAuthProviderOptions,
	provider?: string,
): Promise<KerberoSecOAuthCredentials> {
	const body = {
	...
	const json = (await response.json()) as KerberoSecTokenResponse;
	return toKerberoSecCredentials(
		requireKerberoSecTokenResponse(json, "Invalid token exchange response"),
...
export async function loginKerberoSecOAuth(
	options: KerberoSecOAuthProviderOptions & {
		callbacks: OAuthLoginCallbacks;
	},
): Promise<KerberoSecOAuthCredentials> {
	captureAuthStarted(options.telemetry, options.provider ?? "kerberosec");
	...
	try {
		let credentials: KerberoSecOAuthCredentials;
		if (useWorkOSDeviceAuth) {
			const clientId = getKerberoSecEnvironmentConfig().workOsClientId;
		...
		captureAuthFailed(
			...
			{
				requestId:
					error instanceof KerberoSecOAuthTokenError
						? error.requestId
						: undefined,
			},
		);
...
export async function startKerberoSecDeviceAuth(options?: {
	requestTimeoutMs?: number;
}): Promise<{
	deviceCode: string;
	userCode: string;
	...
	return await requestWorkOSDeviceAuthorization(
		getKerberoSecEnvironmentConfig().workOsClientId,
		options,
	);
}

export async function completeKerberoSecDeviceAuth(options: {
	deviceCode: string;
	expiresInSeconds: number;
	pollIntervalSeconds: number;
	apiBaseUrl: string;
	...
	const providerName = options.provider ?? "kerberosec";
	captureAuthStarted(options.telemetry, providerName);
	try {
		const workosTokens = await pollWorkOSTokens({
			clientId: getKerberoSecEnvironmentConfig().workOsClientId,
...
export async function refreshKerberoSecToken(
	current: KerberoSecOAuthCredentials,
	options: KerberoSecOAuthProviderOptions,
): Promise<KerberoSecOAuthCredentials> {
	...
	sdkDebug(
		`kerberosec.refresh.request url=${refreshUrl} refreshTokenHash=${hashSecret(current.refresh)}`,
	);
	...
	if (!response.ok) {
		...
		sdkDebug(
			`kerberosec.refresh.error status=${response.status} errorCode=${details.code ?? "none"} message=${details.message ?? "none"} requestId=${requestId ?? "none"}`,
		);
		throw new KerberoSecOAuthTokenError(
	...
	const result = toKerberoSecCredentials(
		requireKerberoSecTokenResponse(json, "Invalid token refresh response"),
		provider,
		current,
	);
	sdkDebug(
		`kerberosec.refresh.success newAccessTokenHash=${hashSecret(result.access)} newRefreshTokenHash=${hashSecret(result.refresh)} expires=${result.expires}`,
	);
	return result;
}

/**
 * Resolve valid KerberoSec credentials, refreshing when needed.
 *
 ...
 */
export async function getValidKerberoSecCredentials(
	currentCredentials: KerberoSecOAuthCredentials | null,
	providerOptions: KerberoSecOAuthProviderOptions,
	options?: KerberoSecTokenResolution,
): Promise<KerberoSecOAuthCredentials | null> {
	if (!currentCredentials) {
		sdkDebug("kerberosec.getCredentials outcome=no_current_credentials");
		return null;
	}
	...
	if (
		...
		sdkDebug(
			`kerberosec.getCredentials outcome=still_valid forceRefresh=${forceRefresh}`,
		);
		return currentCredentials;
	}

	sdkDebug(
		`kerberosec.getCredentials outcome=needs_refresh forceRefresh=${forceRefresh} accessTokenHash=${hashSecret(currentCredentials.access)}`,
	);

	try {
		return await refreshKerberoSecToken(currentCredentials, providerOptions);
	} catch (error) {
		const authTelemetryDetails = getAuthTelemetryDetails(currentCredentials);
		const requestIdDetails =
			error instanceof KerberoSecOAuthTokenError && error.requestId
				? { request_id: error.requestId }
				: {};
		const failureDetails = {
			status:
				error instanceof KerberoSecOAuthTokenError ? error.status : undefined,
			errorCode:
				error instanceof KerberoSecOAuthTokenError
					? error.errorCode
					: undefined,
		...
		if (
			...
			sdkDebug(
				`kerberosec.getCredentials outcome=invalid_grant status=${error.status} errorCode=${error.errorCode ?? "none"}`,
			);
			// This is the single owner of the involuntary-logout event for the
			// KerberoSec provider — callers observing the `null` return must NOT
			...
			captureAuthLoggedOut(
				...
				providerOptions.provider ?? "kerberosec",
				"token_invalid",
				{
					status: error.status,
		...
		if (currentCredentials.expires - Date.now() > retryableTokenGraceMs) {
			...
			sdkDebug(
				`kerberosec.getCredentials outcome=transient_failure_kept_current error=${error instanceof Error ? error.message : String(error)}`,
			);
			captureAuthRefreshSoftFailure(
				providerOptions.telemetry,
				providerOptions.provider ?? "kerberosec",
		...
		// credentials over it, logging out every KerberoSec process on the machine.
		sdkDebug(
			`kerberosec.getCredentials outcome=transient_failure_rethrown error=${error instanceof Error ? error.message : String(error)}`,
		);
