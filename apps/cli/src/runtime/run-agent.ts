...
import type { ConsecutiveMistakeLimitContext } from "@kerberosec/shared";
import { createSessionId } from "@kerberosec/shared";
import { logCliError } from "../logging/errors";
import { createCliCore } from "../session/session";
import { resolveKerberoSecWelcomeLine } from "../tui/interactive-welcome";
import {
...
import { formatCliErrorMessage } from "../utils/kerberosec-pass-errors";
import {
	c,
	emitJsonLine,
	formatUsd,
...
export async function runAgent(
	...
	if (config.verbose) {
		const kerberosecWelcomeLine = await resolveKerberoSecWelcomeLine({
			config,
			kerberosecApiBaseUrl: options?.kerberosecApiBaseUrl,
			kerberosecProviderSettings: options?.kerberosecProviderSettings,
		});
		if (kerberosecWelcomeLine && config.outputMode !== "json") {
			writeln(kerberosecWelcomeLine);
		}
	}
	...
	const shouldZeroCost = await shouldZeroKerberoSecFreeModelCost(config);
