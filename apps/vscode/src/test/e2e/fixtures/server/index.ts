...
import type { BalanceResponse, OrganizationBalanceResponse, UserResponse } from "../../../../shared/KerberoSecAccount"
import {
	E2E_MOCK_API_RESPONSES,
	E2E_MOCK_KERBEROSEC_MODELS,
	E2E_MOCK_KERBEROSEC_RECOMMENDED_MODELS,
	E2E_MOCK_EDITOR_TOOL_CALL,
	E2E_MOCK_POWERSHELL_TOOL_CALL,
	E2E_REGISTERED_MOCK_ENDPOINTS,
} from "./api"
import { KerberoSecDataMock } from "./data"

const E2E_API_SERVER_PORT = 7777

export const MOCK_KERBEROSEC_API_SERVER_URL = `http://localhost:${E2E_API_SERVER_PORT}`

const useVerboseLogging = process.env.KERBEROSEC_E2E_TESTS_VERBOSE === "true"
function log(...args: unknown[]) {
	if (useVerboseLogging) {
		console.log("[KerberoSecApiServerMock]", ...args)
	}
}

export class KerberoSecApiServerMock {
	static globalSharedServer: KerberoSecApiServerMock | null = null
	...
	public readonly API_USER = new KerberoSecDataMock("personal")
	...
	public static async startGlobalServer(): Promise<KerberoSecApiServerMock> {
		log("=== SERVER FIXTURE CALLED ===")
		if (KerberoSecApiServerMock.globalSharedServer) {
			log("Using existing global server")
			return KerberoSecApiServerMock.globalSharedServer
		}

		log("Starting global server...")
		const server = createServer((req: IncomingMessage, res: ServerResponse) => {
			// Parse URL and method
			const parsedUrl = new URL(req.url || "/", MOCK_KERBEROSEC_API_SERVER_URL)
			...
			if (isAuthRequired && authToken) {
				...
				const user = KerberoSecApiServerMock.globalSharedServer!.API_USER.getUserByToken(normalizedAuthToken)
				if (!user) {
					return sendApiError("Invalid token", 401)
				}
				KerberoSecApiServerMock.globalSharedServer!.setCurrentUser(user)
			}
			...
			const handleRequest = async () => {
				...
				const routeMatch = KerberoSecApiServerMock.matchRoute(path, method)
				...
				const controller = KerberoSecApiServerMock.globalSharedServer!
				...
				// API v1 endpoints
				if (baseRoute === "/api/v1") {
					if (endpoint === "/ai/kerberosec/recommended-models" && method === "GET") {
						return sendJson(E2E_MOCK_KERBEROSEC_RECOMMENDED_MODELS)
					}

					if (endpoint === "/ai/kerberosec/models" && method === "GET") {
						return sendJson({ data: E2E_MOCK_KERBEROSEC_MODELS })
					}
					...
					if (endpoint === "/auth/token" && method === "POST") {
						...
						// Return format matching KerberoSecAuthProvider expectations
						return sendApiResponse({
							...
							choices: [
								{
									...
									message: {
										role: "assistant",
										content: "Hello! I'm a mock KerberoSec API response.",
									},
									finish_reason: "stop",
								},
							],
		...
		const controller = new KerberoSecApiServerMock(server)
		KerberoSecApiServerMock.globalSharedServer = controller

		// Track connections for proper cleanup
		server.on("connection", (socket) => {
			KerberoSecApiServerMock.globalSockets.add(socket)
			socket.on("close", () => {
				KerberoSecApiServerMock.globalSockets.delete(socket)
			})
		})
		...
		await new Promise<void>((resolve, reject) => {
			server.listen(E2E_API_SERVER_PORT, (error?: Error) => {
				if (error) {
					...
					log(`KerberoSecApiServerMock listening on port ${E2E_API_SERVER_PORT}`)
					resolve()
				}
			})
		})
	...
	public static async stopGlobalServer(): Promise<void> {
		...
		const server = KerberoSecApiServerMock.globalSharedServer.server

		// Clean shutdown - destroy all socket connections first
		KerberoSecApiServerMock.globalSockets.forEach((socket) => socket.destroy())
		KerberoSecApiServerMock.globalSockets.clear()
		...
		KerberoSecApiServerMock.globalSharedServer = null
	}
}
