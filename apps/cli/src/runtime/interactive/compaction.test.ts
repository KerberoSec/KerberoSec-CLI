...
// key) is needed; every other `@kerberosec/llms` export stays real because
// `@kerberosec/core` re-exports them.
vi.mock("@kerberosec/llms", async (importOriginal) => ({
	...(await importOriginal<typeof import("@kerberosec/llms")>()),
...
function createProviderSettingsManager(): ProviderSettingsManager {
	const tempDir = mkdtempSync(join(tmpdir(), "kerberosec-cli-compact-"));
