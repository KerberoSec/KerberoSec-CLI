...
describe("OpenTelemetryAdapter", () => {
	...
	it("records metrics with merged telemetry attributes and retires gauge series", () => {
		...
		adapter.recordCounter("kerberosec.turns.total", 2, { ulid: "01HXYZ" });
		adapter.recordHistogram("kerberosec.api.duration.seconds", 1.5, {
			ulid: "01HXYZ",
		});
		adapter.recordGauge("kerberosec.workspace.active_roots", 3, {
			workspace: "a",
		});
		...
		adapter.recordGauge("kerberosec.workspace.active_roots", null, {
			workspace: "a",
		});
