...
// Post-install script for KerberoSec CLI.
//
// Creates a hard link (or copy fallback) from the platform-specific binary
// to bin/.kerberosec for fast startup on subsequent runs.
//
...
function shieldRunningHubDiscovery() {
	const explicitPath = process.env.KERBEROSEC_HUB_DISCOVERY_PATH?.trim();
	const dataDir =
		process.env.KERBEROSEC_DATA_DIR?.trim() ||
		path.join(
			process.env.KERBEROSEC_DIR?.trim() ||
				path.join(os.homedir(), ".kerberosec"),
			"data",
		);
	const recordPath =
...
function main() {
	...
	const arch = os.arch();
	const packageName = `@kerberosec/cli-${platform}-${arch}`;
	const binaryName = "kerberosec";
	...
	const binDir =
		path.basename(__dirname) === "script"
			...
			: path.join(__dirname, "bin");
	const target = path.join(binDir, ".kerberosec");
	...
	console.log(`Cached kerberosec binary at ${target}`);
}
