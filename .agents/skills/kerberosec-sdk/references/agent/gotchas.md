...
`@kerberosec/agents` (and by extension, the `Agent` class) is browser-safe with no Node.js dependencies. However, `@kerberosec/core` and `KerberoSecCore` require Node.js 22+. If you import from `@kerberosec/sdk`, you get everything including the Node-only code. For browser usage, import directly from `@kerberosec/agents`:
...
- Using `KerberoSecCore` with compaction for long sessions
...
- `../kerberoseccore/REFERENCE.md` - Use KerberoSecCore for persistence
