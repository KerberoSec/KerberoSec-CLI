...
//
// Builds an SDK ApiHandler (from `@kerberosec/llms`) directly from the extension's
// legacy ApiConfiguration. This is the single inference path: the main task
// loop runs through KerberoSecCore (see kerberosec-session-factory.ts), and standalone
...
import { type ApiHandler, createHandler, type ProviderConfig } from "@kerberosec/llms"
...
export function buildSdkProviderConfig(
	...
	const providerId = (mode === "plan" ? configuration.planModeApiProvider : configuration.actModeApiProvider) ?? "kerberosec"
	...
	const base: ProviderConfig = {
		...
		// configuration (see .kerberosecrules/network.md).
		fetch,
