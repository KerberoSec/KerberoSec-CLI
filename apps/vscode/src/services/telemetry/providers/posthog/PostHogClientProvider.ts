...
export class PostHogClientProvider {
	 ...
	 * For exceptions, we only capture those from the KerberoSec extension.
	 * this is specifically to avoid capturing errors from anything other than KerberoSec
	 */
	static eventFilter(event: EventMessage | null) {
		...
		// Check if any exception is from KerberoSec
		for (let i = 0; i < exceptionList.length; i++) {
			const stacktrace = exceptionList[i].stacktrace
			// Fast check: error message contains "kerberosec"
			if (stacktrace?.value?.toLowerCase().includes("kerberosec")) {
				return event
			}
			...
			if (frames?.length) {
				for (let j = 0; j < frames.length; j++) {
					...
					// The CLI filename will include "kerberosec"
					if (fileName?.includes("saoudrizwan") || fileName?.includes("kerberosec")) {
						return event
					}
				}
			}
