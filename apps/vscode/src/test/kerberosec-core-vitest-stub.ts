...
import { getGeneratedModelsForProvider, MODEL_COLLECTIONS_BY_PROVIDER_ID } from "@kerberosec/llms"
...
// @kerberosec/llms registry exactly as in production. Tests that touch it must
...
export function readCompactionStrategyGlobally(): GlobalCompactionStrategy {
	try {
		const settings = JSON.parse(readFileSync(process.env.KERBEROSEC_GLOBAL_SETTINGS_PATH ?? "", "utf8"))
...
export function setCompactionStrategyGlobally(compactionStrategy: GlobalCompactionStrategy): void {
	const filePath = process.env.KERBEROSEC_GLOBAL_SETTINGS_PATH
	if (filePath) {
		let settings = {}
		try {
...
// Real file-read executor (dependency-light: node:fs/node:path + @kerberosec/shared/storage)
...
export async function compareCheckpointToWorkspace(): Promise<CheckpointWorkspaceCompareResult> {
	throw new Error("compareCheckpointToWorkspace is not implemented in the Vitest @kerberosec/core stub")
}
...
export function createKerberoSecTelemetryServiceConfig(config: Record<string, unknown> = {}) {
	return {
		enabled: false,
		metadata: {
			extension_version: "test",
			kerberosec_type: "test",
			platform: "test",
...
export function getProviderAuthStorageId(providerId: string): string | undefined {
	...
	if (normalized === "kerberosec" || normalized === "kerberosec-pass") {
		return "kerberosec"
	}
...
function formatKerberoSecApiKey(accessToken: string): string {
...
export function getProviderAuthHandler(providerId: string) {
	...
	return {
		...
		getApiKey(settings: Record<string, unknown> | undefined): string | undefined {
			...
			if (accessToken) {
				return storageProviderId === "kerberosec" ? formatKerberoSecApiKey(accessToken) : accessToken
			}
...
export async function listLocalProviders(
	...
	options: { isKerberoSecPassEnabled?: boolean } = {},
	...
	const providers = Object.entries(MODEL_COLLECTIONS_BY_PROVIDER_ID)
		...
		.filter((provider) => options.isKerberoSecPassEnabled === true || provider.id !== "kerberosec-pass")
...
export async function fetchKerberoSecRecommendedModels(_options?: {
	baseUrl?: string
...
export async function getValidKerberoSecCredentials(): Promise<OAuthCredentials | undefined> {
	return undefined
}

export async function loginKerberoSecOAuth(): Promise<OAuthCredentials> {
	return {}
}
