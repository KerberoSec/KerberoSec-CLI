...
import type { BannerRules } from "@shared/KerberoSecBanner"
import { expect } from "chai"
import * as sinon from "sinon"
import { KerberoSecEnv, Environment } from "@/config"
...
describe("BannerService", () => {
	...
	beforeEach(() => {
		...
		// Mock KerberoSecEnv.config() to return a valid config
		sandbox.stub(KerberoSecEnv, "config").returns({
			environment: Environment.production,
			appBaseUrl: "https://app.kerberosec-mock.bot",
			apiBaseUrl: "https://api.kerberosec-mock.bot",
			mcpBaseUrl: "https://api.kerberosec-mock.bot/v1/mcp",
		})
	...
	describe("IDE Type Detection", () => {
		...
		it('should return "jetbrains" when ide is "KerberoSec for JetBrains" (case-insensitive)', async () => {
			stubHostInfo({ ide: "KerberoSec for JetBrains" })
			mockFetch.resolves(createSuccessResponse(emptyResponse))
		...
		it("should prefer ide field over platform field for detection", async () => {
			stubHostInfo({ ide: "KerberoSec for JetBrains", platform: "Visual Studio Code 1.103.0" })
			mockFetch.resolves(createSuccessResponse(emptyResponse))
