...
import type { AgentToolContext } from "@kerberosec/shared";
...
describe("createShellExecutor", () => {
	...
	it("releases a detachable command while it continues in the background", async () => {
		...
		expect(result).toMatch(/kerberosec-command-.*output\.log/);
	...
	it("reaps stale detached logs left by a previous Hub process", async () => {
		...
		const staleDirectory = join(tempDirectory, "kerberosec-command-stale");
		const freshDirectory = join(tempDirectory, "kerberosec-command-fresh");
	...
	it("tracks the detached command across host restarts until it exits", async () => {
		...
		const liveDirectory = join(tempDirectory, "kerberosec-command-live");
		const completedDirectory = join(
			tempDirectory,
			"kerberosec-command-completed",
		);
	...
	it("preserves a live log through an identity probe failure", async () => {
		...
		const directory = join(tempDirectory, "kerberosec-command-probe-recovery");
	...
	it("never retires a live log while identity probing stays unavailable", async () => {
		...
		const directory = join(tempDirectory, "kerberosec-command-probe-timeout");
	...
	it("retries when an identity probe rejects", async () => {
		...
		const directory = join(tempDirectory, "kerberosec-command-probe-rejection");
		let probeAttempts = 0;
		try {
			await mkdir(directory);
			await Promise.all([
	...
	it("honors completion written before a leftover active marker", async () => {
		...
		const directory = join(tempDirectory, "kerberosec-command-completion-race");
