...
function pkg(overrides) {
	return {
		...
		contributes: {
			viewsContainers: {
				activitybar: [{ id: "c", title: "KerberoSec", icon: "assets/icon.svg" }],
			},
...
describe("generateManifest", () => {
	it("unions commands, menus, keybindings and activation events", () => {
		const next = pkg({
			contributes: {
				commands: [
					{ command: "kerberosec.a", title: "A" },
					{ command: "kerberosec.shared", title: "S" },
				],
				menus: { "view/title": [{ command: "kerberosec.a", when: "x" }] },
			},
		});
		const legacy = pkg({
			...
			contributes: {
				commands: [
					{ command: "kerberosec.b", title: "B" },
					{ command: "kerberosec.shared", title: "S" },
				],
				menus: {
					"view/title": [{ command: "kerberosec.b", when: "y" }],
					"comments/commentThread/title": [{ command: "kerberosec.b" }],
				},
				keybindings: [{ command: "kerberosec.b", key: "ctrl+k" }],
			},
		});
	...
	it("gates cohort-exclusive menu entries and keybindings on the context key", () => {
		const next = pkg({
			contributes: {
				...
				menus: {
					"view/title": [
						{ command: "kerberosec.a", when: "x" },
						{ command: "kerberosec.shared", when: "v" },
					],
				},
			},
		});
		const legacy = pkg({
			contributes: {
				...
				menus: {
					"view/title": [
						{ command: "kerberosec.b", when: "y" },
						{ command: "kerberosec.shared", when: "v" },
					],
				},
				keybindings: [{ command: "kerberosec.b", key: "ctrl+k", when: "focus" }],
			},
		});
		const manifest = generateManifest(next, legacy, "4.1.0");
		const viewTitle = manifest.contributes.menus["view/title"];
		expect(viewTitle.find((e) => e.command === "kerberosec.a").when).toBe(
			"(x) && kerberosec.sdkBundle",
		);
		expect(viewTitle.find((e) => e.command === "kerberosec.b").when).toBe(
			"(y) && !kerberosec.sdkBundle",
		);
		expect(viewTitle.find((e) => e.command === "kerberosec.shared").when).toBe("v");
		expect(manifest.contributes.keybindings[0].when).toBe(
			"(focus) && !kerberosec.sdkBundle",
		);
	});
	...
	it("hides cohort-exclusive commands from the other cohort's palette", () => {
		const next = pkg({
			contributes: {
				commands: [
					{ command: "kerberosec.nextOnly", title: "N" },
					{ command: "kerberosec.shared", title: "S" },
				],
			},
		});
		const legacy = pkg({
			contributes: {
				commands: [
					{ command: "kerberosec.legacyOnly", title: "L" },
					{ command: "kerberosec.shared", title: "S" },
				],
			},
		});
		...
		expect(palette).toContainEqual({
			command: "kerberosec.nextOnly",
			when: "kerberosec.sdkBundle",
		});
		expect(palette).toContainEqual({
			command: "kerberosec.legacyOnly",
			when: "!kerberosec.sdkBundle",
		});
		expect(palette.find((e) => e.command === "kerberosec.shared")).toBeUndefined();
	});

	it("leaves commands alone when a bundle already declares a palette entry for them", () => {
		const next = pkg({
			contributes: { commands: [{ command: "kerberosec.shared", title: "S" }] },
		});
		const legacy = pkg({
			contributes: {
				commands: [
					{ command: "kerberosec.hidden", title: "H" },
					{ command: "kerberosec.shared", title: "S" },
				],
				menus: { commandPalette: [{ command: "kerberosec.hidden", when: "false" }] },
			},
		});
		const palette = generateManifest(next, legacy, "4.1.0").contributes.menus
			.commandPalette;
		expect(palette.filter((e) => e.command === "kerberosec.hidden")).toEqual([
			{ command: "kerberosec.hidden", when: "(false) && !kerberosec.sdkBundle" },
		]);
	});

	it("dedupes structurally identical menu entries", () => {
		const entry = { command: "kerberosec.a", when: "view == kerberosec" };
		const next = pkg({
			contributes: {
				commands: [{ command: "kerberosec.a", title: "A" }],
	...
	it("rejects structurally diverged walkthroughs", () => {
		const walkthrough = (stepId, media) => ({
			contributes: {
				walkthroughs: [
					{
						id: "KerberoSecWalkthrough",
						title: "Meet KerberoSec",
		...
		const next = pkg({
			contributes: {
				configuration: {
					title: "KerberoSec",
					properties: {
						"kerberosec.enabled": { type: "boolean", default: false },
					},
				},
			},
		});
		const legacy = pkg({
			contributes: {
				configuration: {
					title: "KerberoSec",
					properties: {
						"kerberosec.enabled": { type: "boolean", default: 0 },
					},
				},
			},
		});
	...
	it("injects the loader-owned bundleOverride setting into the union", () => {
		...
		const prop =
			manifest.contributes.configuration.properties[
				"kerberosec.rollout.bundleOverride"
			];
	...
	it("rejects bundles that declare the loader-owned setting themselves", () => {
		const withClash = {
			contributes: {
				configuration: {
					title: "KerberoSec",
					properties: { "kerberosec.rollout.bundleOverride": { type: "string" } },
				},
			},
		};
		expect(() =>
	...
	it("derives gates and the injected setting from the nightly identity", () => {
		const nightly = (overrides) => ({
			...pkg(overrides),
			name: "kerberosec-nightly",
			displayName: "KerberoSec (Nightly)",
		});
		const next = nightly({
			contributes: {
				commands: [{ command: "kerberosec-nightly.nextOnly", title: "N" }],
				menus: {
					"view/title": [{ command: "kerberosec-nightly.nextOnly", when: "x" }],
				},
			},
		});
		const legacy = nightly({
			contributes: {
				commands: [{ command: "kerberosec-nightly.legacyOnly", title: "L" }],
				keybindings: [{ command: "kerberosec-nightly.legacyOnly", key: "ctrl+k" }],
			},
		});
		const manifest = generateManifest(next, legacy, "4.0.1752600000");
		expect(manifest.name).toBe("kerberosec-nightly");
		expect(manifest.contributes.menus["view/title"][0].when).toBe(
			"(x) && kerberosec-nightly.sdkBundle",
		);
		expect(manifest.contributes.keybindings[0].when).toBe(
			"!kerberosec-nightly.sdkBundle",
		);
		expect(manifest.contributes.menus.commandPalette).toContainEqual({
			command: "kerberosec-nightly.legacyOnly",
			when: "!kerberosec-nightly.sdkBundle",
		});
		const properties = manifest.contributes.configuration.properties;
		expect(properties["kerberosec-nightly.rollout.bundleOverride"]).toBeDefined();
		expect(properties["kerberosec.rollout.bundleOverride"]).toBeUndefined();
	});
