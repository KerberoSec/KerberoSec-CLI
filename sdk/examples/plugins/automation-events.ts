 ...
 * KerberoSecCore automation without importing cron internals.
 *
 * Local demo:
 *   kerberosec plugin install https://github.com/kerberosec/kerberosec/blob/main/sdk/examples/plugins/automation-events.ts --cwd .
 *   mkdir -p .kerberosec/cron/events
 *   cp examples/cron/events/local-plugin-event.event.md .kerberosec/cron/events/local-plugin-event.event.md
 *   perl -0pi -e "s#/absolute/path/to/repo#$PWD#g" .kerberosec/cron/events/local-plugin-event.event.md
 *   KERBEROSEC_LOCAL_EVENT_INTERVAL_MS=2000 kerberosec -i "wait for the plugin event"
 */
...
export const plugin: AgentPlugin = {
	...
	setup(api, ctx) {
		...
		const intervalMs = Number(
			process.env.KERBEROSEC_LOCAL_EVENT_INTERVAL_MS ?? 0,
		);
