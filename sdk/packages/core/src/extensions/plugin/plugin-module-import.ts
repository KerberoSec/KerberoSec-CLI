...
import { PLUGIN_FILE_EXTENSIONS } from "@kerberosec/shared";
...
const HOST_PROVIDED_SDK_SPECIFIERS = [
	...
	"@kerberosec/core",
	"@kerberosec/core/hub",
	"@kerberosec/core/hub/daemon-entry",
	"@kerberosec/core/telemetry",
	"@kerberosec/llms",
	"@kerberosec/llms/browser",
	"@kerberosec/shared",
	"@kerberosec/shared/automation",
	"@kerberosec/shared/browser",
	"@kerberosec/shared/storage",
	"@kerberosec/shared/db",
	"@kerberosec/shared/types",
];
...
function collectWorkspaceAliases(root: string): Record<string, string> {
	...
	const candidates: Record<string, string> = {
		"@kerberosec/sdk": resolve(root, "packages/sdk/src/index.ts"),
		"@kerberosec/agents": resolve(root, "packages/agents/src/index.ts"),
		"@kerberosec/core": resolve(root, "packages/core/src/index.ts"),
		"@kerberosec/llms": resolve(root, "packages/llms/src/index.ts"),
		"@kerberosec/shared": resolve(root, "packages/shared/src/index.ts"),
		"@kerberosec/shared/storage": resolve(
			root,
			"packages/shared/src/storage/index.ts",
		),
		"@kerberosec/shared/db": resolve(root, "packages/shared/src/db/index.ts"),
	};
...
function isKerberoSecSdkSpecifier(specifier: string): boolean {
	return getPackageName(specifier).startsWith("@kerberosec/");
}
...
function getHostPackageSearchRoots(): string[] {
	...
	const wrapperPath = process.env.KERBEROSEC_WRAPPER_PATH?.trim();
	if (wrapperPath) {
...
function isPackageBasedPlugin(pluginFilePath: string): boolean {
	// Walk up from the plugin file looking for a package.json with a `kerberosec`
	// manifest. Stop at the first package.json we encounter; if it doesn't
	// declare `kerberosec` we've left the plugin boundary (e.g. hit the workspace
	// root).  Also cap the traversal so we never wander far from the plugin
	// search root (.kerberosec/plugins).
	const MAX_DEPTH = 4;
	let current = dirname(pluginFilePath);
	for (let depth = 0; depth < MAX_DEPTH; depth++) {
		...
		if (existsSync(packageJsonPath)) {
			try {
				...
				return pkg != null && typeof pkg === "object" && "kerberosec" in pkg;
			} catch {
				return false;
			}
		}
...
function isTypePositionImport(source: string, matchIndex: number): boolean {
	const prefix = source.slice(Math.max(0, matchIndex - 80), matchIndex);
	return /(?:type\s+[\w$]+\s*=\s*|\btypeof\b|\bas\b|\bextends\b|[:<|&,])\s*$/.test(
		prefix,
	);
}
...
function collectStaticModuleSpecifiers(source: string): string[] {
	...
	// 1. import ... from "..." (excluding `import type ...`)
	for (const match of source.matchAll(
		/\bimport\s+(?!type\b)(?:[^"'`]*?\bfrom\s*)?["'`]([^"'`]+)["'`]/g,
	)) {
		if (match[1]) specifiers.add(match[1]);
	}
	// 2. export ... from "..." (excluding `export type ...`)
	for (const match of source.matchAll(
	...
	for (const match of source.matchAll(
		...
		if (
			...
			!isTypePositionImport(source, match.index)
		) {
	...
	for (const specifier of collectStaticModuleSpecifiers(source)) {
		...
		if (isBareSpecifier(specifier)) {
			if (
				...
				(isKerberoSecSdkSpecifier(specifier) &&
...
function collectPluginImportAliases(
	...
	for (const specifier of staticSpecifiers) {
		if (
			...
			(isKerberoSecSdkSpecifier(specifier) || preferHostRuntimeDependencies)
		) {
...
export async function importPluginModule(
	...
	const jiti = createJiti(pluginPath, {
		...
		// every nested import in the plugin, sees `import "@kerberosec/core"` with no
