...
import { KerberoSecAuthContext, KerberoSecAuthContextType, KerberoSecAuthProvider, useKerberoSecAuth } from "@/context/KerberoSecAuthContext"
import {
...
const KerberoSecAuthProviderWithOverrides: React.FC<{
	overrides?: Partial<KerberoSecAuthContextType>
	children: React.ReactNode
}> = ({ overrides, children }) => {
	const authContext = useKerberoSecAuth()
	return <KerberoSecAuthContext.Provider value={{ ...authContext, ...overrides }}>{children}</KerberoSecAuthContext.Provider>
}

export const createStorybookDecorator =
	(overrideStates?: Partial<ExtensionStateContextType>, classNames?: string, authOverrides?: Partial<KerberoSecAuthContextType>) =>
	(Story: any) => (
		<ExtensionStateProviderWithOverrides overrides={overrideStates}>
			<KerberoSecAuthProviderWithOverrides overrides={authOverrides}>
				<div className={cn("max-w-lg mx-auto", classNames)}>
					<Story />
				</div>
			</KerberoSecAuthProviderWithOverrides>
