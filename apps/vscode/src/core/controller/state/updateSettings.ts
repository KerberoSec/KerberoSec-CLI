import { setCompactionStrategyGlobally, setModelToolEnabledGlobally } from "@kerberosec/core"
import { Empty } from "@shared/proto/kerberosec/common"
import { PlanActMode, McpDisplayMode as ProtoMcpDisplayMode, UpdateSettingsRequest } from "@shared/proto/kerberosec/state"
...
export async function updateSettings(controller: Controller, request: UpdateSettingsRequest): Promise<Empty> {
	try {
		if (request.kerberosecEnv !== undefined && request.kerberosecEnv !== "") {
			KerberoSecEnv.setEnvironment(request.kerberosecEnv)
