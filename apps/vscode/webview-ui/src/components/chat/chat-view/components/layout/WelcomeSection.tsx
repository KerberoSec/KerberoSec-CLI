import { BANNER_DATA, BannerAction, BannerActionType, BannerCardData } from "@shared/kerberosec/banner"
import { EmptyRequest } from "@shared/proto/kerberosec/common"
import type { Worktree } from "@shared/proto/kerberosec/worktree"
import { TrackWorktreeViewOpenedRequest } from "@shared/proto/kerberosec/worktree"
...
export const WelcomeSection: React.FC<WelcomeSectionProps> = ({
	...
	const bannerConfig = useMemo((): BannerCardData[] => {
		...
		return BANNER_DATA.filter((banner) => {
			...
			if (banner.isKerberoSecUserOnly !== undefined) {
				return banner.isKerberoSecUserOnly === !!kerberosecUser
			}
	...
	}, [isBannerDismissed, kerberosecUser])
		...
		(action: BannerAction) => {
			switch (action.action) {
				...
				case BannerActionType.InstallCli:
					StateServiceClient.installKerberoSecCli({}).catch((error) =>
	...
	}, [bannerConfig, banners, kerberosecUser, handleBannerAction, handleBannerDismiss])
								...
								{currentWorktree && (
									<Tooltip>
										...
										<TooltipContent side="bottom">
											View and manage git worktrees. Great for running parallel KerberoSec tasks.
										</TooltipContent>
									</Tooltip>
								)}
							</div>
