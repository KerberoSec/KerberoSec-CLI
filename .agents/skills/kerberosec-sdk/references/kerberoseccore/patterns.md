...
```typescript
import { KerberoSecCore, createTool } from "@kerberosec/sdk"
import { z } from "zod"
...
```typescript
config: {
  pluginPaths: ["./my-kerberosec-plugin"],
  extensionContext: {
    workspace: { rootPath: process.cwd(), cwd: process.cwd() },
  },
}
...
const sessions = await kerberosec.list(10)
for (const session of sessions) {
  console.log(`${session.id}: ${session.title}`)
}

// Read messages from a past session
const messages = await kerberosec.readMessages(sessions[0].id)
for (const msg of messages) {
  console.log(`[${msg.role}] ${msg.content}`)
}

// Check usage
const usage = await kerberosec.getAccumulatedUsage(sessions[0].id)
...
process.on("SIGTERM", async () => {
  await kerberosec.dispose("SIGTERM received")
  process.exit(0)
})
...
const viewer = await KerberoSecCore.create({
