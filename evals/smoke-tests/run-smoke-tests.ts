#!/usr/bin/env npx tsx
/**
 * Smoke Test Runner for KerberoSec
 *
...
const DEFAULT_PROVIDER = "kerberosec"
const DEFAULT_MODEL = "anthropic/claude-sonnet-4.5"

// Models to test - can be overridden with --model flag
const MODELS: string[] = [DEFAULT_MODEL]

// Check if kerberosec CLI is available
function checkKerberoSecCli(): boolean {
	try {
		execSync("which kerberosec", { encoding: "utf-8", timeout: 5000 })
		return true
	} catch {
		return false
	}
...
// Use user's existing KerberoSec config (already has auth configured)
// For CI, this would be set up by the auth step before tests run
const KERBEROSEC_CONFIG_DIR = path.join(process.env.HOME || "", ".kerberosec")
...
function configureAuth(options: { provider: string; apiKey: string; modelId: string; baseUrl?: string }): {
	...
	fs.mkdirSync(KERBEROSEC_CONFIG_DIR, { recursive: true })

	try {
		// Run quick auth setup (non-interactive when all flags provided)
		const args = [`kerberosec auth --config "${KERBEROSEC_CONFIG_DIR}"`, `-p "${options.provider}"`, `-k "${options.apiKey}"`, `-m "${options.modelId}"`]
		if (options.baseUrl) {
...
function ensureScenarioAuth(scenario: SmokeScenario, modelId: string): { ok: boolean; error?: string } {
	...
	const apiKeyEnv = scenario.auth?.apiKeyEnv || (provider === DEFAULT_PROVIDER ? "KERBEROSEC_API_KEY" : undefined)
	...
	// For default provider, local developers can rely on existing ~/.kerberosec auth.
...
async function runTrial(scenario: SmokeScenario, modelId: string, trialWorkdir: string): Promise<TrialResult> {
	...
	// Provider is configured via `kerberosec auth` before running tests
	const args = [
		"--config",
		KERBEROSEC_CONFIG_DIR, // Use shared config directory for auth
	...
	try {
		// Run kerberosec CLI
		const result = await runKerberoSecWithTimeout(args, trialWorkdir, scenario.timeout * 1000)
...
// Run kerberosec CLI with timeout
interface KerberoSecResult {
	success: boolean
	error?: string
	stdout: string
	stderr: string
...
function runKerberoSecWithTimeout(args: string[], cwd: string, timeoutMs: number): Promise<KerberoSecResult> {
	return new Promise((resolve) => {
		let stdout = ""
		let stderr = ""

		const proc = spawn("kerberosec", args, {
			cwd,
			env: { ...process.env },
...
async function main() {
	...
	// Check kerberosec CLI is available
	if (!checkKerberoSecCli()) {
		console.error("ERROR: kerberosec CLI not found in PATH")
		console.error("")
		console.error("For local development:")
		console.error("  Install kerberosec from npm or link the SDK CLI from apps/cli")
		console.error("")
		console.error("For CI:")
		console.error("  Ensure a kerberosec binary is available on PATH")
		process.exit(1)
	}
