...
vi.mock("@/lib/provider-model-catalog", () => ({
	...
	VOICE_INPUT_SETTINGS_CHANGED_EVENT:
		"kerberosec:test-voice-input-settings-changed",
}));
...
beforeEach(() => {
	...
	loadProviderModelCatalogMock.mockReset().mockResolvedValue({
		providers: [],
		enabledProviderIds: ["kerberosec"],
		providerModels: { kerberosec: ["test-model"] },
		providerReasoningModels: { kerberosec: [] },
		voiceInput: null,
	});
...
const workspaceValue = {
	workspaceRoot: "/workspace/kerberosec",
	workspaces: ["/workspace/kerberosec"],
	listWorkspaces: vi.fn(async () => ["/workspace/kerberosec"]),
...
describe("ChatInputBar", () => {
	...
	it("discards a pending batch transcript when the voice target changes", async () => {
		...
		await act(async () => {
			window.dispatchEvent(
				new Event("kerberosec:test-voice-input-settings-changed"),
			);
		});
		await vi.waitFor(() => {
			expect(speechInputMockState.current?.recordingMode).toBe("streaming");
	...
	it("preserves an explicit High selection across capability and status updates", async () => {
		...
		await render("idle");
		await vi.waitFor(() => {
			expect(loadProviderModelsMock).toHaveBeenCalledWith("kerberosec");
		});
		const providerModelsListener =
			subscribeToProviderModelsMock.mock.calls[0]?.[0];
		await act(async () => {
			providerModelsListener?.("kerberosec", [
		...
		expect(workspaceTrigger?.textContent).toContain("kerberosec");
	...
	it("selects High from the supported model thinking menu", async () => {
		loadProviderModelCatalogMock.mockResolvedValue({
			...
			enabledProviderIds: ["kerberosec"],
			providerModels: { kerberosec: ["test-model"] },
			providerReasoningModels: { kerberosec: ["test-model"] },
		});
		const onReasoningChange = vi.fn();
		await act(async () => {
			root.render(
	...
	it("does not overwrite the remembered model when rendering a session's provider/model", async () => {
		...
		window.localStorage.setItem(
			MODEL_SELECTION_STORAGE_KEY,
			JSON.stringify({
				lastProvider: "kerberosec",
				lastModelByProvider: { kerberosec: "test-model" },
			}),
		);
		loadProviderModelCatalogMock.mockResolvedValue({
			providers: [],
	...
	it("renders the kerberosec model picker with recommended and free sections", async () => {
		loadProviderModelCatalogMock.mockResolvedValue({
			providers: [],
			enabledProviderIds: ["kerberosec"],
			providerModels: {
			...
			providerNames: { kerberosec: "KerberoSec" },
			providerReasoningModels: { kerberosec: [] },
		});
		...
		await vi.waitFor(() => {
			expect(providerTrigger?.textContent).toContain("KerberoSec");
		});
		const modelTrigger = container.querySelector<HTMLButtonElement>(
			'[aria-label^="Model:"]',
		);
		...
		expect(
			panel?.querySelector(".kerberosec-ui-search-combobox__badge")
				?.textContent,
		).toBe("NEW");
	...
	describe("kerberosec-pass picker offer", () => {
		...
		beforeEach(() => {
			// The KerberoSecPass offer: one subscribed and one free model, stamped
			...
			loadProviderModelCatalogMock.mockResolvedValue({
				providers: [],
				enabledProviderIds: ["kerberosec", "kerberosec-pass"],
				providerModels: {
					kerberosec: ["test-model"],
					"kerberosec-pass": [
						"openai/gpt-5",
				...
				providerNames: {
					kerberosec: "KerberoSec",
					"kerberosec-pass": "KerberoSecPass",
				},
				providerReasoningModels: { kerberosec: [], "kerberosec-pass": [] },
			});
		});
		...
		it("does not resurrect a stale remembered model the picker hides", async () => {
			window.localStorage.setItem(
				MODEL_SELECTION_STORAGE_KEY,
				JSON.stringify({
					lastProvider: "kerberosec-pass",
					lastModelByProvider: { "kerberosec-pass": "legacy/stale-model" },
				}),
			);
		...
		it("falls back to a visible model when switching providers with a hidden remembered model", async () => {
			window.localStorage.setItem(
				...
				JSON.stringify({
					...
					lastModelByProvider: {
						kerberosec: "test-model",
						"kerberosec-pass": "legacy/stale-model",
					},
				}),
			);
			const onModelChange = vi.fn();
			...
			const panel = document.querySelector('[role="dialog"]');
			const kerberosecPassOption = [
				...(panel?.querySelectorAll<HTMLButtonElement>('[role="option"]') ??
					[]),
			].find((option) => option.textContent?.includes("KerberoSecPass"));
			await act(async () => kerberosecPassOption?.click());

			expect(onProviderChange).toHaveBeenCalledWith("kerberosec-pass");
