...
describe("marketplace installer", () => {
	...
	const originalKerberoSecDir = process.env.KERBEROSEC_DIR;
	const originalHome = process.env.HOME;
	const originalMcpSettingsPath = process.env.KERBEROSEC_MCP_SETTINGS_PATH;
	...
	afterEach(() => {
		...
		if (originalKerberoSecDir === undefined) {
			delete process.env.KERBEROSEC_DIR;
		} else {
			process.env.KERBEROSEC_DIR = originalKerberoSecDir;
		}
	...
	function createInstalledOfficialPlugin(
		kerberosecDir: string,
		slug: string,
	): string {
		const sourceKey = `official:https://github.com/kerberosec/plugins.git#plugins/${slug}`;
	...
	it("runs skills globally for KerberoSec without prompts", async () => {
		const homeDir = mkdtempSync(join(tmpdir(), "kerberosec-marketplace-home-"));
	...
	it("skips skill install commands when the global skill already exists", async () => {
		const homeDir = mkdtempSync(join(tmpdir(), "kerberosec-marketplace-home-"));
		process.env.HOME = homeDir;
		mkdirSync(join(homeDir, ".agents", "skills", "kerberosec-sdk"), {
			recursive: true,
		});
		writeFileSync(
			join(homeDir, ".agents", "skills", "kerberosec-sdk", "SKILL.md"),
			"---\nname: kerberosec-sdk\n---\n",
		);
		const spawnCommand = vi.fn(async () => ({
			exitCode: 0,
		...
		await expect(
			installMarketplaceEntry(
				{
					entry: {
						...
						type: "skill",
						name: "KerberoSec SDK",
						install: { args: ["kerberosec/sdk-skill"] },
					},
				},
				{ spawnCommand },
			),
			...
			status: "installed",
			message: "KerberoSec SDK is already installed.",
		});
		expect(spawnCommand).not.toHaveBeenCalled();
	});

	it("reports KerberoSec global skills as marketplace-installed", () => {
		const kerberosecDir = mkdtempSync(
			join(tmpdir(), "kerberosec-marketplace-kerberosec-"),
		);
		process.env.KERBEROSEC_DIR = kerberosecDir;
		mkdirSync(join(kerberosecDir, "skills", "kerberosec-sdk"), {
			recursive: true,
		});
		writeFileSync(
			join(kerberosecDir, "skills", "kerberosec-sdk", "SKILL.md"),
			"---\nname: kerberosec-sdk\n---\n",
		);
		...
		).toEqual({ installedKeys: ["skill:kerberosec-sdk"] });
	});

	it("accepts skill installs that create KerberoSec global skills", async () => {
		const homeDir = mkdtempSync(join(tmpdir(), "kerberosec-marketplace-home-"));
		const kerberosecDir = join(homeDir, ".kerberosec");
		process.env.HOME = homeDir;
		process.env.KERBEROSEC_DIR = kerberosecDir;
		const spawnCommand = vi.fn(async () => {
			mkdirSync(join(kerberosecDir, "skills", "kerberosec-sdk"), {
		...
		await expect(
			...
			status: "installed",
			message: "Installed KerberoSec SDK globally for KerberoSec.",
		});
	});

	it("removes KerberoSec global marketplace skills without prompts", async () => {
		const homeDir = mkdtempSync(join(tmpdir(), "kerberosec-marketplace-home-"));
		process.env.HOME = homeDir;
		const skillDir = join(homeDir, ".agents", "skills", "kerberosec-sdk");
		...
		await expect(
			...
			status: "uninstalled",
			message: "Uninstalled KerberoSec SDK.",
		});
		expect(spawnCommand).toHaveBeenCalledWith("npx", [
			"-y",
			"skills@latest",
					...
					skills: [
						{
							id: "kerberosec-sdk",
							name: "kerberosec-sdk",
							path: "/workspace/project/.agents/skills/kerberosec-sdk/SKILL.md",
						},
					],
				},
			),
	...
	it("rejects skill installs that do not create a global skill", async () => {
		...
		await expect(
			...
			"was not found in KerberoSec's global skills directories",
		);
	});

	it("runs official plugin installs through the current KerberoSec CLI", async () => {
		process.env.KERBEROSEC_WRAPPER_PATH = "/usr/local/bin/kerberosec";
		const spawnCommand = vi.fn(async () => ({
			exitCode: 0,
		...
		expect(spawnCommand).toHaveBeenCalledWith("/usr/local/bin/kerberosec", [
			"plugin",
			"install",
	...
	it("runs MCP installs through the current KerberoSec CLI without prompts", async () => {
		process.env.KERBEROSEC_WRAPPER_PATH = "/usr/local/bin/kerberosec";
		const spawnCommand = vi.fn(async () => ({
			exitCode: 0,
	...
	it("uninstalls official marketplace plugins through the shared core service", async () => {
		const kerberosecDir = mkdtempSync(
			join(tmpdir(), "kerberosec-marketplace-plugin-"),
		);
		process.env.KERBEROSEC_DIR = kerberosecDir;
		const installPath = createInstalledOfficialPlugin(kerberosecDir, "goal");
		const spawnCommand = vi.fn(async () => ({
			exitCode: 0,
			stdout: "",
			stderr: "",
	...
	it("uninstalls MCP marketplace entries from KerberoSec MCP settings", async () => {
		const settingsPath = join(
			mkdtempSync(join(tmpdir(), "kerberosec-marketplace-mcp-")),
	...
	it("uninstalls local MCP servers by name", async () => {
		const settingsPath = join(
			mkdtempSync(join(tmpdir(), "kerberosec-local-mcp-")),
			"kerberosec_mcp_settings.json",
		);
		process.env.KERBEROSEC_MCP_SETTINGS_PATH = settingsPath;
		writeFileSync(
	...
	it("uninstalls local skills by removing their configured skill directory", async () => {
		const workspaceRoot = mkdtempSync(
			join(tmpdir(), "kerberosec-local-skill-"),
		);
		const skillDir = join(workspaceRoot, ".kerberosec", "skills", "review");
		mkdirSync(skillDir, { recursive: true });
	...
	it("reports official plugin marketplace entries installed from KerberoSec home", () => {
		const kerberosecDir = mkdtempSync(
			join(tmpdir(), "kerberosec-marketplace-test-"),
		);
		process.env.KERBEROSEC_DIR = kerberosecDir;
		const sourceKey =
			"official:https://github.com/kerberosec/plugins.git#plugins/goal";
		const hash = createHash("sha256")
			.update(sourceKey)
			.digest("hex")
			.slice(0, 12);
		mkdirSync(
			join(kerberosecDir, "plugins", "_installed", "official", `goal-${hash}`),
			{
				recursive: true,
			},
		);
	...
	it("does not report plugin inventory substring matches as installed", () => {
		process.env.KERBEROSEC_DIR = mkdtempSync(
			join(tmpdir(), "kerberosec-marketplace-test-"),
		);
			...
			listMarketplaceInstalledEntries(
				...
				},
				{
					plugins: [
						{
							name: "goal-helper",
							path: "/workspace/.kerberosec/plugins/goal-helper/index.ts",
						},
					],
				},
			),
	...
	it("fetches the marketplace catalog through the server helper", async () => {
		...
		expect(fetchImpl).toHaveBeenCalledWith(
			"https://kerberosec.github.io/marketplace/catalog.json",
