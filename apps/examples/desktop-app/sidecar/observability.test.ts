...
const mocks = vi.hoisted(() => ({
	...
	createKerberoSecTelemetryServiceConfig: vi.fn((config: unknown) => config),
...
vi.mock("@kerberosec/core", async () => {
	...
	return {
		...
		createKerberoSecTelemetryServiceConfig:
			mocks.createKerberoSecTelemetryServiceConfig,
...
describe("desktop observability", () => {
	...
	it("configures desktop telemetry, identity, activation, and lifecycle", async () => {
		...
		expect(mocks.createKerberoSecTelemetryServiceConfig).toHaveBeenCalledWith({
