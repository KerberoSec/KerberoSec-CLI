...
export async function runAuthCommand(input: AuthCommandInput): Promise<number> {
	...
	if (input.explicitProvider?.trim()) {
		...
		if (providerId === "ollama") {
			const { runOllamaSetupWorkflow } = await import(
				"../utils/ollama-manager"
			);
			const result = await runOllamaSetupWorkflow(
				input.providerSettingsManager,
				{
					explicitModel: input.modelid,
					onLog: (msg) => input.io.writeln(msg),
				},
			);
...
export async function runAuthProviderCommand(
	...
	if (!isOAuthProvider(providerId)) {
		io.writeErr(
			`provider "${providerId}" does not support OAuth login (supported: kerberosec, openai-codex, oca)`,
		);
		return 1;
	}
	try {
