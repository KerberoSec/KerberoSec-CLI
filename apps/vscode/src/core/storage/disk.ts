...
export const GlobalFileNames = {
	...
	kerberosecRecommendedModels: "kerberosec_recommended_models.json",
	mcpSettings: "kerberosec_mcp_settings.json",
	kerberosecRules: ".kerberosecrules",
	workflows: ".kerberosecrules/workflows",
	hooksDir: ".kerberosecrules/hooks",
...
/**
 * Returns the cross-platform path to the KerberoSec home directory (~/.kerberosec).
 * This works on macOS, Linux, and Windows:
 * - macOS: /Users/username/.kerberosec
 * - Linux: /home/username/.kerberosec
 * - Windows: C:\Users\username\.kerberosec
 *
 * This is intended to eventually replace ~/Documents/KerberoSec as the global config location.
 */
...
export async function ensureRulesDirectoryExists(): Promise<string> {
	...
	const kerberosecRulesDir = path.join(userDocumentsPath, "KerberoSec", "Rules")
	try {
		await fs.mkdir(kerberosecRulesDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "KerberoSec", "Rules") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return kerberosecRulesDir
}

export async function ensureWorkflowsDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const kerberosecWorkflowsDir = path.join(userDocumentsPath, "KerberoSec", "Workflows")
	try {
		await fs.mkdir(kerberosecWorkflowsDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "KerberoSec", "Workflows") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return kerberosecWorkflowsDir
}

export async function ensureMcpServersDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const mcpServersDir = path.join(userDocumentsPath, "KerberoSec", "MCP")
	try {
		await fs.mkdir(mcpServersDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "KerberoSec", "MCP") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine since this path is only ever used in the system prompt
	}
...
export async function ensureHooksDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const kerberosecHooksDir = path.join(userDocumentsPath, "KerberoSec", "Hooks")
	try {
		await fs.mkdir(kerberosecHooksDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "KerberoSec", "Hooks") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return kerberosecHooksDir
}

/**
 * Returns the global skills directory path (~/.kerberosec/skills) without creating it.
 */
 ...
 * 3. Each workspace root's .kerberosecrules/hooks directory (if they exist)
 *
 ...
 * identity must come from the window's host, not global state: ~/.kerberosec global
 * state is shared across every KerberoSec instance, so a persisted value can point
...
/**
 * Gets the paths to the workspace's .kerberosecrules/hooks directories to search for
	...
	return (
		await Promise.all(
			workspaceRootPaths.map(async (workspaceRootPath) => {
				// Look for a .kerberosecrules/hooks folder in this workspace root.
