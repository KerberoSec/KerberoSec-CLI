...
function mergeToggleStateAfterScan(
	scanned: KerberoSecRulesToggles,
	preScan: KerberoSecRulesToggles,
	current: KerberoSecRulesToggles,
): KerberoSecRulesToggles {
	const merged: KerberoSecRulesToggles = {}
...
async function doRefreshWorkflowToggles(
	...
	const globalKerberoSecWorkflowsFilePath = await ensureWorkflowsDirectoryExists()
	const scannedGlobalToggles = await synchronizeRuleToggles(globalKerberoSecWorkflowsFilePath, globalWorkflowToggles)
