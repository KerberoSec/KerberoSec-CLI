import { AskResponseRequest } from "@shared/proto/kerberosec/task"
...
const KERBEROSEC_PASS_SUBSCRIBE_PATH = "dashboard/subscription"

const HEADLINE = "This model requires a KerberoSecPass subscription."
...
function buildSubscribeUrl(appBaseUrl?: string): string | undefined {
	...
	try {
		...
		const url = new URL(KERBEROSEC_PASS_SUBSCRIBE_PATH, base)
...
const EntitlementError: React.FC<EntitlementErrorProps> = ({ message }) => {
	...
	const subscribeUrl = buildSubscribeUrl(kerberosecUser?.appBaseUrl)
		...
		<div className="p-2 border-none rounded-md mb-2 bg-(--vscode-textBlockQuote-background)">
			<div className="mb-3">
				...
				<div className="text-(--vscode-descriptionForeground) text-xs">
					Subscribe to KerberoSecPass to use this model, then retry your request.
				</div>
				{backendDetail && (
