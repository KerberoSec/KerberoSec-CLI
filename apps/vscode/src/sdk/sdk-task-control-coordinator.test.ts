...
describe("SdkTaskControlCoordinator", () => {
	...
	it("cancels a running KerberoSec task when the user signs out", async () => {
		const activeSession = makeActiveSession()
		const { coordinator, options } = makeCoordinator({ activeSession })

		await coordinator.cancelKerberoSecTaskOnSignOut(true)

		expect(activeSession.sdkHost.abort).toHaveBeenCalledWith("session-123")
		expect(options.sessions.setRunning).toHaveBeenCalledWith(false)
	})

	it("does not cancel a non-KerberoSec task when the user signs out", async () => {
		const activeSession = makeActiveSession()
		const { coordinator, options } = makeCoordinator({ activeSession })

		await coordinator.cancelKerberoSecTaskOnSignOut(false)
	...
	it("drops the outgoing task's settings overlay when switching to another task", async () => {
		const { coordinator, options } = makeCoordinator({
			...
			hasHistoryItem: true,
			kerberosecMessages: [{ ts: 1, type: "say", say: "task", text: "hello" }],
	...
	it("shows a task by creating a proxy, loading messages, and appending a fresh resume ask", async () => {
		...
		const activeSession = makeActiveSession()
		const sdkKerberoSecMessages: KerberoSecMessage[] = [
		...
		const { coordinator, options, state } = makeCoordinator({
			...
			task: existingTask,
			hasHistoryItem: true,
			kerberosecMessages: sdkKerberoSecMessages,
			sessionStatus: "completed",
		})
		...
		expect(options.taskHistory.getKerberoSecMessages).toHaveBeenCalledWith("task-1")
		expect(state.task?.messageStateHandler.getKerberoSecMessages()).toEqual([
			{ ts: 1, type: "say", say: "task", text: "hello" },
			{ ts: 2, type: "ask", ask: "completion_result", text: "" },
	...
	it("clears a pending approval when switching tasks so the new task input is not consumed", async () => {
		...
		await vi.waitFor(() => expect(pendingTask.messageStateHandler.getKerberoSecMessages()).toHaveLength(1))
	...
	it("shows a legacy task with a warning and a resume ask", async () => {
		const legacyMessages: KerberoSecMessage[] = [{ ts: 1, type: "say", say: "task", text: "legacy task" }]
		const { coordinator, options, state } = makeCoordinator({
			hasHistoryItem: true,
			kerberosecMessages: legacyMessages,
			isLegacyTask: true,
		})
	...
	it("does not show a task that is missing from history", async () => {
		...
		expect(options.taskHistory.getKerberoSecMessages).not.toHaveBeenCalled()
	...
	it("appends a resume ask and sets the resumable phase when showing an interrupted (cancelled) task", async () => {
		...
		expect(state.task?.messageStateHandler.getKerberoSecMessages().at(-1)).toEqual(
			expect.objectContaining({ type: "ask", ask: "resume_task" }),
		)
	...
	it("abandons a superseded showTaskWithId so the newest selection wins", async () => {
		...
		let resolveFirstRead: ((messages: KerberoSecMessage[]) => void) | undefined
		options.taskHistory.getKerberoSecMessages.mockImplementationOnce(
			() =>
				new Promise<KerberoSecMessage[]>((resolve) => {
		...
		expect(state.task?.taskId).toBe("task-new")
		expect(state.task?.messageStateHandler.getKerberoSecMessages().length).toBeGreaterThan(0)
	...
	it("abandons a superseded showTaskWithId when the user clears the task", async () => {
		...
		let resolveRead: ((messages: KerberoSecMessage[]) => void) | undefined
		options.taskHistory.getKerberoSecMessages.mockImplementationOnce(
			() =>
				new Promise<KerberoSecMessage[]>((resolve) => {
					resolveRead = resolve
	...
	it("does not install the new task proxy until its messages are loaded", async () => {
		...
		let resolveGetKerberoSecMessages: ((messages: KerberoSecMessage[]) => void) | undefined
		const getKerberoSecMessagesDeferred = new Promise<KerberoSecMessage[]>((resolve) => {
			resolveGetKerberoSecMessages = resolve
		})
		...
		options.taskHistory.getKerberoSecMessages.mockReturnValueOnce(getKerberoSecMessagesDeferred)

		let setTaskHadMessages: boolean | undefined
		options.setTask.mockImplementation((task: any) => {
			setTaskHadMessages = (task?.messageStateHandler?.getKerberoSecMessages?.() ?? []).length > 0
			state.task = task
		})

		const inFlight = coordinator.showTaskWithId("task-1")

		// While getKerberoSecMessages is still pending, the new task proxy must not be
		...
		resolveGetKerberoSecMessages?.(sdkKerberoSecMessages)
		await inFlight
...
function makeCoordinator(input: Partial<MakeCoordinatorInput> = {}) {
	...
	const options = {
		...
		messages: {
			...
			finalizeMessagesForSave: vi.fn((messages: KerberoSecMessage[]) =>
		...
		taskHistory: {
			getKerberoSecMessages: vi.fn().mockResolvedValue(input.kerberosecMessages ?? []),
		...
		taskHistory: SdkTaskControlCoordinatorOptions["taskHistory"] & {
			...
			getKerberoSecMessages: ReturnType<typeof vi.fn>
...
function makeTask(taskId: string, messages: KerberoSecMessage[] = []) {
	return {
		taskId,
		messageStateHandler: {
			getKerberoSecMessages: vi.fn(() => messages),
			clear: vi.fn(),
		},
	}
}
