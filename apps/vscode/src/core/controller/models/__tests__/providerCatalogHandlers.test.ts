...
import { ApiFormat, ModelOverrides } from "@/shared/proto/kerberosec/models"
...
describe("provider model catalog handlers", () => {
	...
	it("readProviderConfig redacts secrets", async () => {
		...
		const providerId = parseProviderId("kerberosec")
		...
		const response = await readProviderConfig(controller, { value: "kerberosec" })
