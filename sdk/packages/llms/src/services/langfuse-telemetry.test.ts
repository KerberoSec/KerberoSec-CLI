...
describe("langfuse telemetry", () => {
	...
	it("enables telemetry for non-kerberosec providers when langfuse config is available", async () => {
		await expect(ensureLangfuseTelemetry("openrouter")).resolves.toBe(true);
		await expect(ensureLangfuseTelemetry("kerberosec")).resolves.toBe(true);
