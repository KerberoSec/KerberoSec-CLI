...
The KerberoSec SDK supports two models for multi-agent work: sub-agents (parent-child) and teams (peer-to-peer).
...
```
~/.kerberosec/data/teams/[team-name]/
...
```bash
kerberosec --team-name auth-sprint "Continue the auth refactor"
```
...
- `../kerberoseccore/api.md` - Session config for teams
