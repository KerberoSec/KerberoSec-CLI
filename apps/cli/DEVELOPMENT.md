...
This guide covers everything you need to build and run the KerberoSec CLI locally after cloning the repository. It includes setup instructions, a tech stack overview, and a walkthrough of the TUI architecture.
...
```bash
KERBEROSEC_BUILD_ENV=development bun --conditions=development ./src/index.ts
```
...
The `build:sdk` step is required because `bun link` runs without the `--conditions=development` flag, so Bun resolves workspace packages (`@kerberosec/llms`, `@kerberosec/core`, etc.) via their `package.json` exports which point to `dist/`. Without the build, those dist files don't exist and you'll get "Cannot find module" errors.

After linking, you can run `kerberosec` from any directory:
...
If you're using `bun run dev`, you don't need to rebuild after every SDK change -- dev mode resolves packages from source. But if you're using the linked `kerberosec` binary, you do need to rebuild for changes to take effect.
...
```bash
bun run dev -- --interactive --config /tmp/kerberosec-test
```

Or set `KERBEROSEC_FORCE_ONBOARDING=1` to force the onboarding view regardless of existing config.
...
bunx tuistory -s kerberosec --cols 120 --rows 36 -- bun src/index.ts --provider anthropic -m claude-sonnet-4-6 -k test-key
...
bunx tuistory -s kerberosec type "/settings"
bunx tuistory -s kerberosec press enter
bunx tuistory -s kerberosec snapshot --trim     # current screen as text
bunx tuistory -s kerberosec screenshot          # current screen as a styled PNG

# A human can watch/drive the same session from another terminal
tuistory attach -s kerberosec

# Tear down
bunx tuistory -s kerberosec close
```
...
```bash
cd apps/cli
KERBEROSEC_BUILD_ENV=development bun --conditions=development --inspect-brk=6499 ./src/index.ts
```
...
The CLI is published as the `kerberosec` wrapper package on npm with platform-specific binaries under `@kerberosec/cli-*`. The release flow lives in the `publish-cli` skill (`.kerberosec/skills/publish-cli/SKILL.md` at the repo root).
...
`kerberosec` uses a `pino`-backed adapter that targets the core `BasicLogger` contract:

- CLI runtime passes `logger` directly into local `@kerberosec/core` sessions.
...
After login, OAuth credentials are persisted with `auth.expiresAt`, and `@kerberosec/core` refreshes these tokens automatically during session turns. Provider auth and model settings should be changed through `kerberosec auth`, the interactive config UI, or core provider-settings APIs rather than editing provider settings files directly.

On startup, `kerberosec` also attempts a legacy settings import:

- Source files: `<KERBEROSEC_DATA_DIR>/globalState.json` and `<KERBEROSEC_DATA_DIR>/secrets.json`
- Target file: `<KERBEROSEC_DATA_DIR>/settings/providers.json` (or `KERBEROSEC_PROVIDER_SETTINGS_PATH`)
...
- Provider runtime settings continue to persist in `<KERBEROSEC_DATA_DIR>/settings/providers.json`.
...
- User-added OpenAI-compatible provider model catalogs are persisted in `<KERBEROSEC_DATA_DIR>/settings/models.json` (or alongside `KERBEROSEC_PROVIDER_SETTINGS_PATH`).
