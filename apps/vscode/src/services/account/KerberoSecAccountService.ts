...
import { buildBasicKerberoSecHeaders } from "../EnvUtils"

export class KerberoSecAccountService {
	private static instance: KerberoSecAccountService
	...
	/**
	 * Returns the singleton instance of KerberoSecAccountService
	 * @returns Singleton instance of KerberoSecAccountService
	 */
	public static getInstance(): KerberoSecAccountService {
		if (!KerberoSecAccountService.instance) {
			KerberoSecAccountService.instance = new KerberoSecAccountService()
		}
		return KerberoSecAccountService.instance
	}

	/**
	 * Returns the base URL for the KerberoSec API
	 * @returns The base URL as a string
	 */
	get baseUrl(): string {
		return KerberoSecEnv.config().apiBaseUrl
	}

	/**
	 * Helper function to make authenticated requests to the KerberoSec API
	...
	private async authenticatedRequest<T>(
		...
		const kerberosecAccountAuthToken = options?.authToken ?? (await this._authService.getAuthToken())
		if (!kerberosecAccountAuthToken) {
			throw new Error("No KerberoSec account auth token found")
		}
		const requestConfig: AxiosRequestConfig = {
			...config,
			headers: {
				Authorization: `Bearer ${kerberosecAccountAuthToken}`,
	...
	async fetchMe(): Promise<UserResponse | undefined> {
		try {
			const data = await this.authenticatedRequest<UserResponse>(KERBEROSEC_API_ENDPOINT.USER_INFO)
			return data
		} catch (error) {
	...
	async fetchFeaturebaseToken(): Promise<FeaturebaseTokenResponse | undefined> {
		try {
			const data = await this.authenticatedRequest<FeaturebaseTokenResponse>(KERBEROSEC_API_ENDPOINT.FEATUREBASE_TOKEN)
			return data
		} catch (error) {
	...
	async fetchUserRemoteConfig(): Promise<UserRemoteConfigDiscoveryResponse | undefined> {
		...
		const data = await this.authenticatedRequest<UserRemoteConfigDiscoveryResponse | null>(
			KERBEROSEC_API_ENDPOINT.USER_REMOTE_CONFIG,
			{},
	...
	async switchAccount(organizationId?: string | null): Promise<void> {
		...
		try {
			...
			const _response = await this.authenticatedRequest<string>(KERBEROSEC_API_ENDPOINT.ACTIVE_ACCOUNT, {
				method: "PUT",
				headers: {
					"Content-Type": "application/json",
				},
