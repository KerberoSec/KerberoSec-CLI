 ...
 * (manifest name "claude-dev", contribution IDs under "kerberosec.*") and the
 * nightly (name "kerberosec-nightly", IDs under "kerberosec-nightly.*" — the nightly
 * packaging rewrites every `"kerberosec.` prefix in the manifest, see
 ...
 */
export const NIGHTLY_EXTENSION_NAME = "kerberosec-nightly";
export type IdPrefix = "kerberosec" | "kerberosec-nightly";

export function idPrefix(extensionName: string | undefined): IdPrefix {
	return extensionName === NIGHTLY_EXTENSION_NAME ? "kerberosec-nightly" : "kerberosec";
}
 ...
 */
export const COHORT_STATE_KEY = "kerberosec.rollout.bundle";
/** Version of the combined VSIX whose `next` bundle failed to activate, if any. */
export const FAILED_VERSION_STATE_KEY =
	"kerberosec.rollout.nextActivationFailedVersion";
/** Epoch ms of the previous loader activation, for launch-cadence telemetry. */
export const LAST_ACTIVATION_STATE_KEY = "kerberosec.rollout.lastActivationAt";

/**
 * PostHog rollout flag (created in the KerberoSec PostHog project). Must be a
...
export const BUNDLE_OVERRIDE_ENV = "KERBEROSEC_BUNDLE_OVERRIDE";
...
export interface CohortInputs {
	/** KERBEROSEC_BUNDLE_OVERRIDE, if set. */
