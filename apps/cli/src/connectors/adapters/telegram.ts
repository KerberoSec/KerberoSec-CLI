...
import type { ChatStartSessionRequest } from "@kerberosec/core";
import {
...
> {
		...
		return (
			super
				...
				.option(
					...
					"RPC address",
					process.env.KERBEROSEC_RPC_ADDRESS?.trim() ||
						resolveDefaultCliRpcAddress(),
				)
				.addHelpText(
					"after",
					[
						...
						"  - Use --allowed-user-id or `kerberosec connect` to restrict Telegram access.",
	...
	protected override readOptions(command: Command): ConnectTelegramOptions {
		...
		const hookCommand =
			...
			process.env.KERBEROSEC_CONNECT_HOOK_COMMAND?.trim();
	...
	protected override async runWithOptions(
		...
		if (
			...
			process.env.KERBEROSEC_TELEGRAM_CONNECT_CHILD !== "1"
		) {
		...
		const backgroundExitCode = await this.maybeRunInBackground({
			...
			childEnvVar: "KERBEROSEC_TELEGRAM_CONNECT_CHILD",
			statePath,
			readState: (path) => this.readConnectorState(path),
			...
			foregroundHint:
				"[telegram] use `kerberosec connect telegram -i ...` to run in the foreground",
