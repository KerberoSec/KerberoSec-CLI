...
```bash
mkdir -p .kerberosec/hooks
cp examples/hooks/PreToolUse.sh .kerberosec/hooks/
chmod +x .kerberosec/hooks/PreToolUse.sh
kerberosec -i "do something"  # See tool calls logged to stderr
```
...
```bash
mkdir -p .kerberosec/hooks
cp examples/hooks/PostToolUse.sh .kerberosec/hooks/
chmod +x .kerberosec/hooks/PostToolUse.sh
kerberosec -i "do something"  # See tool results logged and enriched
```
...
```bash
mkdir -p .kerberosec/hooks
cp examples/hooks/PreToolUse_BlockDestructive.sh .kerberosec/hooks/PreToolUse.sh
chmod +x .kerberosec/hooks/PreToolUse.sh
kerberosec -i "clean up the repo"  # Destructive operations will be blocked
```
...
```bash
mkdir -p .kerberosec/hooks
cp examples/hooks/PreToolUse_RequireReview.sh .kerberosec/hooks/PreToolUse.sh
chmod +x .kerberosec/hooks/PreToolUse.sh
kerberosec -i "update dependencies"  # Critical file writes will pause for review
```
...
```bash
mkdir -p .kerberosec/hooks
cp examples/hooks/PreToolUse_InjectFileContext.sh .kerberosec/hooks/PreToolUse.sh
chmod +x .kerberosec/hooks/PreToolUse.sh
kerberosec -i "review the configuration"  # Related files will be mentioned automatically
```
...
```bash
mkdir -p .kerberosec/hooks
cp examples/hooks/TaskStart.sh .kerberosec/hooks/
cp examples/hooks/TaskComplete.sh .kerberosec/hooks/
cp examples/hooks/SessionShutdown.sh .kerberosec/hooks/
chmod +x .kerberosec/hooks/Task*.sh .kerberosec/hooks/SessionShutdown.sh
kerberosec -i "do something"  # Session lifecycle will be logged
```
...
```bash
mkdir -p .kerberosec/hooks
cp examples/hooks/PreToolUse.py .kerberosec/hooks/
chmod +x .kerberosec/hooks/PreToolUse.py
kerberosec -i "do something"  # Python hook will log tool calls
```
...
```bash
mkdir -p .kerberosec/hooks
cp examples/hooks/PostToolUse.py .kerberosec/hooks/
chmod +x .kerberosec/hooks/PostToolUse.py
kerberosec -i "do something"  # Python hook will enrich tool results
```
...
```bash
mkdir -p .kerberosec/hooks
cp examples/hooks/PreToolUse_InjectContext.py .kerberosec/hooks/PreToolUse.py
chmod +x .kerberosec/hooks/PreToolUse.py
kerberosec -i "add a new feature"  # Related files and environment will be injected
```
...
```bash
mkdir -p .kerberosec/hooks
cp examples/hooks/PreToolUse.ts .kerberosec/hooks/
chmod +x .kerberosec/hooks/PreToolUse.ts
kerberosec -i "do something"  # TypeScript hook will execute via bun
```
...
```bash
mkdir -p .kerberosec/hooks
cp examples/hooks/PostToolUse.ts .kerberosec/hooks/
chmod +x .kerberosec/hooks/PostToolUse.ts
kerberosec -i "do something"  # TypeScript hook will execute via bun
```
...
```bash
mkdir -p .kerberosec/hooks
cp examples/hooks/PreToolUse_ModifyInput.ts .kerberosec/hooks/PreToolUse.ts
chmod +x .kerberosec/hooks/PreToolUse.ts
kerberosec -i "install dependencies"  # npm install will have --save-exact added automatically
```
...
**File hooks** go in `.kerberosec/hooks/` and must be named after the event they handle:

```bash
mkdir -p .kerberosec/hooks

# Copy PreToolUse example (pick your language)
cp examples/hooks/PreToolUse.sh .kerberosec/hooks/PreToolUse.sh      # Bash
cp examples/hooks/PreToolUse.py .kerberosec/hooks/PreToolUse.py      # Python
cp examples/hooks/PreToolUse.ts .kerberosec/hooks/PreToolUse.ts      # TypeScript

# Copy PostToolUse example
cp examples/hooks/PostToolUse.sh .kerberosec/hooks/PostToolUse.sh    # Bash
cp examples/hooks/PostToolUse.py .kerberosec/hooks/PostToolUse.py    # Python
cp examples/hooks/PostToolUse.ts .kerberosec/hooks/PostToolUse.ts    # TypeScript
```

### 2. Make it executable

```bash
chmod +x .kerberosec/hooks/PreToolUse.*
chmod +x .kerberosec/hooks/PostToolUse.*
```

### 3. Test it

```bash
kerberosec -i "test prompt"
# Or load from a custom hooks directory:
kerberosec --hooks-dir ./my-hooks -i "test prompt"
```
...
```bash
kerberosec --verbose "your prompt"
```

### Test a hook manually
```bash
echo '{"tool_call": {"name": "read_files", "input": {"filePath": "test.ts"}}}' | .kerberosec/hooks/PreToolUse.sh
```

### Check hook output
```bash
.kerberosec/hooks/PreToolUse.sh < input.json | jq .
```
...
```bash
kerberosec plugin install https://github.com/kerberosec/kerberosec/blob/main/sdk/examples/hooks/custom-compaction-hook.example.ts --cwd .

kerberosec -i "Search the codebase for dispatcher usage, then summarize it"
```
...
| ------- | --------------- | ------------- | -------- |
| `custom-compaction-hook.example.ts` (in `.kerberosec/plugins/`) | `hooks.beforeModel` runtime hook | Agent runtime request messages with runtime parts such as `tool-call`, `tool-result`, `reasoning`, `image`, and `file` | Cases needing runtime-hook context, current runtime snapshot, or direct request mutation |
...
- **Use multiple hooks** — different event files can coexist in `.kerberosec/hooks/`
