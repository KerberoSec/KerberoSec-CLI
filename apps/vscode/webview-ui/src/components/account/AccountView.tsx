import type { UsageTransaction as KerberoSecAccountUsageTransaction, PaymentTransaction } from "@shared/KerberoSecAccount"
import { isKerberoSecInternalTester } from "@shared/internal/account"
import type { UserOrganization } from "@shared/proto/kerberosec/account"
...
import { type KerberoSecUser, handleSignOut } from "@/context/KerberoSecAuthContext"
...
import { convertProtoUsageTransactions, getKerberoSecUris, getMainRole } from "./helpers"
import { RemoteConfigToggle } from "./RemoteConfigToggle"

type AccountViewProps = {
	kerberosecUser: KerberoSecUser | null
	organizations: UserOrganization[] | null
	activeOrganization: UserOrganization | null
	onDone: () => void
}

type KerberoSecAccountViewProps = {
	kerberosecUser: KerberoSecUser
	userOrganizations: UserOrganization[] | null
	activeOrganization: UserOrganization | null
	kerberosecEnv: "Production" | "Staging" | "Local"
}

type CachedData = {
	balance: number | null
	usageData: KerberoSecAccountUsageTransaction[]
	paymentsData: PaymentTransaction[]
	lastFetchTime: number
}

const KerberoSecEnvOptions = ["Production", "Staging", "Local"] as const

const AccountView = ({ onDone, kerberosecUser, organizations, activeOrganization }: AccountViewProps) => {
	...
	return (
		<div className="fixed inset-0 flex flex-col overflow-hidden">
			...
			<div className="grow flex flex-col px-5 overflow-y-auto">
				{kerberosecUser?.uid ? (
					<KerberoSecAccountView
						activeOrganization={activeOrganization}
						kerberosecEnv={environment === "local" ? "Local" : environment === "staging" ? "Staging" : "Production"}
						kerberosecUser={kerberosecUser}
						key={kerberosecUser.uid}
...
const KerberoSecAccountView = ({ kerberosecUser, userOrganizations, activeOrganization, kerberosecEnv }: KerberoSecAccountViewProps) => {
	const { email, displayName, appBaseUrl, uid } = kerberosecUser
	...
	const [usageData, setUsageData] = useState<KerberoSecAccountUsageTransaction[]>([])
	...
	const isKerberoSecTester = useMemo(() => (email ? isKerberoSecInternalTester(email) : false), [email])
	...
	const kerberosecUrl = appBaseUrl || "https://app.kerberosec.bot"
			...
			<div className="flex flex-col h-full">
				...
				<div className="w-full flex gap-2 flex-col min-[225px]:flex-row">
					<div className="w-full min-[225px]:w-1/2">
						<VSCodeButtonLink appearance="primary" className="w-full" href={getKerberoSecUris(kerberosecUrl, "dashboard").href}>
							Dashboard
						</VSCodeButtonLink>
					</div>
				...
				<CreditBalance
					balance={balance}
					creditUrl={getKerberoSecUris(kerberosecUrl, "credits", dropdownValue === uid ? "account" : "organization")}
				...
				{isKerberoSecTester && environment !== "selfHosted" && (
					<div className="w-full gap-1 items-end">
						<VSCodeDivider className="w-full my-3" />
						<div className="text-sm font-semibold">KerberoSec Environment</div>
						<VSCodeDropdown
							className="w-full mt-1"
							currentValue={kerberosecEnv}
							onChange={async (e) => {
								const target = e.target as HTMLSelectElement
								if (target?.value) {
									const value = target.value as "Local" | "Staging" | "Production"
									updateSetting("kerberosecEnv", value.toLowerCase())
								}
							}}>
							{KerberoSecEnvOptions.map((env) => (
