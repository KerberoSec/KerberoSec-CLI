...
vi.mock("@/config", () => ({ KerberoSecEnv: { config: () => ({ apiBaseUrl: "https://api.example.test" }) } }))
vi.mock("@/services/EnvUtils", () => ({ buildBasicKerberoSecHeaders: async () => ({}) }))
...
describe("SdkRemoteConfigControlPlane", () => {
	...
	it("wraps discovered remote config in a bundle when no organization is active", async () => {
		...
		expect(bundle?.source).toBe("kerberosec-extension-remote-config")
