/**
 * Early SDK logger for components that operate before/outside of `KerberoSecCore`
 * sessions, plus a secret-hashing helper for credential diagnostics.
 *
 * `KerberoSecCore.create({ logger })` receives a `BasicLogger` but it is only
 * threaded to session-scoped components. `ProviderSettingsManager`,
 * `RuntimeOAuthTokenManager`, and the KerberoSec auth functions in `kerberosec.ts` are
 * constructed or called before `KerberoSecCore` exists or outside a session.
