...
function sameOptimisticMessage(left: KerberoSecMessage, right: KerberoSecMessage): boolean {
...
export function hasPendingMessageConfirmation(messages: KerberoSecMessage[], pending: PendingUserMessage): boolean {
	return messages.some((message) => message.ts > pending.afterTs && sameOptimisticMessage(message, pending.message))
}

export function withPendingUserMessage(messages: KerberoSecMessage[], pending: PendingUserMessage | undefined): KerberoSecMessage[] {
