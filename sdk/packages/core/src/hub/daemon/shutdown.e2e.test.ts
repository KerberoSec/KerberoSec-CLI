...
async function startDaemon(): Promise<ReadyDaemon> {
	const dataDir = await mkdtemp(join(tmpdir(), "kerberosec-hub-shutdown-e2e-"));
	...
	const child = spawn(
		...
		["--conditions=development", entryPath],
		{
			...
			env: {
				...
				KERBEROSEC_HUB_DISCOVERY_PATH: discoveryPath,
				KERBEROSEC_HUB_TEST_PORT: "0",
...
async function openAuthenticatedSocket(
	...
	for (;;) {
		const socket = new WebSocket(url, `kerberosec-hub-auth.${authToken}`);
		try {
			await withTimeout(
