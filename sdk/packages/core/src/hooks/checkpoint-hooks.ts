...
import type { AgentHooks, BasicLogger } from "@kerberosec/shared";
import { countUserRunMessages } from "../session/user-run-messages";

const execFile = promisify(execFileCallback);
const CHECKPOINT_STASH_MESSAGE_PREFIX = "kerberosec checkpoint session=";
...
async function createUntrackedParentCommit(
	...
	const tempDir = await mkdtemp(join(tmpdir(), "kerberosec-checkpoint-"));
	...
	try {
		...
		const commit = await runGitWithIndex(cwd, indexFile, [
			...
			tree,
			"-m",
			"untracked files on kerberosec checkpoint",
		]);
...
async function createWorktreeStashCommit(
	...
	const indexParent = (
		await runGit(cwd, [
			...
			head,
			"-m",
			"index on kerberosec checkpoint",
		])
	).stdout;
	if (!indexParent) {
		return undefined;
...
/**
 * Deletes all private git refs under refs/kerberosec/checkpoints/{sessionId}/ that
...
export async function deleteCheckpointRefs(
	...
	if (!cwd) return;
	const prefix = `refs/kerberosec/checkpoints/${sessionId}/`;
	try {
...
export async function retainCheckpointRefs(
	...
	await Promise.allSettled(
		checkpoints.map((entry) =>
			runGit(cwd, [
				"update-ref",
				`refs/kerberosec/checkpoints/${sessionId}/${entry.runCount}`,
				entry.ref,
			]),
		),
	);
...
export function createCheckpointHooks(
	...
	const createCheckpoint = async (
		...
		const privateRef = `refs/kerberosec/checkpoints/${options.sessionId}/${runCount}`;
		try {
