...
import os from "os"
import { KerberoSecConfigurationError, KerberoSecEndpoint, KerberoSecEnv, Environment } from "../config"

describe("KerberoSecEndpoint configuration", () => {
	...
	beforeEach(async () => {
		...
		// Create .kerberosec directory
		await fs.mkdir(path.join(tempDir, ".kerberosec"), { recursive: true })
		...
		;(KerberoSecEndpoint as any)._instance = null
		;(KerberoSecEndpoint as any)._initialized = false
		;(KerberoSecEndpoint as any)._extensionFsPath = undefined
	})
	...
	describe("valid config parsing", () => {
		it("should parse valid endpoints.json with all required fields", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".kerberosec", "endpoints.json"), JSON.stringify(validConfig), "utf8")

			await KerberoSecEndpoint.initialize(tempDir)

			const config = KerberoSecEndpoint.config
		...
		it("should work without endpoints.json (standard mode)", async () => {
			...
			config.appBaseUrl.should.equal("https://app.kerberosec.bot")
			config.apiBaseUrl.should.equal("https://api.kerberosec.bot")
		})
		...
		it("should accept URLs with paths", async () => {
			const validConfig = {
				...
				apiBaseUrl: "https://proxy.enterprise.com/kerberosec/api",
				mcpBaseUrl: "https://proxy.enterprise.com/kerberosec/mcp",
			}
			...
			const config = KerberoSecEndpoint.config
			config.appBaseUrl.should.equal("https://proxy.enterprise.com/kerberosec/app")
		})
	})

	describe("invalid JSON handling", () => {
		it("should throw KerberoSecConfigurationError for invalid JSON syntax", async () => {
			await fs.writeFile(path.join(tempDir, ".kerberosec", "endpoints.json"), "{ invalid json }", "utf8")

			try {
				await KerberoSecEndpoint.initialize(tempDir)
				throw new Error("Should have thrown")
			} catch (error: any) {
				error.should.be.instanceof(KerberoSecConfigurationError)
				error.message.should.containEql("Invalid JSON")
			}
		})

		it("should throw KerberoSecConfigurationError for truncated JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".kerberosec", "endpoints.json"), '{"appBaseUrl": "https://test.com"', "utf8")
		...
		it("should throw KerberoSecConfigurationError for empty file", async () => {
			await fs.writeFile(path.join(tempDir, ".kerberosec", "endpoints.json"), "", "utf8")
		...
		it("should throw KerberoSecConfigurationError for non-object JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".kerberosec", "endpoints.json"), '"just a string"', "utf8")
		...
		it("should throw KerberoSecConfigurationError for array JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".kerberosec", "endpoints.json"), "[]", "utf8")
		...
		it("should throw KerberoSecConfigurationError for null JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".kerberosec", "endpoints.json"), "null", "utf8")
	...
	describe("missing required fields", () => {
		it("should throw KerberoSecConfigurationError when appBaseUrl is missing", async () => {
			const config = {
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
			}

			await fs.writeFile(path.join(tempDir, ".kerberosec", "endpoints.json"), JSON.stringify(config), "utf8")
		...
		it("should throw KerberoSecConfigurationError when apiBaseUrl is missing", async () => {
			const config = {
				appBaseUrl: "https://app.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
			}
		...
		it("should throw KerberoSecConfigurationError when mcpBaseUrl is missing", async () => {
			const config = {
				appBaseUrl: "https://app.enterprise.com",
				apiBaseUrl: "https://api.enterprise.com",
			}
		...
		it("should throw KerberoSecConfigurationError when all fields are missing", async () => {
			await fs.writeFile(path.join(tempDir, ".kerberosec", "endpoints.json"), "{}", "utf8")
		...
		it("should throw KerberoSecConfigurationError when field is null", async () => {
			const config = {
				appBaseUrl: null,
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw KerberoSecConfigurationError when field is empty string", async () => {
			const config = {
				appBaseUrl: "",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw KerberoSecConfigurationError when field is whitespace only", async () => {
			const config = {
				appBaseUrl: "   ",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw KerberoSecConfigurationError when field is non-string", async () => {
			const config = {
				appBaseUrl: 12345,
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
	...
	describe("invalid URL detection", () => {
		it("should throw KerberoSecConfigurationError for invalid URL format", async () => {
			const config = {
		...
		it("should throw KerberoSecConfigurationError for URL without protocol", async () => {
			const config = {
		...
		it("should throw KerberoSecConfigurationError for malformed URL", async () => {
			const config = {
				appBaseUrl: "https://",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
	...
	describe("environment switching blocked in self-hosted mode", () => {
		it("should throw error when trying to change environment in self-hosted mode", async () => {
			...
			KerberoSecEndpoint.config.environment.should.equal(Environment.selfHosted)

			// Try to change environment - should throw
			try {
				KerberoSecEnv.setEnvironment("staging")
				throw new Error("Should have thrown")
			} catch (error: any) {
		...
		it("should throw error for all environment values in self-hosted mode", async () => {
			...
			for (const env of environments) {
				try {
					KerberoSecEnv.setEnvironment(env)
		...
		it("should allow environment switching in standard mode", async () => {
			...
			KerberoSecEndpoint.config.environment.should.not.equal(Environment.selfHosted)

			// Should be able to change environment
			KerberoSecEnv.setEnvironment("staging")
			KerberoSecEnv.getEnvironment().environment.should.equal("staging")

			KerberoSecEnv.setEnvironment("local")
			KerberoSecEnv.getEnvironment().environment.should.equal("local")

			KerberoSecEnv.setEnvironment("production")
			KerberoSecEnv.getEnvironment().environment.should.equal("production")
		})
	})
	...
	describe("self-hosted mode behavior", () => {
		it("should report selfHosted environment in self-hosted mode", async () => {
			...
			const envConfig = KerberoSecEndpoint.config
		...
		it("should use custom endpoints from file", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".kerberosec", "endpoints.json"), JSON.stringify(customConfig), "utf8")
	...
	describe("initialization behavior", () => {
		it("should only initialize once", async () => {
			await KerberoSecEndpoint.initialize(tempDir)
			KerberoSecEndpoint.isInitialized().should.be.true()
		...
		it("should throw error when accessing config before initialization", async () => {
			...
			try {
				const _ = KerberoSecEndpoint.config
				throw new Error("Should have thrown")
			} catch (error: any) {
	...
	describe("isSelfHosted() method", () => {
		it("should return true when not initialized (safety fallback)", async () => {
			...
			KerberoSecEndpoint.isInitialized().should.be.false()
			KerberoSecEndpoint.isSelfHosted().should.be.true()
		})
		...
		it("should return false when in normal mode (no endpoints.json)", async () => {
			...
			KerberoSecEndpoint.isSelfHosted().should.be.false()
		})
	})
	...
	describe("bundled endpoints.json behavior", () => {
		...
		it("should use bundled endpoints.json when available", async () => {
			...
			await KerberoSecEndpoint.initialize(bundledDir)
		...
		it("should prefer bundled endpoints.json over user file", async () => {
			...
			// Set up both configs
			await fs.writeFile(path.join(bundledDir, "endpoints.json"), JSON.stringify(bundledConfig), "utf8")
			await fs.writeFile(path.join(tempDir, ".kerberosec", "endpoints.json"), JSON.stringify(userConfig), "utf8")
		...
		it("should throw KerberoSecConfigurationError for invalid bundled file", async () => {
			const invalidConfig = {
		...
		it("should throw KerberoSecConfigurationError for invalid JSON in bundled file", async () => {
