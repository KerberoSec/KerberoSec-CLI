...
const authMocks = vi.hoisted(() => ({
	...
	normalizeProviderId: vi.fn(
		(providerId?: string) => providerId ?? "kerberosec",
	),
...
const migrationNoticeMocks = vi.hoisted(() => ({
	getKerberoSecCliMigrationNotice: vi.fn<
		(
			dataDir?: string,
...
vi.mock("@kerberosec/core", async () => {
	return {
		...(await vi.importActual("@kerberosec/core")),
...
describe("runCli lightweight command dispatch", () => {
	...
	beforeEach(() => {
		...
		globalSettingsRoot = mkdtempSync(
			join(tmpdir(), "kerberosec-cli-main-test-"),
		);
		process.env.KERBEROSEC_GLOBAL_SETTINGS_PATH = join(
			globalSettingsRoot,
		...
		migrationNoticeMocks.getKerberoSecCliMigrationNotice.mockReset();
		migrationNoticeMocks.getKerberoSecCliMigrationNotice.mockReturnValue(
			undefined,
		);
	...
	it("runs logout command successfully", async () => {
		process.argv = ["bun", "src/index.ts", "logout"];
		const { runCli } = await import("./main");
	...
	it("rejects a single bare positional prompt token", async () => {
		...
		expect(consoleError).toHaveBeenCalledWith(
			expect.stringContaining('Use "kerberosec --help"'),
		);
	...
	it("creates a worktree and runs prompt sessions from it", async () => {
		...
		expect(runtimeMocks.runAgent).toHaveBeenCalledWith(
			"say hello",
			expect.objectContaining({
				cwd: "/tmp/kerberosec-worktree",
				workspaceRoot: "/tmp/kerberosec-worktree",
			}),
			expect.anything(),
		);
	});
	...
	it("passes the migration notice marker into interactive mode", async () => {
		const notice = {
			id: "kerberosec-cli-kerberosec-pass-intro",
			title: "Try KerberoSecPass",
		};
		migrationNoticeMocks.getKerberoSecCliMigrationNotice.mockReturnValue(
			notice,
		);
		...
		expect(
			migrationNoticeMocks.markKerberoSecCliMigrationNoticeShown,
	...
	it("passes the active KerberoSecPass provider into the migration notice gate", async () => {
		providerSettingsMocks.getLastUsedProviderSettings.mockReturnValue({
			provider: "kerberosec-pass",
			model: "kerberosec-pass/test-model",
		});
		process.argv = ["bun", "src/index.ts"];
		...
		await expect(runCli()).resolves.toBeUndefined();
		expect(
			migrationNoticeMocks.getKerberoSecCliMigrationNotice,
	...
	it("does not start OAuth before onboarding in interactive mode", async () => {
		...
		authMocks.normalizeProviderId.mockReturnValue("kerberosec");
		authMocks.getPersistedProviderApiKey.mockReturnValue(undefined);
	...
	describe("persisted general settings at startup", () => {
		function writePersistedSettings(settings: Record<string, unknown>) {
			const path = process.env.KERBEROSEC_GLOBAL_SETTINGS_PATH;
			if (!path) {
				throw new Error("KERBEROSEC_GLOBAL_SETTINGS_PATH is not set");
			}
	...
	it("does not pass non-KerberoSec provider settings as KerberoSec account options", async () => {
		providerSettingsMocks.getLastUsedProviderSettings.mockReturnValue({
			provider: "openrouter",
		...
		expect(runtimeMocks.runInteractive).toHaveBeenCalledWith(
			...
			undefined,
			expect.objectContaining({
				kerberosecApiBaseUrl: undefined,
				kerberosecProviderSettings: undefined,
			}),
		);
	});

	it("passes KerberoSec provider settings as KerberoSec account options", async () => {
		const kerberosecSettings = {
			provider: "kerberosec",
		...
		expect(runtimeMocks.runInteractive).toHaveBeenCalledWith(
			...
			undefined,
			expect.objectContaining({
				kerberosecApiBaseUrl: "https://api.example.test",
				kerberosecProviderSettings: kerberosecSettings,
			}),
		);
	});

	it("seeds feature flag identity from persisted KerberoSec account id before refreshing flags", async () => {
		const kerberosecSettings = {
			provider: "kerberosec",
			model: "anthropic/claude-sonnet-4.6",
			auth: {
	...
	it("identifies saved KerberoSec accountId for telemetry before runtime events", async () => {
		// KERBEROSEC-2406: when persisted KerberoSec auth includes an accountId, the
	...
	it("does not call identifyTelemetryAccount in runtime path when no saved KerberoSec accountId", async () => {
		// KERBEROSEC-2406: when no persisted accountId is found (anonymous/unauthenticated),
	...
	it("does not call identifyTelemetryAccount from runtime path when provider is not kerberosec", async () => {
		// KERBEROSEC-2406: identity identification from saved settings only applies
		// to KerberoSec-provider sessions; other providers use different auth flows.
		providerSettingsMocks.getLastUsedProviderSettings.mockReturnValue({
			provider: "openrouter",
			model: "openai/gpt-5",
		});
	...
	it("runs dashboard before loading runtime modules", async () => {
		process.argv = [
			...
			"dashboard",
			"--config",
			"/tmp/kerberosec-config",
			"--data-dir",
			".kerberosec-dashboard-data",
			"--port",
			"9090",
			"--no-open",
		];
