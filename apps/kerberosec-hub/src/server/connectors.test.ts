...
describe("connector launch command", () => {
	const originalLaunchSpec = process.env[KERBEROSEC_CONNECTOR_CLI_LAUNCH_ENV];

	afterEach(() => {
		if (originalLaunchSpec === undefined) {
			delete process.env[KERBEROSEC_CONNECTOR_CLI_LAUNCH_ENV];
		} else {
			process.env[KERBEROSEC_CONNECTOR_CLI_LAUNCH_ENV] = originalLaunchSpec;
		}
	});
	...
	it("uses compiled CLI subcommands without Bun flags", () => {
		expect(
			__test__.buildCliConnectCommand(["telegram", "--bot-token", "token"], {
				execPath: "/Applications/KerberoSec/bin/kerberosec",
				cliPath: "/repo/apps/cli/src/index.ts",
				exists: () => true,
			}),
		).toEqual({
