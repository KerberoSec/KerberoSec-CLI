...
//
// Creates and manages SDK sessions using KerberoSecCore. This factory handles:
// - Creating KerberoSecCore instances with proper configuration
// - Building session config from legacy state (provider, model, API key)
// - Custom session persistence adapter reading ~/.kerberosec/data/tasks/
...
import type { ProviderApiLine, ModelInfo as SdkModelInfo } from "@kerberosec/llms"
import {
...
import { buildKerberoSecSystemPrompt, isKerberoSecProvider } from "@kerberosec/shared"
import type { ApiConfiguration } from "@shared/api"
import { KerberoSecClient } from "@shared/kerberosec"
 ...
 * standalone kerberosec-core (JetBrains), where `vscode` is a Proxy-stub module and
 * direct API reads would yield non-string values. The hostbridge returns the
 * per-host values (e.g. "KerberoSec for JetBrains" + IDE version on JetBrains).
 */
...
const PROVIDER_API_KEY_MAP: Record<string, keyof ApiConfiguration> = {
	...
	kerberosec: "kerberosecApiKey",
	"kerberosec-pass": "kerberosecApiKey",
	ollama: "ollamaApiKey",
...
const PROVIDER_MODEL_ID_MAP: Record<string, { plan: keyof ApiConfiguration; act: keyof ApiConfiguration }> = {
	...
	kerberosec: { plan: "planModeKerberoSecModelId", act: "actModeKerberoSecModelId" },
	"kerberosec-pass": { plan: "planModeKerberoSecPassModelId", act: "actModeKerberoSecPassModelId" },
...
const DEFAULT_PROVIDER_ID = "kerberosec"
...
export function resolveApiKey(providerId: string, config: ApiConfiguration): string | undefined {
	...
	if (authHandler) {
		...
		// resolve shared storage (e.g. kerberosec-pass -> kerberosec) without VS Code
...
export function resolveModelId(providerId: string, mode: Mode, config: ApiConfiguration): string | undefined {
	...
	// session use a different model than the KerberoSec provider UI shows.
...
export async function buildSessionConfig(input: SessionConfigInput): Promise<CoreSessionConfig> {
	...
	// Include rich workspace metadata so KerberoSec API observability can extract
	...
	try {
		...
		systemPrompt = buildKerberoSecSystemPrompt({
			ide: "VS Code",
			workspaceRoot,
			workspaceName,
		...
		systemPrompt = "You are KerberoSec, a highly skilled software engineer. Help the user with their request."
	}
	...
	try {
		...
		// the @kerberosec/llms registry. Reading models from that registry ensures custom
	...
	const providerConfig = {
		...
		// otherwise falls back to a small default output cap (KERBEROSEC-2911).
	...
	const config: CoreSessionConfig = {
		...
		extensionContext: {
			...
			client: {
				name: hostIdentity?.kerberosecType || KerberoSecClient.VSCode,
				version: hostIdentity?.kerberosecVersion || ExtensionRegistryInfo.version,
 ...
 */
export function buildStartSessionInput(config: CoreSessionConfig, input: SessionConfigInput): KerberoSecCoreStartInput {
	return {
		config,
