import type { ConfiguredTelemetryHandle, ITelemetryService } from "@kerberosec/core"
...
vi.mock("@kerberosec/core", () => ({
	createKerberoSecTelemetryServiceConfig: coreTelemetryMocks.createConfig,
	createConfiguredTelemetryHandle: coreTelemetryMocks.createHandle,
}))

const telemetryState = vi.hoisted(() => ({
	kerberosecTelemetrySetting: "unset" as string | undefined,
	hostSetting: 1,
	hostVersion: {
		platform: "VS Code",
		version: "1.103.0",
		kerberosecType: "VSCode Extension",
	} as { platform?: string; version?: string; kerberosecType?: string; kerberosecVersion?: string },
	...
	StateManager: {
		get: () => ({
			getGlobalSettingsKey: (key: string) =>
				key === "telemetrySetting" ? telemetryState.kerberosecTelemetrySetting : undefined,
		}),
	},
}))
...
describe("VscodeTelemetryPolicyService", () => {
	beforeEach(() => {
		telemetryState.kerberosecTelemetrySetting = "unset"
		...
		vi.stubEnv("KERBEROSEC_ROLLOUT_VARIANT", "")
	})
	...
	it("adds rollout metadata as SDK common properties", () => {
		vi.stubEnv("KERBEROSEC_ROLLOUT_VARIANT", "next")
		coreTelemetryMocks.createHandle.mockReturnValue(createHandle())
	...
	it("allows ordinary events when host telemetry is enabled and KerberoSec telemetry is not disabled", async () => {
		const handle = createHandle()
		const service = new VscodeTelemetryPolicyService(handle)
	...
	it("drops ordinary events when KerberoSec telemetry is disabled but allows required events while host telemetry is enabled", async () => {
		telemetryState.kerberosecTelemetrySetting = "disabled"
		const handle = createHandle()
		const service = new VscodeTelemetryPolicyService(handle)
		await settlePromises()
	...
	it("applies the full host identity metadata before enabling events", async () => {
		telemetryState.hostVersion = {
			...
			version: "2026.1.1",
			kerberosecType: "KerberoSec for JetBrains",
			kerberosecVersion: "1.1.61",
		}
		const handle = createHandle()
		const service = new VscodeTelemetryPolicyService(handle)
		await settlePromises()
		...
		expect(handle.telemetry.updateMetadata).toHaveBeenCalledWith({
			...
			kerberosec_type: "KerberoSec for JetBrains",
			platform: "IntelliJ IDEA Ultimate",
	...
	it("omits the metadata fields the host does not report", async () => {
		// The default host version has no kerberosecVersion, so host_plugin_version must be absent.
		const handle = createHandle()
		const service = new VscodeTelemetryPolicyService(handle)
		await settlePromises()
		...
		expect(handle.telemetry.updateMetadata).toHaveBeenCalledWith({
			kerberosec_type: "VSCode Extension",
			platform: "VS Code",
