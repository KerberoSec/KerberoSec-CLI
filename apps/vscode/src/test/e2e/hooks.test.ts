...
// (.kerberosecrules/hooks/UserPromptSubmit[.ps1]) that writes hook-ran.json into its
...
// window's workspace root and name it — regardless of what any other KerberoSec
...
hooksE2e("Hooks - workspace hook runs from this window's workspace root", async ({ helper, sidebar, workspaceDir }) => {
	...
	try {
		...
		await expect(sidebar.getByText("mock KerberoSec API response")).toBeVisible()
