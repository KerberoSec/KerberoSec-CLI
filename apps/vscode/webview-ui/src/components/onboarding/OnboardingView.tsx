import { buildModelInfoNameMap, type ModelInfo, openAiModelInfoSafeDefaults, resolveKerberoSecPassModelInfo } from "@shared/api"
import { StringRequest } from "@shared/proto/kerberosec/common"
import type { OnboardingModel, OnboardingModelGroup, OpenRouterModelInfo } from "@shared/proto/index.kerberosec"
...
import KerberoSecLogoWhite from "@/assets/KerberoSecLogoWhite"
...
import { setPendingKerberoSecPassSubscribe } from "./kerberosecPassSubscribe"
import {
...
type ModelSelectionProps = {
	userType: NEW_USER_TYPE.FREE | NEW_USER_TYPE.POWER | NEW_USER_TYPE.KERBEROSEC_PASS
...
function getModelGroupKey(userType: ModelSelectionProps["userType"]): keyof OnboardingModelsByGroup {
	if (userType === NEW_USER_TYPE.KERBEROSEC_PASS) {
...
const ModelSelection = ({
	...
	const isKerberoSecPass = userType === NEW_USER_TYPE.KERBEROSEC_PASS
	const modelGroups = onboardingModels[getModelGroupKey(userType)]
	// KerberoSecPass costs are covered by the subscription, so prices are hidden.
	const hidePrice = isKerberoSecPass
	...
	// No curated KerberoSecPass models available: show an empty state rather than other models.
		...
		<div className="flex flex-col w-full items-center px-2">
			<div className="flex w-full max-w-lg flex-col gap-6 my-4">
				{modelGroups.map((group) => {
					const isKerberoSecPassGroup = group.group === KERBEROSECPASS_GROUP
					return (
						<div className="flex flex-col gap-3" key={group.group}>
							<h4
								className={cn(
									"text-sm font-bold text-foreground/70 mb-2",
									isKerberoSecPassGroup ? "normal-case" : "uppercase",
								)}>
			...
			{/* SEARCH MODEL — hidden for KerberoSecPass, whose selection is constrained to the curated list. */}
			{!isKerberoSecPass && (
...
const OnboardingStepContent = ({
	...
	if (userType === NEW_USER_TYPE.FREE || userType === NEW_USER_TYPE.POWER || userType === NEW_USER_TYPE.KERBEROSEC_PASS) {
		return (
			<ModelSelection
				models={models}
...
const OnboardingViewContent = ({ onboardingModels }: { onboardingModels: OnboardingModelGroup }) => {
	...
	const { commitSelection } = useProviderConfig("kerberosec")
	...
	const models = useMemo(() => getKerberoSecUIOnboardingGroups(onboardingModels), [onboardingModels])
	// Gate on models so a fallback/empty response can't route users into the dead-end empty step.
	const showKerberoSecPass = models.kerberosecPass.length > 0
	const userTypeSelections = useMemo(() => getUserTypeSelections(showKerberoSecPass), [showKerberoSecPass])
	// KerberoSecPass model IDs (e.g. "kerberosec-pass/glm-5.2") aren't keyed in openRouterModels,
	// so resolve their info via the slug-based lookup used by KerberoSecPassProvider.
	...
	useEffect(() => {
		setSearchTerm("")
		const groupKey = userType === NEW_USER_TYPE.KERBEROSEC_PASS ? "kerberosecPass" : userType === NEW_USER_TYPE.POWER ? "power" : "free"
		// KerberoSecPass must stay within its curated list (never fall back to a free/OpenRouter model
		// under the kerberosec-pass provider). Free/Frontier fall back to free if their group is empty.
		const modelGroup = userType === NEW_USER_TYPE.KERBEROSEC_PASS ? models[groupKey][0] : (models[groupKey][0] ?? models.free[0])
	...
	const finishOnboarding = useCallback(
		async (updateModelId: boolean, step: number, markCompleted = true) => {
			...
			// Guard: never save a non-KerberoSecPass model id under the kerberosec-pass provider.
			const isKerberoSecPassModel = selectedModelId.startsWith("kerberosec-pass/")
			if (modelSelected) {
				if (userType === NEW_USER_TYPE.KERBEROSEC_PASS && isKerberoSecPassModel) {
					const kerberosecPassModelInfo = resolveKerberoSecPassModelInfo(selectedModelId, openRouterModelsByName)
					await handleFieldsChange({
						planModeKerberoSecPassModelId: selectedModelId,
						actModeKerberoSecPassModelId: selectedModelId,
						planModeKerberoSecPassModelInfo: kerberosecPassModelInfo,
						actModeKerberoSecPassModelInfo: kerberosecPassModelInfo,
						planModeApiProvider: "kerberosec-pass",
						actModeApiProvider: "kerberosec-pass",
					})
				} else if (userType !== NEW_USER_TYPE.KERBEROSEC_PASS) {
					const selectedModelInfo = kerberosecModels[selectedModelId] ??
					...
					await handleFieldsChange({
						planModeKerberoSecModelId: selectedModelId,
						actModeKerberoSecModelId: selectedModelId,
						planModeKerberoSecModelInfo: selectedModelInfo,
						actModeKerberoSecModelInfo: selectedModelInfo,
						planModeApiProvider: "kerberosec",
						actModeApiProvider: "kerberosec",
					})
				} else {
					// KerberoSecPass selected but the id isn't a kerberosec-pass/ model: skip the write
					// (avoids a bad provider config) and log so the no-op is observable.
					console.error(`Skipped KerberoSecPass provider setup: unexpected model id "${selectedModelId}"`)
				}
			}
	...
	const handleFooterAction = useCallback(
		async (action: "signin" | "next" | "back" | "done" | "signup") => {
			...
			switch (action) {
				case "signup":
					// KerberoSecPass: record the intent so App opens the subscription page once auth
					// completes (App outlives this view, which unmounts on auth). Login flow unchanged.
					setPendingKerberoSecPassSubscribe(userType === NEW_USER_TYPE.KERBEROSEC_PASS)
					captureNavigation("signup_clicked", stepNumber + 1)
					setStepNumber(stepNumber + 1)
					await loginAndFinishOnboarding(true, stepNumber + 1)
					break
				case "signin":
					setPendingKerberoSecPassSubscribe(false)
				...
				case "next":
					...
					break
				case "back":
					// Abandon any pending KerberoSecPass subscription redirect when the user goes back.
					setPendingKerberoSecPassSubscribe(false)
	...
	return (
		<div className="fixed inset-0 p-0 flex flex-col w-full">
			<div className="h-full px-5 xs:mx-10 overflow-auto flex flex-col gap-4 items-center justify-center">
				<KerberoSecLogoWhite className="size-16 flex-shrink-0" />
				...
				<div className="flex-1 w-full flex max-w-lg overflow-y-auto min-h-0">
					<OnboardingStepContent
						models={Object.keys(kerberosecModels).length > 0 ? kerberosecModels : openRouterModels}
				...
				<footer className="flex w-full max-w-lg flex-col gap-3 my-2 px-2 overflow-hidden flex-shrink-0">
					{stepDisplayInfo.buttons.map((btn) => {
						// Block KerberoSecPass signup when no KerberoSecPass model is selected (e.g. empty list).
						const isLoginAction = btn.action === "signin" || btn.action === "signup"
						const showSpinner = isActionLoading && isLoginAction
						const disabled =
							isActionLoading ||
							(btn.action === "signup" && userType === NEW_USER_TYPE.KERBEROSEC_PASS && !selectedModelId)
						return (
							<Button
