import type { Banner, BannerAction, BannerRules, BannersResponse } from "@shared/KerberoSecBanner"
import { BannerActionType, type BannerCardData } from "@shared/kerberosec/banner"
...
export class BannerService {
	...
	public getWelcomeBanners(): BannerCardData[] | undefined {
		const isLocal = process.env.IS_DEV === "true" || process.env.KERBEROSEC_ENVIRONMENT === "local"
		const flagEnabled = isLocal || featureFlagsService.getBooleanFlagEnabled(FeatureFlag.REMOTE_WELCOME_BANNERS)

		if (!flagEnabled) {
			return undefined
		}
		const bypassDismissals = process.env.IS_DEV === "true" || process.env.KERBEROSEC_ENVIRONMENT === "local"
	...
	public async sendBannerEvent(bannerId: string, eventType: "dismiss"): Promise<void> {
		try {
			const url = new URL("/banners/v2/messages", KerberoSecEnv.config().apiBaseUrl).toString()
	...
	private buildFetchUrl(): string {
		const url = new URL("/banners/v2/messages", KerberoSecEnv.config().apiBaseUrl)
