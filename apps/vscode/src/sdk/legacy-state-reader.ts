...
//
// Reads on-disk state written in the pre-SDK storage format from the KerberoSec
...
/**
 * Resolve the KerberoSec data directory.
 * Priority: KERBEROSEC_DATA_DIR env > KERBEROSEC_DIR env + "/data" > ~/.kerberosec/data
 */
...
function mcpSettingsPath(dataDir?: string): string {
	return path.join(resolveDataDir(dataDir), "settings", "kerberosec_mcp_settings.json")
}
 ...
 * Returns an empty array if the file is missing or corrupt.
 */
export function readUiMessages(taskId: string, dataDir?: string): KerberoSecMessage[] {
	const messages = readJsonFile<KerberoSecMessage[]>(uiMessagesPath(taskId, dataDir), [])
