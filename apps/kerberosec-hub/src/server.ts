import { CORE_BUILD_VERSION } from "@kerberosec/core";
...
export interface KerberoSecHubDashboardServer {
	listenUrl: string;
	publicUrl: string;
	inviteUrl: string;
	bindHost: string;
...
export async function startKerberoSecHubDashboardServer(): Promise<KerberoSecHubDashboardServer> {
...
export function printKerberoSecHubDashboardServerInfo(
	server: KerberoSecHubDashboardServer,
): void {
	console.log(`KerberoSec Hub dashboard listening: ${server.listenUrl}`);
	console.log(`KerberoSec Hub public URL: ${server.publicUrl}`);
	console.log(`hub endpoint: ${server.hubUrl}`);
	if (server.inviteRequired) {
		console.log(`KerberoSec Hub invite URL: ${server.inviteUrl}`);
...
if (import.meta.main) {
	const server = await startKerberoSecHubDashboardServer();
	printKerberoSecHubDashboardServerInfo(server);
}
