...
type SessionManager = KerberoSecCore;

/** Minimal plugin host interface injected by the runtime via globalThis. */
interface KerberoSecPluginHost {
	emitEvent?: (name: string, payload?: unknown) => void;
}

declare global {
	var __kerberosecPluginHost: KerberoSecPluginHost | undefined;
}
...
function resolveKerberoSecDirPath(): string {
	const explicitDir = process.env.KERBEROSEC_DIR?.trim();
	if (explicitDir) {
		return explicitDir;
	}
	return join(resolveDefaultHomeDir(), ".kerberosec");
}

function resolveKerberoSecDataDirPath(): string {
	const explicitDir = process.env.KERBEROSEC_DATA_DIR?.trim();
	if (explicitDir) {
		return explicitDir;
	}
	return join(resolveKerberoSecDirPath(), "data");
}

function resolveGlobalAgentsDirPath(): string {
	return join(resolveKerberoSecDataDirPath(), "settings", "agents");
}

const HANDOFFS_DIR = join(
	resolveKerberoSecDataDirPath(),
	"plugins",
	"subagents",
	"handoffs",
);
...
const DEFAULT_PROVIDER_ID = envOr(
	"KERBEROSEC_SUBAGENT_PROVIDER_ID",
	"kerberosec",
);
const DEFAULT_MODEL_ID = envOr(
	"KERBEROSEC_SUBAGENT_MODEL_ID",
	"anthropic/claude-sonnet-4.6",
);
type SubagentBackendMode = "auto" | "hub" | "local";

const DEFAULT_BACKEND_MODE = envOr("KERBEROSEC_SUBAGENTS_BACKEND_MODE", "auto");
const DEFAULT_AGENT_PRESET = envOr(
	"KERBEROSEC_SUBAGENT_DEFAULT_PRESET",
	"phantom",
);
...
function parseFrontmatter(md: string): {
	...
	// UTF-8 BOM (see kerberosec/kerberosec#12151).
	md = stripUtf8Bom(md);
	const m = md.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n?([\s\S]*)$/);
...
function readAgentDefinitions(baseCwd: string): AgentDefinition[] {
	const dirs: Array<{ path: string; source: AgentDefinition["source"] }> = [
		...
		{ path: join(baseCwd, ".kerberosec", "agents"), source: "project" },
	];
...
function readSkillDefinitions(baseCwd: string): SkillDefinition[] {
	const dirs: Array<{ path: string; source: SkillDefinition["source"] }> = [
		...
		{ path: join(baseCwd, ".kerberosec", "skills"), source: "project" },
	];
...
function emitSteer(sessionId: string | undefined, prompt: string): void {
	if (sessionId && prompt.trim()) {
		globalThis.__kerberosecPluginHost?.emitEvent?.("steer_message", {
			sessionId,
			prompt,
		});
	}
...
async function getSessionManager(): Promise<SessionManager> {
	sessionManagerPromise ??= KerberoSecCore.create({
			...
			toRegisteredTool(
				createTool({
					...
					async execute(input, ctx) {
						const mgr = await getSessionManager();
						const baseCwd = envOr("KERBEROSEC_SUBAGENT_CWD", process.cwd());
