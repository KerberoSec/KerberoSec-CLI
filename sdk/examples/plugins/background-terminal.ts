 ...
 * under KerberoSec's data directory, and optionally steers a completion summary back
 * into the current session when the command exits.
 *
 * CLI usage:
 *   kerberosec plugin install https://github.com/kerberosec/kerberosec/blob/main/sdk/examples/plugins/background-terminal.ts --cwd .
 *   kerberosec -i "Start the dev server in the background and keep working"
 */
...
const KERBEROSEC_DATA_DIR =
	process.env.KERBEROSEC_DATA_DIR || join(homedir(), ".kerberosec", "data");
const JOBS_DIR = join(
	KERBEROSEC_DATA_DIR,
	"plugins",
	"background-shell",
