import { readTuiThemeGlobally, setTuiThemeGlobally } from "@kerberosec/core";
...
/**
 * Resolves the theme to boot with: KERBEROSEC_THEME env override first, then the
 * persisted setting, falling back to terminal auto-detection.
 */
export function getInitialThemeId(): string {
	const fromEnv = process.env.KERBEROSEC_THEME?.trim();
	if (fromEnv) {
