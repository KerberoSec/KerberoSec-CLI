...
import { setKerberoSecDir } from "@kerberosec/shared/storage";
...
describe("createTaskWorktree", () => {
	...
	beforeEach(async () => {
		sandboxRoot = await mkdtemp(
			path.join(tmpdir(), "kerberosec-sdk-worktree-"),
		);
		kerberosecDir = path.join(sandboxRoot, ".kerberosec");
		...
		setKerberoSecDir(kerberosecDir);
	...
	afterEach(async () => {
		...
		setKerberoSecDir(originalKerberoSecDir ?? path.join("~", ".kerberosec"));
		await rm(sandboxRoot, { recursive: true, force: true });
	});

	it("places worktrees under ~/.kerberosec/worktrees", () => {
		expect(getTaskWorktreesHomePath()).toBe(
			path.join(kerberosecDir, "worktrees"),
		);
	});

	it("creates a detached worktree at ~/.kerberosec/worktrees/<taskId>/<repoName>", async () => {
		...
		expect(result.path).toBe(
			path.join(kerberosecDir, "worktrees", "my-task", "myrepo"),
		);
	...
	it("generates a Kanban-style short taskId when none is provided", async () => {
		...
		expect(result.path).toBe(
			path.join(kerberosecDir, "worktrees", result.taskId, "myrepo"),
		);
	});
	...
	it("cleans up the task directory when git worktree add fails", async () => {
		...
		expect(
			await pathExists(path.join(kerberosecDir, "worktrees", "empty")),
		).toBe(false);
	});
