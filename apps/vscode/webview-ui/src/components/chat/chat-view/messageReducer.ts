...
// Convergent-replica reducer for the webview's kerberosecMessages transcript.
//
...
function seqOf(message: KerberoSecMessage): number {
	return message.seq ?? 0
}

/** Replace the replica's transcript wholesale at a new epoch (new task / history load). */
function resetTo(epoch: number, messages: KerberoSecMessage[], stateVersion: number, turnState?: TurnState): ReplicaState {
...
/**
 * Apply one incoming KerberoSecMessage (from the partial-message stream OR from within a state
 ...
 */
export function applyMessage(state: ReplicaState, incoming: KerberoSecMessage): ReplicaState {
...
export function applyStateSnapshot(
	state: ReplicaState,
	snapshotMessages: KerberoSecMessage[],
	snapshotEpoch = 0,
	snapshotVersion = 0,
