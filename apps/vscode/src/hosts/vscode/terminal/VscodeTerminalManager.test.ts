...
describe("VscodeTerminalManager", () => {
	...
	it("creates a fresh terminal after timing out an unconfirmed reused-terminal cwd change", async () => {
		...
		const targetCwd = "/tmp/kerberosec-target"
		...
		const terminalInfo: TerminalInfo = {
			...
			terminal: {
				shellIntegration: {
					cwd: vscode.Uri.file("/tmp/kerberosec-original"),
	...
	it("reuses a terminal after its cwd change is confirmed", async () => {
		const targetCwd = "/tmp/kerberosec-target"
		let currentCwd = vscode.Uri.file("/tmp/kerberosec-original")
		const terminalInfo: TerminalInfo = {
			id: 1,
			busy: false,
			lastCommand: "",
	...
	it("releases a reused terminal reservation when showing it fails", async () => {
		const terminalInfo: TerminalInfo = {
			...
			lastActive: Date.now(),
			terminal: {
				shellIntegration: { cwd: vscode.Uri.file("/tmp/kerberosec-original") },
				show: sandbox.stub().throws(new Error("terminal closed")),
			} as unknown as vscode.Terminal,
		}
		sandbox.stub(TerminalRegistry, "getAllTerminals").returns([terminalInfo])

		await assert.rejects(manager.getOrCreateTerminal("/tmp/kerberosec-target"), /terminal closed/)
	...
	it("creates a fresh terminal when the reused-terminal cwd command cannot start", async () => {
		...
		const terminal = (await manager.getOrCreateTerminal("/tmp/kerberosec-target")) as unknown as TerminalInfo
		try {
			assert.notEqual(terminal, terminalInfo)
			assert.equal(terminal.busy, true)
			assert.equal(terminalInfo.busy, false)
	...
	it("releases a reused terminal reservation when it closes during cwd setup", async () => {
		...
		const rejectedAcquisition = assert.rejects(
			manager.getOrCreateTerminal("/tmp/kerberosec-target"),
	...
	it("reserves different terminals for parallel acquisitions", async () => {
		setVscodeHostProviderMock()
		const first = (await manager.getOrCreateTerminal("/tmp/kerberosec-parallel")) as unknown as TerminalInfo
		const second = (await manager.getOrCreateTerminal("/tmp/kerberosec-parallel")) as unknown as TerminalInfo
	...
	it("does not reuse a terminal after its command stream fails", async () => {
		setVscodeHostProviderMock()
		const terminalInfo = TerminalRegistry.createTerminal("/tmp/kerberosec-stream-error")
		sandbox.stub(terminalInfo.terminal, "shellIntegration").get(() => ({
			cwd: vscode.Uri.file("/tmp/kerberosec-stream-error"),
		...
		const nextTerminal = (await manager.getOrCreateTerminal("/tmp/kerberosec-stream-error")) as unknown as TerminalInfo
		try {
	...
	it("continues terminal acquisition when pending cleanup fails", async () => {
		...
		try {
			acquiredTerminal = (await manager.getOrCreateTerminal(
				"/tmp/kerberosec-after-cleanup-error",
	...
	it("defers fallback terminal disposal until the next terminal acquisition", async () => {
		...
		try {
			...
			nextTerminal = (await nextManager.getOrCreateTerminal("/tmp/kerberosec-next-command")) as unknown as TerminalInfo
	...
	it("preserves a detached fallback terminal across the next terminal acquisition", async () => {
		...
		try {
			...
			nextTerminal = (await manager.getOrCreateTerminal("/tmp/kerberosec-next-command")) as unknown as TerminalInfo
