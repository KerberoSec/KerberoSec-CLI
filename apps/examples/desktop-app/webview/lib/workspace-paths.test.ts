...
describe("workspace paths", () => {
	...
	it("normalizes trailing separators and Windows path casing", () => {
		expect(normalizeWorkspacePath(" /workspace/kerberosec/ ")).toBe(
			"/workspace/kerberosec",
		);
		expect(normalizeWorkspacePath("C:\\Users\\Saoud\\KerberoSec\\")).toBe(
			"c:\\users\\saoud\\kerberosec",
		);
		expect(normalizeWorkspacePath("/")).toBe("/");
	});

	it("detects absolute file paths across platforms", () => {
		expect(isAbsoluteFilePath("/Users/renee/kerberosec/docs/a.mdx")).toBe(true);
	...
	it("resolves relative diff paths against the session cwd", () => {
		expect(
			resolveWorkspaceFilePath("docs/a.mdx", "/Users/renee/kerberosec"),
		).toBe("/Users/renee/kerberosec/docs/a.mdx");
		expect(
			resolveWorkspaceFilePath("./docs/a.mdx", "/Users/renee/kerberosec/"),
		).toBe("/Users/renee/kerberosec/docs/a.mdx");
		expect(
			resolveWorkspaceFilePath(
				"/Users/renee/kerberosec/docs/a.mdx",
				"/elsewhere",
			),
		).toBe("/Users/renee/kerberosec/docs/a.mdx");
	...
	it("excludes .kerberosec-internal paths from the workspace catalog", () => {
		expect(
			isExcludedWorkspacePath(
				"/Users/beatrix/.kerberosec/worktrees/5e0b3/sdk-wip",
			),
		).toBe(true);
		expect(
			isExcludedWorkspacePath(
				"/Users/beatrix/.kerberosec/plugins/_installed/git/github.com/example-plugin",
			),
		).toBe(true);
		expect(
			isExcludedWorkspacePath("C:\\Users\\Saoud\\.kerberosec\\worktrees\\abc"),
		).toBe(true);
	});

	it("excludes the SDK chat workspace from discovery and stored selections", () => {
		const temporaryWorkspace = "/home/host/.kerberosec/data/workspaces/chat";
	...
	describe("with a registered host home directory", () => {
		...
		it("matches Windows homes case-insensitively", () => {
			...
			expect(isExcludedWorkspacePath("D:\\Homes\\Bea\\kerberosec")).toBe(false);
		});
	});
	...
	it("excludes home and Desktop directories but keeps projects inside them", () => {
		...
		expect(isExcludedWorkspacePath("/Users/beatrix/dev/kerberosec")).toBe(
			false,
		);
		...
		expect(isExcludedWorkspacePath("/workspace/kerberosec")).toBe(false);
		expect(isExcludedWorkspacePath("C:\\Users\\Saoud\\KerberoSec")).toBe(false);
	});

	it("filters excluded paths out of session-derived workspaces", () => {
		const paths = workspacePathsFromSessions([
			{ workspaceRoot: "/projects/app" },
			{ workspaceRoot: "/Users/beatrix/.kerberosec/worktrees/97815/sdk-wip" },
