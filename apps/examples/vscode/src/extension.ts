...
export function activate(context: vscode.ExtensionContext): void {
	...
	context.subscriptions.push(
		...
		vscode.window.registerWebviewViewProvider(
			"kerberosecVscode.chatView",
			sidebarProvider,
			{ webviewOptions: { retainContextWhenHidden: true } },
		),
		vscode.commands.registerCommand("kerberosecVscode.openChat", () => {
...
class KerberoSecChatViewProvider implements vscode.WebviewViewProvider {
	constructor(
...
type StartConfig = {
	...
	logger: BasicLogger;
	extensionContext?: import("@kerberosec/shared").ExtensionContext;
...
class CoreChatWebviewController implements vscode.Disposable {
	...
	private host: KerberoSecCore | undefined;
	...
	constructor(
		...
		if (sharedTelemetry) {
			...
			const { telemetry } = createConfiguredTelemetryService(
				createKerberoSecTelemetryServiceConfig({
					metadata: createKerberoSecTelemetryServiceMetadata({
						extension_version: version,
						kerberosec_type: displayName,
						platform: vscode.env.appName,
	...
	private async initialize(): Promise<void> {
		try {
			...
			await this.post({ type: "status", text: "KerberoSec is Ready" });
...
<body>
	...
	private buildExtensionContext(workspaceRoot: string, cwd: string) {
		return {
			client: { name: "kerberosec-vscode", version },
			workspace: {
	...
	private async buildStartConfigFromSession(
		session: NonNullable<Awaited<ReturnType<KerberoSecCore["get"]>>>,
	): Promise<StartConfig> {
		const mode: "act" | "plan" =
	...
	private async ensureSession(
		...
		const providerId = Llms.normalizeProviderId(
			config?.provider?.trim() || "kerberosec",
		);
	...
	private async getSessionHost(): Promise<KerberoSecCore> {
		if (!this.host) {
			await this.ensureHub();
			const defaults = this.resolveWorkspaceDefaults();
			this.host = await KerberoSecCore.create({
				backendMode: "hub",
