...
import "should"
import { KerberoSecFileStorage } from "@shared/storage/KerberoSecFileStorage"
...
describe("vscode-to-file-migration", () => {
	...
	beforeEach(() => {
		...
		originalMcpSettingsPath = process.env.KERBEROSEC_MCP_SETTINGS_PATH
		tempDir = path.join(os.tmpdir(), `migration-test-${Date.now()}-${Math.random().toString(36).slice(2)}`)
		fs.mkdirSync(tempDir, { recursive: true })
		process.env.KERBEROSEC_MCP_SETTINGS_PATH = path.join(tempDir, "runtime-mcp-settings", "kerberosec_mcp_settings.json")

		storageContext = createStorageContext({
			kerberosecDir: tempDir,
	...
	afterEach(() => {
		...
		if (originalMcpSettingsPath === undefined) {
			...
			process.env.KERBEROSEC_MCP_SETTINGS_PATH = originalMcpSettingsPath
		}
		try {
	...
	describe("sentinel behavior", () => {
		...
		it("should run only MCP settings migration when v1 storage export sentinels are already present", async () => {
			...
			mockCtx._workspaceStateStore.set("localKerberoSecRulesToggles", { "rule-1": true })
			...
			fs.writeFileSync(
				path.join(extensionStorage, "settings", "kerberosec_mcp_settings.json"),
			...
			;(storageContext.workspaceState.get("localKerberoSecRulesToggles") === undefined).should.be.true()
			storageContext.globalState.get("__vscodeMigrationVersion")!.should.equal(3)
			storageContext.workspaceState.get("__vscodeMigrationVersion")!.should.equal(3)
		})
		...
		it("should migrate workspace state when globals already migrated (new workspace)", async () => {
			...
			const stored = storageContext.workspaceState.get("localKerberoSecRulesToggles") as any
		...
		it("should migrate globals when workspace already migrated", async () => {
			...
			const mockCtx = createMockVSCodeContext()
			mockCtx._globalStateStore.set("mode", "plan")
			mockCtx._workspaceStateStore.set("localKerberoSecRulesToggles", { "rule-1": true }) // should be skipped
	...
	describe("legacy MCP settings migration", () => {
		function sharedMcpSettingsPath() {
			return process.env.KERBEROSEC_MCP_SETTINGS_PATH!
		}
		...
		it("writes to the runtime MCP settings resolver path rather than storage.dataDir", async () => {
			...
			fs.existsSync(path.join(storageContext.dataDir, "settings", "kerberosec_mcp_settings.json")).should.be.false()
	...
	describe("workspace state migration", () => {
		it("should migrate workspace state keys", async () => {
			...
			const mockCtx = createMockVSCodeContext()
			mockCtx._workspaceStateStore.set("localKerberoSecRulesToggles", toggles)
		...
		it("should NOT overwrite existing workspace state", async () => {
			...
			storageContext.workspaceState.set("localKerberoSecRulesToggles", existingToggles)

			const mockCtx = createMockVSCodeContext()
			mockCtx._workspaceStateStore.set("localKerberoSecRulesToggles", { "rule-vscode": true })
...
describe("createStorageContext", () => {
	...
	it("should create all three stores", () => {
		const ctx = createStorageContext({ kerberosecDir: tempDir, workspacePath: "/fake/workspace" })

		ctx.globalState.should.be.instanceOf(KerberoSecFileStorage)
		ctx.secrets.should.be.instanceOf(KerberoSecFileStorage)
		ctx.workspaceState.should.be.instanceOf(KerberoSecFileStorage)
	})
	...
	it("should produce deterministic workspace hashes", () => {
		const ctx1 = createStorageContext({ kerberosecDir: tempDir, workspacePath: "/some/project" })
		const ctx2 = createStorageContext({ kerberosecDir: tempDir, workspacePath: "/some/project" })
	...
	it("should produce different hashes for different workspaces", () => {
		const ctx1 = createStorageContext({ kerberosecDir: tempDir, workspacePath: "/project-a" })
		const ctx2 = createStorageContext({ kerberosecDir: tempDir, workspacePath: "/project-b" })
	...
	it("should store and retrieve values correctly", () => {
		const ctx = createStorageContext({ kerberosecDir: tempDir, workspacePath: "/test" })
	...
	// The data dir must resolve like the SDK's resolveKerberoSecDataDir and the
	// legacy reader's resolveDataDir: KERBEROSEC_DATA_DIR first. Ignoring it split
	...
	describe("environment-based data dir resolution", () => {
		const previousKerberoSecDir = process.env.KERBEROSEC_DIR
		const previousKerberoSecDataDir = process.env.KERBEROSEC_DATA_DIR

		afterEach(() => {
			restoreEnv("KERBEROSEC_DIR", previousKerberoSecDir)
			restoreEnv("KERBEROSEC_DATA_DIR", previousKerberoSecDataDir)
		})
		...
		it("should honor KERBEROSEC_DATA_DIR when no kerberosecDir option is given", () => {
			const isolatedDataDir = path.join(tempDir, "isolated-data")
			process.env.KERBEROSEC_DATA_DIR = isolatedDataDir
		...
		it("should prefer KERBEROSEC_DATA_DIR over KERBEROSEC_DIR", () => {
			const isolatedDataDir = path.join(tempDir, "isolated-data")
			process.env.KERBEROSEC_DATA_DIR = isolatedDataDir
			process.env.KERBEROSEC_DIR = path.join(tempDir, "kerberosec-home")
		...
		it("should let an explicit kerberosecDir option win over KERBEROSEC_DATA_DIR", () => {
			process.env.KERBEROSEC_DATA_DIR = path.join(tempDir, "isolated-data")
		...
		it("should trim whitespace-padded env values like the SDK resolvers", () => {
			...
			ctx.dataDir.should.equal(path.join(tempDir, "kerberosec-home", "data"))
		})
	})
})
