...
title: "Tools"
sidebarTitle: "Tools"
description: "Reference for KerberoSec's built-in tool system, aligned with SDK tool architecture."
---

KerberoSec tools are executable functions the model can call while working. The model decides which tool to call, KerberoSec runs it, then returns results back to the model.

## Built-In Tools (KerberoSecCore)

When running through `KerberoSecCore`, these built-ins are available:
...
<Note>
`Agent` from `@kerberosec/agents` does not include built-ins by default. You provide tools explicitly.
</Note>
...
KerberoSec can run tools with approval or auto-approval, depending on settings/policies. Typical patterns:
...
KerberoSec can also call tools discovered from MCP servers configured in `.kerberosec/mcp.json`.

MCP tools are loaded alongside built-ins, so KerberoSec can use both local tools and external integrations in one task.
...
Some older docs/examples reference XML-style names like `read_file`, `replace_in_file`, or `execute_command`. Current SDK/KerberoSecCore runtime uses the built-in tool names listed above (`read_files`, `apply_patch`, `bash`, etc.).
