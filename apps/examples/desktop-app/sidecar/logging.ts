...
function resolveRuntimeConfig(): Required<RuntimeLoggerConfig> {
	const enabledValue = process.env.KERBEROSEC_LOG_ENABLED?.trim().toLowerCase();
	return {
		enabled: enabledValue !== "0" && enabledValue !== "false",
		level: resolveLogLevel(process.env.KERBEROSEC_LOG_LEVEL),
		destination:
			process.env.KERBEROSEC_LOG_PATH?.trim() ||
			join(resolveKerberoSecDataDir(), "logs", "code.log"),
		name: process.env.KERBEROSEC_LOG_NAME?.trim() || "kerberosec-code.sidecar",
		bindings: {},
	};
}
...
function writeDestinationFallbackWarning(path: string, error: unknown): void {
	try {
		...
		process.stderr.write(
			`[kerberosec-code] Unable to open log file ${path}; falling back to stderr (${message})\n`,
		);
	} catch {
