# KerberoSec Extension Architecture & Development Guide

## Project Overview

KerberoSec is a VSCode extension that provides AI assistance through a combination of a core extension backend and a React-based webview frontend. The extension is built with TypeScript and follows a modular architecture pattern.
...
The WebviewProvider maintains a reference to the Controller and delegates message handling to it. It also handles the creation of both sidebar and tab panel webviews, allowing KerberoSec to be used in different contexts within VSCode.
...
KerberoSec supports multiple AI providers through a modular API provider system. Each provider is implemented as a separate module in the `src/api/providers/` directory and follows a common interface.
...
KerberoSec supports separate model configurations for Plan and Act modes:
...
class Task {
  async initiateTaskLoop(userContent: UserContent, isNewTask: boolean) {
    while (!this.abort) {
      ...
      const recDidEndLoop = await this.recursivelyMakeKerberoSecRequests(
...
class Task {
  async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
    ...
    const previousRequest = this.kerberosecMessages[previousApiReqIndex]
...
class Task {
  async resumeTaskFromHistory() {
    // 1. Load saved state
    this.kerberosecMessages = await getSavedKerberoSecMessages(this.getContext(), this.taskId)
  ...
  private async saveTaskState() {
    ...
    await saveKerberoSecMessages(this.getContext(), this.taskId, this.kerberosecMessages)
...
KerberoSec implements a dual-mode system that separates planning from execution:
...
KerberoSec supports two types of MCP server connections:
...
class Controller {
  ...
  async downloadMcp(mcpId: string) {
    ...
    const response = await axios.post<McpDownloadResponse>(
      "https://api.kerberosec.bot/v1/mcp/download",
      { mcpId },
      {
    ...
    await this.initKerberoSecWithTask(task)
  }
}
```
...
This guide provides a comprehensive overview of the KerberoSec extension architecture, with special focus on state management, data persistence, and code organization. Following these patterns ensures robust feature implementation with proper state handling across the extension's components.
...
Contributions to the KerberoSec extension are welcome! Please follow these guidelines:
...
The `.kerberosecignore` file allows users to specify files and directories that KerberoSec should not access. When implementing new features, respect the `.kerberosecignore` rules and ensure that your code does not attempt to read or modify ignored files.
