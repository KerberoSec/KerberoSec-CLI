...
import type { KerberoSecAccountUserInfo } from "@/services/auth/AuthService"
...
function makeUserInfo(
	overrides: Partial<KerberoSecAccountUserInfo> & { orgOverrides?: Record<string, unknown> } = {},
): KerberoSecAccountUserInfo {
...
describe("OpenTelemetryTelemetryProvider.identifyUser", () => {
	...
	it("should handle user with no active organization", () => {
		...
		const userInfo: KerberoSecAccountUserInfo = {
			id: "user-1",
	...
	it("should clear stale org attributes when switching from org to no-org", () => {
		...
		const userNoOrg: KerberoSecAccountUserInfo = {
			id: "user-1",
			displayName: "Test User",
			email: "test@example.com",
			createdAt: new Date().toISOString(),
