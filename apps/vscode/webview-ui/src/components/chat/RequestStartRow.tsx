...
const getActivityText = (tool: KerberoSecSayTool): string | null => {
...
const collectToolsInRange = (
	...
	startIdx: number,
	endIdx: number,
	stopCondition?: (msg: KerberoSecMessage) => boolean,
	...
	for (let i = startIdx; i < endIdx; i++) {
		...
		try {
			const tool = JSON.parse(msg.text || "{}") as KerberoSecSayTool
...
const findCurrentApiReq = (messages: KerberoSecMessage[]): { index: number; hasCost: boolean } | null => {
...
const findPrevCompletedApiReq = (messages: KerberoSecMessage[], beforeIdx: number): number => {
...
export const RequestStartRow: React.FC<RequestStartRowProps> = ({
	...
	const hasCompletionResult = kerberosecMessages.some(
		(msg) =>
	...
	const willBeAbsorbed = useMemo(() => {
		return isApiReqAbsorbable(message.ts, kerberosecMessages)
	}, [message.ts, kerberosecMessages])
	...
	const currentActivities = useMemo(() => {
		const currentApiReq = findCurrentApiReq(kerberosecMessages)
		if (!currentApiReq) {
			return []
		}
		...
		if (!currentApiReq.hasCost) {
			...
			return collectToolsInRange(kerberosecMessages, currentApiReq.index + 1, kerberosecMessages.length)
		}
	...
	const hasCompletedTools = useMemo(() => {
		...
		return kerberosecMessages.some((msg, idx) => {
			if (msg.say === "tool" && isLowStakesTool(msg)) {
				...
				for (let i = idx - 1; i >= 0; i--) {
					const prevMsg = kerberosecMessages[i]
