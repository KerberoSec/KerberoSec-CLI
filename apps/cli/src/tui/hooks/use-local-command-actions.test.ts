...
describe("runLocalSlashCommandAction", () => {
	...
	it("exits KerberoSec with quit", () => {
		vi.useFakeTimers();
		const exitKerberoSec = vi.fn();
		const actions = makeActions({ exitKerberoSec });
		...
		try {
			...
			expect(handled).toBe(true);
			expect(exitKerberoSec).not.toHaveBeenCalled();

			vi.runAllTimers();

			expect(exitKerberoSec).toHaveBeenCalledOnce();
		} finally {
			vi.useRealTimers();
		}
	});

	it("invokes logout action on logout command", async () => {
		const logout = vi.fn(async () => {});
		const actions = makeActions({ logout });
