...
const envKeys = [
	"KERBEROSEC_DATA_DIR",
	"KERBEROSEC_LOG_PATH",
	"KERBEROSEC_LOG_LEVEL",
	"KERBEROSEC_LOG_NAME",
	"KERBEROSEC_LOG_ENABLED",
] as const;
...
describe("createCliLoggerAdapter", () => {
	...
	it("resolves default runtime config from data dir", () => {
		...
		delete process.env.KERBEROSEC_LOG_PATH;
		delete process.env.KERBEROSEC_LOG_LEVEL;
		delete process.env.KERBEROSEC_LOG_NAME;
	...
	it("maps core logger metadata with error payload", () => {
		...
		const snapshot = withEnvSnapshot();
		process.env.KERBEROSEC_DATA_DIR = dataDir;
		process.env.KERBEROSEC_LOG_ENABLED = "0";
		try {
			const adapter = createCliLoggerAdapter({ runtime: "cli" });
			expect(() => {
	...
	it("falls back when log destination path is not writable", () => {
		...
		process.env.KERBEROSEC_LOG_PATH = unwritablePath;
		delete process.env.KERBEROSEC_LOG_LEVEL;
		delete process.env.KERBEROSEC_LOG_NAME;
		delete process.env.KERBEROSEC_LOG_ENABLED;
