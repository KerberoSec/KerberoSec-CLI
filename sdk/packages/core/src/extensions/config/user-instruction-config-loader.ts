...
import { stripUtf8Bom } from "@kerberosec/shared";
import {
...
function isIgnorableDirectoryError(error: unknown): boolean {
	...
	return (
		...
		// ENOTDIR: a path component is a file, e.g. `.kerberosecrules/workflows`
		// when `.kerberosecrules` is a legacy single-file ruleset. Treat it like a
...
async function discoverManagedPluginRoots(
	kerberosecDirectoryPath: string,
): Promise<string[]> {
	try {
		const entries = await readdir(kerberosecDirectoryPath, {
			withFileTypes: true,
		});
		...
		for (const entry of entries) {
			...
			const pluginRoot = join(kerberosecDirectoryPath, entry.name);
...
function parseMarkdownFrontmatter(
	...
	// regex below never matches a file that starts with "\uFEFF---" (see kerberosec/kerberosec#12151).
...
async function discoverSkillFiles(
	...
	if (basename(directoryPath) === ".kerberosec") {
...
export function createSkillsConfigDefinition(
	...
	const managedRoot = options?.workspacePath
		? join(options.workspacePath, ".kerberosec")
		: undefined;
