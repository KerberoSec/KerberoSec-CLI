	...
	const handleTerminal = (
		...
		if (kind === "completed") {
			...
			void notify({
				...
				body: "KerberoSec finished working and the result is ready.",
			});
			return;
		}
		void notify({
			...
			title: "Task failed",
			body:
				detail || "KerberoSec encountered an error while running this task.",
		});
	};
	...
	const subscriptions = [
		...
		desktopClient.subscribe("ask_question_requested", (payload) => {
			...
			void notify({
				...
				sessionId,
				title: "KerberoSec has a question",
				body:
					asNonEmptyString(item.question) ||
					"Open this task to answer KerberoSec's question.",
			});
		}),
	];
