...
Global settings, secrets and workspace state are stored in **file-backed JSON stores** under `~/.kerberosec/data/`. This is the shared storage layer used by VSCode, CLI, and JetBrains.

## Key Abstractions

### `StorageContext` (src/shared/storage/storage-context.ts)
The entry point. Created via `createStorageContext()` and passed to `StateManager.initialize()`. Contains three `KerberoSecFileStorage` instances:
- `globalState` → `~/.kerberosec/data/globalState.json`
- `secrets` → `~/.kerberosec/data/secrets.json` (mode 0o600)
- `workspaceState` → `~/.kerberosec/data/workspaces/<hash>/workspaceState.json`

### `KerberoSecFileStorage` (src/shared/storage/KerberoSecFileStorage.ts)
...
Remember that your data may be read by a different client than the one that wrote it. For example, a value written by KerberoSec in JetBrains may be read by KerberoSec CLI.
