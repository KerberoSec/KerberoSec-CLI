...
// fake "Conversation Summary" instead of compacting (KERBEROSEC-2503).

import type { Message as SdkMessage } from "@kerberosec/llms"
import type { KerberoSecCompactionInfo, KerberoSecMessage } from "@shared/ExtensionMessage"
...
const COMPACTION_UNSUPPORTED_MESSAGE = "Compaction is not supported by this runtime yet. Please update KerberoSec and try again."
...
export class SdkCompactionCoordinator {
	...
	private async runCompaction(sdkHost: SdkSessionHost, sessionId: string): Promise<void> {
		...
		let noticeInfo: KerberoSecCompactionInfo | undefined
		try {
	...
	private emitCompactionRow(info: KerberoSecCompactionInfo, ts: number, sessionId: string): void {
	...
	private emitInfo(text: string, sessionId?: string): void {
		...
		const infoMessage: KerberoSecMessage = {
			ts: Date.now(),
			type: "say",
			say: "info",
			text,
