...
import {
	...
	KerberoSecPassSubscriptionContent,
...
import { buildFeaturedModelEntries } from "../components/model-selector/kerberosec-model-picker";
import {
	BROWSE_ALL_ACTION,
	KerberoSecModelSelectorDialogContent,
} from "../components/model-selector/kerberosec-model-selector";
import {
	buildModelOptions,
...
function providerToExistingProviderOptions(input: {
	...
	if (input.providerId !== "kerberosec-pass") {
		return [];
	}
...
async function runProviderChange(
	...
	// working; only the KerberoSec providers accept a dashboard API key.
	const supportsManualApiKey = isKerberoSecProvider(newProviderId);
		...
		async (options?: OpenModelSelectorOptions) => {
			...
			while (pickingModel) {
				...
				if (
					...
					// KerberoSecPass gets the same sectioned picker with Subscribed/Free
					// sections — free models are selectable while staying on KerberoSecPass
					const featuredProviderId = config.providerId;
					const kerberosecResult = await dialog.choice<string>({
						style: { maxHeight: termHeight - 2 },
						content: (ctx: ChoiceContext<string>) => (
							<KerberoSecModelSelectorDialogContent
								{...ctx}
								...
								loadEntries={async () =>
									buildFeaturedModelEntries(
										featuredProviderId,
										await fetchKerberoSecRecommendedModels(),
									)
								}
							/>
						),
					...
					if (kerberosecResult === CHANGE_PROVIDER_ACTION) {
						await changeProvider();
						continue;
					}
					if (kerberosecResult === BROWSE_ALL_ACTION) {
						const browseResult = await dialog.choice<string>({
							...
							content: (ctx: ChoiceContext<string>) => (
								<ModelSelectorContent
									...
									currentProviderName={providerDisplayName}
									models={modelOptions}
									showCustomModelId={config.providerId !== "kerberosec-pass"}
								/>
							),
						});
					...
					config.modelId = kerberosecResult;
					const selectedModel = modelOptions.find(
						(m: ModelOption) => m.key === kerberosecResult,
					);
