...
function parseSubagentRowData(message: KerberoSecMessage): SubagentRowData | null {
	if (!message.text) {
		return null
	}
	...
	try {
		if (message.ask === "use_subagents" || message.say === "use_subagents") {
			const parsed = JSON.parse(message.text) as KerberoSecAskUseSubagents
		...
		const parsed = JSON.parse(message.text) as KerberoSecSaySubagentStatus
...
export default function SubagentStatusRow({ message, isLast, lastModifiedMessage }: SubagentStatusRowProps) {
	...
	const title = singular ? "KerberoSec wants to use a subagent:" : "KerberoSec wants to use subagents:"
