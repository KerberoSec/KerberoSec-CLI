...
if (!target || !["sdk", "cli"].includes(target)) {
	...
	console.error(
		"  sdk   Publish @kerberosec/{shared,llms,agents,core,sdk} to npm",
	);
	console.error(
		"  cli   Publish kerberosec from an existing cli-vX.Y.Z git tag",
	);
	console.error("");
...
async function releaseSDK(version: string): Promise<number> {
	...
	for (const workspace of SDK_PUBLISH_ORDER) {
		...
		const name = `@kerberosec/${workspace}`;
	...
	if (dryRun) {
		...
		for (const workspace of SDK_PUBLISH_ORDER) {
			console.log(`    - @kerberosec/${workspace}@${version}`);
		}
	}
	console.log(`${"═".repeat(60)}\n`);
...
async function releaseCLI(version: string): Promise<number> {
	...
	if (dryRun) {
		...
		console.log("    npm install -g kerberosec");
	}
	console.log(`${"═".repeat(60)}\n`);
