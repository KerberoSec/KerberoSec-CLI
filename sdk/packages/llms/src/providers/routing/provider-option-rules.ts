...
const kerberosecGatewayReasoningRule: ProviderOptionRule = {
	id: "provider.kerberosec.reasoning",
	phase: "provider-reasoning",
	description: "KerberoSec gateway accepts the shared gateway reasoning shape.",
	applies: (input) => isKerberoSecProvider(input.request.providerId),
	build: (input) =>
...
const kerberosecMiniMaxM3GatewayReasoningRule: ProviderOptionRule = {
	id: "provider.kerberosec.minimax-m3.gateway-reasoning",
	phase: "provider-reasoning",
	description:
		"KerberoSec-routed MiniMax M3 keeps the gateway reasoning shape instead of leaking generic thinking.",
	applies: (input) =>
		isKerberoSecProvider(input.request.providerId) && isMiniMaxM3(input),
	suppresses: { genericThinking: true },
	build: () => undefined,
};
...
const kerberosecReasoningDisabledThinkingRule: ProviderOptionRule = {
	id: "provider.kerberosec.disable-thinking",
	phase: "provider",
	description:
		"KerberoSec-routed non-Kimi-K2.6 Moonshot Kimi models use thinking.type=disabled when reasoning is disabled.",
	applies: (input) =>
		isKerberoSecProvider(input.request.providerId) &&
...
export const PROVIDER_OPTION_RULES: ReadonlyArray<ProviderOptionRule> = [
	...
	kerberosecGatewayReasoningRule,
	openRouterReasoningRule,
	kerberosecMiniMaxM3GatewayReasoningRule,
	vercelReasoningRule,
	directMoonshotReasoningRule,
	fireworksReasoningRule,
	geminiThinkingRule,
	kerberosecReasoningDisabledThinkingRule,
	kimiK26ThinkingRule,
