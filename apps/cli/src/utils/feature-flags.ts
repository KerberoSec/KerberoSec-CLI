...
import {
	buildKerberoSecPostHogClient,
	PostHogFeatureFlagsProvider,
} from "@kerberosec/core/services/feature-flags/posthog";
import { resolveKerberoSecDataDir } from "@kerberosec/shared/storage";

let cliFeatureFlagsContext: FeatureFlagsContext = {
	clientName: "kerberosec-cli",
};
...
function resolveCliFeatureFlagsCachePath(): string {
	return join(resolveKerberoSecDataDir(), "cache", "feature-flags.json");
}
		...
		const provider =
			...
			process.env.E2E_TEST !== "true"
				? new PostHogFeatureFlagsProvider({
						client: buildKerberoSecPostHogClient(apiKey),
						config: {
