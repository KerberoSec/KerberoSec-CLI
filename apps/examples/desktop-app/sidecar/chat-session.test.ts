...
describe("hasProviderChanged", () => {
	it("distinguishes provider switches from model switches", () => {
		expect(
			hasProviderChanged(
				{ provider: "kerberosec", model: "anthropic/claude-sonnet-4.6" },
		...
		expect(
			hasProviderChanged(
				{ provider: "kerberosec", model: "anthropic/claude-sonnet-4.6" },
				{ provider: "kerberosec", model: "openai/gpt-5.3-codex" },
			),
		).toBe(false);
	});
	...
	it("omits workspace paths and returns the SDK-resolved chat workspace", async () => {
		const start = vi.fn(async (input: { config: Record<string, unknown> }) => {
			...
			return {
				...
				manifest: {
					cwd: "/home/host/.kerberosec/data/workspaces/chat",
					workspace_root: "/home/host/.kerberosec/data/workspaces/chat",
				},
		...
		expect(result).toEqual({
			sessionId: "session-pathless",
			cwd: "/home/host/.kerberosec/data/workspaces/chat",
			workspaceRoot: "/home/host/.kerberosec/data/workspaces/chat",
		});
...
describe("session forks", () => {
	...
	it("allows a workspace restore after a queued turn completes through the event stream", async () => {
		...
		const dataDir = mkdtempSync(join(tmpdir(), "kerberosec-queued-restore-"));
		const originalDataDir = process.env.KERBEROSEC_SESSION_DATA_DIR;
		process.env.KERBEROSEC_SESSION_DATA_DIR = dataDir;
		try {
			const restore = vi.fn(async () => ({
				sessionId,
			...
			if (originalDataDir === undefined) {
				...
				process.env.KERBEROSEC_SESSION_DATA_DIR = originalDataDir;
			}
...
describe("first-send connection updates", () => {
	...
	it.each([
		...
		const { ctx, send, sessionId } = createContext();
		const previousSessionDataDir = process.env.KERBEROSEC_SESSION_DATA_DIR;
		const testSessionDataDir = join(
			tmpdir(),
			`kerberosec-desktop-attachments-${Date.now()}-${delivery ?? "immediate"}`,
		);
		...
		try {
			process.env.KERBEROSEC_SESSION_DATA_DIR = testSessionDataDir;
			await handleChatSessionCommand(ctx, {
				action: "send",
				sessionId,
				prompt: "",
			...
			if (previousSessionDataDir === undefined) {
				...
				process.env.KERBEROSEC_SESSION_DATA_DIR = previousSessionDataDir;
			}
	...
	it("deletes materialized attachments when a queued prompt is removed", async () => {
		...
		const testSessionDataDir = join(
			tmpdir(),
			`kerberosec-desktop-attachments-remove-${Date.now()}`,
		);
	...
	it("deletes tracked attachments when a session is reset", async () => {
		...
		const testSessionDataDir = join(
			tmpdir(),
			`kerberosec-desktop-attachments-reset-${Date.now()}`,
		);
	...
	it("preserves tracked attachments across re-attach", async () => {
		...
		const testSessionDataDir = join(
			tmpdir(),
			`kerberosec-desktop-attachments-attach-${Date.now()}`,
		);
	...
	it("restores the previous provider runtime when replacement startup fails", async () => {
		...
		const previousKanbanDataDir = process.env.KERBEROSEC_KANBAN_DATA_DIR;
		const testKanbanDataDir = join(
			tmpdir(),
			`kerberosec-provider-rollback-${process.pid}`,
		);
		process.env.KERBEROSEC_KANBAN_DATA_DIR = testKanbanDataDir;
		start
		...
		try {
			...
			await handleChatSessionCommand(ctx, {
				action: "send",
				sessionId,
				prompt: "continue with KerberoSec",
				config: { ...baseConfig },
			});
			expect(send).toHaveBeenCalledOnce();
		} finally {
			if (previousKanbanDataDir === undefined) {
				delete process.env.KERBEROSEC_KANBAN_DATA_DIR;
			} else {
				process.env.KERBEROSEC_KANBAN_DATA_DIR = previousKanbanDataDir;
			}
	...
	it("restores the previous provider when replacement label sync fails", async () => {
		...
		const testKanbanDataDir = join(
			tmpdir(),
			`kerberosec-provider-label-rollback-${process.pid}`,
		);
		process.env.KERBEROSEC_KANBAN_DATA_DIR = testKanbanDataDir;
		try {
			updateSessionConnection
...
describe("workspace metadata prewarming", () => {
	it("reuses one in-flight scan and consumes it only once", async () => {
		...
		const cwd = "/tmp/kerberosec-desktop-prewarm-reuse";
	...
	it("evicts failed scans so the next session can retry", async () => {
		...
		const cwd = "/tmp/kerberosec-desktop-prewarm-retry";
	...
	it("keeps different workspaces in separate single-flight entries", () => {
		...
		prewarmWorkspaceMetadata("/tmp/kerberosec-desktop-prewarm-a", load);
		prewarmWorkspaceMetadata("/tmp/kerberosec-desktop-prewarm-b", load);
	...
	it("refreshes a prewarm that is older than the startup window", async () => {
		...
		const cwd = "/tmp/kerberosec-desktop-prewarm-expired";
...
Follow the desktop send skill instructions.`,
		...
		const workflowsDir = join(workspace, ".kerberosec", "workflows");
...
Follow the desktop send workflow instructions.`,
	...
	function createContext(workspace: string) {
		...
		const session = {
			config: { provider: "kerberosec", model: "test-model", cwd: workspace },
			messages: [],
			promptsInQueue: [],
			busy: false,
			startedAt: Date.now(),
