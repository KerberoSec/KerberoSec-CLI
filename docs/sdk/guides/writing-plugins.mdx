...
To load this plugin from a file in KerberoSecCore, pass its path in `pluginPaths`:

```typescript
// /absolute/path/to/github.ts
import { type AgentPlugin, createTool } from "@kerberosec/sdk"
...
```bash
kerberosec plugin install https://github.com/your-org/your-repo/blob/main/plugins/github-plugin.ts
```

Single-file plugins can only import Node builtins and `@kerberosec/*`. As soon as you need an npm dependency (`zod`, an HTTP client, etc.) you must ship as a package: a directory with a `package.json` that declares a `kerberosec` field for entry points and your runtime `dependencies`. Dependencies under the `@kerberosec/` scope are provided by the host runtime -- the installer strips these and runs `npm install` for the rest, so declare any `@kerberosec/*` package you import as an optional peer dependency:
...
```bash
kerberosec plugin install https://github.com/your-org/kerberosec-github-plugin.git
```
...
Each bundled skill follows the same directory format as any other [KerberoSec skill](/customization/skills). When the plugin is installed or loaded through `pluginPaths`, KerberoSec discovers those skills automatically.

See [Plugins](/customization/plugins) for the full manifest format, directory layout, and the [typescript-lsp-plugin](https://github.com/kerberosec/typescript-lsp-plugin) for a complete working example.
