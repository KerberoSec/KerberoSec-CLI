...
export function generateManifest(nextPkg, legacyPkg, version) {
	...
	// The nightly packaging rewrites the whole `kerberosec.*` ID namespace to
	// `kerberosec-nightly.*` (scripts/nightlify.mjs), so the context key and the
	...
	const prefix = nextPkg.name === "kerberosec-nightly" ? "kerberosec-nightly" : "kerberosec";
...
function loaderSettings(prefix) {
	return {
		[`${prefix}.rollout.bundleOverride`]: {
			...
			scope: "application",
			markdownDescription:
				"Manual override for KerberoSec's staged extension rollout. `next` forces the new (SDK-based) extension, `legacy` forces the previous one, `auto` follows the remote rollout assignment. Takes effect on window reload.",
		},
	};
}
...
function injectLoaderConfiguration(configuration, prefix) {
	...
	return { title: "KerberoSec", ...(configuration ?? {}), properties };
}
