# `@kerberosec/ui` adoption primer

This guide is for KerberoSec engineering teams that want a web application to share
the KerberoSec visual language and agent-chat presentation without copying desktop
styles or adopting desktop product structure.

## The short version

`@kerberosec/ui` has two opt-in layers:
...
- KerberoSec-owned Slate, Violet, Ruby, Green, Amber, and Sky solid/alpha palettes
...
This boundary gives KerberoSec products a shared visual and interaction language
without turning `@kerberosec/ui` into a second agent runtime.
...
copy — that is the direction `@kerberosec/ui` is headed, and more shared modules
are expected over time. Two exist today:

- `@kerberosec/ui/components/agent-chat/tool-summary` — pure, framework-free
  ...
  `@kerberosec/core` or transport events.
- `@kerberosec/ui/components/agent-chat/tool-diff` — `ToolFileDiff`, a thin
...
`@kerberosec/ui` is configured for public npm publication with its own version and
manual release workflow. Check availability with `npm view @kerberosec/ui version`;
...
| --- | --- | --- | --- |
| Use only light/dark CSS variables | `@kerberosec/ui/theme/tokens.css` | No | No |
| Render shared root React primitives without host resets | `scoped-tokens.css`, `components.css`, and `@kerberosec/ui` | Tailwind v4 | React 18.3 or 19 |
| Use tokens through Tailwind utilities | `tokens.css` then `theme.css` | Tailwind v4 | No |
| Use the complete theme and shared base behavior | `@kerberosec/ui/theme/index.css` | Tailwind v4 | No |
| Compose shared agent-chat presentation | `@kerberosec/ui/components/agent-chat` plus its CSS | No, if tokens are mapped in plain CSS | React 18.3 or 19 |
...
There is no `@kerberosec/ui/theme` shorthand. Use the explicit paths documented here
so dependencies remain visible.

## Install inside the KerberoSec monorepo
...
# Required for the documented Tailwind-backed theme and KerberoSec fonts
...
@import "tailwindcss";
@import "@kerberosec/ui/theme/index.css";
```
...
@import "@fontsource-variable/geist-mono";
@import "tailwindcss";
@import "@kerberosec/ui/theme/tokens.css";
@import "@kerberosec/ui/theme/theme.css";
```

For an existing Tailwind v4 surface that must preserve its host shell, keep
Tailwind setup host-owned and add the scoped KerberoSec imports:

```css
@import "@kerberosec/ui/theme/scoped-tokens.css";
@import "@kerberosec/ui/components.css";
```

Render shared components inside `.kerberosec-ui-theme`. Dark values activate when
...
registers the complete, generic KerberoSec utility vocabulary and is intended only
for applications that want KerberoSec to own those utility names globally.
...
If the application later opts into KerberoSec's global utility vocabulary and base
...
```css
@import "@kerberosec/ui/theme/index.css";
@import "@kerberosec/ui/components/agent-chat.css";
```
...
import {
  ...
  ToolActivityContent,
  ToolActivityTrigger,
} from "@kerberosec/ui/components/agent-chat";
...
package depend on `@kerberosec/core`, the Vercel AI SDK, desktop schemas, or transport
events.

## Explore components in Storybook

From the KerberoSec repository root:

```bash
bun -F @kerberosec/ui storybook
```
...
```bash
bun -F @kerberosec/ui storybook -- --host 0.0.0.0 --port 3490 --exact-port
```

Build the production Storybook bundle with:

```bash
bun -F @kerberosec/ui build-storybook
```
...
The catalog currently runs from a KerberoSec monorepo checkout. Story sources and
...
Keep the following outside `@kerberosec/ui`:
...
- Keep KerberoSec-owned palette values in `palette.css` and author UI through visual
  or status roles
...
1. Adopt the theme and chat primitives in a second production-shaped KerberoSec app.
...
- [Desktop theme integration test (monorepo)](https://github.com/kerberosec/kerberosec/blob/main/apps/examples/desktop-app/webview/styles/theme-integration.test.ts)
