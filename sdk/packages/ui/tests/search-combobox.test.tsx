...
const options = [
	{ label: "kerberosec/kerberosec", value: "kerberosec" },
	{
		description: "Cloud dashboard",
		label: "kerberosec/core-platform",
		value: "core-platform",
	},
];
...
describe("SearchCombobox", () => {
	it("filters and selects an option", async () => {
		...
		expect(trigger?.getAttribute("aria-label")).toBe(
			"Repository: kerberosec/kerberosec",
		);
		...
		expect(panel?.textContent).not.toContain("kerberosec/kerberosec");
		const match = Array.from(panel?.querySelectorAll("button") ?? []).find(
			(button) =>
				button.textContent === "kerberosec/core-platformCloud dashboard",
		);
	...
	it("navigates with arrow keys and selects with Enter", async () => {
		...
		expect(active?.textContent).toContain("kerberosec/core-platform");
		await act(async () => {
			search?.dispatchEvent(
	...
	it("renders section headers and badges, and flattens while searching", async () => {
		...
		expect(
			searchedPanel?.querySelector(".kerberosec-ui-search-combobox__match")
				?.textContent,
		).toBe("DeepSeek");
	});
	...
	it("renders loading and disabled states", async () => {
		...
		const optionButtons = container.querySelectorAll<HTMLButtonElement>(
			".kerberosec-ui-search-combobox__option",
		);
