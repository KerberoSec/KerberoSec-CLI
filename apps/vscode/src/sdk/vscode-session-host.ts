// VscodeSessionHost — wraps KerberoSecCore with VSCode-specific customizations
//
// Uses KerberoSecCore.create() so the SDK owns session input normalization,
...
import {
	...
	KerberoSecCore,
	type KerberoSecCoreListHistoryOptions,
...
export class VscodeSessionHost implements SdkSessionHost {
	...
	private readonly inner: KerberoSecCore
	private readonly prepareStartSessionInput?: (input: KerberoSecCoreStartInput) => Promise<KerberoSecCoreStartInput>

	private constructor(
		inner: KerberoSecCore,
		prepareStartSessionInput?: (input: KerberoSecCoreStartInput) => Promise<KerberoSecCoreStartInput>,
	) {
		this.inner = inner
	...
	static async create(options: VscodeSessionHostOptions): Promise<VscodeSessionHost> {
		...
		// the VSCode extra tools. Used by KerberoSecCore's prepare hook for normal
		// starts AND by restore() for checkpoint-restore replacement sessions,
		// which KerberoSecCore starts without running the prepare hook.
		const prepareStartSessionInput = async (input: KerberoSecCoreStartInput): Promise<KerberoSecCoreStartInput> => {
		...
		const inner = await KerberoSecCore.create({
			backendMode: "local",
			capabilities: {
		...
		Logger.log("[VscodeSessionHost] Initialized with KerberoSecCore + VSCode extra tools")
		if (options.getTerminalManager) {
	...
	async start(input: KerberoSecCoreStartInput): Promise<StartSessionResult>
	async start(input: StartSessionInput | KerberoSecCoreStartInput): Promise<StartSessionResult> {
		return this.inner.start(input as KerberoSecCoreStartInput)
	}
	...
	async list(limit?: number, options: Omit<KerberoSecCoreListHistoryOptions, "limit"> = {}): Promise<SessionHistoryRecord[]> {
		return this.inner.list(limit, options)
	}

	async listHistory(options: KerberoSecCoreListHistoryOptions = {}): Promise<SessionHistoryRecord[]> {
	...
	async restore(input: RestoreInput): Promise<RestoreResult> {
		// KerberoSecCore.restore starts the checkpoint-restore replacement session
