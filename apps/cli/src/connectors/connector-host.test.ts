...
describe("handleConnectorUserTurn", () => {
	...
	it("sends a first-contact message only once per persisted thread state", async () => {
		...
		const commonInput = {
			...
			transport: "telegram",
			botUserName: "KerberoSecAdapterBot",
			requestStop: vi.fn(),
			bindingsPath,
			systemRules: "rules",
			errorLabel: "Telegram",
	...
	it("keeps tools disabled when connector startup forced no-tools", async () => {
		...
		await handleConnectorUserTurn({
			...commonInput,
			text: "/tools@KerberoSecAdapterBot on",
		});
		...
		await handleConnectorUserTurn({
			...commonInput,
			text: "/yolo@KerberoSecAdapterBot on",
		});
	...
	it("recovers when the bound session is wedged on a run that never drained", async () => {
		// KerberoSec Mom's failure: the thread pointed at a session whose runtime still
	...
	it("mutes a specific participant in the current connector thread", async () => {
		...
		await handleConnectorUserTurn({
			...
			text: "/mute@KerberoSecAdapterBot <@bob>",
		});
		await handleConnectorUserTurn({
			...commonInput,
		...
		await handleConnectorUserTurn({
			...commonInput,
			thread: alice.thread as never,
			text: "/unmute@KerberoSecAdapterBot <@bob>",
		});
		await handleConnectorUserTurn({
			...commonInput,
			thread: bob.thread as never,
	...
	it("does not report thread unmuted when only participant-specific mutes are active", async () => {
		...
		await handleConnectorUserTurn({
			...commonInput,
			thread: alice.thread as never,
			text: "/unmute@KerberoSecAdapterBot",
		});
		await handleConnectorUserTurn({
			...commonInput,
			thread: bob.thread as never,
