...
//
// Translates SDK session events into KerberoSecMessage[] for webview consumption.
// The webview expects KerberoSecMessage objects with ask/say types; this module
// maps SDK CoreSessionEvent and AgentEvent types to that format.
//
// Key mappings:
// - SDK "chunk" event (agent stream) → KerberoSecMessage say="text" with partial=true
// - SDK "agent_event" content_start (text) → KerberoSecMessage say="text" with partial=true
// - SDK "agent_event" content_start (reasoning) → KerberoSecMessage say="reasoning" with partial=true
// - SDK "agent_event" content_start (tool) → KerberoSecMessage say="tool" with partial=true
//   IMPORTANT: The webview's ChatRow.tsx parses message.text as JSON when
//   say==="tool", expecting KerberoSecSayTool format: {tool, path, content, ...}.
//   We must convert SDK tool names (read_files, editor, run_commands, etc.)
//   and their inputs to this format.
// - SDK "agent_event" content_start (tool: MCP) → KerberoSecMessage say="use_mcp_server" with partial=true
//   MCP tools use serverName__toolName naming convention. The webview renders
//   MCP tool calls via say/ask="use_mcp_server" with KerberoSecAskUseMcpServer JSON.
// - SDK "agent_event" content_end (tool: MCP) → say="use_mcp_server" + say="mcp_server_response"
// - SDK "agent_event" content_end → KerberoSecMessage with partial=false
// - SDK "agent_event" content_start (tool: attempt_completion) → KerberoSecMessage say="completion_result"
// - SDK "agent_event" content_end (tool: attempt_completion) → KerberoSecMessage say="completion_result" (final)
...
// - SDK "agent_event" error → KerberoSecMessage say="error"
// - SDK "agent_event" usage → KerberoSecMessage say="api_req_started" with KerberoSecApiReqInfo JSON
// - SDK "ended" event → finalizes the session

import type { CoreSessionEvent } from "@kerberosec/core"
import { PATCH_MARKERS, projectSessionMessagesForDisplay } from "@kerberosec/core"
import type { MessageWithMetadata as SdkMessage } from "@kerberosec/llms"
import { type AgentEvent, formatDisplayUserInput } from "@kerberosec/shared"
import { COMMAND_OUTPUT_STRING } from "@shared/combineCommandSequences"
import type {
	KerberoSecApiReqInfo,
	KerberoSecAskUseMcpServer,
	KerberoSecAskUseSubagents,
	KerberoSecCompactionInfo,
	KerberoSecMessage,
	KerberoSecSay,
	KerberoSecSaySubagentStatus,
	KerberoSecSayTool,
	KerberoSecSubagentUsageInfo,
	SubagentStatusItem,
} from "@shared/ExtensionMessage"
import { Logger } from "@shared/services/Logger"
import * as path from "path"
import { arePathsEqual, getDesktopDir } from "@/utils/path"
import { KERBEROSEC_FREE_PROMOTION_ENDED_ERROR_CODE, isKerberoSecFreePromotionEndedMessage } from "../services/error/KerberoSecError"
...
/**
 * Result of translating a single SDK event into KerberoSecMessages.
 * May produce zero or more messages.
 */
export interface TranslationResult {
	/** Messages produced by this event */
	messages: KerberoSecMessage[]
...
function normalizeUsageEvent(usageEvent: {
	...
	// cache reads/writes included. Classic KerberoSec/webview metrics expect
...
export class MessageTranslatorState {
	...
	/** Build a KerberoSecSaySubagentStatus from the current entries */
	buildSubagentStatus(overallStatus: KerberoSecSaySubagentStatus["status"]): KerberoSecSaySubagentStatus {
...
/**
 * Tools whose KerberoSecSayTool.path is a filesystem path. webFetch/webSearch/
 * useSkill and MCP tools reuse `path` for URLs, queries, and names, so they
 * are deliberately excluded.
 */
const FILESYSTEM_PATH_TOOLS: ReadonlySet<KerberoSecSayTool["tool"]> = new Set([
	"readFile",
	"listFilesTopLevel",
	"listFilesRecursive",
...
/**
 * Relativize a KerberoSecSayTool's filesystem paths against the task cwd before it
 ...
 */
function toDisplaySayTool(sayTool: KerberoSecSayTool, cwd: string | undefined): KerberoSecSayTool {
...
// ---------------------------------------------------------------------------
// SDK tool name → classic KerberoSecSayTool mapping
// ---------------------------------------------------------------------------

/**
 * Map an SDK tool name and its input to a KerberoSecSayTool object that the
 * webview's ChatRow.tsx can render.
 *
 * The webview does `JSON.parse(message.text) as KerberoSecSayTool` when
 * `say === "tool"`, so the text MUST be valid KerberoSecSayTool JSON.
 *
 ...
 *   MCP tools (serverName__toolName)   → (handled before reaching sdkToolToKerberoSecSayTool — emitted as say="use_mcp_server")
 */
function sdkToolToKerberoSecSayTool(toolName: string, input?: unknown): KerberoSecSayTool {
	...
	switch (toolName) {
		...
		case "replace_in_file": {
			...
			// this the card mislabels a prepend/insert as "KerberoSec wants to create a new file".
		...
		default: {
			...
			const filePath =
				...
				""
			return {
				tool: toolName as KerberoSecSayTool["tool"],
				path: filePath,
			}
		}
	}
...
/**
 * Map a read request's line range onto KerberoSecSayTool fields. An omitted start_line with an
 ...
 */
function readLineRangeFields(read: FileReadRequest | undefined): Pick<KerberoSecSayTool, "readLineStart" | "readLineEnd"> {
...
/**
 * Split a multi-file apply_patch string into one KerberoSecSayTool per file so each
 * "KerberoSec wants to edit this file" row renders only that file's diff (kerberosec#9904).
 *
 ...
 */
function splitApplyPatchByFile(patch: string): KerberoSecSayTool[] {
	const lines = patch.split("\n")
	const blocks: { tool: KerberoSecSayTool["tool"]; path: string; lines: string[] }[] = []
	let current: { tool: KerberoSecSayTool["tool"]; path: string; lines: string[] } | undefined
	...
	for (const line of lines) {
		...
		if (marker) {
			...
			const tool: KerberoSecSayTool["tool"] =
	...
	// pathless row can't route to the per-file diff view (kerberosec#9904).
...
/**
 * Build a KerberoSecAskUseMcpServer JSON payload for MCP tool calls.
...
function buildMcpToolPayload(mcpInfo: { serverName: string; toolName: string }, input?: unknown): string {
	const parsedInput = parseToolInput(input)
	// Format arguments as a JSON string (matching classic KerberoSecAskUseMcpServer.arguments)
	...
	} satisfies KerberoSecAskUseMcpServer)
}
...
/**
 * Build the KerberoSec approval ask message for an SDK tool approval request.
 ...
 */
export function buildToolApprovalAskMessage(toolName: string, input: unknown, ts: number, cwd?: string): KerberoSecMessage {
	...
	if (toolName === "spawn_agent") {
		...
		return {
			...
			text: JSON.stringify({
				prompts: [taskPrompt],
			} satisfies KerberoSecAskUseSubagents),
			partial: false,
		}
	}
	...
	return {
		...
		type: "ask",
		ask: "tool",
		text: JSON.stringify(toDisplaySayTool(sdkToolToKerberoSecSayTool(toolName, input), cwd)),
		partial: false,
	}
}
...
/**
 * Translate an SDK AgentEvent into KerberoSecMessage(s).
 */
/**
 ...
 */
export function parseCompactionNoticeMetadata(metadata: Record<string, unknown> | undefined): KerberoSecCompactionInfo | undefined {
...
export function buildCompactionMessage(info: KerberoSecCompactionInfo, ts: number): KerberoSecMessage {
	return {
		ts,
		type: "say",
		say: "compaction",
...
function translateAgentEvent(event: AgentEvent, state: MessageTranslatorState): KerberoSecMessage[] {
	const messages: KerberoSecMessage[] = []
	...
	switch (event.type) {
			...
			switch (event.contentType) {
				...
				case "tool": {
					...
					if (toolName === "spawn_agent") {
						...
						const approvalPayload: KerberoSecAskUseSubagents = {
							prompts: allPrompts,
						}
						messages.push({
							ts: state.getSpawnAgentPromptsTs(),
							type: "say",
							say: "use_subagents" as KerberoSecSay,
					...
					// with KerberoSecAskUseMcpServer JSON, not generic say="tool".
					const mcpInfo = parseMcpToolName(toolName)
					if (mcpInfo) {
						...
						messages.push({
							ts: state.getStreamingToolTs(),
							type: "say",
							say: "use_mcp_server" as KerberoSecSay,
							text: mcpPayload,
							partial: true,
						})
						break
					...
					// All other tools → say="tool" with KerberoSecSayTool JSON
					...
					// reproduce for files ≥2, orphaning those partial rows (kerberosec#9904).
					const sayTool = toDisplaySayTool(sdkToolToKerberoSecSayTool(toolName, input), state.currentCwd())
					messages.push({
						ts: state.getStreamingToolTs(),
						type: "say",
						say: "tool",
		...
		case "content_update": {
			...
			// these into the KerberoSecSaySubagentStatus format for the rich UI.
			...
			switch (event.contentType) {
				...
				case "tool": {
					...
					if (toolName === "spawn_agent") {
						...
						const overallStatus: KerberoSecSaySubagentStatus["status"] = allDone
							? hasFailed
								? "failed"
								: "completed"
							: "running"
						...
						if (allDone) {
							const usagePayload: KerberoSecSubagentUsageInfo = {
								source: "subagents",
							...
							messages.push({
								ts: state.nextTs(),
								type: "say",
								say: "subagent_usage" as KerberoSecSay,
					...
					if (mcpInfoEnd) {
						...
						if (mcpOutputStr) {
							messages.push({
								ts: state.nextTs(),
								type: "say",
								say: "mcp_server_response" as KerberoSecSay,
								text: mcpOutputStr,
								partial: false,
							})
						}
					...
					if (toolName === "read_files" || toolName === "read_file") {
						...
						if (fileReads.length > 1) {
							...
							fileReads.forEach((fileRead, index) => {
								const sayTool: KerberoSecSayTool = {
									tool: "readFile",
									path: fileRead.path,
									...readLineRangeFields(fileRead),
								}
					...
					// instead of the whole multi-file patch (kerberosec#9904). Single-file
					...
					const sayTool = toDisplaySayTool(sdkToolToKerberoSecSayTool(toolName, storedInput), state.currentCwd())
		...
		case "iteration_start": {
			...
			messages.push({
				...
				} satisfies KerberoSecApiReqInfo),
				partial: false,
			})
			break
		}
		...
		case "error": {
			...
			// The webview uses KerberoSecError.parse() on the `api_req_failed` text to
			...
			// may contain JSON from the API (e.g. KerberoSec provider's 402 response with
			// `code: "insufficient_credits"`). We try to reshape it into the
			// KerberoSecError-serialized format the webview expects so that ErrorRow
...
export function translateSessionEvent(event: CoreSessionEvent, state: MessageTranslatorState): TranslationResult {
	...
	switch (event.type) {
		...
		case "agent_event": {
			// Sub-agent events should NOT produce KerberoSecMessages in the main chat.
		...
		case "hook": {
			...
			if (hookName === "tool_call") {
				result.messages.push({
					ts: state.nextTs(),
					type: "say",
					say: "hook_status" as KerberoSecSay,
		...
		case "pending_prompts": {
			...
			// to KerberoSecMessages at this layer
			break
		}
...
function agentEventToMessages(event: AgentEvent, state: MessageTranslatorState): KerberoSecMessage[] {
...
function appendPersistedMetricsMessage(
	kerberosecMessages: KerberoSecMessage[],
...
function finalizePersistedToolUse(
	...
	// produces the final non-partial KerberoSecMessage shape the webview expects.
...
export interface SdkMessagesToKerberoSecMessagesOptions {
	/**
...
/**
 * Convert SDK-persisted LLM messages back into the KerberoSecMessage format used by
 * the webview. Keep this in the live message translator so history rendering
 * and streaming rendering share the same SDK tool → KerberoSec UI mapping.
 */
export function sdkMessagesToKerberoSecMessages(
	messages: SdkMessageWithMetrics[],
	minter?: MessageIdMinter,
	options?: SdkMessagesToKerberoSecMessagesOptions,
): KerberoSecMessage[] {
	const kerberosecMessages: KerberoSecMessage[] = []
	...
	const flushUnmatchedToolUses = () => {
		for (const toolUse of pendingToolUses.values()) {
			kerberosecMessages.push(...finalizePersistedToolUse(toolUse, state))
		}
	...
	const upsertKerberoSecMessages = (updates: KerberoSecMessage[]) => {
		for (const update of updates) {
			const existingIndex = kerberosecMessages.findIndex((m) => m.ts === update.ts)
			if (existingIndex !== -1) {
				kerberosecMessages[existingIndex] = update
			} else {
				kerberosecMessages.push(update)
			}
		}
	}
	...
	for (const { message, sourceIndex } of projectSessionMessagesForDisplay(messages)) {
		...
		if (message.role === "assistant") {
			...
			if (typeof message.content === "string") {
				...
				appendPersistedMetricsMessage(kerberosecMessages, message, state)
				continue
			}
			...
			if (text) {
				...
				if (!isSyntheticSdkUserMessage(message)) {
					kerberosecMessages.push({
						ts: state.nextTs(),
						type: "say",
						say: kerberosecMessages.length === 0 ? "task" : "user_feedback",
						text,
						partial: false,
					})
				}
		...
		for (const block of message.content) {
			...
			kerberosecMessages.push(...finalizePersistedToolUse(toolUse, state, block.content, block.is_error))
		}
	}
...
/**
 * Reshape an SDK error into the serialized KerberoSecError JSON the webview's
...
export function reshapeErrorForWebview(
	...
	// The KerberoSecError-JSON branches below are kerberosec-provider flows (balance,
	// spend limit), so "kerberosec" stays their fallback id. The missing-credential
	// message instead gets the raw value: defaulting there would name the wrong
	// provider when the active provider id is unknown.
	const kerberosecErrorProviderId = providerId ?? "kerberosec"
	const rawMessage = error.message ?? "Unknown error"

	// A retired kerberosec-free/ model answers "model not found" once its free
	...
	if (isKerberoSecFreePromotionEndedMessage(rawMessage, modelId)) {
		return JSON.stringify({
			message: rawMessage,
			code: KERBEROSEC_FREE_PROMOTION_ENDED_ERROR_CODE,
			providerId: kerberosecErrorProviderId,
			modelId,
			details: {
				code: KERBEROSEC_FREE_PROMOTION_ENDED_ERROR_CODE,
				message: rawMessage,
	...
	if (!parsed) {
		...
		// KerberoSecError-compatible JSON the webview expects.
	...
	// KerberoSecError.getErrorType() to return KerberoSecErrorType.Balance
	...
	// KerberoSecError.parse() can still extract what it can.
