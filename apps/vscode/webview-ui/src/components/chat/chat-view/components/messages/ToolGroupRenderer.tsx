import { KerberoSecMessage, KerberoSecSayTool } from "@shared/ExtensionMessage"
...
interface ToolWithReasoning {
	tool: KerberoSecMessage
	parsedTool: KerberoSecSayTool
	reasoning?: string
	isActive?: boolean
	activityText?: string
}
...
const getCurrentActivities = (allMessages: KerberoSecMessage[]): KerberoSecMessage[] => {
	...
	const activities: KerberoSecMessage[] = []
 ...
 */
export function buildToolsWithReasoning(messages: KerberoSecMessage[]): ToolWithReasoning[] {
 ...
 */
function parseToolSafe(text: string | undefined): KerberoSecSayTool {
	try {
		return JSON.parse(text || "{}") as KerberoSecSayTool
	} catch {
		return {} as KerberoSecSayTool
	}
}
 ...
 */
function getToolDisplayInfo(tool: KerberoSecSayTool) {
 ...
 */
export function getToolGroupSummaryFromParsedTools(tools: KerberoSecSayTool[]): string {
	...
	return parts.length === 0 ? "Context" : "KerberoSec" + action + parts.join(", ")
}
