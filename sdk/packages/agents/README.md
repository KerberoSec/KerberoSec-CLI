# [experimental] @kerberosec/agents

`@kerberosec/agents` is the runtime-agnostic agent loop package in the KerberoSec SDK.
...
`@kerberosec/agents` does not ship a full application runtime by itself.

- Default host tools like filesystem access, shell execution, or web fetching live in `@kerberosec/core`
- Session persistence and stateful orchestration live in `@kerberosec/core`
- Shared hub runtime/session transport lives in `@kerberosec/core` (see `@kerberosec/core/hub`)
- Sub-agent and team coordination primitives live in `@kerberosec/core`
...
```bash
npm install @kerberosec/agents @kerberosec/shared @kerberosec/llms
```

## Quick Start

```ts
import { Agent } from "@kerberosec/agents";
import type { AgentTool } from "@kerberosec/shared";
...
when the host already owns gateway construction (this is what `@kerberosec/core`
uses internally):

```ts
import { createGateway } from "@kerberosec/llms";
...
`@kerberosec/shared`. Each tool has a JSON Schema `inputSchema` and an
...
`AgentRuntimeEvent` in `@kerberosec/shared` for the full union.
...
subprocess-backed hooks, MCP extensions), use `@kerberosec/core`.
...
```ts
import type { AgentRuntimePlugin } from "@kerberosec/shared";
...
For multi-agent workflows, use `@kerberosec/core`:
...
- `@kerberosec/agents` — the single package entrypoint. The `package.json`
  `exports` map automatically serves a browser-safe bundle when bundlers
  resolve the `browser` condition.

## Related Packages

- `@kerberosec/shared`: shared types (`AgentTool`, `AgentMessage`,
  `AgentRuntimeEvent`, `AgentRuntimeHooks`, etc.)
- `@kerberosec/llms`: provider settings, model catalogs, and gateway/handler
  creation
- `@kerberosec/core`: stateful runtime assembly, storage, default tools,
  subprocess hooks, hub transport, and MCP integration

## More Examples

- Repo examples:
  [examples/plugins](https://github.com/kerberosec/sdk/tree/main/examples/plugins),
  [examples/hooks](https://github.com/kerberosec/sdk/tree/main/examples/hooks),
  [examples/cron](https://github.com/kerberosec/sdk/tree/main/examples/cron)
- Workspace overview: [README.md](https://github.com/kerberosec/sdk/blob/main/README.md)
- API and architecture references:
  [DOC.md](https://github.com/kerberosec/sdk/blob/main/DOC.md),
  [ARCHITECTURE.md](https://github.com/kerberosec/sdk/blob/main/ARCHITECTURE.md)
