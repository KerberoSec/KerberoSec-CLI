...
import {
	...
	useKerberoSecRecommendedModels,
...
export function useOnboardingController(props: OnboardingControllerProps) {
	...
	const [
		kerberosecPassSubscriptionStatus,
		setKerberoSecPassSubscriptionStatus,
	] = useState<KerberoSecPassSubscriptionStatus>("loading");
	const [kerberosecPassSubscriptionError, setKerberoSecPassSubscriptionError] =
		useState("");
	const [kerberosecPassCurrentPlanName, setKerberoSecPassCurrentPlanName] =
		useState("");
	const [kerberosecPassPlanFeatures, setKerberoSecPassPlanFeatures] = useState<
		string[]
	>([]);
	const [
		kerberosecPassSubscriptionSelected,
		setKerberoSecPassSubscriptionSelected,
	] = useState(0);
	const [
		kerberosecPassSubscriptionOpenStatus,
		setKerberoSecPassSubscriptionOpenStatus,
	] = useState("");
	const kerberosecPassSubscriptionUrl = useMemo(
		() => getCliSubscriptionUrl(),
		[],
	);
	...
	const createCustomModelItem = useCallback(
		(_search: string, filteredItems: SearchableItem[]) => {
			if (activeProviderId === "kerberosec-pass") {
				return undefined;
			}
	...
	// KerberoSec featured model picker (KerberoSecPass gets Subscribed/Free sections)
	const recommended = useKerberoSecRecommendedModels();
	const kerberosecEntries: KerberoSecModelPickerEntry[] = useMemo(
		() =>
			recommended.data
	...
	const [kerberosecModelSelected, setKerberoSecModelSelected] = useState(0);
	const [kerberosecModelReasoningIds, setKerberoSecModelReasoningIds] =
		useState<Set<string>>(new Set());

	useEffect(() => {
		// The featured picker serves both kerberosec and kerberosec-pass, so pool
		// reasoning support from both catalogs. Display names need no catalog
		// here: fetchKerberoSecRecommendedModels resolves them.
		void Promise.allSettled(
			["kerberosec", "kerberosec-pass"].map((providerId) =>
			...
			setKerberoSecModelReasoningIds(ids);
		});
	}, []);
	...
	const loadModelsForProvider = useCallback(
		(providerId: string) => {
			...
			setModelEntries([]);
			(providerId === "ollama"
				? import("../../../utils/ollama-manager")
						.then(({ ensureOllamaServerRunning }) =>
	...
	const refreshKerberoSecPassSubscriptionStatus = useCallback(() => {
		setKerberoSecPassSubscriptionStatus("loading");
		setKerberoSecPassSubscriptionError("");
		setKerberoSecPassCurrentPlanName("");
		setKerberoSecPassSubscriptionOpenStatus("");
			...
			.then(({ currentPlanResult, availablePlansResult }) => {
				...
				if (currentPlanResult.status === "rejected") {
					...
					if (message.trim().toLowerCase() === "no plan found for user") {
						setKerberoSecPassSubscriptionStatus("unsubscribed");
						return;
					}
					setKerberoSecPassSubscriptionError(message);
					setKerberoSecPassSubscriptionStatus("error");
					return;
				}
				...
				if (plan) {
					setKerberoSecPassCurrentPlanName(
						plan.displayName || plan.name || plan.id || "KerberoSecPass",
					);
					setKerberoSecPassSubscriptionStatus("subscribed");
				} else {
					setKerberoSecPassSubscriptionStatus("unsubscribed");
				}
			});
	...
	const transitionToModelPicker = useCallback(
		(providerId: string) => {
			...
			if (shouldUseFeaturedKerberoSecModelPicker(providerId)) {
				setKerberoSecModelSelected(0);
				setStep("kerberosec_model");
	...
	const transitionToKerberoSecPassSubscription = useCallback(() => {
		...
		setActiveProviderName(provider?.name ?? "KerberoSecPass");
		setModelsDefaultId(provider?.defaultModelId ?? "");
	...
	const handleAuthComplete = useCallback(
		(providerId: OnboardingOAuthProviderId) => {
			if (providerId === "kerberosec-pass") {
				transitionToKerberoSecPassSubscription();
				return;
			}
			transitionToModelPicker(providerId);
		},
		[transitionToKerberoSecPassSubscription, transitionToModelPicker],
	);
	...
	const startOAuthFlow = useCallback(
		(providerId: OnboardingOAuthProviderId) => {
			if (isKerberoSecProvider(providerId)) {
	...
	const continueFromKerberoSecPassSubscription = useCallback(() => {
		transitionToModelPicker("kerberosec-pass");
	}, [transitionToModelPicker]);

	const openKerberoSecPassSubscriptionPage = useCallback(() => {
		setKerberoSecPassSubscriptionOpenStatus("Opening subscription page...");
		void open(kerberosecPassSubscriptionUrl, { wait: false })
			.then(() => {
				setKerberoSecPassSubscriptionOpenStatus(
			...
			.catch(() => {
				setKerberoSecPassSubscriptionOpenStatus(
					`Could not open browser automatically. Open ${kerberosecPassSubscriptionUrl}`,
				);
			});
	}, [kerberosecPassSubscriptionUrl]);

	useEffect(() => {
		if (
			step === "kerberosec_pass_subscription" &&
			kerberosecPassSubscriptionStatus === "subscribed"
		) {
			transitionToModelPicker("kerberosec-pass");
		}
	}, [step, kerberosecPassSubscriptionStatus, transitionToModelPicker]);
	...
	const saveKerberoSecModelSelection = useCallback(
		(modelId: string, modelName: string) => {
			...
			setSelectedModelId(modelId);
			if (kerberosecModelReasoningIds.has(modelId)) {
		...
		[activeProviderId, kerberosecModelReasoningIds, providerSettingsManager],
	);
	...
	useOnboardingKeyboard({
		...
		modelList,
		kerberosecEntries,
		kerberosecModelSelected,
		kerberosecPassSubscriptionStatus,
		kerberosecPassSubscriptionOptions: KERBEROSEC_PASS_SUBSCRIPTION_OPTIONS,
		kerberosecPassSubscriptionSelected,
		thinkingSelected,
		setStep,
		setMenuSelected,
		...
		setDeviceError,
		setDeviceStatus,
		setKerberoSecModelSelected,
		setKerberoSecPassSubscriptionSelected,
		setThinkingSelected,
		continueFromKerberoSecPassSubscription,
		refreshKerberoSecPassSubscriptionStatus,
		openKerberoSecPassSubscriptionPage,
		abortOAuth: () => {
	...
	return {
		...
		kerberosecEntries,
		kerberosecModelSelected,
		kerberosecPassCurrentPlanName,
		kerberosecPassPlanFeatures,
		kerberosecPassSubscriptionError,
		kerberosecPassSubscriptionOpenStatus,
		kerberosecPassSubscriptionOptions: KERBEROSEC_PASS_SUBSCRIPTION_OPTIONS,
		kerberosecPassSubscriptionSelected,
		kerberosecPassSubscriptionStatus,
