...
function buildAiSdkRuntimeContext(
	...
	return {
		// `distinctId` is KerberoSec's canonical identity field. Langfuse's data
		...
		...(typeof metadata.clientVersion === "string"
			...
			: {}),
		...(typeof metadata.kerberosecCoreVersion === "string"
			? { kerberosecCoreVersion: metadata.kerberosecCoreVersion }
			: {}),
		...(tags && tags.length > 0 ? { tags } : {}),
		// Keep KerberoSec correlation fields available even when the integration
...
async function createProviderModule(
	...
	switch (kind) {
		case "kerberosec": {
			const { createKerberoSecProviderModule } = await import(
				"./vendors/kerberosec"
			);
			return createKerberoSecProviderModule(config, context);
		}
		case "openai": {
 ...
 * kerberosec, generic OpenAI-compatible endpoints), not just local Ollama. An
...
export const createKerberoSecProvider = createAiSdkProvider("kerberosec");
