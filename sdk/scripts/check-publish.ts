...
async function main(): Promise<number> {
	...
	const testDir = await mkdtemp(join(tmpdir(), "kerberosec-pkg-verify-"));
	const packDir = await mkdtemp(join(tmpdir(), "kerberosec-pkg-packs-"));
	const bunCacheDir = await mkdtemp(
		join(tmpdir(), "kerberosec-pkg-bun-cache-"),
	);
		...
		for (const pkg of published) {
			...
			await writeFile(
				...
				[
					...
					`import { join } from "node:path";`,
					`try {`,
					`  const root = await import("@kerberosec/core");`,
					`  if (typeof root.KerberoSecCore?.create !== "function") {`,
					`    console.error("  FAIL @kerberosec/core: root export is missing KerberoSecCore.create");`,
					`    process.exit(1);`,
					`  }`,
					`} catch (error) {`,
					`  const message = error instanceof Error ? error.message : String(error);`,
					`  console.error("  FAIL @kerberosec/core: published runtime shape is invalid:", message);`,
					`  process.exit(1);`,
					`}`,
					`console.log("  OK @kerberosec/core publish shape");`,
				].join("\n"),
			);
			try {
				const proc = Bun.spawn(["bun", testFile], {
