...
describe("ChatMessages tool disclosures", () => {
	it.each([
		...
		const icon = container.querySelector(".kerberosec-chat-tool-icon svg");
	...
	it("renders a detail-less tool summary as static text", async () => {
		...
		expect(
			container
				.querySelector(".kerberosec-chat-tool")
				?.classList.contains("my-0"),
		).toBe(true);
	});
	...
	it("shimmers a tool title only while its result is pending", async () => {
		...
		const pendingTitle = container.querySelector(
			".kerberosec-chat-tool-label > span",
		);
		expect(
			pendingTitle?.classList.contains("kerberosec-chat-streaming-title"),
		).toBe(true);
		...
		expect(
			completedTitle?.classList.contains("kerberosec-chat-streaming-title"),
		).toBe(false);
	});
	...
	it("auto-expands submit_and_exit and renders its summary as markdown", async () => {
		...
		const markdown = panel?.querySelector(".kerberosec-markdown");
	...
	it("renders consecutive tool calls as individual rows", async () => {
		...
		expect(container.querySelectorAll(".kerberosec-chat-tool")).toHaveLength(5);
	...
	it("pre-expands tool groups that contain edit diffs", async () => {
		...
		const [editBlock, readBlock] = [
			...container.querySelectorAll(".kerberosec-chat-tool"),
		];
		// The edit group's diff panel is visible without a click…
		expect(
			editBlock
				?.querySelector(".kerberosec-chat-disclosure-content-motion")
		...
		expect(
			[
				...container.querySelectorAll(
					".kerberosec-chat-disclosure-content-motion",
				),
	...
	it("preserves interleaved tool activity order", async () => {
		...
		const labels = [...container.querySelectorAll(".kerberosec-chat-tool")].map(
			(row) => row.textContent ?? "",
		);
	...
	it("spaces reasoning, content, and tool blocks with a single gap-2", async () => {
		...
		const message = container.querySelector(
			'.kerberosec-chat-message[data-role="assistant"]',
		);
		...
		const content = message?.querySelector(".kerberosec-chat-message-content");
		...
		for (const element of [
			message,
			content?.querySelector(".kerberosec-chat-reasoning"),
			content?.lastElementChild,
			container.querySelector(".kerberosec-chat-tool"),
		]) {
	...
	it("positions hidden message actions outside the message layout", async () => {
		...
		const userMessage = container.querySelector(
			'.kerberosec-chat-message[data-role="user"]',
		);
		const userActions = userMessage?.querySelector(
			":scope > .kerberosec-chat-message-actions",
		);
		...
		const userAction = userActions?.querySelector(
			".kerberosec-chat-message-action",
		);
	...
	it("leaves vertical scrolling to the conversation viewport", async () => {
		...
		const content = container.querySelector(
			".kerberosec-chat-conversation-content",
		);
...
describe("ChatMessages reasoning disclosure", () => {
	it("shimmers the thinking title only while reasoning is streaming", async () => {
		...
		const streamingTitle = container.querySelector(
			".kerberosec-chat-reasoning-trigger > span",
		);
		expect(
			streamingTitle?.classList.contains("kerberosec-chat-streaming-title"),
		).toBe(true);
	...
	it("shows elapsed thinking time with the border-left disclosure style", async () => {
		...
		expect(trigger?.getAttribute("aria-expanded")).toBe("false");
		expect(
			trigger?.querySelector(".kerberosec-chat-thinking-icon"),
		).not.toBeNull();
		expect(
			trigger?.querySelector(".kerberosec-chat-disclosure-icon"),
		).toBeNull();

		await act(async () => trigger?.click());

		const content = container.querySelector(
			".kerberosec-chat-reasoning-content",
		);
		expect(trigger?.getAttribute("aria-expanded")).toBe("true");
		expect(content?.textContent).toContain("Carefully considered the request.");
		expect(content?.classList.contains("kerberosec-chat-panel-rail")).toBe(
			true,
		);
	});
	...
	it("hangs expanded reasoning and tool panels off the same left rail", async () => {
		...
		const reasoningTrigger = container.querySelector(
			".kerberosec-chat-reasoning-trigger",
		...
		const toolContent = container.querySelector(
			".kerberosec-chat-tool-content",
		);
		expect(reasoningContent).not.toBeNull();
		expect(toolContent).not.toBeNull();

		expect(
			reasoningContent?.classList.contains("kerberosec-chat-panel-rail"),
		).toBe(true);
		expect(toolContent?.classList.contains("kerberosec-chat-panel-rail")).toBe(
			true,
		);

		// Reasoning remains capped and scrolls internally (the shared
		// kerberosec-chat-thinking-content styling); tool output grows into the
		// conversation scroller.
		expect(
			reasoningContent?.classList.contains("kerberosec-chat-thinking-content"),
		).toBe(true);
		expect(
			toolContent?.classList.contains("kerberosec-chat-thinking-content"),
		).toBe(false);
		...
		const details = toolContent?.querySelector(".kerberosec-chat-tool-details");
	...
	it("keeps the reasoning panel inside the shape the hover-suppress rule targets", async () => {
		...
		expect(
			actions?.matches(
				".kerberosec-chat-message:has(> .kerberosec-chat-message-content .kerberosec-chat-reasoning) > .kerberosec-chat-message-actions:not([data-visible='true'])",
			),
		).toBe(true);
	...
	it("keeps reasoning disclosures separate across tool activity", async () => {
		...
		const workTrigger = container.querySelector(
			"button.kerberosec-chat-work-trigger",
		) as HTMLButtonElement | null;
		expect(workTrigger).not.toBeNull();
		await act(async () => workTrigger?.click());

		expect(
			container.querySelectorAll(".kerberosec-chat-reasoning"),
		).toHaveLength(2);
	});
...
describe("ChatMessages work collapse", () => {
	...
	it("folds a finished run into a work summary that expands back into rows", async () => {
		...
		expect(container.querySelector(".kerberosec-chat-tool")).toBeNull();
		expect(container.textContent).toContain("Fixed it.");

		await act(async () => trigger?.click());
		expect(trigger?.getAttribute("aria-expanded")).toBe("true");
		expect(container.querySelectorAll(".kerberosec-chat-tool")).toHaveLength(2);
	});

	it("keeps the live run's rows visible while the session is active", async () => {
		await renderMessages(completedRun, { status: "running" });

		expect(container.querySelector(".kerberosec-chat-work")).toBeNull();
		expect(container.querySelectorAll(".kerberosec-chat-tool")).toHaveLength(2);
	});
	...
	it("shows while starting", async () => {
		...
		expect(
			[...container.querySelectorAll("span")]
				...
				?.classList.contains("kerberosec-chat-streaming-title"),
		).toBe(true);
	});
...
describe("ChatMessages tool approvals", () => {
	it("renders the shared card and forwards its decisions", async () => {
		...
		const card = container.querySelector(".kerberosec-ui-agent-approval-card");
