...
const WATCH_INTERVAL_ENV = "KERBEROSEC_HUB_BUILD_WATCH_INTERVAL_MS";
 ...
 *
 * A mismatch means another KerberoSec installation replaced the shared Hub with a
 ...
 * invoke `onMismatch` when another KerberoSec installation has replaced the Hub
 ...
 * prompts, so the watcher refuses to start when `KERBEROSEC_HUB_PORT` is set.
...
export function watchManagedHubBuildMismatch(
	...
	if (isHubDaemonProcess() || process.env.KERBEROSEC_HUB_PORT?.trim()) {
		return () => {};
	}
