	...
	describe("StdioHookRunner", () => {
		it(
			...
			async () => {
				...
				const hookPath = path.join(tempDir, ".kerberosecrules", "hooks", "PreToolUse")
  ...
  contextModification: "Received tool: " + input.preToolUse.toolName
	...
	describe("PostToolUse Hook", () => {
		it("should receive execution results", async () => {
			const hookPath = path.join(tempDir, ".kerberosecrules", "hooks", "PostToolUse")
			const hookScript = `#!/usr/bin/env node
const input = JSON.parse(require('fs').readFileSync(0, 'utf-8'));
console.log(JSON.stringify({
  cancel: false,
  contextModification: "Tool succeeded: " + input.postToolUse.success
	...
	describe("Hook Discovery", () => {
		it("should generate Windows PowerShell bridge files with real newlines", async () => {
			const hooksDir = path.join(tempDir, ".kerberosecrules", "hooks")
...
const hasAllFields = input.kerberosecVersion && input.hookName && input.timestamp && 
  ...
  contextModification: hasAllFields ? "All fields present" : "Missing fields"
	...
	describe("Global Hooks", () => {
		...
		beforeEach(async () => {
			...
			workspaceHooksDir = path.join(tempDir, ".kerberosecrules", "hooks")
...
console.log(JSON.stringify({
  ...
  contextModification: "GLOBAL: Context added"
			...
			const workspaceHookPath = path.join(tempDir, ".kerberosecrules", "hooks", "PreToolUse")
  ...
  contextModification: "Global observed: " + input.postToolUse.success
	...
	describe("workspace root matching", () => {
		...
		it("determineHookCwd picks the owning root for prefix-sharing and nested roots", () => {
			...
			const hooksDir = (root: string) => path.join(root, ".kerberosecrules", "hooks")
			...
			const globalScript = path.join(path.sep, "home", "Documents", "KerberoSec", "Hooks", "PreToolUse")
			factory
				.determineHookCwd(globalScript, [path.join(path.sep, "home", "Documents", "KerberoSec", "Hooks")], [app, appWeb])
				.should.equal(app)
		})
	})
})
