import { KerberoSecMessage } from "@shared/ExtensionMessage"
...
// Mock data factories
const createMockMessage = (overrides: Partial<KerberoSecMessage> = {}): KerberoSecMessage => ({
	ts: Date.now(),
	type: "say",
	say: "error",
...
const meta: Meta<typeof ErrorRow> = {
	...
	parameters: {
		docs: {
			description: {
				component:
					"Displays different types of error messages in the chat interface, including API errors, credit limit errors, diff errors, and kerberosecignore errors. Handles special error parsing for KerberoSec provider errors and provides appropriate user actions.",
			},
		},
	},
...
export const Default: Story = {
	...
	argTypes: {
		errorType: {
			control: { type: "select" },
			options: ["error", "mistake_limit_reached", "diff_error", "kerberosecignore_error"],
...
// KerberoSec-specific errors
export const KerberoSecBalanceError: Story = {
	args: {
		message: createMockMessage(),
		errorType: "error",
...
export const KerberoSecRateLimitError: Story = {
	args: {
		message: createMockMessage(),
		errorType: "error",
		apiRequestFailedMessage: JSON.stringify({
...
export const KerberoSecSpendLimitDaily: Story = {
	args: {
		message: createMockMessage(),
		errorType: "error",
		apiRequestFailedMessage: JSON.stringify({
...
export const KerberoSecSpendLimitMonthly: Story = {
	args: {
		message: createMockMessage(),
		errorType: "error",
		apiRequestFailedMessage: JSON.stringify({
...
export const KerberoSecSpendLimitMinimal: Story = {
	args: {
		message: createMockMessage(),
		errorType: "error",
		apiRequestFailedMessage: JSON.stringify({
...
// KerberoSecPass entitlement error (user not subscribed to a required model plan)
export const KerberoSecPassEntitlementError: Story = {
	args: {
		message: createMockMessage(),
		errorType: "error",
		apiRequestFailedMessage:
	...
	parameters: {
		docs: {
			description: {
				story: "KerberoSecPass model returns the SDK KerberoSecNotSubscribedError message when the user is not subscribed. A human-readable message with a 'Get KerberoSecPass' subscribe link and a retry button is shown.",
			},
		},
	},
}
...
export const AuthErrorSignedIn: Story = {
	...
	decorators: [
		createStoryDecorator({
			kerberosecUser: { id: "user123", email: "user@example.com" },
...
export const InteractiveSignIn: Story = {
	args: {
		...
		errorType: "error",
		apiRequestFailedMessage: JSON.stringify({
			message: "Please sign in to access KerberoSec services.",
			code: "ERR_BAD_REQUEST",
	...
	play: async ({ canvasElement }) => {
		...
		const signInButton = canvas.getByRole("button", { name: /sign in to kerberosec/i })
