...
// stub API keeps working. `AuthService`/`KerberoSecAccountService` statics and the
...
describe("fetchRemoteConfig", () => {
	...
	let accountService: KerberoSecAccountService
	...
	beforeEach(() => {
		...
		accountService = new KerberoSecAccountService()
		sandbox.stub(KerberoSecAccountService, "getInstance").returns(accountService)
	...
	it("switches org when not in the chosen org", async () => {
		...
		const controller = {
			...
			stateManager: {
				getApiConfiguration: sandbox.stub().returns({ planModeApiProvider: "kerberosec", actModeApiProvider: "kerberosec" }),
