...
// Local `createUID` helper. The kerberosece source imports this from
// `@kerberosec/shared` (see `packages/shared/dist/identifier.ts`), but
...
// exact kerberosece implementation (`${prefix}_${nanoid(length)}`).
 ...
 * `@kerberosec/core`, which constructs models itself to share gateway/telemetry
 ...
 * runtime builds an `AgentModel` internally via `@kerberosec/llms`. This is the
 ...
 * construct the model itself via `@kerberosec/llms`.
 */
...
export class AgentRuntime {
	...
	private async generateAssistantMessage(options?: {
		...
		const modelRequestMetadata = omitUndefinedValues({
			...
			kerberosecCoreVersion: trimNonEmpty(this.config.kerberosecCoreVersion),
	...
	private async emit(event: AgentRuntimeEvent): Promise<void> {
		...
		switch (event.type) {
			case "run-started":
				// Verbatim kerberosece calls `logger?.info?.(...)`. sdk-re's
				// `BasicLogger` does not declare `info` (it uses `log`), so
				// we narrow to an optional-info shape at the call site to
				// preserve the kerberosece runtime contract without mutating
 ...
 *
 * while `@kerberosec/core` (which owns model construction) continues to use
