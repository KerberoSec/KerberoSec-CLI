...
vi.mock("@kerberosec/core", async (importOriginal: any) => {
...
// pulls in @kerberosec/core, @kerberosec/llms and @kerberosec/shared, so the first test to call
...
describe("ProviderCatalog Phase 3.5 listProviders", () => {
	it("returns SDK provider listings with top-level picker metadata", async () => {
		...
		expect(mocks.listLocalProviders).toHaveBeenCalledWith(expect.anything(), { isKerberoSecPassEnabled: true })
	})
	...
	it("refetches provider listings after explicit invalidation", async () => {
		...
		mocks.listLocalProviders
			.mockResolvedValueOnce({
				providers: [{ id: "kerberosec", name: "KerberoSec", protocol: "anthropic", client: "anthropic", source: "system" }],
			})
			.mockResolvedValueOnce({
				providers: [
					{ id: "kerberosec-pass", name: "KerberoSecPass", protocol: "anthropic", client: "anthropic", source: "system" },
				],
			})
		const catalog = createProviderCatalog(makeReader({ providerId: parseProviderId("kerberosec") }))

		const first = await catalog.listProviders()
		catalog.invalidateProviderListings()
		const second = await catalog.listProviders()

		expect(first[0]?.id).toBe("kerberosec")
		expect(second[0]?.id).toBe("kerberosec-pass")
