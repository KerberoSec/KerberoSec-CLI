...
const KERBEROSEC_PASS_FREE_SECTION_DESCRIPTION =
 ...
 * stamps onto `ProviderModel.featured` (see @kerberosec/core's
 * applyKerberoSecFeaturedModels). The `kerberosec` provider gets Recommended / Free /
 * All models; `kerberosec-pass` gets Subscribed / Free only — its offer is exactly
	...
	if (providerId === "kerberosec-pass") {
		...
		return {
			...
			sections: [
				...
				{
					description: KERBEROSEC_PASS_FREE_SECTION_DESCRIPTION,
					id: "free",
					label: "Free",
				},
				...(rest.length > 0
