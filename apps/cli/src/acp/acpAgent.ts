...
import { isKerberoSecOrgIndividualInferenceSubscriptionErrorMessage } from "../utils/kerberosec-pass-errors";
...
import {
	...
	fetchKerberoSecOrganizations,
	getAcpOrgSubscriptionMessage,
	ORGANIZATION_CONFIG_ID,
	PERSONAL_ACCOUNT_VALUE,
	switchKerberoSecOrganization,
	usesKerberoSecAccount,
} from "./organizations";
...
export class AcpAgent implements Agent {
	...
	isSessionReady() {
		...
		if (!this.authResult && !process.env.KERBEROSEC_API_KEY) {
	...
	async newSession(params: NewSessionRequest): Promise<NewSessionResponse> {
		...
		const providerId =
			process.env.KERBEROSEC_PROVIDER ??
		...
		// provider's own catalog: `kerberosec-pass` uses `kerberosec-pass/…` ids that mean
		// nothing to `kerberosec`, and vice versa.
		const defaultModelId = await resolveDefaultModelId(
			providerId,
			process.env.KERBEROSEC_MODEL,
			providerModels,
		);
	...
	async setSessionConfigOption(
		...
		switch (params.configId) {
			case "provider": {
				if (process.env.KERBEROSEC_PROVIDER) {
				...
				// be listed first (for kerberosec-pass that is an unrelated free model).
				const providerModels = await Llms.getModelsForProvider(
					value,
					CHAT_MODEL_QUERY_OPTIONS,
				);
	...
	private get accountApiKey(): string {
		return process.env.KERBEROSEC_API_KEY ?? this.authResult?.apiKey ?? "";
	}

	private async getOrganizationConfigOption(
		providerId: string,
	): Promise<SessionConfigOption | undefined> {
		if (!usesKerberoSecAccount(providerId)) {
			return undefined;
		}
		const organizations = await fetchKerberoSecOrganizations({
			apiKey: this.accountApiKey,
			providerSettingsManager: this.providerSettingsManager,
		});
	...
	private async buildConfig(session: SessionState): Promise<Config> {
		...
		const providerId =
			process.env.KERBEROSEC_PROVIDER ?? session.currentProviderId;
		const apiKey =
			process.env.KERBEROSEC_API_KEY ?? this.authResult?.apiKey ?? "";
