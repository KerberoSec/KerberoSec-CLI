...
export const ACCOUNT_IDENTITY_STORAGE_KEY =
	"kerberosec.code.account-identity.v1";

const SIGNED_OUT_ERROR_MARKERS = [
	"No KerberoSec account auth token found",
	"no longer valid",
];

type AccountContextValue = {
	user: KerberoSecAccountUser | null;
...
export function parseCachedAccountUser(
	raw: string | null,
): KerberoSecAccountUser | null {
	if (!raw) {
		return null;
	}
	try {
		const parsed = JSON.parse(raw) as { user?: KerberoSecAccountUser | null };
...
function readCachedAccountUser(): KerberoSecAccountUser | null {
...
function writeCachedAccountUser(user: KerberoSecAccountUser | null): void {
	if (typeof window === "undefined") {
		return;
	}
	try {
...
export function AccountProvider({ children }: { children: React.ReactNode }) {
	...
	const refreshAccount = useCallback(async () => {
		try {
			const me = await desktopClient.invoke<KerberoSecAccountUser>(
				"kerberosec_account",
				{
			...
			if (isKerberoSecAccountNotAuthenticatedResult(me)) {
				setUser(null);
