...
// (~/.kerberosec/data/settings/kerberosec_mcp_settings.json) under each server's `oauth`
// key, in the format @kerberosec/core (CLI, JetBrains) reads and writes:
//
...
//  - Writes scope to ONE server's `oauth` key via @kerberosec/core's
...
//    @kerberosec/core's authorizeMcpServerOAuth, which binds a local loopback
 ...
 * providers. Matches @kerberosec/core's DEFAULT_HTTP_MCP_REDIRECT_URL — the actual
 ...
 * @kerberosec/core defaults; extras tolerate concurrent flows from other KerberoSec
 ...
 * reads/writes the shared settings file in @kerberosec/core's format.
 *
 ...
 */
class KerberoSecOAuthClientProvider implements OAuthClientProvider {
	constructor(
 ...
 * HTTP-callback authorization flow via @kerberosec/core.
 */
export class McpOAuthManager {
	...
	async getOrCreateProvider(serverName: string, serverUrl: string): Promise<OAuthClientProvider> {
		...
		const provider = new KerberoSecOAuthClientProvider(serverName, await this.getSettingsPath())
	 ...
	 *
	 * Delegates to @kerberosec/core's authorizeMcpServerOAuth (the exact flow the
