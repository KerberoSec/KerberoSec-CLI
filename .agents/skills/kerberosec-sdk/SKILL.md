---
name: kerberosec-sdk
description: Comprehensive KerberoSec SDK skill for building AI agents. Covers the Agent runtime, KerberoSecCore sessions, custom tools, plugins, events, LLM providers, scheduling, multi-agent teams, and production deployment. Use for any task involving @kerberosec/sdk or its sub-packages.
metadata:
   references: agent, kerberoseccore
---

# KerberoSec SDK Skill

Consolidated skill for building AI agents with the KerberoSec SDK. Use the decision trees below to find the right entry point and API surface, then load detailed references.

## Critical Rules

Follow these rules in all KerberoSec SDK code:

1. Install with `npm install @kerberosec/sdk`. The `@kerberosec/sdk` package re-exports everything from `@kerberosec/core`, `@kerberosec/agents`, `@kerberosec/llms`, and `@kerberosec/shared`.
2. Requires Node.js 22 or later.
3. Use `createTool()` from `@kerberosec/sdk` (or `@kerberosec/shared`) to define tools. Tool names must be `snake_case`.
...
6. When using `KerberoSecCore`, always call `dispose()` when done to clean up resources.
7. The standalone `Agent` and `KerberoSecCore` have different event systems. For `Agent`: use `agent.subscribe()` to get `AgentRuntimeEvent` types (text streaming is `"assistant-text-delta"`, result text is `result.outputText`). For `KerberoSecCore`: use `kerberosec.subscribe()` to get `CoreSessionEvent` types (text streaming is `"chunk"` with `payload.type === "text"`, result text is `result.text`). There is no top-level `onEvent` field on `AgentRuntimeConfig` -- use `agent.subscribe()` or `hooks.onEvent` instead. Do not use event types like `"content_update"` or `"content_start"` with `agent.subscribe()` -- those are internal legacy types from the KerberoSecCore adapter layer.

## How to Use This Skill

### Reference File Structure

The two main API surfaces (`Agent` and `KerberoSecCore`) follow a 4-file pattern. Cross-cutting concepts are single-file guides.
...
./references/kerberoseccore/REFERENCE.md       # Start here for full runtime
...
|   +-- agent/ (Agent class from @kerberosec/agents)
+-- I need session persistence, built-in tools, config discovery
|   +-- kerberoseccore/ (KerberoSecCore from @kerberosec/core)
+-- I want built-in file/shell/search/web tools
|   +-- kerberoseccore/ (has built-in tools; Agent does not)
+-- I want scheduled or recurring agents
|   +-- kerberoseccore/ (automation API)
+-- I need multi-process or multi-client session sharing
|   +-- kerberoseccore/ (hub-backed runtime)
...
- Tool errors crashing the agent -> `agent/gotchas.md` or `kerberoseccore/gotchas.md`
- Provider auth failures -> `providers/REFERENCE.md`
- Session not persisting -> `kerberoseccore/gotchas.md`
- Token usage too high -> `production/REFERENCE.md` (cost control)
- Hub connection issues -> `kerberoseccore/gotchas.md`
...
| KerberoSecCore | `./references/kerberoseccore/REFERENCE.md` | Full runtime with sessions, persistence, built-in tools |
...
| Package | Purpose |
|---------|---------|
| `@kerberosec/sdk` | Everything you need, install this one |
| `@kerberosec/core` | Sessions, persistence, built-in tools, config, hub |
| `@kerberosec/agents` | Stateless agent loop, tool orchestration, streaming |
| `@kerberosec/llms` | LLM provider gateway |
| `@kerberosec/shared` | Types, tool helpers, hook engine |
