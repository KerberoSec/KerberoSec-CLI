...
describe("Filesystem Utilities", () => {
	const tmpDir = path.join(os.tmpdir(), "kerberosec-test-" + Math.random().toString(36).slice(2))
	...
	it("should exclude .kerberosecrules/workflows directory specifically", async () => {
		// Create a test directory structure
		const kerberosecrulesDirTest = path.join(tmpDir, "kerberosecrules-test")
		const kerberosecrulesDirPath = path.join(kerberosecrulesDirTest, ".kerberosecrules")

		// Create .kerberosecrules directory and root files
		await fs.mkdir(kerberosecrulesDirPath, { recursive: true })
		await fs.writeFile(path.join(kerberosecrulesDirPath, "config.json"), "{}")
		await fs.writeFile(path.join(kerberosecrulesDirPath, "settings.js"), "// settings")

		// Create .kerberosecrules/other directory and files
		const otherDirPath = path.join(kerberosecrulesDirPath, "other")
		...
		// Create .kerberosecrules/workflows directory and files
		const workflowsDirPath = path.join(kerberosecrulesDirPath, "workflows")
		...
		const allFiles = await readDirectory(kerberosecrulesDirPath)
		...
		const filteredFiles = await readDirectory(kerberosecrulesDirPath, [[".kerberosecrules", "workflows"]])

		// Verify workflows files are excluded but others remain
		filteredFiles.length.should.equal(4) // 2 in root + 2 in other

		const expectedFiles = [
			path.resolve(kerberosecrulesDirPath, "config.json"),
			path.resolve(kerberosecrulesDirPath, "settings.js"),
		...
		const multiExcludeFiles = await readDirectory(kerberosecrulesDirPath, [
			[".kerberosecrules", "workflows"],
			[".kerberosecrules", "other"],
		])

		// Verify both workflows and other directories are excluded
		multiExcludeFiles.length.should.equal(2) // only the 2 files in root

		const rootOnlyFiles = [path.resolve(kerberosecrulesDirPath, "config.json"), path.resolve(kerberosecrulesDirPath, "settings.js")]

		multiExcludeFiles.sort().should.deepEqual(rootOnlyFiles.sort())
	})

	it("should exclude .kerberosecrules/hooks directory specifically", async () => {
		// Create a test directory structure
		const kerberosecrulesDirTest = path.join(tmpDir, "kerberosecrules-hooks-test")
		const kerberosecrulesDirPath = path.join(kerberosecrulesDirTest, ".kerberosecrules")
		...
		// Create .kerberosecrules/hooks directory and files
		const hooksDirPath = path.join(kerberosecrulesDirPath, "hooks")
		...
		const filteredFiles = await readDirectory(kerberosecrulesDirPath, [[".kerberosecrules", "hooks"]])
