...
const WRAPPER_PACKAGE_JSON = {
	name: "kerberosec-installed-plugin",
	private: true,
	kerberosec: {
...
function getPluginRoot(cwd: string | undefined): string {
	return cwd
		? join(cwd, ".kerberosec", "plugins")
		: join(resolveKerberoSecDir(), "plugins");
}
...
function getManifestPaths(manifest: PluginPackageManifest | null): string[] {
	const entries = manifest?.kerberosec?.plugins;
...
function removeInstalledHostProvidedSdkDependencies(
	...
	const kerberosecScopeDir = join(packageRoot, "node_modules", "@kerberosec");
	if (!existsSync(kerberosecScopeDir)) {
		return;
	}
	for (const entry of statSafeReadDir(kerberosecScopeDir)) {
		const packageName = `@kerberosec/${entry.name}`;
		if (packageName === preservePackageName) {
			continue;
		}
		rmSync(join(kerberosecScopeDir, entry.name), {
			recursive: true,
			force: true,
		});
	}
...
async function installNpmPackage(
	...
	await writeFile(
		...
		JSON.stringify(
			{ name: "kerberosec-plugin-install", private: true },
			null,
			2,
		),
		"utf8",
...
async function installOfficialPlugin(
	...
	if (!existsSync(sourceRoot) || !statSync(sourceRoot).isDirectory()) {
		throw new Error(
			`Official KerberoSec plugin "${parsed.slug}" was not found at plugins/${parsed.slug} in ${officialPluginsRepo}`,
		);
	}
...
export async function installPlugin(
	...
	const npmCommand =
		options.npmCommand ?? (process.env.KERBEROSEC_NPM_COMMAND?.trim() || "npm");
