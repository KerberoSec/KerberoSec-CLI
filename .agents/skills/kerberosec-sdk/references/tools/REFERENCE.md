...
Tools are how agents interact with the world. The KerberoSec SDK supports both built-in tools (via KerberoSecCore) and custom tools you define yourself.

## Creating Custom Tools

Use `createTool()` from `@kerberosec/sdk` (or `@kerberosec/shared`):
...
## Built-in Tools (KerberoSecCore Only)

When using `KerberoSecCore` with `enableTools: true`, these tools are available automatically:
...
KerberoSecCore can connect to MCP (Model Context Protocol) servers for additional tools. Configure in `.kerberosec/mcp-servers.json`:
...
- `../kerberoseccore/REFERENCE.md` - Using tools with KerberoSecCore
