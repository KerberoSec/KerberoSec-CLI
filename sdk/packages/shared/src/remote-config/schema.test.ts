...
describe("Remote Config Schema", () => {
	...
	describe("RemoteConfigSchema", () => {
		...
		it("should handle complete config with all fields", () => {
			...
			expect(result.providerSettings?.KerberoSec?.models?.[0].id).to.equal(
				"claude-3-5-sonnet-20241022",
			);
			expect(result.providerSettings?.KerberoSec?.models?.[1].id).to.equal(
