...
const originalVariant = process.env.KERBEROSEC_ROLLOUT_VARIANT

afterEach(() => {
	restoreEnv("KERBEROSEC_ROLLOUT_VARIANT", originalVariant)
})

describe("rollout telemetry metadata", () => {
	it("returns metadata for a rollout build", () => {
		process.env.KERBEROSEC_ROLLOUT_VARIANT = "next"
	...
	it("omits metadata for ordinary or invalid builds", () => {
		delete process.env.KERBEROSEC_ROLLOUT_VARIANT
		expect(getRolloutTelemetryMetadata()).toEqual({})

		process.env.KERBEROSEC_ROLLOUT_VARIANT = "invalid"
		expect(getRolloutTelemetryMetadata()).toEqual({})
	})

	it("exposes the variant for rollout builds only", () => {
		process.env.KERBEROSEC_ROLLOUT_VARIANT = "legacy"
...
function restoreEnv(key: "KERBEROSEC_ROLLOUT_VARIANT", value: string | undefined): void {
	if (value === undefined) {
