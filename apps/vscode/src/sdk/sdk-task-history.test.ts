import type { SessionHistoryRecord } from "@kerberosec/core"
...
import { sdkMessagesToKerberoSecMessages } from "./message-translator"
...
vi.mock("@/hosts/host-provider", () => ({
	HostProvider: {
		get: vi.fn(() => ({ globalStorageFsPath: "/tmp/kerberosec" })),
	},
}))
...
describe("SdkTaskHistory", () => {
	...
	it("converts SDK persisted conversation messages to KerberoSec messages", () => {
		const result = sdkMessagesToKerberoSecMessages([
				...
				content: [
					{
						...
						input: {
							...
							old_text: "## License",
							new_text: "## A Note from KerberoSec\n\n> Why do programmers prefer dark mode?",
						},
					},
				],
			},
	...
	it("retags only the final turn's text, styled by the mode recovered from user_input wrappers", async () => {
		...
		const result = await history.getKerberoSecMessages("task-1")
	...
	it("does not retag the terminal text when no session record exists (unknown outcome)", async () => {
		...
		const result = await history.getKerberoSecMessages("task-without-record")
	...
	it("backfills SDK task size from the session artifact directory", async () => {
		...
		const { history, updateSession } = makeHistory([
			makeSessionRecord("task-1", {
				...
				messagesPath: "/tmp/kerberosec/sessions/task-1/task-1.messages.json",
			}),
		])
		...
		expect(getFolderSize.loose).toHaveBeenCalledWith("/tmp/kerberosec/sessions/task-1", { bigint: false })
	...
	it("reads legacy task UI messages without migrating", async () => {
		...
		const messages = await history.getKerberoSecMessages("legacy-task")
	...
	it("uses pretty legacy UI messages plus resumed SDK messages when both stores exist", async () => {
		...
		const kerberosecMessages = await history.getKerberoSecMessages("legacy-task")
		...
		expect(kerberosecMessages).toEqual([
	...
	it("includes legacy tasks from VS Code extension storage", async () => {
		legacyStateReaderMock.taskHistory = [makeHistoryItem("kerberosec-dir-task", { task: "~/.kerberosec task" })]
		...
		expect(result.map((item) => item.sessionId)).toEqual(["kerberosec-dir-task", "extension-storage-task"])
	})
