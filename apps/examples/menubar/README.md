...
Any Client (CLI, VS Code, agents)
    ...
    ▼
Hub WebSocket Server (@kerberosec/core/hub/server.ts)
    │
...
`apps/cli/src/index.ts` when it is present; packaged builds use `kerberosec` from
`PATH`, or the executable specified by `KERBEROSEC_CLI_PATH`.
