...
import {
	...
	setHomeDir,
	setKerberoSecDir,
} from "@kerberosec/shared/storage";
...
describe("plugin install command", () => {
	...
	let originalKerberoSecDir: string | undefined;
	let originalKerberoSecDataDir: string | undefined;
	...
	beforeEach(() => {
		...
		originalKerberoSecDir = process.env.KERBEROSEC_DIR;
		originalKerberoSecDataDir = process.env.KERBEROSEC_DATA_DIR;
		originalMcpSettingsPath = process.env.KERBEROSEC_MCP_SETTINGS_PATH;
		process.env.HOME = home;
		process.env.KERBEROSEC_DIR = join(home, ".kerberosec");
		process.env.KERBEROSEC_DATA_DIR = join(home, ".kerberosec", "data");
		setHomeDir(home);
		setKerberoSecDir(process.env.KERBEROSEC_DIR);
	});
	...
	async function createOfficialPluginsRepo(
		...
		runGitCommand(repo, ["config", "user.name", "KerberoSec Test"]);
	...
	afterEach(() => {
		...
		if (originalKerberoSecDataDir === undefined) {
			delete process.env.KERBEROSEC_DATA_DIR;
		} else {
			process.env.KERBEROSEC_DATA_DIR = originalKerberoSecDataDir;
		}
	...
	it("parses GitHub plugin file URLs as remote sources", () => {
		expect(
			parsePluginSource(
				"https://github.com/kerberosec/kerberosec/blob/main/sdk/examples/plugins/weather-metrics.ts",
			),
		).toEqual({
			type: "remote",
			url: "https://raw.githubusercontent.com/kerberosec/kerberosec/main/sdk/examples/plugins/weather-metrics.ts",
	...
	it("parses raw plugin file URLs as remote sources", () => {
		expect(
			parsePluginSource(
				"https://raw.githubusercontent.com/kerberosec/kerberosec/main/sdk/examples/plugins/weather-metrics.ts",
			),
		).toEqual({
			type: "remote",
			url: "https://raw.githubusercontent.com/kerberosec/kerberosec/main/sdk/examples/plugins/weather-metrics.ts",
	...
	it("installs a local plugin file into the global plugin root", async () => {
		...
		expect(result.installPath).toContain(join(home, ".kerberosec", "plugins"));
	...
	it("installs a remote plugin file into the workspace plugin root", async () => {
		...
		expect(result.installPath).toContain(
			join(workspace, ".kerberosec", "plugins", "_installed", "remote"),
		);
		expect(result.entryPaths).toHaveLength(1);
		expect(existsSync(result.entryPaths[0] ?? "")).toBe(true);
		...
		expect(
			discoverPluginModulePaths(join(workspace, ".kerberosec", "plugins")),
	...
	it("installs an official plugin slug from the configured collection repo", async () => {
		...
		expect(result.installPath).toContain(
			join(workspace, ".kerberosec", "plugins", "_installed", "official"),
		);
		expect(result.entryPaths).toHaveLength(1);
		expect(readFileSync(result.entryPaths[0] ?? "", "utf8")).toContain(
			"official-web-search",
	...
	it("reports a clear error when an official plugin slug is missing", async () => {
		...
		await expect(
			...
			/Official KerberoSec plugin "missing-plugin" was not found at plugins\/missing-plugin/,
		);
	});
	...
	it("keeps explicit relative paths as local plugin installs", async () => {
		...
		expect(result.installPath).toContain(
			join(workspace, ".kerberosec", "plugins", "_installed", "local"),
		);
		const wrapperManifest = JSON.parse(
			readFileSync(join(result.installPath, "package.json"), "utf8"),
		) as { name?: string };
	...
	it("installs into cwd plugin root when cwd is provided", async () => {
		...
		const wrapperManifest = JSON.parse(
			...
			name?: string;
			kerberosec?: { plugins?: Array<{ paths?: string[] }> };
		};
		expect(wrapperManifest.name).toBe("plugin-package");
		expect(wrapperManifest.kerberosec?.plugins?.[0]?.paths).toHaveLength(1);
		expect(wrapperManifest.kerberosec?.plugins?.[0]?.paths?.[0]).toContain(
			"package/index.ts",
		);
		...
		const discovered = discoverPluginModulePaths(
			join(workspace, ".kerberosec", "plugins"),
		);
		expect(discovered).toEqual(result.entryPaths);
	...
	it("omits and removes host SDK packages from npm-sourced installs", async () => {
		...
		writeFileSync(
			...
			[
				...
				'mkdir -p "$prefix/node_modules/@kerberosec/core"',
				'printf \'%s\\n\' \'{"name":"published-plugin","type":"module","kerberosec":{"plugins":["index.ts"]}}\' > "$prefix/node_modules/published-plugin/package.json"',
				"printf '%s\\n' \"export default { name: 'published-plugin', manifest: { capabilities: ['tools'] } };\" > \"$prefix/node_modules/published-plugin/index.ts\"",
				'printf \'%s\\n\' \'{"name":"@kerberosec/core"}\' > "$prefix/node_modules/@kerberosec/core/package.json"',
				"exit 0",
			].join("\n"),
			{ encoding: "utf8", mode: 0o755 },
		);
	...
	it("does not run MCP OAuth follow-up for JSON plugin installs", async () => {
		process.env.KERBEROSEC_MCP_SETTINGS_PATH = join(root, "mcp-settings.json");
...
`,
		...
		const originalSettingsPath = process.env.KERBEROSEC_MCP_SETTINGS_PATH;
		process.env.KERBEROSEC_MCP_SETTINGS_PATH = join(
			blockedDirectory,
		...
		try {
			...
			if (originalSettingsPath === undefined) {
				...
				process.env.KERBEROSEC_MCP_SETTINGS_PATH = originalSettingsPath;
			}
		}
	});
