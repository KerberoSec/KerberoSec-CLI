...
import { createSessionId } from "@kerberosec/shared"
...
export interface SdkTaskStartCoordinatorOptions {
	...
	isKerberoSecManagedProviderActive: () => boolean
	emitKerberoSecAuthError: (task?: string) => void
...
export class SdkTaskStartCoordinator {
	...
	async initTask(
		...
		try {
			...
			if (usesKerberoSecAccountAuth(config.providerId) && !config.apiKey) {
				Logger.warn(
					`[SdkController] ${config.providerId} provider selected but no KerberoSec auth token — emitting auth error`,
				)
				...
				this.options.emitKerberoSecAuthError(prompt)
				return undefined
			}
	...
	private emitInitialTaskMessage(sessionId: string, task: string, images?: string[], files?: string[]): void {
		...
		const taskMessage: KerberoSecMessage = {
			ts: Date.now(),
			type: "say",
			say: "task",
			text: task,
	...
	private handleInitError(error: unknown, sessionId?: string): void {
		...
		;(globalThis as Record<string, unknown>).__kerberosec_last_init_error = errorDetails
		;(globalThis as Record<string, unknown>).__kerberosec_last_init_error_raw = error
	...
	private handleReinitError(taskId: string, error: unknown): void {
		...
		const isKerberoSecAuthReinit =
			this.options.isKerberoSecManagedProviderActive() &&
			(reinitErrorMsg.includes(KERBEROSEC_ACCOUNT_AUTH_ERROR_MESSAGE) ||
				reinitErrorMsg.toLowerCase().includes("missing api key") ||
				reinitErrorMsg.toLowerCase().includes("unauthorized"))

		if (isKerberoSecAuthReinit) {
