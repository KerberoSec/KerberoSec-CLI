...
- `AgentRuntimeEvent` from `@kerberosec/agents`, delivered through `agent.subscribe(listener)`.
- `AgentEvent` / `CoreSessionEvent` from `@kerberosec/core`, delivered through core session adapters and `kerberosec.subscribe(listener)`.
...
`AgentEvent` is the host-facing event shape used by `@kerberosec/core` session orchestration. For direct `Agent` usage, prefer `agent.subscribe(...)` and `AgentRuntimeEvent`.
...
## KerberoSecCore Session Events

Use `kerberosec.subscribe()` for session-level events:
...
```typescript
const unsubscribe = kerberosec.subscribe(listener, { sessionId })
```
...
For a complete working example of streaming agent events to a browser via SSE, see the [multi-agent example](https://github.com/kerberosec/kerberosec/tree/main/apps/examples/multi-agent). It spawns multiple agents in parallel and streams each agent's events to separate UI cards.
