...
describe("spawnDetachedConnector", () => {
	it("preserves the connect subcommand when building detached connector args", () => {
		expect(
			__test__.buildDetachedConnectorArgs(
				["connect", "telegram"],
				["-m", "KerberoSecAdapterBot", "-k", "token-123"],
			),
		).toEqual([
			"connect",
			"telegram",
			"-m",
			"KerberoSecAdapterBot",
			"-k",
			"token-123",
			"-i",
		]);
	...
	it("uses a dynamic connector inspector port for development node launches", () => {
		...
		expect(
			__test__.buildDetachedConnectorCommand(
				["connect", "telegram"],
				["-m", "KerberoSecAdapterBot"],
				"/usr/local/bin/node",
				"./apps/cli/src/index.ts",
				[],
				repoRoot,
				{ KERBEROSEC_BUILD_ENV: "development" },
			),
		).toEqual({
	...
	it("marks detached children and removes the hub-daemon-only environment flag", () => {
		const env = {
			KERBEROSEC_BUILD_ENV: "production",
			KERBEROSEC_RUN_AS_HUB_DAEMON: "1",
			UNCHANGED: "value",
		};

		expect(
			__test__.buildDetachedConnectorEnv(
				"KERBEROSEC_TELEGRAM_CONNECT_CHILD",
				env,
			),
		).toEqual({
			KERBEROSEC_BUILD_ENV: "production",
			KERBEROSEC_CONNECTOR_DETACHED_CHILD: "1",
			KERBEROSEC_TELEGRAM_CONNECT_CHILD: "1",
			UNCHANGED: "value",
		});
		expect(env.KERBEROSEC_RUN_AS_HUB_DAEMON).toBe("1");
	});
});
...
describe("detached connector log rotation", () => {
	it("keeps one generation once the log grows past the cap", () => {
		const dir = mkdtempSync(join(tmpdir(), "kerberosec-connector-log-"));
		const logPath = join(dir, "kerberosec-slack.log");
