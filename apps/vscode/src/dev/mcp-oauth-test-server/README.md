...
and debugging KerberoSec's MCP OAuth flow locally.
...
2. **MCP StreamableHTTP resource server**:
   - `POST /mcp` — returns `401 + WWW-Authenticate: Bearer resource_metadata="..."`
     until authenticated (this is what triggers KerberoSec's OAuth flow), then a
...
- **State expiry** — KerberoSec's `McpOAuthManager` enforces a state lifetime
...
- **Denial** — the consent page's Deny button (or `--auto-deny`) redirects back
  with `error=access_denied`, so you can observe how KerberoSec handles a denial.
...
the nested `transport` shape used by `kerberosec_mcp_settings.json`) in addition to
the banner — merge it under `mcpServers` in
`~/.kerberosec/data/settings/kerberosec_mcp_settings.json`. Or add an MCP server
(StreamableHTTP) in KerberoSec by hand, pointing at:
...
| `--instances <n>` | Start N independent servers, each on its own random port (implies `--random-port`). Use to add several MCP servers to KerberoSec at once. |
...
each one to KerberoSec as a separate StreamableHTTP server to exercise concurrent
...
Because OAuth state is keyed by **server name** in `kerberosec_mcp_settings.json`,
each KerberoSec server entry gets its own independent tokens — even if two point at
the same URL.

## Reproducing specific bugs

**"OAuth state expired" race** — make the user take longer than KerberoSec's
...
- Under `KERBEROSEC_CAPTURE_BROWSER=1` (see `src/utils/env.ts`), the authorization URL
  KerberoSec tries to open is captured instead of launched. The harness `curl`s the
  ...
  extension via `globalThis.__kerberosecHandleUri(...)` (see the debug harness README,
  "Testing MCP OAuth").
