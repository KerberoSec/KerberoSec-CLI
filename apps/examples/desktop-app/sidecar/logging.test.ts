...
const originalEnv = {
	KERBEROSEC_LOG_ENABLED: process.env.KERBEROSEC_LOG_ENABLED,
	KERBEROSEC_LOG_LEVEL: process.env.KERBEROSEC_LOG_LEVEL,
	KERBEROSEC_LOG_NAME: process.env.KERBEROSEC_LOG_NAME,
	KERBEROSEC_LOG_PATH: process.env.KERBEROSEC_LOG_PATH,
};
...
describe("desktop sidecar logging", () => {
	it("writes structured SDK logs to the configured file", () => {
		const directory = mkdtempSync(join(tmpdir(), "kerberosec-code-logging-"));
		const destination = join(directory, "sidecar.log");
		process.env.KERBEROSEC_LOG_PATH = destination;
		process.env.KERBEROSEC_LOG_LEVEL = "debug";
		delete process.env.KERBEROSEC_LOG_ENABLED;
		...
		try {
			...
			expect(adapter.runtimeConfig.name).toBe("kerberosec-code.sidecar");
		} finally {
	...
	it("warns once before falling back to stderr when the log file cannot open", () => {
		const directory = mkdtempSync(join(tmpdir(), "kerberosec-code-fallback-"));
		process.env.KERBEROSEC_LOG_PATH = directory;
		delete process.env.KERBEROSEC_LOG_ENABLED;
	...
	it("rotates the active log before a write exceeds the size limit", () => {
		const directory = mkdtempSync(join(tmpdir(), "kerberosec-code-rotation-"));
		const destination = join(directory, "sidecar.log");
		process.env.KERBEROSEC_LOG_PATH = destination;
		delete process.env.KERBEROSEC_LOG_ENABLED;
