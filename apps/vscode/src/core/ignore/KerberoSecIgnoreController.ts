 ...
 * Designed to be instantiated once in KerberoSec.ts and passed to file manipulation services.
 * Uses the 'ignore' library to support standard .gitignore syntax in .kerberosecignore files.
 */
export class KerberoSecIgnoreController {
	private cwd: string
	private ignoreInstance: Ignore
	private fileWatcher?: FSWatcher
	kerberosecIgnoreContent: string | undefined

	constructor(cwd: string) {
		this.cwd = cwd
		this.ignoreInstance = ignore()
		this.kerberosecIgnoreContent = undefined
	}
	...
	async initialize(): Promise<void> {
		// Set up file watcher for .kerberosecignore
		this.setupFileWatcher()
		await this.loadKerberoSecIgnore()
	}

	/**
	 * Set up the file watcher for .kerberosecignore changes
	 */
	private setupFileWatcher(): void {
		const ignorePath = path.join(this.cwd, ".kerberosecignore")
		...
		this.fileWatcher.on("error", (error) => {
			Logger.error("Error watching .kerberosecignore file:", error)
		})
	}

	/**
	 * Load custom patterns from .kerberosecignore if it exists.
	 * Supports "!include <filename>" to load additional ignore patterns from other files.
	 */
	private async loadKerberoSecIgnore(): Promise<void> {
		try {
			...
			if (await fileExistsAtPath(ignorePath)) {
				...
				this.kerberosecIgnoreContent = content
				await this.processIgnoreContent(content)
				this.ignoreInstance.add(".kerberosecignore")
			} else {
				this.kerberosecIgnoreContent = undefined
			}
		} catch (error) {
			// Should never happen: reading file failed even though it exists
			Logger.error("Unexpected error loading .kerberosecignore:", error)
		}
	}
	...
	private async processIgnoreContent(content: string): Promise<void> {
		...
		const combinedContent = await this.processKerberoSecIgnoreIncludes(content)
	 ...
	 */
	private async processKerberoSecIgnoreIncludes(content: string): Promise<string> {
		let combinedContent = ""
	...
	private async readIncludedFile(includeLine: string): Promise<string | null> {
		...
		if (!(await fileExistsAtPath(resolvedIncludePath))) {
			Logger.debug(`[KerberoSecIgnore] Included file not found: ${resolvedIncludePath}`)
			return null
		}
	...
	validateAccess(filePath: string): boolean {
		// Always allow access if .kerberosecignore does not exist
		if (!this.kerberosecIgnoreContent) {
			return true
		}
		try {
	...
	validateCommand(command: string): string | undefined {
		// Always allow if no .kerberosecignore exists
		if (!this.kerberosecIgnoreContent) {
			return undefined
		}
