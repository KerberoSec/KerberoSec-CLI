...
function schedule(overrides: Partial<ScheduleRecord> = {}): ScheduleRecord {
	return {
		...
		cwd: WORKSPACE_ROOT,
		modelSelection: { providerId: "kerberosec", modelId: "model-1" },
		enabled: true,
		mode: "yolo",
		maxParallel: 1,
		createdAt: Date.now(),
...
describe("schedule agent tool", () => {
	it("creates a one-time schedule using current session defaults", async () => {
		...
		const tool = scheduleOperations({
			...
			resolveSessionDefaults: async () => ({
				workspaceRoot: WORKSPACE_ROOT,
				cwd: WORKSPACE_ROOT,
				modelSelection: { providerId: "kerberosec", modelId: "model-current" },
				interactive: true,
			}),
		});
	...
	it("rejects a symlink and parent-segment workspace escape", async () => {
		const root = mkdtempSync(join(tmpdir(), "kerberosec-schedule-scope-"));
		try {
