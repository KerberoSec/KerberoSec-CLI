...
console.log(`Building @kerberosec/cli v${version}`);
...
if (!buildOptions.skipSdkBuild) {
	...
	await $`bun -F @kerberosec/cli build`.cwd(rootDir);
}

const hubWebviewSource = join(cliDir, "../kerberosec-hub/src/webview");
const hubWebviewDist = join(cliDir, "../kerberosec-hub/dist/webview");
...
if (shouldBuildHubWebview()) {
	console.log("Building KerberoSec Hub webview...");
	await $`bun -F @kerberosec/kerberosec-hub build:webview`.cwd(rootDir);
}
...
async function buildCompiledBinary(input: {
	...
	const tmpDir = join("/tmp", `kerberosec-build-${input.dirName}`);
	const tmpOutfile = join(
		tmpDir,
		input.outfile.endsWith(".exe") ? "kerberosec.exe" : "kerberosec",
	);
...
for (const item of targets) {
	...
	const name = `@kerberosec/cli-${displayOs}-${item.arch}`;
	const dirName = `cli-${displayOs}-${item.arch}`;
	const binaryName = item.os === "win32" ? "kerberosec.exe" : "kerberosec";
	...
	if (existsSync(hubWebviewDist)) {
		const hubWebviewDest = join(
			cliDir,
			`dist/${dirName}/kerberosec-hub/webview`,
		);
		mkdirSync(join(cliDir, `dist/${dirName}/kerberosec-hub`), {
			recursive: true,
		});
		...
		`${JSON.stringify(
			{
				name,
				version,
				description: `KerberoSec CLI binary for ${displayOs} ${item.arch}`,
				os: [item.os],
				cpu: [item.arch],
				...(repository ? { repository } : {}),
				bin: {
					kerberosec: `bin/${binaryName}`,
				},
			},
			null,
			2,
