...
import type { KerberoSecAccountUser } from "@kerberosec/core";
...
function makeUser(
	overrides: Partial<KerberoSecAccountUser> = {},
...
describe("account context", () => {
	...
	it("classifies signed-out errors separately from transient failures", () => {
		expect(
			isSignedOutAccountError(
				new Error("No KerberoSec account auth token found"),
			),
		).toBe(true);
		expect(
			isSignedOutAccountError(
	...
	it("fetches the signed-in user on mount and caches the identity", async () => {
		...
		await vi.waitFor(() => {
			...
			expect(probeText("account-org")).toBe("KerberoSec Bot Inc");
		});
		expect(invoke).toHaveBeenCalledWith("kerberosec_account", {
