...
const hoisted = vi.hoisted(() => ({
	...
	startKerberoSecDeviceAuth: vi.fn(),
	completeKerberoSecDeviceAuth: vi.fn(),
...
vi.mock("@kerberosec/core", () => ({
	...
	startKerberoSecDeviceAuth: hoisted.startKerberoSecDeviceAuth,
	completeKerberoSecDeviceAuth: hoisted.completeKerberoSecDeviceAuth,
	saveLocalProviderOAuthCredentials: hoisted.saveLocalProviderOAuthCredentials,
	ProviderSettingsManager: class {},
}));

vi.mock("@kerberosec/shared", () => ({
	getKerberoSecEnvironmentConfig: () => ({ apiBaseUrl: "https://api.example" }),
}));
...
describe("onboarding auth telemetry forwarding", () => {
	beforeEach(() => {
		...
		hoisted.startKerberoSecDeviceAuth.mockReset();
		hoisted.completeKerberoSecDeviceAuth.mockReset();
	...
	it("forwards telemetry into completeKerberoSecDeviceAuth for the device-code flow", async () => {
		hoisted.startKerberoSecDeviceAuth.mockResolvedValueOnce({
			deviceCode: "dc",
			userCode: "uc",
		...
		hoisted.completeKerberoSecDeviceAuth.mockResolvedValueOnce({
			access: "a",
			refresh: "r",
			expires: 0,
			accountId: "acct-1",
		...
		expect(hoisted.completeKerberoSecDeviceAuth).toHaveBeenCalledTimes(1);
		const [opts] = hoisted.completeKerberoSecDeviceAuth.mock.calls[0];
		expect(opts.telemetry).toBe(fakeTelemetry);
		// We do NOT forward telemetry to startKerberoSecDeviceAuth — auth_started is
		// emitted by completeKerberoSecDeviceAuth, so passing telemetry to the start
		// helper would double-emit the event.
		expect(hoisted.startKerberoSecDeviceAuth).toHaveBeenCalledWith();
