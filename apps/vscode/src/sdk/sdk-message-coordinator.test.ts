...
describe("SdkMessageCoordinator", () => {
	...
	it("appends messages to the current task", () => {
		...
		expect(task.messageStateHandler.getKerberoSecMessages()).toEqual(messages)
	})
	...
	it("stamps seq and epoch from the shared minter on append", () => {
		...
		const a: KerberoSecMessage = { ts: 1, type: "say" as const, say: "text" as const, text: "a", partial: true }
		const b: KerberoSecMessage = { ts: 2, type: "say" as const, say: "text" as const, text: "b", partial: false }
	...
	it("gives an updated message a NEWER seq than its earlier copy (partial -> final)", () => {
		...
		const partial: KerberoSecMessage = { ts: 10, type: "say" as const, say: "text" as const, text: "Hel", partial: true }
		...
		const final: KerberoSecMessage = { ts: 10, type: "say" as const, say: "text" as const, text: "Hello", partial: false }
	...
	it("reflects a bumped epoch on subsequent messages", () => {
		...
		const before: KerberoSecMessage = { ts: 1, type: "say" as const, say: "text" as const, text: "before", partial: false }
		coordinator.appendMessages([before])
		minter.bumpEpoch()
		const after: KerberoSecMessage = { ts: 2, type: "say" as const, say: "text" as const, text: "after", partial: false }
	...
	it("leaves messages unstamped when no minter is wired (classic/legacy)", () => {
		const task = createTaskProxy("session-123", vi.fn(), vi.fn())
		const coordinator = new SdkMessageCoordinator({ getTask: () => task })
		const m: KerberoSecMessage = { ts: 1, type: "say" as const, say: "text" as const, text: "x", partial: false }
