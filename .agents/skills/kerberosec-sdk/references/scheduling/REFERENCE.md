...
The KerberoSec SDK supports scheduled, one-off, and event-driven agent execution through the automation subsystem in `@kerberosec/core`.
...
kerberosec schedule create "Daily standup" \
  --cron "0 9 * * MON-FRI" \
...
# Pause/resume
kerberosec schedule pause <schedule-id>
kerberosec schedule resume <schedule-id>

# Delete
kerberosec schedule delete <schedule-id>

# View past executions
kerberosec schedule executions <schedule-id>
```
...
Create Markdown files in `~/.kerberosec/cron/` (global) or `.kerberosec/cron/` (workspace):
...
kerberosec.automation.ingestEvent({
  eventId: "evt-123",
  eventType: "github.pull_request.opened",
  source: "github",
  timestamp: Date.now(),
...
const specs = await kerberosec.automation.listSpecs()
const runs = await kerberosec.automation.listRuns()
const events = await kerberosec.automation.listEvents()

// Reconcile specs from directory
await kerberosec.automation.reconcile(specDirectory)
...
Each completed run writes a Markdown report to `.kerberosec/cron/reports/<run-id>.md` with:
...
- `../kerberoseccore/REFERENCE.md` - KerberoSecCore runtime
- `../kerberoseccore/api.md` - Automation API details
