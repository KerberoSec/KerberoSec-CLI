...
interface KerberoSecStorageChangeEvent {
	readonly key: string
}

export type StorageEventListener = (event: KerberoSecStorageChangeEvent) => Promise<void>
 ...
 * VSCode's Memento and KerberoSecSyncStorage both satisfy this interface.
 */
export interface KerberoSecMemento {
 ...
 */
export abstract class KerberoSecStorage {
	/**
	 * The name of the storage, used for logging purposes.
	 */
	protected name = "KerberoSecStorage"
	/**
 ...
 * Unlike KerberoSecStorage (string key-value, async), this stores any JSON-serializable
 * values and provides synchronous access - required for VSCode Memento compatibility.
 */

export type SyncStorageEventListener = (event: KerberoSecStorageChangeEvent) => void

export abstract class KerberoSecSyncStorage<T = any> {
