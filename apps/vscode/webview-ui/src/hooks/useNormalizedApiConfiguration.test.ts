import { ApiFormat, ResolveModelInfoResponse } from "@shared/proto/kerberosec/models"
...
describe("useNormalizedApiConfiguration", () => {
	...
	it("uses KerberoSec-specific model fields instead of stale generic or OpenRouter fields", async () => {
		setApiConfiguration({
			actModeApiProvider: "kerberosec",
			actModeApiModelId: "openai/gpt-5.4",
			actModeOpenRouterModelId: "anthropic/claude-sonnet-4.5",
			actModeKerberoSecModelId: "anthropic/claude-sonnet-4.6",
		})
		mockResolveModelInfo.mockResolvedValue(modelInfoResponse("kerberosec", "anthropic/claude-sonnet-4.6"))

		const { result } = renderHook(() => useNormalizedApiConfiguration("act"))

		await waitFor(() => expect(result.current.selectedModelId).toBe("anthropic/claude-sonnet-4.6"))
		expect(mockResolveModelInfo).toHaveBeenCalledWith({ providerId: "kerberosec", modelId: "anthropic/claude-sonnet-4.6" })
	})

	it("asks the backend for the KerberoSec default when no KerberoSec-specific model is selected", async () => {
		setApiConfiguration({
			actModeApiProvider: "kerberosec",
			actModeApiModelId: "openai/gpt-5.4",
			actModeOpenRouterModelId: "anthropic/claude-sonnet-4.5",
		...
		await waitFor(() => expect(result.current.selectedModelId).toBe("anthropic/claude-sonnet-4.6"))
		expect(mockResolveModelInfo).toHaveBeenCalledWith({ providerId: "kerberosec", modelId: undefined })
	})
