...
import { ResetStateRequest } from "@shared/proto/kerberosec/state"
import type { UserOrganization } from "@shared/proto/index.kerberosec"
import {
	CheckCheck,
	FlaskConical,
	HardDriveDownload,
...
import { type KerberoSecUser, useKerberoSecAuth } from "@/context/KerberoSecAuthContext"
...
interface SettingsTab {
	...
	headerText: string
	icon: LucideIcon
	hidden?: (params?: { user: KerberoSecUser | null; activeOrganization: UserOrganization | null }) => boolean
}
...
const SETTINGS_TABS: SettingsTab[] = [
	...
	// Only show in dev mode
	{
		...
		headerText: "Debug",
		icon: FlaskConical,
		hidden: ({ user } = { user: null, activeOrganization: null }) => !IS_DEV && !isKerberoSecInternalTester(user?.email || ""),
	},
]
...
const SettingsView = ({ onDone, targetSection }: SettingsViewProps) => {
	...
	const { activeOrganization, kerberosecUser } = useKerberoSecAuth()
		...
		<Tab>
			...
			<div className="flex flex-1 overflow-hidden">
				<TabList
					...
					value={activeTab}>
					{SETTINGS_TABS.filter((tab) => !tab.hidden?.({ user: kerberosecUser, activeOrganization })).map(renderTabItem)}
				</TabList>
