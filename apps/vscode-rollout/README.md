...
2. Sets the `kerberosec.sdkBundle` context key (gates cohort-specific menu items /
...
3. `require()`s exactly one bundle and calls its `activate()` with a
   ...
   `~/.kerberosec/data` + VS Code storage they used as standalone extensions.
...
(`kerberosec.rollout.nextActivationFailedVersion`), reports a `fallback` telemetry
...
- **Manual overrides, in either direction.** The `kerberosec.rollout.bundleOverride`
  ...
  reload. `KERBEROSEC_BUNDLE_OVERRIDE=next|legacy` (env var) does the same for
...
- **Crash pinning is local, not remote.** If the next bundle throws during
  ...
  that VSIX version on this machine (`kerberosec.rollout.nextActivationFailedVersion`);
...
- Menu entries / keybindings declared by only one get `when` AND-ed with
  `kerberosec.sdkBundle` / `!kerberosec.sdkBundle`, so a cohort never sees a button its
...
node scripts/stitch.mjs \
  ...
  --loader dist/extension.js --version 4.1.0 --out /tmp/kerberosec-ab-staging
node scripts/smoke-loader.mjs /tmp/kerberosec-ab-staging   # loader behavior smoke
cd /tmp/kerberosec-ab-staging && vsce package --no-dependencies --allow-package-secrets sendgrid
```
...
entirely and everyone stays on legacy unless `KERBEROSEC_BUNDLE_OVERRIDE` is set.
...
publishes this same combined package as **`saoudrizwan.kerberosec-nightly`**. Before
...
| | stable | nightly |
|---|---|---|
| manifest `name` | `claude-dev` | `kerberosec-nightly` |
| contribution IDs / context key / settings | `kerberosec.*` | `kerberosec-nightly.*` |
...
status-bar indicator (`KerberoSec: Next` / `KerberoSec: Legacy`); stable builds never do.
...
- **`extension.rollout.bundle_activated`** (authoritative, captured by the
  ...
  requires the bundle to be built with `KERBEROSEC_ROLLOUT_VARIANT`): attempted vs
