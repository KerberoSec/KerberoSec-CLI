...
} from "./kerberosec-model-picker";
import { CHANGE_PROVIDER_ACTION } from "./model-selector";
import { ProviderRow } from "./provider-row";

export const BROWSE_ALL_ACTION = "__browse_all__";

type KerberoSecModelEntriesState =
	| { status: "loading"; message: string }
	| { status: "loaded"; entries: KerberoSecModelPickerEntry[] }
...
export function KerberoSecModelSelectorContent(
			...
			if (entry.kind === "model") {
				if (entry.tier !== lastTier) {
					...
					rows.push({
						key: `tier-${entry.tier}`,
						kind: "header",
						label: KERBEROSEC_MODEL_PICKER_TIER_LABELS[entry.tier],
						description:
...
export function KerberoSecModelSelectorDialogContent(
	props: ChoiceContext<string> & {
		currentModel: string;
		currentProviderName: string;
		loadEntries: () => Promise<KerberoSecModelPickerEntry[]>;
	},
) {
	...
	if (state.status === "loaded") {
		return (
			<KerberoSecModelSelectorContent {...props} entries={state.entries} />
		);
	}
