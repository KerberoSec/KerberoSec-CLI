...
import { UpdateApiConfigurationRequestNew } from "@/shared/proto/index.kerberosec"
...
export async function updateApiConfiguration(controller: Controller, request: UpdateApiConfigurationRequestNew): Promise<Empty> {
	try {
		...
		if (Object.keys(options).length > 0) {
			...
			clearOrganizationForKerberoSecPassProviderSelection(controller, controller.stateManager.getApiConfiguration())
		}
