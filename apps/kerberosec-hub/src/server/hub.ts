...
import type { HubUINotifyPayload } from "@kerberosec/shared";
...
export async function attachHub(ctx: HubContext): Promise<void> {
	...
	ctx.kerberosec = await KerberoSecCore.create({
		clientName: "kerberosec-hub",
		backendMode: "hub",
		capabilities: {
		...
		hub: {
			...
			clientType: "kerberosec-hub-chat",
			displayName: "KerberoSec Hub Chat",
			workspaceRoot,
		},
	});
	...
	ctx.uiClient = new HubUIClient({
		address: ctx.hubUrl,
		authToken: ctx.hubAuthToken,
		clientType: "kerberosec-hub-server",
		displayName: "KerberoSec Hub Server",
	});
	...
	ctx.kerberosec.subscribe((event) => handleSessionEvent(ctx, event));
...
export async function detachHub(ctx: HubContext): Promise<void> {
	...
	try {
		await ctx.kerberosec?.dispose();
	} catch {
		// ignore
	}
	ctx.kerberosec = undefined;
	ctx.clients.clear();
	ctx.sessions.clear();
