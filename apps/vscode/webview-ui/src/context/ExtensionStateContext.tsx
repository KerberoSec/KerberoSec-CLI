...
import type { UserInfo } from "@shared/proto/kerberosec/account"
import { EmptyRequest } from "@shared/proto/kerberosec/common"
import type { OpenRouterCompatibleModelInfo, ProviderModelsResponse } from "@shared/proto/kerberosec/models"
import { OnboardingModelGroup, type TerminalProfile } from "@shared/proto/kerberosec/state"
import { convertProtoToKerberoSecMessage } from "@shared/proto-conversions/kerberosec-message"
...
export interface ExtensionStateContextType extends ExtensionState {
	...
	setGlobalKerberoSecRulesToggles: (toggles: Record<string, boolean>) => void
	setLocalKerberoSecRulesToggles: (toggles: Record<string, boolean>) => void
...
export const ExtensionStateContextProvider: React.FC<{
	...
	// Convergent-replica state for kerberosecMessages. The partial-message stream and the full state
					...
					try {
						...
						setState((prevState) => {
							...
							replicaRef.current = reducerApplyStateSnapshot(
								replicaRef.current,
								stateData.kerberosecMessages ?? [],
								stateData.epoch ?? 0,
								stateData.stateVersion ?? 0,
								stateData.turnState,
							)
							stateData.kerberosecMessages = replicaRef.current.messages
		...
		partialMessageUnsubscribeRef.current = UiServiceClient.subscribeToPartialMessage(EmptyRequest.create({}), {
			onResponse: (protoMessage: any) => {
				try {
					...
					const partialMessage = convertProtoToKerberoSecMessage(protoMessage)
					setState((prevState) => {
						...
						return { ...prevState, kerberosecMessages: replicaRef.current.messages }
					})
				} catch (error) {
	...
	const contextValue: ExtensionStateContextType = {
		...
		showWorktrees,
		showAnnouncement,
		globalKerberoSecRulesToggles: state.globalKerberoSecRulesToggles || {},
		localKerberoSecRulesToggles: state.localKerberoSecRulesToggles || {},
		...
		setShowMcp,
		closeMcpView,
		setGlobalKerberoSecRulesToggles: (toggles) =>
			setState((prevState) => ({
				...prevState,
				globalKerberoSecRulesToggles: toggles,
			})),
		setLocalKerberoSecRulesToggles: (toggles) =>
			setState((prevState) => ({
				...prevState,
				localKerberoSecRulesToggles: toggles,
			})),
