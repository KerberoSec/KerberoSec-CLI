import { GeneratedMediaContent } from "@kerberosec/ui"
import { COMMAND_OUTPUT_STRING } from "@shared/combineCommandSequences"
import {
	KerberoSecApiReqInfo,
	KerberoSecAskQuestion,
	KerberoSecAskUseMcpServer,
	KerberoSecMessage,
	KerberoSecPlanModeResponse,
	KerberoSecSayTool,
	COMPLETION_RESULT_CHANGES_FLAG,
} from "@shared/ExtensionMessage"
import { BooleanRequest, StringRequest } from "@shared/proto/kerberosec/common"
...
interface ChatRowProps {
	...
	lastModifiedMessage?: KerberoSecMessage
	isLast: boolean
	...
	({
		...
		const [cost, _apiReqCancelReason, apiReqStreamingFailedMessage] = useMemo(() => {
			if (message.text != null && message.say === "api_req_started") {
				const info: KerberoSecApiReqInfo = JSON.parse(message.text)
			...
			switch (type) {
				...
				case "mistake_limit_reached":
					return [
						...
						<span className="text-error font-bold">KerberoSec is having trouble...</span>,
					]
				case "command":
					return [
						<TerminalIcon className="text-foreground size-2" />,
						<span className="font-bold text-foreground">KerberoSec wants to execute this command:</span>,
					]
				case "use_mcp_server":
					const mcpServerUse = JSON.parse(message.text || "{}") as KerberoSecAskUseMcpServer
					return [
						...
						<span className="ph-no-capture font-bold text-foreground break-words">
							KerberoSec wants to {mcpServerUse.type === "use_mcp_tool" ? "use a tool" : "access a resource"} on the{" "}
				...
				case "followup":
					return [
						...
						<span className="font-bold text-foreground">KerberoSec has a question:</span>,
					]
				default:
					return [null, null]
			}
		...
		const tool = useMemo(() => {
			if (message.ask === "tool" || message.say === "tool") {
				return JSON.parse(message.text || "{}") as KerberoSecSayTool
			}
			return null
			...
			switch (tool.tool) {
				case "editedExistingFile":
					...
					const editToolTitle = isApplyingPatch
						? "KerberoSec is creating patches to edit this file:"
						: "KerberoSec wants to edit this file:"
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "This file is outside of your workspace")}
								<span style={{ fontWeight: "bold" }}>KerberoSec wants to delete this file:</span>
							</div>
							<CodeAccordian
								// isLoading={message.partial}
								code={tool.content}
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "This file is outside of your workspace")}
								<span className="font-bold">KerberoSec wants to create a new file:</span>
							</div>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "This file is outside of your workspace")}
								<span className="font-bold">KerberoSec wants to read this file:</span>
							</div>
						...
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span style={{ fontWeight: "bold" }}>
									{message.type === "ask"
										? "KerberoSec wants to view the top level files in this directory:"
										: "KerberoSec viewed the top level files in this directory:"}
								</span>
							</div>
							<CodeAccordian
								code={tool.content!}
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span style={{ fontWeight: "bold" }}>
									{message.type === "ask"
										? "KerberoSec wants to recursively view all files in this directory:"
										: "KerberoSec recursively viewed all files in this directory:"}
								</span>
							</div>
							<CodeAccordian
								code={tool.content!}
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span style={{ fontWeight: "bold" }}>
									{message.type === "ask"
										? "KerberoSec wants to view source code definition names used in this directory:"
										: "KerberoSec viewed source code definition names used in this directory:"}
								</span>
							</div>
							<CodeAccordian
								code={tool.content!}
						...
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">
									KerberoSec wants to search this directory for <code className="break-all">{tool.regex}</code>:
								</span>
							</div>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">KerberoSec is condensing the conversation:</span>
							</div>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">
									{message.type === "ask"
										? "KerberoSec wants to fetch content from this URL:"
										: "KerberoSec fetched content from this URL:"}
								</span>
							</div>
							<div
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">
									{message.type === "ask"
										? "KerberoSec wants to search the web for:"
										: "KerberoSec searched the web for:"}
								</span>
							</div>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">KerberoSec loaded the skill:</span>
							</div>
		...
		if (message.ask === "use_mcp_server" || message.say === "use_mcp_server") {
			const useMcpServer = JSON.parse(message.text || "{}") as KerberoSecAskUseMcpServer
			...
			case "say":
				switch (message.say) {
					case "api_req_started":
						return (
							<RequestStartRow
								...
								kerberosecMessages={kerberosecMessages}
								cost={cost}
					...
					case "user_feedback":
						return (
							<UserMessage
								canRestoreWorkspace={canRestoreWorkspaceFromMessage(kerberosecMessages, message.ts)}
								files={message.files}
					...
					case "user_feedback_diff":
						const tool = JSON.parse(message.text || "{}") as KerberoSecSayTool
						return (
					...
					case "kerberosecignore_error":
						return <ErrorRow errorType="kerberosecignore_error" message={message} />
						...
						return (
							<div className="flex flex-col bg-warning/20 p-2 rounded-xs border border-error">
								...
								<div className="text-foreground opacity-80">
									KerberoSec may have trouble viewing the command's output. Please update VSCode (
									<code>CMD/CTRL + Shift + P</code> → "Update") and make sure you're using a supported shell:
									zsh, bash, fish, or PowerShell (<code>CMD/CTRL + Shift + P</code> → "Terminal: Select Default
									Profile").
									<a
										className="px-1"
										href="https://github.com/kerberosec/kerberosec/wiki/Troubleshooting-%E2%80%90-Shell-Integration-Unavailable">
			...
			case "ask":
				switch (message.ask) {
					...
					case "followup":
						...
						try {
							const parsedMessage = JSON.parse(message.text || "{}") as KerberoSecAskQuestion
					...
					case "new_task":
						return (
							<div>
								<div className={HEADER_CLASSNAMES}>
									<FilePlus2Icon className="size-2" />
									<span className="text-foreground font-bold">KerberoSec wants to start a new task:</span>
								</div>
					...
					case "condense":
						return (
							<div>
								<div className={HEADER_CLASSNAMES}>
									<FilePlus2Icon className="size-2" />
									<span className="text-foreground font-bold">KerberoSec wants to condense your conversation:</span>
								</div>
								<NewTaskPreview context={message.text || ""} />
							</div>
						)
					case "report_bug":
						return (
							<div>
								<div className={HEADER_CLASSNAMES}>
									<FilePlus2Icon className="size-2" />
									<span className="text-foreground font-bold">KerberoSec wants to create a Github issue:</span>
								</div>
					...
					case "plan_mode_respond": {
						...
						let selected: string | undefined
						try {
							const parsedMessage = JSON.parse(message.text || "{}") as KerberoSecPlanModeResponse
