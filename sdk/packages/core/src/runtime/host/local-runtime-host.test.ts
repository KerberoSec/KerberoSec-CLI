...
describe("LocalRuntimeHost", () => {
	...
	beforeEach(() => {
		...
		process.env.KERBEROSEC_DIR = join(isolatedHomeDir, ".kerberosec");
	...
	afterEach(() => {
		...
		process.env.KERBEROSEC_DIR = envSnapshot.KERBEROSEC_DIR;
		if (envSnapshot.KERBEROSEC_DATA_DIR === undefined) {
			delete process.env.KERBEROSEC_DATA_DIR;
		} else {
			process.env.KERBEROSEC_DATA_DIR = envSnapshot.KERBEROSEC_DATA_DIR;
		}
		setHomeDir(envSnapshot.HOME ?? "~");
		setKerberoSecDir(envSnapshot.KERBEROSEC_DIR ?? join("~", ".kerberosec"));
		rmSync(isolatedHomeDir, { recursive: true, force: true });
	});

	it("recovers stale detached command logs for non-daemon hosts", async () => {
		const detachedLogDirectory = mkdtempSync(
			join(tmpdir(), "kerberosec-command-local-host-recovery-"),
		);
	...
	it("emits session lifecycle telemetry when configured", async () => {
		...
		// `session.started` is emitted from `KerberoSecCore.start` (see
		// `KerberoSecCore.test.ts`), not from `LocalRuntimeHost`, so that the
		...
		await manager.startSession(
			normalizeStartInput({
				config: createConfig({
					...
					modelId: "kerberosec-pass/glm-5.2",
					apiKey: undefined,
				}),
				prompt: "hello",
			}),
