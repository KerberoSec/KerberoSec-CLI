...
export function normalizeAutomationOptions(
	options: KerberoSecCoreAutomationOptions | boolean | undefined,
): KerberoSecCoreAutomationOptions | undefined {
...
export function normalizeAutomationCronScope(
	scope: KerberoSecCoreAutomationOptions["cronScope"],
): "global" | "workspace" | undefined {
	if (scope === "user") return "global";
	return scope;
}

export class KerberoSecCoreAutomationController
	implements KerberoSecCoreAutomationApi
{
...
export interface KerberoSecCoreAutomationRuntimeHandlersInput {
	host: RuntimeHost;
	getExtensionContext(): ExtensionContext | undefined;
}

export function createKerberoSecCoreAutomationRuntimeHandlers(
	input: KerberoSecCoreAutomationRuntimeHandlersInput,
	...
	return {
		...
		async sendSession(sessionId, request) {
			...
			if (!result) {
				throw new Error("KerberoSecCore automation runtime returned no result");
			}
			return { result: toChatTurnResult(result) };
		},
		async abortSession(sessionId) {
			await host.abort(sessionId, new Error("KerberoSecCore automation abort"));
			return { applied: true };
		},
...
export interface KerberoSecCoreAutomationExtensionContextInput {
	automationService?: CronService;
	automation: KerberoSecCoreAutomationApi;
...
export function createKerberoSecCoreAutomationExtensionContext(
	input: KerberoSecCoreAutomationExtensionContextInput,
