...
import { KerberoSecDefaultTool, setDynamicToolUseNames } from "@shared/tools"
...
/** Default Directory for agent configurations: ~/Documents/KerberoSec/Agents */
...
const AgentBaseConfigSchema = z.object({
	...
	tools: z.array(z.nativeEnum(KerberoSecDefaultTool)).default([]),
...
function normalizeToolName(toolName: string): KerberoSecDefaultTool {
	const trimmed = toolName.trim()
	if (!trimmed) {
		throw new Error("Tool name cannot be empty.")
	}

	const asDefaultTool = trimmed as KerberoSecDefaultTool
	if (Object.values(KerberoSecDefaultTool).includes(asDefaultTool)) {
		return asDefaultTool
	}

	throw new Error(
		`Unknown tool '${trimmed}'. Expected a KerberoSecDefaultTool value (for example: read_file, list_files, search_files).`,
	)
}

function parseTools(tools: string | string[] | undefined): KerberoSecDefaultTool[] {
	if (!tools) {
		return []
	}
...
function getAgentsConfigPath(homeDir = os.homedir()): string {
	return path.join(homeDir, "Documents", "KerberoSec", AGENTS_CONFIG_DIRECTORY_NAME)
}
