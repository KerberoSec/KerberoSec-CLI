...
import { shouldSuppressKerberoSecCliMigrationNoticeForActiveProvider } from "../kanban-migration/notice";
...
import { logoutKerberoSecAccount } from "./kerberosec-account";
...
function App(props: TuiProps) {
	...
	const [appView, setAppView] = useState<AppView>(() => {
		if (process.env.KERBEROSEC_FORCE_ONBOARDING === "1") return "onboarding";
	...
	const exitKerberoSec = useCallback(() => {
		session.requestExit();
	}, [session]);
	...
	useEffect(() => {
		if (props.config.providerId === "ollama") {
			import("../utils/ollama-manager")
				.then(({ ensureOllamaServerRunning }) => {
					const baseUrl = props.config.baseUrl || "http://127.0.0.1:11434";
					void ensureOllamaServerRunning(baseUrl).catch(() => {});
				})
				.catch(() => {});
		}
	}, [props.config.baseUrl, props.config.providerId]);
	...
	useEffect(() => {
		...
		void dialog
			...
			.then((update) => {
				if (update) {
					(onHubUpdateRestart ?? exitKerberoSec)();
					return;
				}
				showToast(
					"Hub still differs from this CLI. Run 'kerberosec update' and restart when convenient.",
					"info",
				);
				refocusTextareaRef.current();
			})
	...
	}, [dialog, exitKerberoSec, hubBuildMismatch, onHubUpdateRestart, showToast]);
