...
} from "../webview/lib/kerberosec-integrations-types";
...
export interface KerberoSecIntegrationsRequestOptions {
	apiBaseUrl: string;
...
export async function listKerberoSecIntegrations(
	options: KerberoSecIntegrationsRequestOptions,
): Promise<KerberoSecIntegration[]> {
	const data = await requestKerberoSecApiJson("/api/v1/integrations", options);
	return Array.isArray(data) ? (data as KerberoSecIntegration[]) : [];
}

export async function listKerberoSecGitHubRepositories(
	options: KerberoSecIntegrationsRequestOptions,
): Promise<KerberoSecGitHubRepository[]> {
	const data = await requestKerberoSecApiJson(
		"/api/v1/integrations/github/repositories",
		options,
	);
	return Array.isArray(data) ? (data as KerberoSecGitHubRepository[]) : [];
}
...
function formatRequestFailure(
	...
	if (body) {
		...
		return `KerberoSec integrations request failed with status ${status}: ${preview}`;
	}
	return `KerberoSec integrations request failed with status ${status}`;
}

async function requestKerberoSecApiJson(
	endpoint: string,
	options: KerberoSecIntegrationsRequestOptions,
): Promise<unknown> {
	const fetchImpl = options.fetchImpl ?? fetch;
	...
	try {
		...
		if (text.trim()) {
			try {
				...
				throw new Error("KerberoSec integrations response was not valid JSON");
			}
		}
		...
		if (typeof parsed === "object" && parsed !== null && "success" in parsed) {
			...
			if (typeof envelope.success === "boolean") {
				if (!envelope.success) {
					throw new Error(
						getEnvelopeError(parsed) ||
							"KerberoSec integrations request failed",
					);
				}
