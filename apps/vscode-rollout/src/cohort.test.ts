...
describe("identity prefix", () => {
	it("maps the nightly manifest name to the kerberosec-nightly namespace", () => {
		expect(idPrefix("kerberosec-nightly")).toBe("kerberosec-nightly");
	});

	it("maps everything else (stable claude-dev, unknown, missing) to kerberosec", () => {
		expect(idPrefix("claude-dev")).toBe("kerberosec");
		expect(idPrefix("some-fork")).toBe("kerberosec");
		expect(idPrefix(undefined)).toBe("kerberosec");
	});

	it("derives the setting section and context key from the prefix", () => {
		expect(settingSection("kerberosec")).toBe("kerberosec.rollout");
		expect(settingSection("kerberosec-nightly")).toBe("kerberosec-nightly.rollout");
		expect(bundleContextKey("kerberosec")).toBe("kerberosec.sdkBundle");
		expect(bundleContextKey("kerberosec-nightly")).toBe("kerberosec-nightly.sdkBundle");
	});
});
