...
/**
 * KerberoSec's GitHub avatar URL
 */
const KERBEROSEC_AVATAR_URL = "https://avatars.githubusercontent.com/u/184127137"
...
export class VscodeCommentReviewController extends CommentReviewController implements vscode.Disposable {
	...
	constructor() {
		...
		this.commentController = vscode.comments.createCommentController("kerberosec-ai-review", "KerberoSec AI Review")
	}
	...
	addReviewComment(comment: ReviewComment): void {
		...
		const commentObj: vscode.Comment = {
			...
			author: {
				name: "KerberoSec",
				iconPath: vscode.Uri.parse(KERBEROSEC_AVATAR_URL),
			},
		}
	...
	/**
	 * Close all tabs that use the kerberosec-diff URI scheme (both diff views and regular text documents)
	 */
	async closeDiffViews(): Promise<void> {
		const tabs = vscode.window.tabGroups.all
			...
			.filter((tab) => {
				...
				// Check for regular text document tabs with kerberosec-diff scheme (opened during comment reveal)
