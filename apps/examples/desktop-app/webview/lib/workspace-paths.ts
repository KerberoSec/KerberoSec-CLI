import { isChatWorkspacePath } from "@kerberosec/shared/browser";
...
/**
 * Sessions can run anywhere (KerberoSec-internal worktrees and plugin installs
 * under `.kerberosec`, or a shell's default cwd like the home or Desktop
...
export function isExcludedWorkspacePath(path: string): boolean {
	...
	if (normalized.split(/[\\/]/).includes(".kerberosec")) {
		return true;
	}
	return (
