...
import * as Llms from "@kerberosec/llms";
import {
...
async function mergeKnownModels(
	...
	if (providerId === "kerberosec-pass" && Object.keys(liveModels).length > 0) {
	...
	if (providerId === "kerberosec") {
		// KerberoSec recommendations can use Vercel-style ids while the broader
		// catalog includes OpenRouter aliases for the same models. Image-output
		// models are temporarily unavailable through KerberoSec's inference backend,
		// so filter them only at the KerberoSec catalog boundary.
		return Llms.sortModelsByReleaseDate(
...
function resolveCatalogModels(
	...
	// KerberoSec uses OpenRouter-backed catalog models, so live catalog lookups must
