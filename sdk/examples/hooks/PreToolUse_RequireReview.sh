#!/usr/bin/env bash
# KerberoSec Hook: PreToolUse (Require Review)
# Pauses for review before writing to critical files
# Copy to ~/.kerberosec/hooks/PreToolUse.sh and chmod +x
...
if [ "$tool" = "editor" ] || [ "$tool" = "write_file" ]; then
  if [[ $file =~ (package\.json|tsconfig|\.env|secrets|config\.(ts|js)|\.kerberosec) ]]; then
