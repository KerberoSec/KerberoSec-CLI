# [experimental] @kerberosec/shared
...
`@kerberosec/shared` owns shared cross-package primitives (session common types/utilities).

Node-only filesystem path resolvers live under the storage subpath export:

- `@kerberosec/shared/storage`
- examples: `resolveKerberoSecDataDir`, `resolveDbDataDir`, `resolveSessionDataDir`, `resolveTeamDataDir`
...
(`@kerberosec/cli`, `@kerberosec/code`) so request/response contracts are not duplicated
...
- KerberoSec account action payloads (`KerberoSecAccountActionRequest`)
