export const KERBEROSEC_RUN_AS_HUB_DAEMON_ENV = "KERBEROSEC_RUN_AS_HUB_DAEMON";
export const KERBEROSEC_CONNECTOR_CLI_LAUNCH_ENV =
	"KERBEROSEC_CONNECTOR_CLI_LAUNCH";
export const KERBEROSEC_CONNECTOR_STARTING_INSTANCE_ENV =
	"KERBEROSEC_CONNECTOR_STARTING_INSTANCE";
export const KERBEROSEC_CONNECTOR_SUPERVISED_ENV =
	"KERBEROSEC_CONNECTOR_SUPERVISED";
 ...
 * on EADDRINUSE against the real hub. Observed as every `kerberosec` invocation from
 * a Slack connector agent failing, `kerberosec --help` included, because the
...
export function claimHubDaemonProcess(
	...
	claimedHubDaemonProcess = env[KERBEROSEC_RUN_AS_HUB_DAEMON_ENV] === "1";
	delete env[KERBEROSEC_RUN_AS_HUB_DAEMON_ENV];
...
export function isHubDaemonProcess(
	...
	if (env) {
		return env[KERBEROSEC_RUN_AS_HUB_DAEMON_ENV] === "1";
	}
	return (
...
export function setConnectorCliLaunchSpec(
	...
	env[KERBEROSEC_CONNECTOR_CLI_LAUNCH_ENV] = JSON.stringify(spec);
}
 ...
 * — inherits its environment. An inherited marker makes a nested `kerberosec connect`
...
export function claimSupervisedConnectorProcess(
	...
	claimedSupervisedConnectorProcess =
		env[KERBEROSEC_CONNECTOR_SUPERVISED_ENV] === "1";
	delete env[KERBEROSEC_CONNECTOR_SUPERVISED_ENV];
...
export function isSupervisedConnectorProcess(
	...
	if (env) {
		return env[KERBEROSEC_CONNECTOR_SUPERVISED_ENV] === "1";
	}
	return (
		claimedSupervisedConnectorProcess ??
...
export function setStartingConnectorInstance(
	...
	env[KERBEROSEC_CONNECTOR_STARTING_INSTANCE_ENV] = JSON.stringify(ref);
}

export function readStartingConnectorInstance(
	env: Record<string, string | undefined> = process.env,
): ConnectorInstanceRef | undefined {
	const raw = env[KERBEROSEC_CONNECTOR_STARTING_INSTANCE_ENV];
	if (!raw) {
		return undefined;
	}
	try {
...
export function readConnectorCliLaunchSpec(
	...
	const raw = env[KERBEROSEC_CONNECTOR_CLI_LAUNCH_ENV];
	if (!raw) {
		return undefined;
	}
	try {
