...
describe("chat commands", () => {
	it("shows connector help for /help and /start", async () => {
		for (const { command, botUserName } of [
			{ command: "/help" },
			{ command: "/start" },
			{ command: "/help@kerberosecbot", botUserName: "kerberosecbot" },
			{ command: "/start@kerberosec_bot", botUserName: "@kerberosec_bot" },
		]) {
			...
			expect(reply).toHaveBeenCalledWith(
				expect.stringContaining("KerberoSec connector commands:"),
			);
			expect(reply).toHaveBeenCalledWith(
	...
	it("requires a bot suffix when requested", async () => {
		...
		expect(await maybeHandleChatCommand("/help@kerberosecbot", context)).toBe(
			true,
		);
		expect(reply).toHaveBeenCalledWith(
			expect.stringContaining("KerberoSec connector commands:"),
	...
	it("detects commands addressed to the configured bot", () => {
		expect(isCommandAddressedToBot("/new@kerberosecbot", "kerberosecbot")).toBe(
			true,
		);
		expect(
			isCommandAddressedToBot("/new@kerberosec_bot", "@kerberosec_bot"),
		).toBe(true);
		expect(
			isCommandAddressedToBot("/new@kerberosec.bot", "kerberosec.bot"),
		).toBe(true);
		expect(
			isCommandAddressedToBot("/new@kerberosec-bot", "kerberosec-bot"),
		).toBe(true);
		expect(isCommandAddressedToBot("/new", "kerberosecbot")).toBe(false);
		expect(isCommandAddressedToBot("/new@otherbot", "kerberosecbot")).toBe(
			false,
		);
		expect(isCommandAddressedToBot("/new@kerberosecbot", undefined)).toBe(
			false,
		);
	});

	it("normalizes commands addressed to dotted and hyphenated bot names", () => {
		expect(normalizeCommandName("/new@kerberosec.bot", "kerberosec.bot")).toBe(
			"/new",
		);
		expect(normalizeCommandName("/new@kerberosec-bot", "kerberosec-bot")).toBe(
			"/new",
	...
	it("leaves bot-suffixed commands unmatched without a known bot username", async () => {
		...
		const handled = await maybeHandleChatCommand("/help@kerberosecbot", {
			enabled: true,
			getState: async () => ({
				enableTools: true,
				autoApproveTools: false,
