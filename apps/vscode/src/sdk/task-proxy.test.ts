...
describe("MessageStateHandler", () => {
	it("should start with empty messages", () => {
		...
		expect(handler.getKerberoSecMessages()).toEqual([])
	})
	...
	it("should add and retrieve messages", () => {
		...
		expect(handler.getKerberoSecMessages()).toHaveLength(2)
		expect(handler.getKerberoSecMessages()[0].text).toBe("hello")
		expect(handler.getKerberoSecMessages()[1].text).toBe("tool call")
	})

	it("should return a copy of messages", () => {
		const handler = new MessageStateHandler()
		handler.addMessages([{ ts: 1, type: "say", say: "text", text: "hello", partial: false }])
		const copy = handler.getKerberoSecMessages()
		copy.push({ ts: 2, type: "say", say: "text", text: "extra", partial: false })
		expect(handler.getKerberoSecMessages()).toHaveLength(1)
	})
...
describe("createTaskProxy", () => {
	...
	it("should provide a messageStateHandler", () => {
		...
		expect(proxy.messageStateHandler.getKerberoSecMessages()).toEqual([])
	})
	...
	it("should accumulate messages in messageStateHandler", () => {
		...
		expect(proxy.messageStateHandler.getKerberoSecMessages()).toHaveLength(1)
	})
})
