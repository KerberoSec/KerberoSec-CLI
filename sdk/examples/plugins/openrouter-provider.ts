 ...
 * run inference against it, no changes to KerberoSec itself required.
 *
 * This example registers an OpenAI-compatible provider pointed at OpenRouter.
 * OpenRouter ships as a built-in provider in KerberoSec already, so this example
 ...
 * (a self-hosted LiteLLM/vLLM gateway, an internal proxy, a niche provider KerberoSec
 * does not bundle, etc.).
 *
 * How it works:
...
import { type AgentPlugin, Llms } from "@kerberosec/core";
