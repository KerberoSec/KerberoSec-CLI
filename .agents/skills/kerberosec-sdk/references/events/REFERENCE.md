...
The KerberoSec SDK has three event layers. Which one you use depends on whether you're working with the standalone `Agent` class or `KerberoSecCore`.
...
| `KerberoSecCore` | `kerberosec.subscribe()` | `CoreSessionEvent` | `chunk` (with `payload.type === "text"`) |
...
## Layer 2: AgentEvent (KerberoSecCore Internal)

When using `KerberoSecCore`, a `RuntimeEventAdapter` translates Layer 1 events into a legacy format called `AgentEvent`. You do not interact with this layer directly -- it is projected into `CoreSessionEvent` for subscribers. The key mappings:
...
## Layer 3: CoreSessionEvent (KerberoSecCore Subscriber)

Emitted by `KerberoSecCore` via `kerberosec.subscribe()`. These are higher-level session events.
...
```typescript
kerberosec.subscribe(handler, { sessionId: "specific-session-id" })
```

## Hub Events (Layer 3b)

When KerberoSecCore runs in hub mode (via `backendMode: "hub"` or `"auto"` when a hub is available), events are projected over WebSocket using `HubEventName` types like `assistant.delta`, `iteration.started`, `tool.started`, etc. You do not interact with these directly -- `kerberosec.subscribe()` still gives you `CoreSessionEvent` regardless of backend mode.

## Result Type Differences

The standalone Agent and KerberoSecCore return different result types:

| API | Result type | Text property |
|---|---|---|
| `agent.run()` | `AgentRunResult` | `result.outputText` |
| `kerberosec.start()` / `kerberosec.send()` | `AgentResult` | `result.text` |
...
- `../kerberoseccore/REFERENCE.md` - KerberoSecCore session management
