...
function listMatchingProcesses(pattern: string): ProcessRecord[] {
	...
	// (e.g. the "--kerberosec-hub-daemon" marker) are treated as patterns.
...
function resolveCliLogPath(): string {
	...
	return join(resolveKerberoSecDataDir(), "logs", `${name}.log`);
}
...
export async function runDoctorCommand(
	...
	if (!fix) {
		...
		if (
			...
			io.writeln(
				"\nRun `kerberosec doctor fix` to kill all stale local processes, including stale sidecars.",
			);
		}
		return 0;
	}
