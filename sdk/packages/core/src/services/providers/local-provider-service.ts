...
async function resolveProviderModelMap(
	...
	const isKerberoSecPass = providerId === KERBEROSEC_PASS_PROVIDER_ID;
	if (!config && !isKerberoSecPass) {
	...
	const resolved = await resolveProviderConfig(
		providerId,
		{
			loadLatestOnInit: isKerberoSecPass,
	...
	if (isKerberoSecPass && resolved?.knownModels) {
		return resolved.knownModels;
	}
...
export async function listLocalProviders(
	...
	const featuredData = peekKerberoSecRecommendedModels();
	...
	const providerEntries = await Promise.all(
		ids.map(
			async (id): Promise<{ provider: ProviderListItem; rank: number }> => {
				...
				const modelList = applyKerberoSecFeaturedModels(
					id,
	...
	if (options.isKerberoSecPassEnabled !== true) {
		providers = providers.filter(
			(provider) => provider.id !== KERBEROSEC_PASS_PROVIDER_ID,
		);
	}
...
export async function getLocalProviderModels(
	...
	if (id === KERBEROSEC_PROVIDER_ID || id === KERBEROSEC_PASS_PROVIDER_ID) {
		...
		models = applyKerberoSecFeaturedModels(
			id,
			models,
...
export function resolveLocalKerberoSecAuthToken(
	settings: ProviderSettings | undefined,
): string | undefined {
