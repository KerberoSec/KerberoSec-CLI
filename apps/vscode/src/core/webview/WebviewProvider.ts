...
import { KerberoSecExtensionContext } from "@/shared/kerberosec"
		...
		return /*html*/ `
			...
			<html lang="en">
				...
				<meta http-equiv="Content-Security-Policy" content="default-src 'none';
					connect-src https://*.posthog.com https://*.kerberosec.bot; 
					font-src ${this.getCspSource()} data:; 
					style-src ${this.getCspSource()} 'unsafe-inline'; 
					img-src ${this.getCspSource()} https: data:; 
					script-src 'nonce-${nonce}' 'unsafe-eval';">
				<title>KerberoSec</title>
			</head>
			<body>
		...
		try {
			...
			if (process.env.IS_DEV) {
				HostProvider.window.showMessage({
					...
					message:
						"KerberoSec: Local webview dev server is not running, HMR will not work. Please run 'bun run dev:webview' before launching the extension to enable HMR. Using bundled assets.",
				})
			}
