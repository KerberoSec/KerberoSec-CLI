...
async function requestDesktopToolApprovalFromCore(
	...
	if (!cachedDesktopApprovalRequester) {
		cachedDesktopApprovalRequester = import("@kerberosec/core")
			.then((module) => {
				const fn = (
					module as {
				...
				if (typeof fn !== "function") {
					throw new Error(
						"Installed @kerberosec/core does not expose requestDesktopToolApproval",
					);
				}
				return fn;
			})
	...
	const approvalDir = process.env.KERBEROSEC_TOOL_APPROVAL_DIR?.trim();
...
export async function requestToolApproval(
	...
	const mode = process.env.KERBEROSEC_TOOL_APPROVAL_MODE?.trim().toLowerCase();
	if (mode === "desktop") {
