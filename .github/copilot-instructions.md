# Copilot Instructions for KerberoSec

This is a VS Code extension. Read `.kerberosecrules/general.md` for tribal knowledge and nuanced patterns.
...
1. **Define** in `proto/kerberosec/*.proto`. Naming: `PascalCaseService`, `camelCase` RPCs, `PascalCase` Messages. Use `common.proto` shared types for simple data.
...
- Adding enums (e.g. `KerberoSecSay`) → also update `src/shared/proto-conversions/kerberosec-message.ts`.
...
1. `proto/kerberosec/models.proto` — add to `ApiProvider` enum.
...
1. Add enum to `KerberoSecDefaultTool` in `src/shared/tools.ts`.
...
6. If tool has UI: add `KerberoSecSay` enum in proto → `ExtensionMessage.ts` → `kerberosec-message.ts` → `ChatRow.tsx`.
