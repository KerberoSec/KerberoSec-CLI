...
import {
	fetchKerberoSecRecommendedModelsPayload,
	normalizeKerberoSecRecommendedProviderModels,
} from "./catalog-kerberosec-recommended";
import {
...
export async function fetchLiveProviderModels(
	...
	const [providerModels, kerberosecRecommendedPayload] = await Promise.all([
		fetchModelsDevProviderModels(modelsDevUrl, fetcher).catch(
			() => emptyProviderModels,
		),
		fetchKerberoSecRecommendedModelsPayload(fetcher).catch(() => undefined),
	]);
	const kerberosecRecommended = kerberosecRecommendedPayload
		? normalizeKerberoSecRecommendedProviderModels(
				kerberosecRecommendedPayload,
