...
describe("useChatSession", () => {
	...
	it("starts without a selected workspace and adopts the SDK temporary path", async () => {
		...
		invokeMock.mockImplementation(
			async (command: string, args?: Record<string, unknown>) => {
				...
				if (request?.action === "start") {
					...
					const workspacePath = "/home/host/.kerberosec/data/workspaces/chat";
					return {
						sessionId,
						cwd: workspacePath,
		...
		const expectedWorkspacePath = "/home/host/.kerberosec/data/workspaces/chat";
	...
	it("resets to the remembered provider/model after viewing a historical session", async () => {
		window.localStorage.setItem(
			...
			JSON.stringify({
				...
				lastModelByProvider: { kerberosec: "remembered-model" },
			}),
		);
	...
	it("falls back to process context when the remembered workspace is stale", async () => {
		await act(async () => root.unmount());
		window.localStorage.setItem(
			"kerberosec.code.workspace-selection.v1",
			JSON.stringify({
		...
		await vi.waitFor(() => {
			expect(current.config.workspaceRoot).toBe("/workspace/kerberosec");
			expect(current.config.cwd).toBe("/workspace/kerberosec");
		});
