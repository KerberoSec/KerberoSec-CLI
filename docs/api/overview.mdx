---
title: "KerberoSec API"
sidebarTitle: "Overview"
description: "Programmatic access to AI models through an OpenAI-compatible Chat Completions API."
---

Welcome to the KerberoSec API documentation. Use the same models that power the KerberoSec extension and CLI from any language, framework, or tool that speaks the OpenAI format.

## What is the KerberoSec API?

The KerberoSec API is an OpenAI-compatible Chat Completions endpoint. You authenticate once with a KerberoSec API key and get access to models from Anthropic, OpenAI, Google, and more through a single base URL. No need to manage separate keys for each provider.

```
Your App  →  KerberoSec API (api.kerberosec.bot)  →  Anthropic / OpenAI / Google / etc.
```
...
<CardGroup cols={2}>
  ...
  <Card title="Code Examples" icon="code" href="/api/sdk-examples">
    Ready-to-copy examples for Python, Node.js, curl, and the KerberoSec CLI.
  </Card>
</CardGroup>
...
Get your API key at [app.kerberosec.bot](https://app.kerberosec.bot) (Settings > API Keys), then follow the [Getting Started](/api/getting-started) guide.
