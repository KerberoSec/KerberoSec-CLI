...
export async function synchronizeRuleToggles(
	...
	currentToggles: KerberoSecRulesToggles,
	allowedFileExtension = "",
	excludedPaths: string[][] = [],
): Promise<KerberoSecRulesToggles> {
...
export function synchronizeRemoteRuleToggles(
	...
	currentToggles: KerberoSecRulesToggles,
): KerberoSecRulesToggles {
	const updatedToggles: KerberoSecRulesToggles = {}
 ...
 */
export function combineRuleToggles(toggles1: KerberoSecRulesToggles, toggles2: KerberoSecRulesToggles): KerberoSecRulesToggles {
 ...
 */
const getRuleFilesTotalContent = async (rulesFilePaths: string[], basePath: string, toggles: KerberoSecRulesToggles) => {
	return (await getRuleFilesTotalContentWithMetadata(rulesFilePaths, basePath, toggles)).content
}

const LOCAL_RULE_PATHS = {
	kerberosecRules: ".kerberosecrules",
	workflows: ".kerberosecrules/workflows",
} as const
...
type RuleFileController = {
	stateManager: {
		getGlobalSettingsKey(key: "globalWorkflowToggles" | "globalKerberoSecRulesToggles"): KerberoSecRulesToggles
		setGlobalState(key: "globalWorkflowToggles" | "globalKerberoSecRulesToggles", value: KerberoSecRulesToggles): void
 ...
 * Used by getGlobalKerberoSecRules and getLocalKerberoSecRules.
 */
...
export const getRuleFilesTotalContentWithMetadata = async (
	...
	basePath: string,
	toggles: KerberoSecRulesToggles,
...
function getRemoteRulesTotalContentWithMetadata(
	remoteRules: GlobalInstructionsFile[],
	remoteToggles: KerberoSecRulesToggles,
...
/**
 * Handles converting any directory into a file (specifically used for .kerberosecrules and .kerberosecrules/workflows)
 * The old .kerberosecrules file or .kerberosecrules/workflows file will be renamed to a default filename
 * Doesn't do anything if the dir already exists or doesn't exist
 * Returns whether there are any uncaught errors
 */
async function ensureLocalKerberoSecDirExists(kerberosecrulePath: string, defaultRuleFilename: string): Promise<boolean> {
	try {
		const exists = await fileExistsAtPath(kerberosecrulePath)

		if (exists && !(await isDirectory(kerberosecrulePath))) {
			// logic to convert .kerberosecrules file into directory, and rename the rules file to {defaultRuleFilename}
			const content = await fs.readFile(kerberosecrulePath, "utf8")
			const tempPath = kerberosecrulePath + ".bak"
			await fs.rename(kerberosecrulePath, tempPath) // create backup
			try {
				await fs.mkdir(kerberosecrulePath, { recursive: true })
				await fs.writeFile(path.join(kerberosecrulePath, defaultRuleFilename), content, "utf8")
				...
				try {
					await fs.rm(kerberosecrulePath, { recursive: true, force: true }).catch(() => {})
					await fs.rename(tempPath, kerberosecrulePath) // restore backup
				} catch (_restoreError) {}
...
export const createRuleFile = async (isGlobal: boolean, filename: string, cwd: string, type: string) => {
	try {
		...
		if (isGlobal) {
			...
			if (type === "workflow") {
				const globalKerberoSecWorkflowFilePath = await disk.ensureWorkflowsDirectoryExists()
				filePath = path.join(globalKerberoSecWorkflowFilePath, filename)
			} else {
				const globalKerberoSecRulesFilePath = await disk.ensureRulesDirectoryExists()
				filePath = path.join(globalKerberoSecRulesFilePath, filename)
			}
		} else {
			const localKerberoSecRulesFilePath = path.resolve(cwd, LOCAL_RULE_PATHS.kerberosecRules)

			const hasError = await ensureLocalKerberoSecDirExists(localKerberoSecRulesFilePath, "default-rules.md")
			if (hasError === true) {
				return { filePath: null, fileExists: false }
			}

			await fs.mkdir(localKerberoSecRulesFilePath, { recursive: true })

			if (type === "workflow") {
				const localWorkflowsFilePath = path.resolve(cwd, LOCAL_RULE_PATHS.workflows)

				const hasError = await ensureLocalKerberoSecDirExists(localWorkflowsFilePath, "default-workflows.md")
				if (hasError === true) {
					return { filePath: null, fileExists: false }
				}
				...
				// kerberosecrules file creation
				filePath = path.join(localKerberoSecRulesFilePath, filename)
			}
		}
