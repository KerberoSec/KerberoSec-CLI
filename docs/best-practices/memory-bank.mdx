...
description: "A structured documentation system that helps KerberoSec maintain context across sessions."
---

Memory Bank is a documentation methodology that transforms KerberoSec from a stateless assistant into a persistent development partner. Through structured markdown files, KerberoSec can "remember" your project details across sessions.

## Quick Setup

1. Copy the [custom instructions below](#memory-bank-custom-instructions)
2. Add them to a [KerberoSec Rules file](/customization/kerberosec-rules), such as `.kerberosecrules/memory-bank.md`
3. Ask KerberoSec to "initialize memory bank"

## How It Works

Memory Bank files are regular markdown files in your project that both you and KerberoSec can access. They're organized hierarchically to build a complete picture of your project:
...
<Frame>
	<img src="https://storage.googleapis.com/kerberosec_public_images/docs/assets/image%20(16).png" alt="Memory Bank file hierarchy showing projectbrief.md at the top flowing into productContext, systemPatterns, and techContext, which feed into activeContext and progress" />
</Frame>
...
- **"follow your custom instructions"** - Tells KerberoSec to read Memory Bank and continue where you left off
...
These work alongside KerberoSec's built-in [slash commands](/core-workflows/using-commands). In particular, [`/newtask`](/core-workflows/using-commands#newtask) and [`/smol`](/core-workflows/using-commands#smol) help you manage context windows without losing progress.
...
1. Ask KerberoSec to "update memory bank" to document the current state
2. Start a new conversation
3. Ask KerberoSec to "follow your custom instructions"
...
- Let KerberoSec help create the initial structure
...
- Use [KerberoSec Rules](/customization/kerberosec-rules) to store the Memory Bank instructions per project

---

## Memory Bank Custom Instructions

Copy this into a KerberoSec Rules file (for example, `.kerberosecrules/memory-bank.md`) or your global custom instructions:

```markdown
# KerberoSec's Memory Bank
...
**Custom instructions or KerberoSec Rules?**
Either works. Custom instructions apply globally across all projects. A [KerberoSec Rules file](/customization/kerberosec-rules) is project-specific and stored in your repo, which makes it easy to share with collaborators. You can also use [conditional rules](/customization/kerberosec-rules#conditional-rules) to activate Memory Bank instructions only when working with `memory-bank/` files.
