...
import { getProviderCollectionSync } from "@kerberosec/llms"
...
describe("Provider key mapping", () => {
	...
	it("returns the openrouter default for the openrouter-routed providers", () => {
		...
		expect(getProviderDefaultModelId("kerberosec")).to.equal(openrouterDefault)
	...
	it("uses provider-specific model key behavior for KerberoSec", () => {
		expect(getProviderModelIdKey("kerberosec", "act")).to.equal("actModeKerberoSecModelId")
		expect(getProviderModelIdKey("kerberosec", "plan")).to.equal("planModeKerberoSecModelId")
	})

	it("uses separate model keys for KerberoSecPass", () => {
		expect(getProviderModelIdKey("kerberosec-pass", "act")).to.equal("actModeKerberoSecPassModelId")
		expect(getProviderModelIdKey("kerberosec-pass", "plan")).to.equal("planModeKerberoSecPassModelId")
	})
