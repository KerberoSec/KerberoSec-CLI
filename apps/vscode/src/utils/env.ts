 ...
 *
 * When KERBEROSEC_CAPTURE_BROWSER is set (debug harness mode), the URL is captured
...
async function captureBrowserUrl(url: string): Promise<void> {
	...
	// Write to JSONL file in KERBEROSEC_DIR/data/
	try {
		const fs = await import("node:fs")
		const path = await import("node:path")
		const os = await import("node:os")
		const kerberosecDir = process.env.KERBEROSEC_DIR || path.join(os.homedir(), ".kerberosec")
		const dataDir = path.join(kerberosecDir, "data")
	...
	const harnessPort = process.env.KERBEROSEC_DEBUG_HARNESS_PORT
	if (harnessPort) {
		try {
