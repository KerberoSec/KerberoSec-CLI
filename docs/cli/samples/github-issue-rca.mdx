...
description: "Automated GitHub issue analysis using KerberoSec CLI to identify root causes."
---

Automated GitHub issue analysis using KerberoSec CLI. This script uses KerberoSec's autonomous AI capabilities to fetch, analyze, and identify root causes of GitHub issues, outputting clean, parseable results that can be easily integrated into your development workflows.

<Note>
**New to KerberoSec CLI?** This sample assumes you have already completed the [Installation Guide](https://docs.kerberosec.bot/getting-started/installing-kerberosec) and authenticated with `kerberosec auth`. If you haven't set up KerberoSec CLI yet, please start there first.
</Note>

<Frame>
  <img src="https://storage.googleapis.com/kerberosec_public_images/cli-rca.gif" alt="CLI Root Cause Analysis Demo" width="600" />
</Frame>
...
- **KerberoSec CLI** installed and authenticated ([Installation Guide](https://docs.kerberosec.bot/getting-started/installing-kerberosec))
- **At least one AI model provider** configured (e.g., OpenRouter, Anthropic, OpenAI)
- **Basic familiarity** with KerberoSec CLI commands
...
```bash
curl -O https://raw.githubusercontent.com/kerberosec/kerberosec/main/src/samples/cli/github-issue-rca/analyze-issue.sh
```
...
<Note>
The script will automatically handle everything: fetching the issue, analyzing it with KerberoSec, and displaying the results. The analysis typically takes 30-60 seconds depending on the issue complexity.
</Note>
...
**1. `kerberosec --auto-approve true --json "$PROMPT: $ISSUE_URL"`**
   - `kerberosec` is the KerberoSec CLI binary
...
This sample demonstrates how to build an autonomous GitHub issue analysis tool using KerberoSec CLI:

1. **Building autonomous CLI tools** using KerberoSec's capabilities
2. **Parsing structured JSON output** from KerberoSec CLI
...
- [CLI Installation Guide](https://docs.kerberosec.bot/getting-started/installing-kerberosec)
- [CLI Reference Documentation](https://docs.kerberosec.bot/cli/cli-reference)
- [Headless Mode](https://docs.kerberosec.bot/usage/cli-overview#headless-mode)
