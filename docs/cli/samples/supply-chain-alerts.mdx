...
description: "Schedule the KerberoSec CLI to scan your machine for compromised packages with Bumblebee and text you on Telegram when it finds one."
---
...
- The KerberoSec CLI scheduler, which runs an agent on a cron schedule.
- The KerberoSec CLI Telegram connector, which delivers the result to a chat.

The end result: every morning, a KerberoSec agent pulls the latest threat intelligence, runs a read-only scan, and texts you a green check if you are clean or a red alert with details if you are exposed.

```mermaid
flowchart TD
    cron["kerberosec schedule (daily)"] --> agent["KerberoSec agent"]
...
- Node.js 22 or newer (for the KerberoSec CLI).
- Go 1.22 or newer (to build Bumblebee).
- A Telegram account.
- An AI provider key, or a KerberoSec account.

## 1. Install the KerberoSec CLI

```bash
npm install -g kerberosec
kerberosec # run once to configure inference provider and model
```
...
<Steps>
  ...
  <Step title="Start the connector">
    ...
    ```bash
    kerberosec connect telegram -k "<BOT-TOKEN>" --cwd ~/tools/bumblebee
    ```
...
Create the schedule with the KerberoSec CLI on the same machine and pass the delivery method explicitly. The running Telegram connector delivers the result to its chat:

```bash
kerberosec schedule create "supply-chain-watch" \
  --cron "0 8 * * *" \
...
- The connector process (`kerberosec connect telegram`) must stay running for delivery to work. Run it under a process manager (systemd, launchd, `pm2`, or a `tmux`/`screen` session) so it survives reboots.
- The hub runs the schedule and starts automatically when you create one. If it is not running, start it with `kerberosec hub start`.
- Manage schedules anytime with `kerberosec schedule list`, `kerberosec schedule pause <id>`, `kerberosec schedule resume <id>`, and `kerberosec schedule delete <id>`.
