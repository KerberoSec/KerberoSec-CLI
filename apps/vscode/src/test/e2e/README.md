...
This directory contains the end-to-end tests for the KerberoSec VS Code extension using Playwright. These tests simulate user interactions with the extension in a real VS Code environment.
...
- **`fixtures/server/`** - Mock API server for testing KerberoSec's backend interactions
...
e2e("Test description", async ({ sidebar, helper, page }) => {
  ...
  await inputbox.fill("Hello, KerberoSec!")
...
- **`sidebar`** - Playwright Frame object for the KerberoSec extension's sidebar
...
await addSelectedCodeToKerberoSecWebview(page)
```
...
2. **Playwright will open:**
   - A VS Code window with KerberoSec extension loaded
...
- **VS Code Configuration:**
  ...
  - Extension development mode with KerberoSec loaded
  - Temporary user data and extensions directories

- **Mock API Server:**
  - Runs on `http://localhost:7777`
  - Provides mock responses for KerberoSec API calls
...
- `KERBEROSEC_E2E_TESTS_VERBOSE=true` - Enable verbose logging
