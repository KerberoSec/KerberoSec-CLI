...
describe("composeAiSdkProviderOptions: Anthropic thinking precedence", () => {
	...
	runCases([
		...
		{
			name: "KerberoSec-routed Sonnet 4.5 -> gateway reasoning, no thinking, no effort",
			request: {
		...
		{
			// KerberoSecPass is served by the shared "kerberosec" AI SDK provider (same
			// KerberoSec API), which only reads the "kerberosec" providerOptions bucket.
			// Uses an explicit budget because effort-based reasoning is portable
			// and never reaches provider-option buckets.
			name: "KerberoSecPass-routed Sonnet 4.5 budget -> gateway reasoning under the shared kerberosec bucket",
			request: {
...
describe("composeAiSdkProviderOptions: family/provider thinking patches", () => {
	runCases([
		...
		{
			name: "kerberosec GLM thinking-disabled -> routed reasoning only, no thinking leak",
			request: {
				providerId: "kerberosec",
				modelId: "z-ai/glm-4.7",
				reasoning: { enabled: false },
		...
		{
			name: "kerberosec Kimi K2.6 family reasoning.enabled=false -> thinking.type=disabled",
			request: {
				providerId: "kerberosec",
			...
			expect: [
				{ bucket: "kerberosec", has: { thinking: { type: "disabled" } } },
				{
					bucket: "openaiCompatible",
					has: { thinking: { type: "disabled" } },
				},
		...
		{
			name: "kerberosec Kimi K2.6 family reasoning.enabled=true -> thinking.type=enabled",
			request: {
				providerId: "kerberosec",
				modelId: "moonshotai/kimi-k2.6",
				reasoning: { enabled: true },
			...
			context: { family: "kimi-k2.6" },
			expect: [
				{ bucket: "kerberosec", has: { thinking: { type: "enabled" } } },
				{ bucket: "openaiCompatible", has: { thinking: { type: "enabled" } } },
			],
		},
		{
			name: "kerberosec generic reasoning.enabled=false -> gateway reasoning only, no thinking patch",
			request: {
				providerId: "kerberosec",
				modelId: "gpt-5.4",
				reasoning: { enabled: false },
		...
		{
			name: "kerberosec non-K2.6 Moonshot Kimi reasoning.enabled=false -> thinking.type=disabled",
			request: {
				providerId: "kerberosec",
		...
		{
			name: "kerberosec qwen prompt-cache-only route reasoning.enabled=true -> cache control, no gateway reasoning",
			request: {
				providerId: "kerberosec",
		...
		{
			name: "kerberosec unregistered qwen reasoning.enabled=true -> no gateway reasoning",
			request: {
				providerId: "kerberosec",
		...
		{
			name: "kerberosec Kimi K2.6 family reasoning.enabled=false also keeps gateway reasoning shape",
			request: {
				providerId: "kerberosec",
				modelId: "moonshotai/kimi-k2.6",
				reasoning: { enabled: false },
			...
			context: { family: "kimi-k2.6" },
			expect: [
				{ bucket: "kerberosec", has: { reasoning: { enabled: false } } },
			],
		},
		{
			name: "kerberosec Claude Fable omits an unadvertised disabled control",
			request: {
				providerId: "kerberosec",
		...
		{
			name: "kerberosec Claude Fable ignores an advertised toggle because reasoning is mandatory",
			request: {
				providerId: "kerberosec",
				modelId: "anthropic/claude-fable-5",
				reasoning: { enabled: false },
		...
		{
			name: "kerberosec StepFun 3.7 Flash reasoning.enabled=false omits disabled reasoning",
			request: {
				providerId: "kerberosec",
		...
		{
			name: "kerberosec StepFun 3.7 Flash variants reasoning.enabled=false omit disabled reasoning",
			request: {
				providerId: "kerberosec",
		...
		{
			name: "kerberosec MiniMax M3 reasoning enabled -> gateway reasoning without thinking leak",
			request: {
				providerId: "kerberosec",
				modelId: "minimax/minimax-m3",
				reasoning: { enabled: true, effort: "high" },
		...
		{
			name: "kerberosec MiniMax M3 reasoning disabled -> gateway reasoning disabled",
			request: {
				providerId: "kerberosec",
				modelId: "minimax/minimax-m3",
				reasoning: { enabled: false },
...
describe("composeAiSdkProviderOptions: KerberoSecPass bucket normalization", () => {
	it("keys KerberoSecPass options to the shared kerberosec bucket only", () => {
		const result = composeAiSdkProviderOptions(
			makeRequest({
				providerId: "kerberosec-pass",
				modelId: "anthropic/claude-sonnet-4-5",
		...
		// The shared "kerberosec" AI SDK provider serves both gateway ids and only
		// reads the "kerberosec" providerOptions bucket, so nothing may be emitted
		// under the concrete "kerberosec-pass" id or its camelCase alias.
		expect(result.kerberosec).toEqual(
			expect.objectContaining({
				reasoning: { enabled: true, max_tokens: 2048 },
			}),
		);
		expect(result).not.toHaveProperty("kerberosec-pass");
		expect(result).not.toHaveProperty("kerberosecPass");
	});
});
