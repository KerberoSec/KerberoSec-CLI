...
import { createKerberoSecAPI } from "./exports"
...
import { registerKerberoSecOutputChannel } from "./hosts/vscode/hostbridge/env/debugLog"
import {
...
export async function activate(context: vscode.ExtensionContext) {
	...
	// After this, all platforms (VSCode, CLI, JetBrains) read from ~/.kerberosec/data/.
	...
	const handleUri = async (uri: vscode.Uri) => {
		...
		if (isTaskUri) {
			await openKerberoSecSidebarForTaskUri()
		}
	...
	// `ext.evaluate`. Gated on KERBEROSEC_CAPTURE_BROWSER so it never ships in prod.
	if (process.env.KERBEROSEC_CAPTURE_BROWSER === "1" || process.env.KERBEROSEC_CAPTURE_BROWSER === "true") {
		;(globalThis as Record<string, unknown>).__kerberosecHandleUri = (url: string) => SharedUriHandler.handleUri(url)
	}

	// Register size testing commands in development mode
	if (IS_DEV) {
		vscode.commands.executeCommand("setContext", "kerberosec.isDevMode", IS_DEV)
		// Use dynamic import to avoid loading the module in production
		import("./dev/commands/tasks")
			.then((module) => {
				const devTaskCommands = module.registerTaskCommands(webview.controller)
				context.subscriptions.push(...devTaskCommands)
				Logger.log("[KerberoSec Dev] Dev mode activated & dev commands registered")
			})
			.catch((error) => {
				Logger.log(`[KerberoSec Dev] Failed to register dev commands: ${error}`)
			})
	}
		...
		vscode.languages.registerCodeActionsProvider(
			...
			new (class implements vscode.CodeActionProvider {
				...
				provideCodeActions(
					...
					// Add to KerberoSec (Always available)
					const addAction = new vscode.CodeAction("Add to KerberoSec", vscode.CodeActionKind.QuickFix)
					addAction.command = {
						command: commands.AddToChat,
						title: "Add to KerberoSec",
					}
					actions.push(addAction)

					// Explain with KerberoSec (Always available)
					const explainAction = new vscode.CodeAction("Explain with KerberoSec", vscode.CodeActionKind.RefactorExtract) // Using a refactor kind
					explainAction.command = {
						command: commands.ExplainCode,
						title: "Explain with KerberoSec",
					}
					actions.push(explainAction)

					// Improve with KerberoSec (Always available)
					const improveAction = new vscode.CodeAction("Improve with KerberoSec", vscode.CodeActionKind.RefactorRewrite) // Using a refactor kind
					improveAction.command = {
						command: commands.ImproveCode,
						title: "Improve with KerberoSec",
					}
					actions.push(improveAction)

					// Fix with KerberoSec (Only if diagnostics exist)
					if (context.diagnostics.length > 0) {
						const fixAction = new vscode.CodeAction("Fix with KerberoSec", vscode.CodeActionKind.QuickFix)
						fixAction.isPreferred = true
						fixAction.command = {
							command: commands.FixWithKerberoSec,
							title: "Fix with KerberoSec",
						}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.AddToChat, async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
			...
			await addToKerberoSec(context.controller, context.commandContext)
		}),
	)
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.FixWithKerberoSec, async (range: vscode.Range, diagnostics: vscode.Diagnostic[]) => {
			const context = await getContextForCommand(range, diagnostics)
			if (!context) {
				return
			}
			await fixWithKerberoSec(context.controller, context.commandContext)
		}),
	)
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.ExplainCode, async (range: vscode.Range) => {
			...
			await explainWithKerberoSec(context.controller, context.commandContext)
		}),
	)
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.ImproveCode, async (range: vscode.Range) => {
			...
			await improveWithKerberoSec(context.controller, context.commandContext)
		}),
	)
...
\`\`\``
				...
				await addToKerberoSec(ctx.controller, ctx.commandContext, notebookContext)
			},
		),
	)
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
				...
				await explainWithKerberoSec(ctx.controller, ctx.commandContext, notebookContext)
			},
		),
	)
...
\`\`\``
				...
				await improveWithKerberoSec(ctx.controller, ctx.commandContext, notebookContext)
			},
		),
	)
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.Walkthrough, async () => {
			await vscode.commands.executeCommand("workbench.action.openWalkthrough", `${context.extension.id}#KerberoSecWalkthrough`)
	...
	const unsubSecrets = storageContext.secrets.onDidChange((event) => {
		if (event.key === "kerberosec:kerberosecAccountId") {
	...
	Logger.log(`[KerberoSec] extension activated in ${performance.now() - activationStartTime} ms`)

	return createKerberoSecAPI(webview.controller)
}
...
function setupHostProvider(context: ExtensionContext) {
	const outputChannel = registerKerberoSecOutputChannel(context)
	outputChannel.appendLine("[KerberoSec] Setting up VS Code host...")
...
async function openKerberoSecSidebarForTaskUri(): Promise<void> {
	...
	Logger.warn("Task URI handling timed out waiting for KerberoSec sidebar visibility")
}
...
async function cleanupLegacyVSCodeStorage(context: ExtensionContext): Promise<void> {
	try {
		...
		// Migrate custom instructions to global KerberoSec rules (one-time cleanup)
