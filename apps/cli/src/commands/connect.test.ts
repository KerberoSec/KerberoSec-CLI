...
import {
	...
	KERBEROSEC_CONNECTOR_DETACHED_CHILD_ENV,
...
describe("runConnectAdapter", () => {
	const previousDetachedChild =
		process.env[KERBEROSEC_CONNECTOR_DETACHED_CHILD_ENV];
	const io: ConnectIo = {
		writeln: vi.fn(),
		writeErr: vi.fn(),
	};
	...
	beforeEach(() => {
		...
		mocks.run.mockImplementation(
			async (_args: string[], _io: ConnectIo, context: ConnectRunContext) => {
				context.setPersistenceInstanceId("kerberosec_bot");
				return 0;
			},
		);
	...
	afterEach(() => {
		if (previousDetachedChild === undefined) {
			delete process.env[KERBEROSEC_CONNECTOR_DETACHED_CHILD_ENV];
		} else {
			process.env[KERBEROSEC_CONNECTOR_DETACHED_CHILD_ENV] =
				previousDetachedChild;
		}
	});
	...
	it("leaves persistence unchanged when an internal detached child exits", async () => {
		process.env[KERBEROSEC_CONNECTOR_DETACHED_CHILD_ENV] = "1";
	...
	it("validates a replacement before stopping the active instance", async () => {
		...
		mocks.listActiveConnectors.mockReturnValue([
			{
				id: "telegram:kerberosec_bot",
				type: "telegram",
				instanceId: "kerberosec_bot",
				pid: 123,
				hubUrl: "ws://127.0.0.1:4317",
				botUsername: "kerberosec_bot",
			},
		]);
		mocks.getConnector.mockResolvedValue({
			name: "telegram",
	...
	it("restores the last successful launch when a replacement fails", async () => {
		...
		expect(stopInstance).toHaveBeenCalledWith("kerberosec_bot", io);
	...
	it("does not count an already-running instance as a successful replacement", async () => {
		...
		expect(io.writeErr).toHaveBeenCalledWith(
			"[connect] replacement was not started because telegram instance kerberosec_bot is still running",
		);
		expect(mocks.persistConnectorConnection).not.toHaveBeenCalled();
	});
});
...
describe("runCleanupConnectorInstance", () => {
	...
	it("reaps one instance without disabling its autostart", async () => {
		...
		await expect(
			runCleanupConnectorInstance("slack", "kerberosec-slack", io),
		).resolves.toBe(0);

		expect(stopInstance).toHaveBeenCalledWith("kerberosec-slack", io);
...
describe("hub-delegated connector starts", () => {
	...
	beforeEach(() => {
		...
		mocks.getConnector.mockResolvedValue({
			...
			validate: mocks.validate,
			showHelp: vi.fn(),
			resolveInstanceId: () => "kerberosec-slack",
		});
	...
	afterEach(() => {
		delete process.env.KERBEROSEC_CONNECTOR_SUPERVISED;
	});
	...
	it("runs locally inside a supervised process instead of asking the hub again", async () => {
		process.env.KERBEROSEC_CONNECTOR_SUPERVISED = "1";
