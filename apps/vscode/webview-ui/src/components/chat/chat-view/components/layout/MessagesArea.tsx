...
const WAITING_ROW: KerberoSecMessage = {
	ts: WAITING_ROW_TS,
	type: "say",
	say: "reasoning",
	partial: true,
...
interface MessagesAreaProps {
	task: KerberoSecMessage
	groupedMessages: (KerberoSecMessage | KerberoSecMessage[])[]
	modifiedMessages: KerberoSecMessage[]
...
export const MessagesArea: React.FC<MessagesAreaProps> = ({
	...
	const { kerberosecMessages, turnState } = useExtensionState()
	const lastRawMessage = useMemo(() => kerberosecMessages.at(-1), [kerberosecMessages])
	...
	const scrolledPastUserMessageIndex = useMemo(() => {
		...
		return kerberosecMessages.findIndex((msg) => msg.ts === scrolledPastUserMessage.ts)
	}, [kerberosecMessages, scrolledPastUserMessage])
	...
	const forcePendingResponseLoader = isPendingResponseUnconfirmed(chatState.pendingResponse, turnState, kerberosecMessages.length)
	...
	const displayedGroupedMessages = useMemo<(KerberoSecMessage | KerberoSecMessage[])[]>(() => {
