...
install_system_deps() {
    ...
    if [ "$OS" = "Linux" ]; then
        if command -v apt-get &>/dev/null; then
            ...
            echo -e "${YELLOW}⚠️ Unknown Linux package manager. Please ensure curl, git, and build tools are installed.${NC}"
        fi
    elif [ "$OS" = "Darwin" ]; then
...
if ! command -v git &>/dev/null || ! command -v curl &>/dev/null; then
    ...
    echo -e "${GREEN}✓ System essentials (git, curl) already present.${NC}"
fi

# ------------------------------------------------------------------------------
# 2. Check and Install Bun Runtime
# ------------------------------------------------------------------------------
echo -e "\n${YELLOW}⚡ Checking Bun JavaScript/TypeScript Runtime...${NC}"
...
if command -v bun &>/dev/null; then
    ...
    echo -e "${RED}❌ Failed to locate Bun. Please check your installation.${NC}"
    exit 1
fi
...
echo -e "\n${YELLOW}🏗️ Building KerberoSec Core SDK...${NC}"
bun run build:sdk
...
echo -e "  • ${CYAN}/logout${NC}          - Sign out and return to onboarding"
echo -e "  • ${CYAN}/model${NC}           - Switch LLM providers & models"
echo -e "  • ${CYAN}Tab${NC}              - Switch between Plan & Act modes"
