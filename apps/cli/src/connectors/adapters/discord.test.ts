...
import type { ConnectDiscordOptions } from "@kerberosec/shared";
...
describe("discordConnector", () => {
	...
	it("resolves outbound Discord mention names to user mention ids", async () => {
		const fetchMock = vi.fn(async (url: string | URL) => {
			expect(String(url)).toContain(
				"/guilds/guild-123/members/search?query=kerberosec-test-bot&limit=10",
			);
			return new Response(
				JSON.stringify([
					{
						nick: "kerberosec-test-bot",
						user: {
							id: "1509620637721821224",
							username: "kerberosectestbot",
							bot: true,
						},
					},
				]),
		...
		await expect(
			__test__.resolveDiscordOutboundMentions({
				...
				text: "@kerberosec-test-bot how is your day?",
			}),
	...
	it("repairs adapter-split hyphenated Discord mention names before resolving", async () => {
		const fetchMock = vi.fn(async (url: string | URL) => {
			expect(String(url)).toContain("query=kerberosec-test-bot");
			return new Response(
				JSON.stringify([
					{
						nick: "kerberosec-test-bot",
