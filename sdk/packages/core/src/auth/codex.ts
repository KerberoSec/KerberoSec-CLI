...
import { decodeJwtPayload, type ITelemetryService } from "@kerberosec/shared";
...
export async function loginOpenAICodex(options: {
	...
	if (!server.callbackUrl && !options.onManualCodeInput) {
		const error = new Error(
			`Port ${callbackConfig.port} is already in use, so the OpenAI sign-in callback cannot be received. Close the application using that port (for example another Codex or KerberoSec sign-in) and try again.`,
		);
