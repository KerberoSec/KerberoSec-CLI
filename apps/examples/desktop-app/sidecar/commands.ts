...
import { resolveAudioTranscriptionRoute } from "@kerberosec/llms";
import {
...
import { readFileSyncStrippingUtf8Bom } from "@kerberosec/shared/node";
import packageJson from "../package.json";
import { KERBEROSEC_ACCOUNT_NOT_AUTHENTICATED_RESULT } from "../webview/lib/kerberosec-account-state";
import { MAX_RECORDED_AUDIO_BYTES } from "../webview/lib/voice-input-limits";
import {
	listKerberoSecGitHubRepositories,
	listKerberoSecIntegrations,
...
// KerberoSec access tokens expire between app launches, so account requests must
...
let kerberosecOAuthTokenManager: RuntimeOAuthTokenManager | undefined;
...
function syncFeatureFlagsAccountFromSettings(
	...
	void identifyDesktopFeatureFlagsAccount(
		{ id: manager.getProviderSettings("kerberosec")?.auth?.accountId },
		{ logger: ctx.logger, telemetry: ctx.telemetry },
	);
}

async function resolveFreshKerberoSecAuthToken(
	ctx: SidecarContext,
	manager: ProviderSettingsManager,
): Promise<string | undefined> {
	let refreshError: Error | undefined;
	try {
		kerberosecOAuthTokenManager ??= new RuntimeOAuthTokenManager();
		const resolution = await kerberosecOAuthTokenManager.resolveProviderApiKey({
	...
	const persisted = resolveLocalKerberoSecAuthToken(
	...
	if (!persisted && refreshError) {
		ctx.logger?.error?.(
			"KerberoSec auth token refresh failed with no fallback",
			{
				error: refreshError,
			},
		);
		captureAuthRefreshSoftFailure(ctx.telemetry, "kerberosec", {
...
async function handleRoutineScheduleCommand(
	...
	if (command === "create_routine_schedule") {
		...
		const created = await clientCommand("schedule.create", {
			...
			prompt,
			modelSelection: {
				providerId: asTrimmedString(args?.provider) ?? "kerberosec",
				modelId: asTrimmedString(args?.model) ?? KERBEROSEC_DEFAULT_MODEL_ID,
			},
...
async function listUserInstructionConfigs(
	...
	// (apps/kerberosec-hub/src/server/user-instructions.ts) even if the preset
	// defaults change.
...
export async function handleCommand(
	...
	if (command === "kerberosec_account") {
		...
		const authToken = await resolveFreshKerberoSecAuthToken(ctx, manager);
		if (!authToken) {
			...
			return KERBEROSEC_ACCOUNT_NOT_AUTHENTICATED_RESULT;
		}
		const settings = manager.getProviderSettings("kerberosec");
		const accountService = new KerberoSecAccountService({
			apiBaseUrl:
				settings?.baseUrl?.trim() ||
				getKerberoSecEnvironmentConfig().apiBaseUrl,
			getAuthToken: async () => authToken,
		});
		const result = await executeKerberoSecAccountAction(
			args as KerberoSecAccountActionRequest,
			accountService,
		);
	...
	// ── KerberoSec integrations (GitHub App) ────────────────────────────────
	if (command === "kerberosec_integrations") {
		const operation = String(args?.operation ?? "").trim();
		if (!operation) throw new Error("operation is required");
		const manager = new ProviderSettingsManager();
		...
		const settings = manager.getProviderSettings("kerberosec");
		const environment = getKerberoSecEnvironmentConfig();
		...
		switch (operation) {
			case "list":
				return await listKerberoSecIntegrations(requestOptions);
			case "listGitHubRepositories":
				return await listKerberoSecGitHubRepositories(requestOptions);
			case "githubInstallUrl":
				return await resolveGitHubInstallUrl(requestOptions);
			default:
				throw new Error(
					`Unsupported KerberoSec integrations operation: ${operation}`,
				);
		}
	}
	...
	if (command === "list_provider_catalog") {
		...
		return await listLocalProviders(manager, { isKerberoSecPassEnabled: true });
	}
	...
	if (command === "list_kerberosec_recommended_models") {
		// Tiered picker data (recommended / free / kerberosecPass) with
		// display-ready names; falls back to a bundled list offline.
		return await fetchKerberoSecRecommendedModels();
	}
	...
	if (command === "save_provider_settings") {
		...
		// Sign-out is a `save_provider_settings` that blanks the kerberosec auth block
		...
		if (
			saved.providerId === "kerberosec" ||
			saved.providerId === "kerberosec-pass"
		) {
