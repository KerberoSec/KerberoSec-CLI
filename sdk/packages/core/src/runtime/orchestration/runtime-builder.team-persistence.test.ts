...
describe("DefaultRuntimeBuilder team persistence boundary", () => {
	...
	it("forwards kerberosec workspace metadata to teammate runtime bootstrap config", async () => {
		const { DefaultRuntimeBuilder } = await import("./runtime-builder");
  ...
  "workspaces": {
    "/repo/demo": {
      ...
      "latestGitBranchName": "main"
		...
		const kerberosecBootstrapCall = (
			bootstrapAgentTeamsMock.mock.calls as unknown as Array<[BootstrapCall]>
		)[0]?.[0];
		expect(kerberosecBootstrapCall).toBeDefined();
		expect(
			kerberosecBootstrapCall?.teammateConfigProvider.getRuntimeConfig(),
		).toEqual(
			expect.objectContaining({
				providerId: "kerberosec",
				cwd: "/repo/demo",
