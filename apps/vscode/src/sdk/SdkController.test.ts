...
describe("isKerberoSecManagedProvider", () => {
	it("treats both KerberoSec account providers as KerberoSec providers", () => {
		expect(isKerberoSecManagedProvider("kerberosec")).toBe(true)
		expect(isKerberoSecManagedProvider("kerberosec-pass")).toBe(true)
		expect(isKerberoSecManagedProvider("anthropic")).toBe(false)
		expect(isKerberoSecManagedProvider(undefined)).toBe(false)
	})
})
...
describe("SDK remote-config coordination", () => {
	...
	it("refreshes remote config after login before posting authenticated state", async () => {
		...
		await SdkController.prototype.handleAuthCallback.call(controller as never, "token", "kerberosec")
