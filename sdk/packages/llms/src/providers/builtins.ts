...
// Vercel-only model ids surfaced for the KerberoSec provider while the OpenRouter
// catalog lacks them (KerberoSec's backend routes these to Vercel AI Gateway).
// Remove an id once the OpenRouter catalog lists it.
const VERCEL_ONLY_KERBEROSEC_MODEL_IDS: readonly string[] = [
...
function buildKerberoSecModels(): Record<string, ModelInfo> {
	// KerberoSec is OpenRouter-backed generally, but its recommended-model endpoint
		...
		Object.entries(generatedModels("vercel-ai-gateway")).filter(
			([modelId]) =>
				isCanonicalModelIdForAliasRules(
					modelId,
					VERCEL_OPENROUTER_MODEL_ID_ALIAS_RULES,
				) || VERCEL_ONLY_KERBEROSEC_MODEL_IDS.includes(modelId),
		),
	);
	...
	// KerberoSec's inference backend currently rejects image-output models. Keep
...
function createKerberoSecLikeSpec(
...
async function handleKerberoSecResponseError(
	response: Response,
	providerId: string,
): Promise<void> {
	...
	if (isKerberoSecOrgIndividualInferenceSubscriptionMessage(body)) {
		throw new KerberoSecOrgIndividualInferenceSubscriptionError(providerId);
	}

	if (isKerberoSecFreeModelLimitMessage(body)) {
		throw new KerberoSecFreeModelLimitError(body, providerId);
	}

	const kerberosecPassLimitMessage = extractKerberoSecPassLimitMessage(body);
	if (kerberosecPassLimitMessage) {
		throw new KerberoSecPassLimitError(kerberosecPassLimitMessage, providerId);
	}

	if (isKerberoSecNotSubscribedMessage(body)) {
		throw new KerberoSecNotSubscribedError(providerId);
	}
}
	...
	defaults: {
		options: {
			onResponseError: async (response: Response) => {
				await handleKerberoSecResponseError(response, "kerberosec");
			},
		},
	},
});
...
const kerberosecPass = createKerberoSecLikeSpec({
	...
	defaults: {
		options: {
			onResponseError: async (response: Response) => {
				await handleKerberoSecResponseError(
					response,
					KERBEROSEC_PASS_PROVIDER_ID,
				);
			},
		},
	},
...
/**
 * Handwritten providers plus generated providers that require KerberoSec-specific
...
const OPENAI_COMPATIBLE_SPEC_OVERRIDES: BuiltinSpecOverride[] = [
	...
	},
	{
		...
		// routing policy (`apiLineBaseUrls`), which is KerberoSec-specific.
		id: "moonshot",
		apiLineBaseUrls: {
 ...
 * in providers.generated.ts and only retain KerberoSec-owned behavior here.
 */
