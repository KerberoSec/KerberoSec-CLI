...
import type { GatewayStreamRequest } from "@kerberosec/shared";
import { estimateTokens } from "@kerberosec/shared";
...
function readCaptureMode(): CaptureMode {
	const raw =
		process.env.KERBEROSEC_CAPTURE_PROVIDER_REQUEST?.trim().toLowerCase();
...
function isWireCaptureEnabled(): boolean {
	return process.env.KERBEROSEC_CAPTURE_WIRE?.trim().toLowerCase() === "true";
}

function readMaxPreviewBytes(): number {
	const parsed = Number(process.env.KERBEROSEC_CAPTURE_MAX_PREVIEW_BYTES);
...
function isCleanupEnabled(): boolean {
	return process.env.KERBEROSEC_CAPTURE_CLEANUP?.trim().toLowerCase() !== "off";
}

function resolveCaptureDir(): string | undefined {
	const explicit = process.env.KERBEROSEC_CAPTURE_DIR?.trim();
	if (explicit) {
		return resolve(explicit);
	}
	const dataDir = process.env.KERBEROSEC_DATA_DIR?.trim();
	if (dataDir) {
