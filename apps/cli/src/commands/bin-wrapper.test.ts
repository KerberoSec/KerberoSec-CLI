...
const sourceWrapperPath = fileURLToPath(
	new URL("../../bin/kerberosec", import.meta.url),
);

function createWrapperCopy(): string {
	const dir = mkdtempSync(join(tmpdir(), "kerberosec-bin-package-"));
	const binDir = join(dir, "bin");
	mkdirSync(binDir, { recursive: true });
	const wrapperPath = join(binDir, "kerberosec");
...
function createExecutableScript(contents: string): string {
	const dir = mkdtempSync(join(tmpdir(), "kerberosec-bin-wrapper-"));
...
function runWrapper(target: string, args: string[] = []) {
	...
	return spawnSync(process.execPath, [wrapperPath, ...args], {
		env: {
			...process.env,
			KERBEROSEC_BIN_PATH: target,
		},
		encoding: "utf8",
	});
}

describe("bin/kerberosec wrapper", () => {
...
`);
	...
	it("passes the wrapper path to the compiled binary", () => {
		const target = createExecutableScript(`
console.log(process.env.KERBEROSEC_WRAPPER_PATH ?? "");
`);
		...
		expect(result.stdout.trim()).toMatch(/bin[/\\]kerberosec$/);
	});
