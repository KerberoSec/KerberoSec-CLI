...
import { isModelToolEnabledGlobally, readCompactionStrategyGlobally } from "@kerberosec/core"
...
import { getKerberoSecOnboardingModels } from "../models/getKerberoSecOnboardingModels"
...
export async function getStateToPostToWebview(controller: {
	...
	const onboardingModels = getKerberoSecOnboardingModels()
	...
	const globalKerberoSecRulesToggles = stateManager.getGlobalStateKey("globalKerberoSecRulesToggles")
	...
	const localKerberoSecRulesToggles = stateManager.getWorkspaceStateKey("localKerberoSecRulesToggles")
	...
	const currentTaskItem = controller.task?.taskId
		...
		: undefined
	const kerberosecMessages = [...(controller.task?.messageStateHandler?.getKerberoSecMessages?.() || [])]
	...
	const kerberosecConfig = KerberoSecEnv.config()
	const environment = kerberosecConfig.environment
	...
	return {
		...
		environment,
		distinctId,
		globalKerberoSecRulesToggles: globalKerberoSecRulesToggles || {},
		localKerberoSecRulesToggles: localKerberoSecRulesToggles || {},
