...
describe("KerberoSecAccountService.fetchUserRemoteConfig", () => {
	let service: KerberoSecAccountService
	...
	beforeEach(() => {
		...
		service = new KerberoSecAccountService()
	})
...
describe("KerberoSecAccountService.fetchFeaturebaseToken", () => {
	...
	it("returns undefined when the request throws due to missing auth token", async () => {
		sandbox
			.stub(service as unknown as { authenticatedRequest: () => unknown }, "authenticatedRequest")
			.rejects(new Error("No KerberoSec account auth token found"))
