...
The KerberoSec CLI (`kerberosec`) is distributed as compiled binaries via npm. Users run `npm i -g kerberosec` and get a working `kerberosec` command without needing Bun, Zig, or any other runtime installed.
...
| Package | Description |
|---|---|
| `@kerberosec/cli-darwin-arm64` | macOS Apple Silicon binary |
| `@kerberosec/cli-darwin-x64` | macOS Intel binary |
| `@kerberosec/cli-linux-arm64` | Linux ARM binary |
| `@kerberosec/cli-linux-x64` | Linux x64 binary |
| `@kerberosec/cli-windows-x64` | Windows x64 binary |
| `@kerberosec/cli-windows-arm64` | Windows ARM binary |
| `kerberosec` | Wrapper package (pulls the right binary via `optionalDependencies`) |

Each platform package contains a compiled binary and a minimal `package.json` with `os` and `cpu` fields:

```json
{
  "name": "@kerberosec/cli-darwin-arm64",
  "version": "0.1.0",
  "os": ["darwin"],
  "cpu": ["arm64"],
  "bin": {
    "kerberosec": "bin/kerberosec"
  }
}
```
...
The `kerberosec` wrapper package contains no binary -- just the resolver script, postinstall script, and `optionalDependencies` pointing to all platform packages:
...
```json
{
  ...
  "optionalDependencies": {
    "@kerberosec/cli-darwin-arm64": "0.1.0",
    "@kerberosec/cli-darwin-x64": "0.1.0",
    "@kerberosec/cli-linux-arm64": "0.1.0",
    "@kerberosec/cli-linux-x64": "0.1.0",
    "@kerberosec/cli-windows-x64": "0.1.0",
    "@kerberosec/cli-windows-arm64": "0.1.0"
  }
}
```

After installing, users run `kerberosec`:

```bash
npm i -g kerberosec

kerberosec              # interactive mode
kerberosec "prompt"     # single-prompt mode
kerberosec auth         # authenticate a provider
```
...
2. Choose the release version. Because this publishes over the existing `kerberosec` package, the version must be greater than the current published `kerberosec` version. The handoff release is `3.0.0`.
...
The release helper checks the working tree, verifies the tag points at `HEAD` locally and on `origin`, runs tests, builds all platform packages, and publishes the platform packages plus the generated `kerberosec` wrapper package to npm. The package version and tag must match.

By default, `bun release cli` publishes with the npm dist-tag `latest` (what users get with `npm i -g kerberosec`). To publish under a different dist-tag like `next`, pass `--tag`:
...
CI publishing uses npm trusted publishing. Configure npm trusted publishers for the `kerberosec` wrapper package and every platform package before relying on the workflow.

## How It Works Under the Hood

```
User runs: npm i -g kerberosec
  |
  v
npm installs kerberosec (wrapper package)
  + optionalDependencies (only the matching platform gets installed):
    - @kerberosec/cli-darwin-arm64
    - @kerberosec/cli-darwin-x64
    - @kerberosec/cli-linux-arm64
    - @kerberosec/cli-linux-x64
    - @kerberosec/cli-windows-x64
    - @kerberosec/cli-windows-arm64
  |
  v
postinstall script runs:
  ...
  |
  v
User runs: kerberosec
  |
  v
bin/kerberosec (Node.js resolver) executes:
  1. Check KERBEROSEC_BIN_PATH env var override
  2. Check cached binary at bin/.kerberosec
...
apps/cli/
  bin/
    kerberosec                   # Node.js resolver script (npm entry point)
  script/
...
2. Builds SDK packages (`bun run build:sdk`) and the CLI JS bundle (`bun -F @kerberosec/cli build`)
3. For each target platform:
   ...
   - Runs a smoke test on the current platform's binary (`kerberosec --version`)
...
2. Publishes all 6 platform packages in parallel (`@kerberosec/cli-darwin-arm64`, etc.)
3. Generates a clean main package (`kerberosec`) with:
   - `bin.kerberosec` pointing to the resolver script
   - `postinstall` running the binary caching script
   - `optionalDependencies` listing all platform packages
4. Publishes the generated `kerberosec` wrapper package

Platform packages must be published before the generated `kerberosec` wrapper package because npm validates that `optionalDependencies` exist.

The publish script generates a separate `package.json` for the published `kerberosec` wrapper package. The development `package.json` (with `bin` pointing to `src/index.ts` for `bun link`) is never published directly.

## Binary Resolver (`bin/kerberosec`)

A Node.js script that serves as the entry point when users run `kerberosec`. It finds and executes the correct platform-specific binary.
...
Resolution chain:
1. `KERBEROSEC_BIN_PATH` env var (for development or custom deployments)
2. `bin/.kerberosec` cached hard link (created by postinstall for fast startup)
3. Walk up `node_modules` from the script directory to find the platform package

## Postinstall (`script/postinstall.mjs`)

Runs after `npm install kerberosec`. Creates a hard link from the platform binary to `bin/.kerberosec` for fast startup on subsequent runs. Falls back to file copy if hard linking fails (NFS, cross-device, network-mounted filesystems).
...
| `bun link` + `kerberosec` | src/index.ts | Bun (source) | Yes |
| `npm i -g kerberosec` | bin/kerberosec resolver | Compiled binary | No |
...
Platform packages are published under the `@kerberosec` scope. The generated wrapper package is published as `kerberosec`, so npm trusted publishing must be configured for all 7 package names.
...
Windows application control (Smart App Control, WDAC, AppLocker) blocks unsigned executables at launch, regardless of how they were installed — npm distribution gets no exemption ([#12934](https://github.com/kerberosec/kerberosec/issues/12934)). The publish workflow Authenticode-signs `cli-windows-x64/bin/kerberosec.exe` and `cli-windows-arm64/bin/kerberosec.exe` with Azure Trusted Signing before publishing, via the `.github/actions/sign-windows-cli` composite action. Signing runs on the Linux publish runner using [jsign](https://ebourg.github.io/jsign/) (`--storetype TRUSTEDSIGNING`) with an OIDC-federated Entra app, then verifies the signature chain with `osslsigncode` against the Microsoft Identity Verification Root CA 2020. If all `AZURE_*` / `AZURE_TRUSTED_SIGNING_*` repository secrets are absent, the action logs a warning and the release ships unsigned rather than failing; if only some resolve (a typo'd or renamed secret), the release fails loudly instead. The certificate profile secret is suffixed `_CLI` because the desktop app will later get its own profile; the other five secrets are shared. Note that signing bun-compiled executables requires Bun >= 1.2.23 (earlier versions located the embedded bundle relative to the end of the file, which signing corrupts).
