...
describe("createKerberoSecAccountService", () => {
	beforeEach(() => {
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
	...
	it("refreshes persisted KerberoSec OAuth credentials before creating the account service", async () => {
		...
		const { createKerberoSecAccountService } = await import(
			"./kerberosec-account"
		);
	...
	it("asks the user to re-authenticate when KerberoSec OAuth credentials cannot refresh", async () => {
		vi.spyOn(Date, "now").mockReturnValue(100_000);
		mockFetchJson(
			{
				error: "invalid_grant",
		...
		await expect(
			createKerberoSecAccountService({ config: makeConfig() }),
		).rejects.toThrow(
...
describe("loadKerberoSecAccountSnapshot", () => {
	beforeEach(() => {
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
		coreMocks.getProviderSettings.mockReset();
	...
	it("identifies the loaded KerberoSec account for telemetry and feature flags", async () => {
		coreMocks.getProviderSettings.mockReturnValue({
			provider: "kerberosec",
			apiKey: "account-token",
		});
		const { loadKerberoSecAccountSnapshot } = await import(
			"./kerberosec-account"
		);
		...
		await loadKerberoSecAccountSnapshot({ config: makeConfig() });
...
describe("isKerberoSecAccountCreditsErrorMessage", () => {
	it("matches the raw insufficient_credits JSON payload from the KerberoSec API 402", async () => {
		const { isKerberoSecAccountCreditsErrorMessage } = await import(
			"./kerberosec-account"
		);
		expect(
	...
	it("matches the plain human-readable KerberoSec API message", async () => {
		const { isKerberoSecAccountCreditsErrorMessage } = await import(
			"./kerberosec-account"
		);
		expect(
			isKerberoSecAccountCreditsErrorMessage("Not enough credits available"),
		).toBe(true);
	});
	...
	it("matches the legacy insufficient balance phrasing", async () => {
		...
		expect(
			isKerberoSecAccountCreditsErrorMessage(
				"Insufficient balance. Your KerberoSec credits balance is $0.00.",
			),
		).toBe(true);
	});
	...
	it("does not match unrelated errors", async () => {
		...
		expect(isKerberoSecAccountCreditsErrorMessage("Payment Required")).toBe(
			false,
		);
		expect(
			isKerberoSecAccountCreditsErrorMessage(
		...
		expect(
			isKerberoSecAccountCreditsErrorMessage("insufficient balance on gateway"),
		).toBe(false);
	});
});
