# Sidecar Architecture — @kerberosec/code

## Overview

The sidecar is a Bun process that adapts the desktop UI and native operations to
the shared KerberoSec Hub.

It imports `@kerberosec/core`, discovers or starts the canonical shared Hub, registers
...
`KerberoSecCore` uses Hub mode without an explicit endpoint. Core therefore reuses
the same compatible Hub discovered by the CLI or starts the canonical detached
Hub when the desktop is the first client:

```typescript
const sessionManager = await KerberoSecCore.create({
  clientName: "kerberosec-code",
  backendMode: "hub",
  hub: {
    ...
    displayName: "KerberoSec Desktop sidecar",
  },
  capabilities: {
...
```typescript
import { ProviderSettingsManager, listLocalProviders, ... } from "@kerberosec/core";
...
```typescript
import { SqliteSessionStore, resolveSessionBackend } from "@kerberosec/core";
...
|---------|---------------|
| `chat_session_command` | shared Hub through `KerberoSecCore` |
