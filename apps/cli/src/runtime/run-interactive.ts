...
import { formatModeSwitchNotice } from "@kerberosec/shared";
...
import {
	...
	resolveKerberoSecWelcomeLine,
} from "../tui/interactive-welcome";
import {
	loadIndividualSubscriptionPlans,
	loadKerberoSecAccountSnapshot,
	onProviderChange,
	switchKerberoSecAccount,
} from "../tui/kerberosec-account";
...
export async function resumeInteractiveSession(
	...
	const previousAgentResume = process.env.KERBEROSEC_HOOK_AGENT_RESUME;
	...
	try {
		...
		if (previousAgentResume === undefined) {
			...
			process.env.KERBEROSEC_HOOK_AGENT_RESUME = previousAgentResume;
		}
		throw error;
	}
...
export async function runInteractive(
	...
	tuiApp = await renderOpenTui({
		...
		loadWelcomeLine: async () =>
			await resolveKerberoSecWelcomeLine({
				config,
				kerberosecApiBaseUrl: options?.kerberosecApiBaseUrl,
				kerberosecProviderSettings: options?.kerberosecProviderSettings,
			}),
		loadKerberoSecAccount: async () =>
			await loadKerberoSecAccountSnapshot({
				config,
				kerberosecApiBaseUrl: options?.kerberosecApiBaseUrl,
			}),
		...
		switchKerberoSecAccount: async (organizationId) =>
		...
		onSubmit: async (input, mode, delivery, attachments, onCommandOutput) => {
			...
			try {
				...
				if (config.providerId === "ollama") {
					const { ensureOllamaServerRunning } = await import(
						"../utils/ollama-manager"
					);
				...
				zeroTurnCost = await shouldZeroKerberoSecFreeModelCost(config);
		...
		onAccountChange: async () => {
			...
			await loadKerberoSecAccountSnapshot({
				...
				logCliError(
					config.logger,
					"KerberoSec account refresh after account change failed",
					{ error },
				);
			});
			await sessionRuntime.restartWithCurrentMessages();
		...
		// when the TUI was launched through `kerberosec history`.
	...
	if (updateCliAfterExit) {
		...
		writeln(
			"The shared KerberoSec Hub was updated by another KerberoSec installation. Updating this CLI…",
		);
		const { checkForUpdates } = await import("../commands/update");
		const exitCode = await checkForUpdates({ includeKanban: false });
		writeln(
			exitCode === 0
				? "Start kerberosec again to reconnect to the updated Hub."
				: "Update did not complete. Run 'kerberosec update' manually, then start kerberosec again.",
		);
	}
}
