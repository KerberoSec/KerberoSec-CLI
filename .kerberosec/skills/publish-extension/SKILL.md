...
|---|---|---|---|---|
| Stable (combined) | `kerberosec.kerberosec-vscode` | `ext-vscode-ab-package.yml` | dispatch only; `publish` input defaults false | manual input (semver, e.g. `4.1.0`) |
| Nightly (combined) | `saoudrizwan.kerberosec-nightly` | `ext-vscode-publish-nightly.yml` | cron 12:00 UTC + dispatch | auto `<major>.<minor>.<unix-ts>` from main's `apps/vscode/package.json` |
| Legacy hotfix (standalone) | `kerberosec.kerberosec-vscode` | `ext-vscode-publish-legacy.yml` | dispatch | from `apps/vscode/package.json` on `legacy-extension` |
| Stable standalone (post-cutover) | `kerberosec.kerberosec-vscode` | `ext-vscode-publish-stable.yml` | dispatch | from `apps/vscode/package.json` on `main` |
...
2. **Check the flag BEFORE any stable combined publish.** `ext-sdk-bundle-rollout` is **shared between nightly and stable** -  the loader sends only a machine id to `/decide`, no channel property, so there is no per-channel targeting. If the flag is high (nightly dogfooding) and you publish stable, stable users get the next bundle at that same percentage. Verify the effective percentage empirically (no PostHog admin needed -  sample `/decide` with random ids using the key inlined in any shipped loader):
   ...
   (async () => {
     ...
     for (let i = 0; i < n; i += 20) {
       const rs = await Promise.all(Array.from({length: 20}, (_, j) =>
         fetch("https://data.kerberosec.bot/decide?v=3", { method: "POST",
   ...
   Flag changes are made in the PostHog UI (KerberoSec project). **0% is the kill switch** -  the flag is two-way; there is no separate killswitch flag. Dialing down demotes machines back to legacy on their next window reload.
...
```bash
gh api repos/kerberosec/kerberosec/actions/runs/<run-id>/pending_deployments
```
...
1. Verify the marketplace serves the new version (query from rule 1) -  expect minutes-to-an-hour of validation lag after "Published" appears in the logs. Also verify Open VSX:
   ...
   ```bash
   curl -s "https://open-vsx.org/api/saoudrizwan/kerberosec-dev" | python3 -c "import json,sys; d=json.load(sys.stdin); print(d['version'], d['timestamp'])"
   ```
...
3. Thorough artifact check (`gh run download <run-id>`): union `package.json` is `kerberosec.kerberosec-vscode@<VERSION>`, `next/package.json` and `legacy/package.json` carry the SAME version, `grep -c 'phc_' extension/extension.js` ≥ 1 (loader key inlined), no leftover `process.env.TELEMETRY_SERVICE_API_KEY` / `process.env.KERBEROSEC_ROLLOUT_VARIANT` literals in either bundle's dist (leftovers = a build ran without its env and telemetry is silently dead).
...
5. Dial the flag per the rollout plan (e.g. 0% at publish → 1% → up), verifying each change with the probe from rule 2. Announce demotions ahead of time -  dialing down also demotes nightly dogfooders unless they set `"kerberosec-nightly.rollout.bundleOverride": "next"`.
...
No changelog/version prep -  the version is computed. Verify with the marketplace query against `saoudrizwan.kerberosec-nightly`.
...
- `bun run package` in `apps/vscode` does not build `@kerberosec/*` workspace deps -  fresh checkouts need `bun run build:sdk` first (workflows handle this).
...
- Local forcing for manual testing: `KERBEROSEC_BUNDLE_OVERRIDE=next|legacy` env (launch VS Code fresh from a terminal) or the `<prefix>.rollout.bundleOverride` setting + reload; both report as `override` in telemetry so they don't pollute cohort data.
