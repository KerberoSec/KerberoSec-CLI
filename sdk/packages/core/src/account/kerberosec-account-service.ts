...
interface KerberoSecApiEnvelope<T> {
	success?: boolean;
	error?: string;
	data?: T;
}

function getKerberoSecApiEnvelopeError(parsed: unknown): string | undefined {
...
function formatKerberoSecAccountRequestFailure(
	status: number,
	bodyText: string,
	parsed: unknown,
): string {
	const envelopeError = getKerberoSecApiEnvelopeError(parsed);
	if (envelopeError) {
	...
	if (body) {
		...
		return `KerberoSec account request failed with status ${status}: ${preview}`;
	}

	return `KerberoSec account request failed with status ${status}`;
}

export interface KerberoSecAccountServiceOptions {
	apiBaseUrl: string;
...
export class KerberoSecAccountService {
	private readonly apiBaseUrl: string;
	private readonly getAuthTokenFn: KerberoSecAccountServiceOptions["getAuthToken"];
	private readonly getCurrentUserIdFn: KerberoSecAccountServiceOptions["getCurrentUserId"];
	private readonly getOrganizationMemberIdFn: KerberoSecAccountServiceOptions["getOrganizationMemberId"];
	private readonly getHeadersFn: KerberoSecAccountServiceOptions["getHeaders"];
	private readonly requestTimeoutMs: number;
	private readonly fetchImpl: typeof fetch;

	constructor(options: KerberoSecAccountServiceOptions) {
	...
	public async fetchMe(): Promise<KerberoSecAccountUser> {
		return this.request<KerberoSecAccountUser>("/api/v1/users/me");
	}
	...
	public async fetchUsageTransactions(
		...
		const response = await this.request<{
			items: KerberoSecAccountUsageTransaction[];
	...
	public async fetchPaymentTransactions(
		...
		const resolvedUserId = await this.resolveUserId(userId);
		const response = await this.request<{
			paymentTransactions: KerberoSecAccountPaymentTransaction[];
	...
	public async fetchAvailableSubscriptionPlans(input?: {
		...
		return this.request<KerberoSecSubscriptionPlan[]>(
	...
	public async fetchOrganization(
		organizationId: string,
	): Promise<KerberoSecOrganization> {
		const orgId = organizationId.trim();
		if (!orgId) {
			throw new Error("organizationId is required");
		}
		return this.request<KerberoSecOrganization>(
	...
	public async fetchOrganizationUsageTransactions(input: {
		...
		const response = await this.request<{
			items: KerberoSecAccountOrganizationUsageTransaction[];
		}>(
	...
	private async request<T>(
		...
		try {
			...
			if (response.status === 204 || input?.expectNoContent) {
				if (!response.ok) {
					throw new Error(
						`KerberoSec account request failed with status ${response.status}`,
					);
				}
			...
			if (text.trim()) {
				try {
					...
					if (!response.ok) {
						throw new Error(
							formatKerberoSecAccountRequestFailure(
								response.status,
								text,
								undefined,
							),
					...
					throw new Error("KerberoSec account response was not valid JSON");
				}
			}

			if (!response.ok) {
				throw new Error(
					formatKerberoSecAccountRequestFailure(response.status, text, parsed),
				);
			}

			if (typeof parsed === "object" && parsed !== null) {
				const envelope = parsed as KerberoSecApiEnvelope<T>;
				if (typeof envelope.success === "boolean") {
					if (!envelope.success) {
						throw new Error(
							envelope.error || "KerberoSec account request failed",
						);
					}
			...
			if (parsed === undefined || parsed === null) {
				throw new Error("KerberoSec account response payload was empty");
			}
			return parsed as T;
		} finally {
