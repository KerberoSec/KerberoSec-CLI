...
const THEME_BOOTSTRAP_SCRIPT = `<script id="kerberosec-hub-theme-bootstrap">
  (() => {
    try {
      const theme = window.localStorage.getItem("kerberosec-hub-theme");
...
export function normalizeWebviewIndexHtml(html: string): string {
	...
	if (normalized.includes('id="kerberosec-hub-theme-bootstrap"')) {
		return normalized;
	}
...
export class WebviewAssets {
	...
	private async serveIndex(): Promise<Response> {
		...
		return createTextResponse(
			"KerberoSec Hub webview is not built. Run `bun run build:webview` from apps/kerberosec-hub.",
			503,
		);
	}
