...
import { version as kerberosecCoreVersion } from "../../../package.json";
import {
...
describe("createAgentRuntimeConfig", () => {
	...
	it("maps telemetry identity fields from AgentConfig", () => {
		...
		expect(runtimeConfig.clientName).toBe("kerberosec-cli");
		expect(runtimeConfig.clientVersion).toBe("3.0.38");
		expect(runtimeConfig.kerberosecCoreVersion).toBe(kerberosecCoreVersion);
	});
