...
afterEach(() => {
	...
	delete document.documentElement.dataset.kerberosecAccent;
	delete document.documentElement.dataset.kerberosecHubTheme;
...
describe("hub theme", () => {
	it("applies a saved theme before the system preference", () => {
		...
		expect(document.documentElement.dataset.kerberosecHubTheme).toBe("dark");
	});
	...
	it("applies the system preference before the first paint when unsaved", () => {
		...
		expect(document.documentElement.dataset.kerberosecHubTheme).toBe("light");
	});
...
describe("hub accent", () => {
	...
	it("round-trips through storage and the html dataset", () => {
		...
		expect(document.documentElement.dataset.kerberosecAccent).toBe("graphite");
	...
	it("clears the dataset attribute for the default accent", () => {
		...
		expect(document.documentElement.dataset.kerberosecAccent).toBe("ember");
		applyHubAccent(DEFAULT_HUB_ACCENT);
		expect(document.documentElement.dataset.kerberosecAccent).toBeUndefined();
	});
});
