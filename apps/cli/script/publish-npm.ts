...
// Publishes kerberosec and all platform-specific binary packages to npm.
//
// Usage:
...
const wrapperPackageName = "kerberosec";
...
const hostSdkPackages = [
	{ name: "@kerberosec/sdk", directory: "sdk" },
	{ name: "@kerberosec/core", directory: "core" },
	{ name: "@kerberosec/agents", directory: "agents" },
	{ name: "@kerberosec/llms", directory: "llms" },
	{ name: "@kerberosec/shared", directory: "shared" },
] as const;
...
const platformTasks = Object.keys(binaries)
	.sort()
	.map(async (name) => {
		const dirName = name.replace("@kerberosec/", "");
...
const wrapperPackageJson = {
	...
	version,
	description: description || "KerberoSec CLI",
