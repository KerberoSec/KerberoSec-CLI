...
export type KerberoSecPassSubscriptionAction =
	| "subscribe"
	| "refresh"
	| "skip"
	| "back";

export interface KerberoSecPassSubscriptionOption {
	value: KerberoSecPassSubscriptionAction;
	label: string;
}
...
export function getMainMenuOptions(options?: {
	...
	return MAIN_MENU.filter(
		(option) =>
			option.value !== "kerberosec-pass" || options?.isKerberoSecPassEnabled,
	);
}

export const KERBEROSEC_PASS_SUBSCRIPTION_OPTIONS: KerberoSecPassSubscriptionOption[] =
	[
		{
			value: "subscribe",
...
export type KerberoSecPassSubscriptionStatus =
	| "loading"
	| "subscribed"
	| "unsubscribed"
	| "error";
...
export function shouldUseFeaturedKerberoSecModelPicker(
	providerId: string,
): boolean {
	// KerberoSecPass uses the featured picker too, with Subscribed/Free sections
