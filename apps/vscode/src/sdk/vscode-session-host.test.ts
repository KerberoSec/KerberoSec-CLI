import type { KerberoSecCoreStartInput, ITelemetryService } from "@kerberosec/core"
import { beforeEach, describe, expect, it, vi } from "vitest"

const mockKerberoSecCoreCreate = vi.hoisted(() => vi.fn())
const mockCreateVscodeExtraTools = vi.hoisted(() => vi.fn(async () => []))

vi.mock("@kerberosec/core", async () => {
	const actual = await vi.importActual<typeof import("@kerberosec/core")>("@kerberosec/core")
	return {
		...actual,
		KerberoSecCore: {
			create: mockKerberoSecCoreCreate,
		},
	}
})
...
describe("VscodeSessionHost telemetry wiring", () => {
	beforeEach(() => {
		mockKerberoSecCoreCreate.mockReset()
		mockKerberoSecCoreCreate.mockResolvedValue({ runtimeAddress: undefined })
		mockCreateVscodeExtraTools.mockReset().mockResolvedValue([])
	})

	it("passes shared telemetry to KerberoSecCore.create", async () => {
		...
		expect(mockKerberoSecCoreCreate).toHaveBeenCalledWith(expect.objectContaining({ telemetry }))
	})
	...
	it("injects shared telemetry into CoreSessionConfig when remote config did not provide one", async () => {
		...
		const prepare = mockKerberoSecCoreCreate.mock.calls[0][0].prepare
	...
	it("preserves telemetry already supplied by remote config", async () => {
		...
		await VscodeSessionHost.create({
			...
			getRemoteConfigIntegration: () =>
				({
					applyToStartSessionInput: (input: KerberoSecCoreStartInput) => ({
						...input,
						config: {
							...input.config,
	...
	it("passes custom editor and apply_patch executors into tool executor capabilities", async () => {
		...
		const capabilities = mockKerberoSecCoreCreate.mock.calls[0][0].capabilities
	...
	it("waits for policy readiness before selecting and applying remote config", async () => {
		...
		const applyToStartSessionInput = vi.fn(async (input: KerberoSecCoreStartInput) => {
			events.push("apply")
			return input
		})
		await VscodeSessionHost.create({
	...
	it("runs the session gate and remote-config integration on a checkpoint restore with a replacement session", async () => {
		...
		mockKerberoSecCoreCreate.mockResolvedValue({ runtimeAddress: undefined, restore: innerRestore })
		...
		// The gate must resolve before the integration is read; KerberoSecCore.restore
		// does not run the prepare hook, so the host must apply it itself.
		expect(events).toEqual(["gate", "integration"])
		const restoredInput = innerRestore.mock.calls[0][0] as { start: KerberoSecCoreStartInput }
