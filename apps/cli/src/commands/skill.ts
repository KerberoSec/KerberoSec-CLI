...
// `kerberosec skill` is a thin wrapper around the open skills CLI
...
// `kerberosec skill` command we default these to KerberoSec unless the user picked their
...
// interactively instead — not what someone scoping to KerberoSec would expect.
const KERBEROSEC_SCOPED_SUBCOMMANDS = new Set([
	"add",
	"install",
	"i",
	"update",
...
/**
 * Build the argument list passed to `npx`, injecting `--agent kerberosec` for
...
export function buildSkillsArgs(userArgs: readonly string[]): string[] {
	...
	if (
		subcommand &&
		KERBEROSEC_SCOPED_SUBCOMMANDS.has(subcommand) &&
		!hasAgentFlag(args)
	) {
		args.push("--agent", "kerberosec");
	}
	...
	return new Promise<number>((resolve) => {
		...
		child.once("error", (error: NodeJS.ErrnoException) => {
			...
			if (error.code === "ENOENT") {
				io.writeErr(
					'npx was not found. Install Node.js (which includes npx) to use "kerberosec skill".',
				);
			} else {
