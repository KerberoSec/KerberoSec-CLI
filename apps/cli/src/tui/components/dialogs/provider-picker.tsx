...
function KerberoSecPassBrowserPageContent(
...
export function KerberoSecPassSubscriptionContent(
	props: ChoiceContext<boolean> & {
		providerName: string;
	},
) {
	...
	return (
		<KerberoSecPassBrowserPageContent
			{...props}
...
export function OAuthLoginContent(
	...
	const [mode, setMode] = useState<"browser" | "device">(
		providerId === "kerberosec" ? "device" : "browser",
	);
	const [status, setStatus] = useState("Opening browser...");
