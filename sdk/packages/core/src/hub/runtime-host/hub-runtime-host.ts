import { resolveProviderRequestHeaders } from "@kerberosec/llms";
import type {
	AgentEvent,
	AgentFinishReason,
	AgentResult,
...
export class HubRuntimeHost implements RuntimeHost {
	...
	async restoreSession(
		...
		const messages = Array.isArray(reply.payload?.messages)
			? (reply.payload.messages as import("@kerberosec/llms").Message[])
			: undefined;
	...
	async readSessionMessages(
		sessionId: string,
	): Promise<import("@kerberosec/llms").MessageWithMetadata[]> {
		const target = sessionId.trim();
		if (!target) {
			return [];
		}
		...
		return Array.isArray(messages)
			? (messages as import("@kerberosec/llms").MessageWithMetadata[])
			: [];
	}
