import type { MessageWithMetadata } from "@kerberosec/llms";
import type {
	AgentConfig,
...
export type { KerberoSecCoreSettingsApi } from "../settings";
...
export interface KerberoSecCoreAutomationOptions {
...
export type KerberoSecAutomationSpec = CronSpecRecord;
export type KerberoSecAutomationRun = CronRunRecord;
export type KerberoSecAutomationEventLog = CronEventLogRecord;
export type KerberoSecAutomationEventSuppression = CronEventSuppression;
export type KerberoSecAutomationRunStatus =
	| "queued"
	| "running"
	| "done"
	| "failed"
...
export interface KerberoSecAutomationListSpecsOptions {
...
export interface KerberoSecAutomationListRunsOptions {
	specId?: string;
	status?: KerberoSecAutomationRunStatus | KerberoSecAutomationRunStatus[];
	limit?: number;
}

export interface KerberoSecAutomationListEventsOptions {
	eventType?: string;
	source?: string;
	processingStatus?:
		| "received"
...
export interface KerberoSecAutomationEventIngressResult {
	event: KerberoSecAutomationEventLog;
	duplicate: boolean;
	matchedSpecIds: string[];
	queuedRuns: KerberoSecAutomationRun[];
	suppressions: KerberoSecAutomationEventSuppression[];
}

export interface KerberoSecCoreAutomationApi {
	start(): Promise<void>;
	stop(): Promise<void>;
	...
	listEvents(
		options?: KerberoSecAutomationListEventsOptions,
	): KerberoSecAutomationEventLog[];
	getEvent(eventId: string): KerberoSecAutomationEventLog | undefined;
	listSpecs(
		options?: KerberoSecAutomationListSpecsOptions,
	): KerberoSecAutomationSpec[];
	listRuns(
		options?: KerberoSecAutomationListRunsOptions,
	): KerberoSecAutomationRun[];
}

export type KerberoSecCoreListHistoryOptions = SessionHistoryListOptions;

export interface KerberoSecCoreStartInput
	extends Omit<StartSessionInput, "config" | "localRuntime"> {
	config: KerberoSecCoreStartConfig;
...
export interface RestoreInput {
	...
	start?: KerberoSecCoreStartInput;
	cwd?: string;
...
export interface KerberoSecCoreOptions {
	/**
	...
	/**
	 * Feature flags service for this KerberoSecCore instance.
	...
	/**
	 * Enables file-based and event-driven automation through this KerberoSecCore
	 * instance. When configured, callers use `kerberosec.automation.*` instead of
	 * constructing cron services directly.
	 */
	automation?: boolean | KerberoSecCoreAutomationOptions;
	/**
	 ...
	 *     `@kerberosec/core/hub` for the scheduler
	 */
	fetch?: typeof fetch;
	/**
...
export interface StartSessionBootstrap {
	applyToStartSessionInput(
		input: KerberoSecCoreStartInput,
	): Promise<KerberoSecCoreStartInput> | KerberoSecCoreStartInput;
