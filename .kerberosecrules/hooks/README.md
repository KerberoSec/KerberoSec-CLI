# KerberoSec Hooks Documentation

## Overview

KerberoSec hooks allow you to execute custom scripts at specific points in the agentic workflow. Hooks can be placed in either:
- **Global hooks directory**: `~/Documents/KerberoSec/Hooks/` (applies to all workspaces)
- **Workspace hooks directory**: `.kerberosecrules/hooks/` (applies to the workspace the repo is part of)

Hooks run automatically when enabled.

## Enabling Hooks

1. Open KerberoSec settings in VSCode
...
- **Global Location**: `~/Documents/KerberoSec/Hooks/TaskStart`
- **Workspace Location**: `.kerberosecrules/hooks/TaskStart`
...
- **Global Location**: `~/Documents/KerberoSec/Hooks/TaskResume`
- **Workspace Location**: `.kerberosecrules/hooks/TaskResume`
...
- **Global Location**: `~/Documents/KerberoSec/Hooks/TaskCancel`
- **Workspace Location**: `.kerberosecrules/hooks/TaskCancel`
...
- **Global Location**: `~/Documents/KerberoSec/Hooks/TaskComplete`
- **Workspace Location**: `.kerberosecrules/hooks/TaskComplete`
...
- **Global Location**: `~/Documents/KerberoSec/Hooks/UserPromptSubmit`
- **Workspace Location**: `.kerberosecrules/hooks/UserPromptSubmit`

### PreToolUse Hook
- **When**: Runs BEFORE a tool is executed
- **Purpose**: Validate parameters, block execution, or add context
- **Global Location**: `~/Documents/KerberoSec/Hooks/PreToolUse`
- **Workspace Location**: `.kerberosecrules/hooks/PreToolUse`

### PostToolUse Hook
- **When**: Runs AFTER a tool completes
- **Purpose**: Observe results, track patterns, or add context
- **Global Location**: `~/Documents/KerberoSec/Hooks/PostToolUse`
- **Workspace Location**: `.kerberosecrules/hooks/PostToolUse`
...
- **Global Location**: `~/Documents/KerberoSec/Hooks/PreCompact`
- **Workspace Location**: `.kerberosecrules/hooks/PreCompact`

## Cross-Platform Hook Format

KerberoSec uses a git-style approach for hooks that works consistently across all platforms:
...
Like git hooks, KerberoSec executes hook files through a shell that interprets the shebang line:
...
```bash
# Create hook file
nano ~/Documents/KerberoSec/Hooks/PreToolUse

# Make executable
chmod +x ~/Documents/KerberoSec/Hooks/PreToolUse
```
...
# Log to file
echo "$input" >> ~/.kerberosec/hook-logs/tool-usage.jsonl
...
KerberoSec supports two levels of hooks:

### Global Hooks
- **Location**: `~/Documents/KerberoSec/Hooks/` (macOS/Linux)
...
### Workspace Hooks
- **Location**: `.kerberosecrules/hooks/` in each workspace root
...
1. The global hooks directory is automatically created at:
   - macOS/Linux: `~/Documents/KerberoSec/Hooks/`
...
3. Enable hooks in KerberoSec settings
...
```bash
#!/usr/bin/env bash
# ~/Documents/KerberoSec/Hooks/PreToolUse
...
```bash
#!/usr/bin/env bash
# .kerberosecrules/hooks/PreToolUse
...
If you have multiple workspace roots, you can place hooks in each root's `.kerberosecrules/hooks/` directory. All hooks (global and workspace) may execute concurrently. Their results will be combined:
...
- Look for errors in VSCode's Output panel (KerberoSec channel)
