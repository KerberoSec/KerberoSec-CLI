# KerberoSecCore API Reference

## Creating KerberoSecCore

```typescript
import { KerberoSecCore } from "@kerberosec/sdk"

const kerberosec = await KerberoSecCore.create(options: KerberoSecCoreOptions)
```

### KerberoSecCoreOptions

```typescript
interface KerberoSecCoreOptions {
...
```typescript
const session = await kerberosec.start(input: KerberoSecCoreStartInput)
```
...
```typescript
interface KerberoSecCoreStartInput {
  prompt: string
...
`extensions` passes plugin objects directly. `pluginPaths` points to directories with `package.json` containing a `kerberosec.plugins` field. Set `extensionContext.workspace` so plugins receive `ctx.workspaceInfo` in their `setup()` call -- without it, `ctx.workspaceInfo` is undefined.
...
```typescript
const result = await kerberosec.send({
...
```typescript
const unsubscribe = kerberosec.subscribe(
...
```typescript
const sessions: SessionRecord[] = await kerberosec.list(50)
```

### get(sessionId)

```typescript
const session: SessionRecord = await kerberosec.get(sessionId)
```

### readMessages(sessionId)

```typescript
const messages: AgentMessage[] = await kerberosec.readMessages(sessionId)
```

### getAccumulatedUsage(sessionId)

```typescript
const usage = await kerberosec.getAccumulatedUsage(sessionId)
...
```typescript
await kerberosec.update(sessionId, { title: "New title" })
```

### abort(sessionId, reason?)

```typescript
await kerberosec.abort(sessionId, "User cancelled")
```
...
```typescript
await kerberosec.delete(sessionId)
```
...
```typescript
await kerberosec.restore({ sessionId, checkpointId })
```
...
```typescript
await kerberosec.dispose("Shutting down")
```
...
When `automation` is enabled in `KerberoSecCore.create()`:
...
kerberosec.automation.start()
kerberosec.automation.stop()
kerberosec.automation.reconcile(specs)
kerberosec.automation.ingestEvent(event)
kerberosec.automation.listEvents()
kerberosec.automation.listSpecs()
kerberosec.automation.listRuns()
```
...
```typescript
// Read settings
const settings = await kerberosec.settings.list()

// Toggle tools, plugins, MCP servers
await kerberosec.settings.toggle({ type: "tool", name: "bash", enabled: true })
```
