...
describe("getPersistedProviderApiKey", () => {
	it("does not double-prefix persisted KerberoSec OAuth tokens", () => {
		expect(
			getPersistedProviderApiKey("kerberosec", {
				provider: "kerberosec",
				auth: {
...
describe("loadAuthTuiRuntime", () => {
	it("loads OpenTUI React after provider catalog initialization", async () => {
		...
		const script = `
import { ProviderSettingsManager, ensureCustomProvidersLoaded, listLocalProviders } from "@kerberosec/core";
