...
/**
 * Simple KerberoSec gRPC Server
 *
 * This script provides a minimal way to run the KerberoSec core gRPC service
 ...
 *   2. KerberoSecApiServerMock (mock implementation of the KerberoSec API)
 ...
 *   KERBEROSEC_DIST_DIR - Override distribution directory (default: PROJECT_ROOT/dist-standalone)
 *   KERBEROSEC_CORE_FILE - Override core file name (default: kerberosec-core.js)
 ...
 *   KERBEROSEC_ENVIRONMENT - Environment setting (default: local)
 *
...
import { KerberoSecApiServerMock } from "../src/test/e2e/fixtures/server/index"
...
const KERBEROSEC_ENVIRONMENT = process.env.KERBEROSEC_ENVIRONMENT || "local"
...
const distDir = process.env.KERBEROSEC_DIST_DIR || path.join(projectRoot, "dist-standalone")
const kerberosecCoreFile = process.env.KERBEROSEC_CORE_FILE || "kerberosec-core.js"
const coreFile = path.join(distDir, kerberosecCoreFile)

const childProcesses: ChildProcess[] = []

async function main(): Promise<void> {
	console.log("Starting Simple KerberoSec gRPC Server...")
	...
	if (!fs.existsSync(coreFile)) {
		...
		console.error("  KERBEROSEC_DIST_DIR - Override distribution directory")
		console.error("  KERBEROSEC_CORE_FILE - Override core file name")
		console.error("")
	...
	try {
		...
		console.log("KerberoSec API Server started in-process")
	} catch (error) {
		console.error("Failed to start KerberoSec API Server:", error)
		process.exit(1)
	}

	const extensionsDir = path.join(distDir, "vsce-extension")
	const userDataDir = mkdtempSync(path.join(os.tmpdir(), "vsce"))
	const kerberosecTestWorkspace = mkdtempSync(path.join(os.tmpdir(), "kerberosec-test-workspace-"))
	...
	const hostbridge: ChildProcess = spawn("bun", [path.join(__dirname, "test-hostbridge-server.ts")], {
		...
		env: {
			...process.env,
			TEST_HOSTBRIDGE_WORKSPACE_DIR: kerberosecTestWorkspace,
	...
	const baseArgs = ["--enable-source-maps", "--require", workosFetchMockPath, path.join(distDir, "kerberosec-core.js")]
	...
	console.log(`Starting KerberoSec Core Service... (useC8=${USE_C8})`)
	...
	const coreService: ChildProcess = spawn(spawnCommand, spawnArgs, {
		...
		env: {
			...
			E2E_TEST,
			KERBEROSEC_ENVIRONMENT,
			KERBEROSEC_DIR: userDataDir,
	...
	const shutdown = async () => {
		...
		await KerberoSecApiServerMock.stopGlobalServer()

		try {
			rmSync(userDataDir, { recursive: true, force: true })
			rmSync(kerberosecTestWorkspace, { recursive: true, force: true })
	...
	console.log(`KerberoSec gRPC Server is running on 127.0.0.1:${PROTOBUS_PORT}`)
	console.log("Press Ctrl+C to stop")
}

if (require.main === module) {
	main().catch((err) => {
		console.error("Failed to start simple KerberoSec server:", err)
		process.exit(1)
	})
}
