...
const DASHBOARD_PORT_ENV = "KERBEROSEC_HUB_DASHBOARD_PORT";
const WEBVIEW_DIST_ENV = "KERBEROSEC_HUB_WEBVIEW_DIST_DIR";
...
async function withDashboardEnvironment<T>(
	...
	const restore = [
		...
		setEnvValue("KERBEROSEC_DIR", options.configDir?.trim() || undefined),
	...
	if (options.dataDir || process.env.KERBEROSEC_SANDBOX?.trim() === "1") {
...
function resolveDefaultWebviewDistDir(): string | undefined {
	...
	const candidates = [
		...
		join(moduleDir, "../../../kerberosec-hub/dist/webview"),
		// Node bundle: apps/cli/dist/index.js
		join(moduleDir, "kerberosec-hub/webview"),
		// Compiled platform package: apps/cli/dist/<platform>/bin/kerberosec
		join(dirname(process.execPath), "../kerberosec-hub/webview"),
	];
...
function resolveInstalledPlatformPackageWebviewCandidates(): string[] {
	...
	const starts = [
		process.env.KERBEROSEC_WRAPPER_PATH
			? dirname(process.env.KERBEROSEC_WRAPPER_PATH)
			: undefined,
	...
	for (const start of starts) {
		...
		for (;;) {
			candidates.push(
				join(current, "node_modules", packageName, "kerberosec-hub/webview"),
			);
...
function resolvePlatformPackageName(): string {
	...
	return `@kerberosec/cli-${platformName}-${arch()}`;
}

async function startDefaultDashboardServer(): Promise<DashboardServerHandle> {
	const { startKerberoSecHubDashboardServer } = await import(
		"@kerberosec/kerberosec-hub"
	);
	return await startKerberoSecHubDashboardServer();
}
...
export async function runDashboardCommand(
	...
	try {
		...
		options.io.writeln(
			`${c.green}KerberoSec dashboard listening at${c.reset} ${dashboardUrl}`,
		);
		if (server.hubUrl) {
