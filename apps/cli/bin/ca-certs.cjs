// Auto-discovery of OS trust anchors for the KerberoSec CLI.
//
// Bun does not read the OS trust store, so the 3.x CLI cannot see corporate
// MITM / self-signed CAs out of the box. This runs in the Node `bin/kerberosec`
...
function resolveKerberoSecDir(env, os, path) {
	return env.KERBEROSEC_DIR?.trim() || path.join(os.homedir(), ".kerberosec");
}

/**
 * True when the api-unavailable warning should print. Stamped per Node version
 * in the kerberosec dir so the nudge shows once rather than on every command; a
...
function shouldWarnApiUnavailable(env, deps = {}) {
	...
	const dir = resolveKerberoSecDir(env, os, path);
...
function configureNodeExtraCaCerts(env, deps = {}) {
	...
	const managedDir = resolveKerberoSecDir(env, os, path);
