...
type EnvSnapshot = {
	...
	KERBEROSEC_HOOKS_LOG_PATH: string | undefined;
	KERBEROSEC_SESSION_ID: string | undefined;
...
function captureEnv(): EnvSnapshot {
	return {
		...
		KERBEROSEC_HOOKS_LOG_PATH: process.env.KERBEROSEC_HOOKS_LOG_PATH,
		KERBEROSEC_SESSION_ID: process.env.KERBEROSEC_SESSION_ID,
...
function restoreEnv(snapshot: EnvSnapshot): void {
	...
	process.env.KERBEROSEC_HOOKS_LOG_PATH = snapshot.KERBEROSEC_HOOKS_LOG_PATH;
	process.env.KERBEROSEC_SESSION_ID = snapshot.KERBEROSEC_SESSION_ID;
...
describe("parseArgs", () => {
	...
	it("parses provider via -P shorthand", () => {
		const parsed = parseArgs(["-P", "kerberosec"]);
		expect(parsed.provider).toBe("kerberosec");
	});

	it("enables sandbox automatically when --data-dir is set", () => {
		const parsed = parseArgs(["--data-dir", "./.tmp-kerberosec"]);
		expect(parsed.sandbox).toBe(true);
		expect(parsed.dataDir).toBe("./.tmp-kerberosec");
	});
...
describe("format helpers", () => {
	...
	// Regression tests for https://github.com/kerberosec/kerberosec/issues/13036:
...
describe("hook payload validation and audit logging", () => {
	...
	it("writes hook audits to global log", async () => {
		...
		delete process.env.KERBEROSEC_HOOKS_LOG_PATH;
		delete process.env.KERBEROSEC_SESSION_ID;
...
describe("sandbox environment", () => {
	it("sets sandbox-specific storage paths", () => {
		...
		const previous = {
			KERBEROSEC_SANDBOX: process.env.KERBEROSEC_SANDBOX,
			KERBEROSEC_SANDBOX_DATA_DIR: process.env.KERBEROSEC_SANDBOX_DATA_DIR,
			KERBEROSEC_DATA_DIR: process.env.KERBEROSEC_DATA_DIR,
			KERBEROSEC_DB_DATA_DIR: process.env.KERBEROSEC_DB_DATA_DIR,
			KERBEROSEC_SESSION_DATA_DIR: process.env.KERBEROSEC_SESSION_DATA_DIR,
		...
		try {
			...
			expect(process.env.KERBEROSEC_SANDBOX).toBe("1");
			expect(process.env.KERBEROSEC_SANDBOX_DATA_DIR).toBe(
				path.join(root, "sandbox-state"),
			);
			expect(process.env.KERBEROSEC_DATA_DIR).toBe(
				path.join(root, "sandbox-state"),
			);
			expect(process.env.KERBEROSEC_DB_DATA_DIR).toBe(
				path.join(root, "sandbox-state", "db"),
			);
			expect(process.env.KERBEROSEC_SESSION_DATA_DIR).toBe(
				path.join(root, "sandbox-state", "sessions"),
			);
			expect(process.env.KERBEROSEC_TEAM_DATA_DIR).toBe(
				path.join(root, "sandbox-state", "teams"),
			);
			expect(process.env.KERBEROSEC_PROVIDER_SETTINGS_PATH).toBe(
				path.join(root, "sandbox-state", "settings", "providers.json"),
			);
			expect(process.env.KERBEROSEC_HOOKS_LOG_PATH).toBe(
				path.join(root, "sandbox-state", "logs", "hooks.jsonl"),
			);
		} finally {
			process.env.KERBEROSEC_SANDBOX = previous.KERBEROSEC_SANDBOX;
			process.env.KERBEROSEC_SANDBOX_DATA_DIR =
				previous.KERBEROSEC_SANDBOX_DATA_DIR;
			process.env.KERBEROSEC_DATA_DIR = previous.KERBEROSEC_DATA_DIR;
			process.env.KERBEROSEC_DB_DATA_DIR = previous.KERBEROSEC_DB_DATA_DIR;
			process.env.KERBEROSEC_SESSION_DATA_DIR =
				previous.KERBEROSEC_SESSION_DATA_DIR;
			process.env.KERBEROSEC_TEAM_DATA_DIR = previous.KERBEROSEC_TEAM_DATA_DIR;
			process.env.KERBEROSEC_PROVIDER_SETTINGS_PATH =
				previous.KERBEROSEC_PROVIDER_SETTINGS_PATH;
			process.env.KERBEROSEC_HOOKS_LOG_PATH =
				previous.KERBEROSEC_HOOKS_LOG_PATH;
