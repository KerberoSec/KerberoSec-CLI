import type { BasicLogger, ITelemetryService } from "@kerberosec/shared";
import { CronService } from "./cron/service/cron-service";
import {
	createKerberoSecCoreAutomationExtensionContext,
	createKerberoSecCoreAutomationRuntimeHandlers,
	KerberoSecCoreAutomationController,
	normalizeAutomationCronScope,
	normalizeAutomationOptions,
} from "./kerberosec-core/automation";
import {
	createKerberoSecCorePendingPromptsApi,
	createKerberoSecCoreSettingsApi,
	type RuntimeHostServiceExtensions,
} from "./kerberosec-core/runtime-services";
import {
	normalizeKerberoSecCoreStartInput,
	toKerberoSecCoreStartInput,
} from "./kerberosec-core/start-input";
import { emitSessionStartedTelemetry } from "./kerberosec-core/telemetry";
import type {
	CompareCheckpointInput,
	CompareCheckpointResult,
	KerberoSecCoreAutomationApi,
	KerberoSecCoreAutomationOptions,
	KerberoSecCoreListHistoryOptions,
	KerberoSecCoreOptions,
	KerberoSecCoreSettingsApi,
	KerberoSecCoreStartInput,
	RestoreInput,
	RestoreResult,
	StartSessionBootstrap,
} from "./kerberosec-core/types";
...
export type {
	...
	CompareCheckpointResult,
	HubOptions,
	KerberoSecAutomationEventIngressResult,
	KerberoSecAutomationEventLog,
	KerberoSecAutomationEventSuppression,
	KerberoSecAutomationListEventsOptions,
	KerberoSecAutomationListRunsOptions,
	KerberoSecAutomationListSpecsOptions,
	KerberoSecAutomationRun,
	KerberoSecAutomationRunStatus,
	KerberoSecAutomationSpec,
	KerberoSecCoreAutomationApi,
	KerberoSecCoreAutomationOptions,
	KerberoSecCoreListHistoryOptions,
	KerberoSecCoreOptions,
...
/**
 * The primary entry point for the KerberoSec Core SDK.
 *
 * @example
 * ```ts
 * import { KerberoSecCore } from "@kerberosec/core";
 *
 * const kerberosec = await KerberoSecCore.create({ clientName: "my-app" });
 * const session = await kerberosec.start({ ... });
 * ```
 */
export class KerberoSecCore {
	readonly clientName: string | undefined;
	readonly runtimeAddress: string | undefined;
	readonly automation: KerberoSecCoreAutomationApi;
	readonly settings: KerberoSecCoreSettingsApi;
	readonly featureFlags: FeatureFlagsService;
	readonly pendingPrompts: PendingPromptsServiceApi;
	private readonly host: RuntimeHost;
	private readonly prepare: KerberoSecCoreOptions["prepare"] | undefined;
	...
	private constructor(
		...
		prepare: KerberoSecCoreOptions["prepare"],
		...
		automationOptions:
			| (KerberoSecCoreAutomationOptions & { logger?: BasicLogger })
			| undefined,
	) {
		...
		this.settings = createKerberoSecCoreSettingsApi(host);
		this.pendingPrompts = createKerberoSecCorePendingPromptsApi(host);
		this.automation = new KerberoSecCoreAutomationController(() => {
			if (!this.automationService) {
				throw new Error(
					"KerberoSecCore automation is not enabled. Pass `automation: true` or automation options to KerberoSecCore.create().",
				);
			}
		...
		this.automationService = automationOptions
			? new CronService({
					...
					runtimeHandlers: createKerberoSecCoreAutomationRuntimeHandlers({
						host,
						getExtensionContext: () =>
							createKerberoSecCoreAutomationExtensionContext({
	...
	/**
	 * Creates a new KerberoSecCore instance.
	 *
	 ...
	 * @returns A promise that resolves to a new KerberoSecCore instance
	 *
	 * @example
	 * ```ts
	 ...
	 */
	static async create(
		options: KerberoSecCoreOptions = {},
	): Promise<KerberoSecCore> {
	...
	/**
	 * Starts a new KerberoSec session with the provided configuration.
	 *
	...
	/**
	 * Starts a new KerberoSec session with extended core-specific configuration.
	 * This overload allows specifying local runtime options and config overrides.
	 */
	start(input: KerberoSecCoreStartInput): Promise<StartSessionResult>;
	async start(
		input: StartSessionInput | KerberoSecCoreStartInput,
	): Promise<StartSessionResult> {
		const kerberosecCoreInput = toKerberoSecCoreStartInput(input);
		const bootstrap = await this.prepare?.(kerberosecCoreInput);
		try {
			const preparedInput = bootstrap
				? await bootstrap.applyToStartSessionInput(kerberosecCoreInput)
				: kerberosecCoreInput;
			const result = await this.host.startSession(
				normalizeKerberoSecCoreStartInput(preparedInput, {
	 ...
	 * @example
	 * ```ts
	 * await kerberosec.send(sessionId, {
	 *   type: "user_message",
	 ...
	 * @example
	 * ```ts
	 * const usageSummary = await kerberosec.getAccumulatedUsage(sessionId);
	 ...
	 * await kerberosec.abort(sessionId);
	 * ```
	 */
	 ...
	 * await kerberosec.stop(sessionId);
	 * ```
	 */
	...
	/**
	 * Disposes the KerberoSecCore instance and all associated resources.
	 *
	 ...
	 * @example
	 * ```ts
	 * const session = await kerberosec.get(sessionId);
	 ...
	 */
	listHistory = async (
		options: KerberoSecCoreListHistoryOptions = {},
	 ...
	 * @example
	 * ```ts
	 * const sessions = await kerberosec.list(50);
	...
	list = async (
		limit = 200,
		options: Omit<KerberoSecCoreListHistoryOptions, "limit"> = {},
	): Promise<SessionHistoryRecord[]> =>
	 ...
	 * @example
	 * ```ts
	 * const deleted = await kerberosec.delete(sessionId);
	 * if (deleted) {
	 ...
	 * @example
	 * ```ts
	 * await kerberosec.update(sessionId, {
	 ...
	 * @example
	 * ```ts
	 * const messages = await kerberosec.readMessages(sessionId);
	...
	async restore(input: RestoreInput): Promise<RestoreResult> {
		const normalizedStart = input.start
			? normalizeKerberoSecCoreStartInput(input.start, {
					defaultCapabilities: this.capabilities,
					withExtensionContext: (context) =>
						createKerberoSecCoreAutomationExtensionContext({
							automationService: this.automationService,
	 ...
	 * @example
	 * ```ts
	 * const unsubscribe = kerberosec.subscribe((event) => {
	 ...
	 * await kerberosec.updateSessionModel(sessionId, "claude-opus-4-1");
	 * ```
	 */
