 ...
 * id: the loader must never write to the shared ~/.kerberosec state files.
 */
async function getDistinctId(): Promise<string> {
	const generated = await readSharedGlobalStateKey("kerberosec.generatedMachineId");
...
async function readSharedGlobalStateKey(key: string): Promise<unknown> {
	try {
		...
		const raw = await readFile(
			path.join(kerberosecDir, "data", "globalState.json"),
			"utf8",
		);
