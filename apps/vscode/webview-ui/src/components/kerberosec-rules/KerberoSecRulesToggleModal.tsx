...
import {
	...
	ToggleKerberoSecRuleRequest,
	ToggleCursorRuleRequest,
	ToggleSkillRequest,
	ToggleWindsurfRuleRequest,
	ToggleWorkflowRequest,
} from "@shared/proto/kerberosec/file"
...
const KerberoSecRulesToggleModal: React.FC = () => {
	const {
		...
		hooksEnabled,
		setGlobalKerberoSecRulesToggles,
		setLocalKerberoSecRulesToggles,
	...
	const globalRules = Object.entries(globalKerberoSecRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))

	// Format local rules for display with proper typing
	const localRules = Object.entries(localKerberoSecRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))
	...
	const toggleRule = (isGlobal: boolean, rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleKerberoSecRule(
			ToggleKerberoSecRuleRequest.create({
			...
			.catch((error) => {
				console.error("Error toggling KerberoSec rule:", error)
			})
	}
	...
	const toggleWindsurfRule = (rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleWindsurfRule(
			...
			.then((response: KerberoSecRulesToggles) => {
				if (response.toggles) {
								...
								{workspaceHooks.map((workspace, index) => (
									<div
										...
										<div className="text-sm font-normal mb-2">
											{workspace.workspaceName}/.kerberosecrules/hooks/
										</div>
										<div className="flex flex-col gap-0">
											{workspace.hooks
												.sort((a, b) => a.name.localeCompare(b.name))
...
export default KerberoSecRulesToggleModal
