 ...
 *   4. KERBEROSEC_BUNDLE_OVERRIDE / the kerberosec.rollout.bundleOverride setting
 ...
 *      once per window
 *   7. the nightly identity (manifest name kerberosec-nightly) switches the
...
function makeSandbox({
	...
	const sandbox = mkdtempSync(path.join(tmpdir(), "kerberosec-ab-smoke-"));
	cpSync(
	...
	writeFileSync(
		...
		JSON.stringify({ "kerberosec.generatedMachineId": "smoke-machine" }),
	);
	return sandbox;
}
...
await runScenario(
	...
	{ seed: { "kerberosec.rollout.bundle": "next" } },
	async ({ api, sandbox }) => {
...
await runScenario(
	...
	async ({ context, api, fetchCalls }) => {
		...
		assert.deepEqual(api, { bundle: "legacy" });
		assert.equal(context.globalState._dump()["kerberosec.rollout.bundle"], "next");
...
await runScenario(
	...
	async ({ context, api, fetchCalls }) => {
		...
		assert.deepEqual(api, { bundle: "next" });
		assert.equal(context.globalState._dump()["kerberosec.rollout.bundle"], "legacy");
		const [featureFlagCalled] = featureFlagCalledCaptures(fetchCalls);
		assert.ok(featureFlagCalled, "rollout refresh must emit the PostHog feature-flag exposure event");
...
await runScenario(
	...
	{ env: { KERBEROSEC_BUNDLE_OVERRIDE: "next" } },
	async ({ api }) => {
		assert.deepEqual(api, { bundle: "next" });
	},
);
...
await runScenario(
	...
	{
		seed: { "kerberosec.rollout.bundle": "next" },
		settings: { "kerberosec.rollout.bundleOverride": "legacy" },
	},
	async ({ api }) => {
		assert.deepEqual(api, { bundle: "legacy" });
	},
...
await runScenario(
	...
	{
		seed: { "kerberosec.rollout.bundle": "legacy" },
		settings: { "kerberosec.rollout.bundleOverride": "next" },
	},
	async ({ api }) => {
		assert.deepEqual(api, { bundle: "next" });
	},
...
await runScenario(
	...
	async ({ context, api, fetchCalls }) => {
		...
		assert.equal(state["kerberosec.rollout.bundle"], "legacy");
		assert.equal(
			state["kerberosec.rollout.nextActivationFailedVersion"],
			"4.1.0-smoke",
		);
		assert.equal(
...
await runScenario(
	...
	{
		contextPackageJSON: { name: "kerberosec-nightly" },
		settings: { "kerberosec-nightly.rollout.bundleOverride": "next" },
	},
	async ({ api, context }) => {
		assert.deepEqual(api, { bundle: "next" });
		assert.deepEqual(executedCommands[0], [
		...
		assert.equal(item.text, "KerberoSec: Next");
