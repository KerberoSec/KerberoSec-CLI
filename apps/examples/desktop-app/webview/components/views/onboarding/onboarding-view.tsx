...
import { AgentWelcomeHero, Button, IconButton } from "@kerberosec/ui";
import {
	ArrowLeft,
	CheckCircle2,
	ChevronDown,
...
import { KerberoSecLogo } from "@/components/kerberosec-logo";
...
type SetupMethod = "kerberosec" | "api-key";
 ...
 * providers (KerberoSec itself, KerberoSecPass, ChatGPT, OCA) are excluded because they
		...
		<OnboardingContent surface="transparent">
			<div className="flex flex-col items-center py-4 text-center">
				...
				<p className="mt-6 text-md text-muted-foreground">
					KerberoSec is an AI coding agent. It reads your code, edits files,
					runs commands, and works through tasks with you — in any project on
					your machine.
				</p>
				<Button
...
function ConnectStep({
	...
	const [kerberosecApiKey, setKerberoSecApiKey] = useState("");
	const [kerberosecKeySaving, setKerberoSecKeySaving] = useState(false);
	const [kerberosecKeyError, setKerberoSecKeyError] = useState<string | null>(
		null,
	);
	...
	const signInWithKerberoSec = useCallback(async () => {
		...
		try {
			...
			rememberProviderSelection({ id: "kerberosec" });
			await refreshAccount();
			onConnected({ kind: "kerberosec" });
		} catch (error) {
			if (signInAttemptRef.current !== attempt) {
				return;
			}
	...
	const cancelSignInWithKerberoSec = useCallback(() => {
		signInAttemptRef.current += 1;
		setSigningIn(false);
	...
	const connectWithKerberoSecApiKey = useCallback(async () => {
		const key = kerberosecApiKey.trim();
		if (!key) {
			return;
		}
		setKerberoSecKeySaving(true);
		setKerberoSecKeyError(null);
		try {
			...
			try {
				const verified = await desktopClient.invoke("kerberosec_account", {
				...
				if (isKerberoSecAccountNotAuthenticatedResult(verified)) {
					throw new Error("no KerberoSec account credentials were found");
				}
			} catch (verifyError) {
			...
			setKerberoSecKeyError(getErrorMessage(error));
		} finally {
			// Credentials may have been saved (or rolled back); drop the
			// short-lived catalog cache so consumers reload the persisted state.
			invalidateProviderCatalogCache();
			setKerberoSecKeySaving(false);
		}
	}, [kerberosecApiKey, onConnected, refreshAccount]);

	const kerberosecBusy = signingIn || kerberosecKeySaving;
	...
	const [selectedMethod, setSelectedMethod] =
		useState<SetupMethod>("kerberosec");
	const [kerberosecKeyFormExpanded, setKerberoSecKeyFormExpanded] =
		useState(false);
		...
		<OnboardingContent surface="panel">
			<div className="flex flex-col">
				...
				<p className="mt-4 text-sm text-muted-foreground">
					Choose how KerberoSec connects to models. You can add more providers
					anytime in Settings.
				</p>
			</div>
			...
			<div className="mt-8 flex flex-col gap-3">
				<SetupOptionCard
					id="kerberosec"
					onSelect={() => setSelectedMethod("kerberosec")}
					selectLabel="Sign in with KerberoSec"
					selected={selectedMethod === "kerberosec"}
				>
					<SetupOptionHeader
						accessory={
							<Badge
						...
						icon={<KerberoSecLogo className="size-5" />}
						title="Sign in with KerberoSec"
					/>
					{user ? (
						...
						<div className="mt-8 ml-12 flex flex-wrap items-center gap-1 max-[720px]:ml-0">
							<Button
								disabled={kerberosecBusy}
								onClick={() => void signInWithKerberoSec()}
								size="md"
								tone="accent"
								type="button"
								variant="fill"
							...
							{signingIn ? (
								<Button
									onClick={cancelSignInWithKerberoSec}
									size="md"
									tone="neutral"
									type="button"
									variant="ghost"
					...
					{!user ? (
						<div className="mt-6 ml-10 -mb-2 max-[720px]:ml-0">
							<Button
								aria-controls="onboarding-kerberosec-key-form"
								aria-expanded={kerberosecKeyFormExpanded}
								disabled={kerberosecBusy}
								onClick={() => {
									setSelectedMethod("kerberosec");
									setKerberoSecKeyFormExpanded(!kerberosecKeyFormExpanded);
								}}
								size="xs"
								tone="neutral"
								type="button"
								variant="ghost"
							>
								Use a KerberoSec API key
								<ChevronDown aria-hidden="true" className="size-3.5" />
							</Button>
							<ExpandablePanel
								data-onboarding-kerberosec-key-form
								expanded={kerberosecKeyFormExpanded}
								id="onboarding-kerberosec-key-form"
							>
								<div className="flex flex-col gap-2 pt-3 ml-2 max-[720px]:ml-0">
									<div className="flex flex-wrap items-center gap-2">
										<Input
											aria-label="KerberoSec API key"
											autoComplete="off"
											className="min-w-52 flex-1 bg-background"
											disabled={kerberosecKeySaving}
											onChange={(event) => {
												setKerberoSecApiKey(event.target.value);
												setKerberoSecKeyError(null);
											}}
											onKeyDown={(event) => {
												if (
													event.key === "Enter" &&
													kerberosecApiKey.trim() &&
													!kerberosecKeySaving
												) {
													void connectWithKerberoSecApiKey();
												}
											}}
											placeholder="KerberoSec API key"
											type="password"
											value={kerberosecApiKey}
										/>
										<Button
											disabled={!kerberosecApiKey.trim() || kerberosecKeySaving}
											onClick={() => void connectWithKerberoSecApiKey()}
											size="md"
											tone="accent"
											type="button"
											variant="fill"
										>
											...
											{kerberosecKeySaving ? "Connecting..." : "Connect"}
										</Button>
									</div>
									<Button
										...
										disabled={kerberosecKeySaving}
										onClick={() =>
											void openExternalUrl(KERBEROSEC_DASHBOARD_URL)
										}
										size="xs"
										tone="neutral"
										type="button"
									...
									{kerberosecKeyError ? (
										<p className="text-xs text-destructive" role="alert">
											Failed to save API key: {kerberosecKeyError}
										</p>
									) : null}
								</div>
							</ExpandablePanel>
				...
				<SetupOptionCard
					...
					onSelect={() => {
						...
						setKerberoSecKeyFormExpanded(false);
					}}
			...
			<div className="flex flex-col items-center py-4 text-center">
				...
				<p className="mt-3 text-md text-muted-foreground">
					{connection?.kind === "provider"
						...
						: "Your KerberoSec account is connected."}
				</p>
				<Button
					className="mt-8 w-full max-w-64"
					onClick={onFinish}
				...
				{step === "welcome" ? (
					...
					<ConnectStep
						...
						onConnected={(nextConnection) => {
							...
							// The GitHub integration lives on the KerberoSec account, so the
							// step only applies when one is connected
							setStep(
								nextConnection.kind === "kerberosec" && githubStepEnabled
									? "github"
									: "done",
							);
						}}
