...
description: "Configure Single Sign-On (SSO) for KerberoSec Enterprise via WorkOS AuthKit."
---

## Overview
KerberoSec Enterprise integrates with your identity provider (IdP) via **WorkOS AuthKit** for SSO.

This page describes, at a high level, how SSO is set up for KerberoSec Enterprise using WorkOS AuthKit.
...
1) **KerberoSec Dashboard (app.kerberosec.bot)**
...
## Using the KerberoSec Dashboard
Use the KerberoSec Dashboard at https://app.kerberosec.bot to:
...
To connect your IdP to KerberoSec Enterprise, configure your identity provider in **WorkOS AuthKit**:
...
KerberoSec Enterprise’s default SSO integration is **via WorkOS**.
...
1) Attempt an SSO sign-in from https://app.kerberosec.bot.
2) Confirm the sign-in completes (you are redirected back successfully).

## Troubleshooting
- **Redirect URI mismatch**: confirm the redirect/callback URL configured in WorkOS matches what was provided during your KerberoSec Enterprise onboarding.
