...
async function main(): Promise<void> {
	const packDir = await mkdtemp(join(tmpdir(), "kerberosec-node-smoke-packs-"));
	const smokeDir = await mkdtemp(join(tmpdir(), "kerberosec-node-smoke-"));
	const sessionsDir = await mkdtemp(
		join(tmpdir(), "kerberosec-node-sessions-"),
	);
	const npmCacheDir = await mkdtemp(
		join(tmpdir(), "kerberosec-node-npm-cache-"),
	);
	...
	try {
		...
		await writeFile(
			...
			`${JSON.stringify(
				{
					name: "kerberosec-node-smoke",
					private: true,
					type: "module",
					dependencies: {
						"@kerberosec/core": `file:${tarballs.core}`,
						"@kerberosec/agents": `file:${tarballs.agents}`,
						"@kerberosec/llms": `file:${tarballs.llms}`,
						"@kerberosec/shared": `file:${tarballs.shared}`,
					},
		...
		const smokeSource =
			nodeMajor >= 24
				? `
					const { SqliteSessionStore } = await import("@kerberosec/core");
					const store = new SqliteSessionStore({ sessionsDir: process.env.KERBEROSEC_DATA_DIR });
					try {
						store.init();
				...
				`
				: `
					const { resolveSessionBackend } = await import("@kerberosec/core");
		...
		await runCommand(["node", smokeFile], {
			...
			env: {
				...npmEnv,
				KERBEROSEC_DATA_DIR: sessionsDir,
			},
			timeoutMs: 2 * 60_000,
		});
	} finally {
