# KerberoSec SDK Examples

Learn how to build with the KerberoSec SDK through practical, runnable examples.

## 📁 Plugin, Hook, and Automation Examples

Plugins extend the CLI and SDK with custom capabilities. Install local files, GitHub file URLs, package directories, git repos, and npm packages with `kerberosec plugin install`:
...
- Export a reusable plugin module for `.kerberosec/plugins`
...
```bash
kerberosec plugin install https://github.com/kerberosec/kerberosec/blob/main/sdk/examples/plugins/weather-metrics.ts
...
```bash
kerberosec plugin install https://github.com/kerberosec/kerberosec/blob/main/sdk/examples/plugins/typescript-lsp/index.ts
kerberosec -i "Find where createTool is defined"
```
...
kerberosec -i "Use subagents to inspect this repository and report back."
```
...
**Example file-based and event-driven automation specs** for global `~/.kerberosec/cron/`:
...
```bash
mkdir -p ~/.kerberosec/cron
cp examples/cron/changelog-generator.cron.md ~/.kerberosec/cron/
mkdir -p ~/.kerberosec/cron/events
cp examples/cron/events/pr-changelog-check.event.md ~/.kerberosec/cron/events/
```
...
Hooks live in `.kerberosec/hooks/` and are named after the event they handle (PreToolUse, PostToolUse, TaskStart, etc.):
...
# Or Python
cp examples/hooks/PreToolUse.py ~/.kerberosec/hooks/PreToolUse.py
chmod +x ~/.kerberosec/hooks/PreToolUse.py

# Or TypeScript (runs via bun)
cp examples/hooks/PreToolUse.ts ~/.kerberosec/hooks/PreToolUse.ts
chmod +x ~/.kerberosec/hooks/PreToolUse.ts

kerberosec -i "do something"  # Hooks will execute automatically
```
...
```bash
npm add @kerberosec/core
```

Add `@kerberosec/agents` or `@kerberosec/llms` only if you intentionally want lower-level control. For RPC client helpers, prefer importing from `@kerberosec/core` when you want the app-facing SDK surface.

Current SDK layering:

- `@kerberosec/core` owns config discovery/watchers, runtime plugin loading, and the context pipeline
- context compaction is core-owned and runs through turn preparation before model calls
- most app integrations should stay on the `@kerberosec/core` surface unless they intentionally need lower-level agent or model control
...
- [KerberoSec SDK README](../packages/README.md)
