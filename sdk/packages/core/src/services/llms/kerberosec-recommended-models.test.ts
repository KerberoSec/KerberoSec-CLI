...
import {
	...
	peekKerberoSecRecommendedModels,
	resetKerberoSecRecommendedModelsCacheForTests,
} from "./kerberosec-recommended-models";
...
const ENDPOINT_PAYLOAD = {
	...
	free: [
		...
		{
			id: "kerberosec-free/glm-5.2",
			name: "kerberosec-free/glm-5.2",
			description: "",
		},
		{
	...
	kerberosecPass: [
		{
			...
			name: "kerberosec-pass/glm-5.2",
			description: "",
		},
		{
			id: "kerberosec-pass/mystery",
			name: "kerberosec-pass/mystery",
			description: "",
		},
	],
};
...
const CATALOG = {
	...
	kerberosec: {
		...
		"kerberosec-free/glm-5.2": model(
			"kerberosec-free/glm-5.2",
			"GLM-5.2 (free)",
		),
	...
	"kerberosec-pass": {
		"kerberosec-pass/glm-5.2": model("kerberosec-pass/glm-5.2", "GLM-5.2"),
	},
};

function namesOf(data: KerberoSecRecommendedModelsData) {
	return {
		recommended: data.recommended.map((m) => m.name),
		free: data.free.map((m) => m.name),
		kerberosecPass: data.kerberosecPass.map((m) => m.name),
	};
}

describe("fetchKerberoSecRecommendedModels", () => {
	it("resolves display names from the models catalog", async () => {
		const data = await fetchKerberoSecRecommendedModels({
			baseUrl: BASE_URL,
		...
		expect(namesOf(data)).toEqual({
			...
			kerberosecPass: ["GLM-5.2", "mystery"],
		});
	...
	it("returns the bundled fallback untouched when the endpoint fails", async () => {
		...
		expect(data).toEqual(FALLBACK_KERBEROSEC_RECOMMENDED_MODELS);
	});
});

describe("applyKerberoSecFeaturedModels", () => {
	const data: KerberoSecRecommendedModelsData = {
		recommended: [
			{
				id: "anthropic/claude-opus-5",
				name: "Claude Opus 5",
	...
	it("stamps recommended and free tiers onto the kerberosec model list", () => {
		const models = applyKerberoSecFeaturedModels(
			"kerberosec",
			[
	...
	it("stamps subscribed and free tiers for kerberosec-pass and skips other providers", () => {
		const models = applyKerberoSecFeaturedModels(
			"kerberosec-pass",
			[
				{ id: "kerberosec-pass/kimi-k3", name: "Kimi K3" },
				{ id: "deepseek/deepseek-v4-flash", name: "DeepSeek V4 Flash" },
			],
			data,
		);
		...
		expect(applyKerberoSecFeaturedModels("anthropic", untouched, data)).toBe(
			untouched,
		);
	});

	it("matches vendor-prefix mismatches by unambiguous slug without duplicating tiers", () => {
		const fallbackVintage: KerberoSecRecommendedModelsData = {
			recommended: [],
			free: [
				{
		...
		// Catalog knows the model only under the kerberosec-free prefix: the slug
		// fallback stamps it even though no alias rule covers the prefix.
		const slugOnly = applyKerberoSecFeaturedModels(
			"kerberosec",
			[{ id: "kerberosec-free/kat-coder-pro", name: "Kat Coder Pro" }],
			fallbackVintage,
		);
		...
		const bothSpellings = applyKerberoSecFeaturedModels(
			"kerberosec",
			[
				{ id: "kwaipilot/kat-coder-pro", name: "Kat Coder Pro" },
				{ id: "kerberosec-free/kat-coder-pro", name: "Kat Coder Pro (free)" },
			],
			fallbackVintage,
		);
		...
		const ambiguous = applyKerberoSecFeaturedModels(
			"kerberosec",
			[{ id: "kerberosec-free/shared-slug", name: "Shared" }],
			{
				recommended: [
...
describe("getCachedKerberoSecRecommendedModels", () => {
	it("serves one fetch to concurrent and subsequent callers", async () => {
		...
		const [first, second] = await Promise.all([
			getCachedKerberoSecRecommendedModels(options),
			getCachedKerberoSecRecommendedModels(options),
		]);
		const third = await getCachedKerberoSecRecommendedModels(options);
	...
	it("caches the bundled fallback so offline callers do not re-pay the timeout", async () => {
		...
		const first = await getCachedKerberoSecRecommendedModels(options);
		const second = await getCachedKerberoSecRecommendedModels(options);

		expect(calls).toBe(1);
		expect(first).toEqual(FALLBACK_KERBEROSEC_RECOMMENDED_MODELS);
		expect(second).toEqual(FALLBACK_KERBEROSEC_RECOMMENDED_MODELS);
		resetKerberoSecRecommendedModelsCacheForTests();
	});
	...
	it("does not let an in-flight request repopulate the cache after a reset", async () => {
		...
		const stale = getCachedKerberoSecRecommendedModels({
			baseUrl: BASE_URL,
...
describe("peekKerberoSecRecommendedModels", () => {
	it("returns the bundled fallback when the cache is cold", () => {
		resetKerberoSecRecommendedModelsCacheForTests();
		expect(peekKerberoSecRecommendedModels()).toEqual(
			FALLBACK_KERBEROSEC_RECOMMENDED_MODELS,
		);
	});
	...
	it("returns the cached live feed once warmed, without another fetch", async () => {
		...
		const peeked = peekKerberoSecRecommendedModels();
