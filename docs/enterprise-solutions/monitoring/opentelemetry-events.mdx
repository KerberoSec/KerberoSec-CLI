...
description: "Complete reference of OpenTelemetry log events emitted by KerberoSec"
---

This page documents all OpenTelemetry log events currently instrumented in KerberoSec. These events are emitted when OpenTelemetry integration is enabled and provide detailed insights into user behavior, task execution, and system operations.
...
KerberoSec emits events across several categories, each prefixed with a namespace:
...
| `task.rule_toggled` | KerberoSec rule toggled on/off | rule_name, enabled, is_global |
| `task.yolo_mode_toggled` | YOLO mode toggled | enabled |
| `task.kerberosec_web_tools_toggled` | KerberoSec web tools setting toggled | enabled |
...
| Event | Description | Key Attributes |
|-------|-------------|----------------|
| `kerberosec.test.connection` | OTEL connection test event from "Test OTEL Connection" flow | success, exporter_type, endpoint |

### Example: kerberosec.test.connection

```json
{
  "event": "kerberosec.test.connection",
...
| `extension_version` | string | KerberoSec extension version |
...
sequenceDiagram
    participant User
    participant KerberoSec
    participant OTel

    User->>KerberoSec: Start Task
    KerberoSec->>OTel: task.created
    
    User->>KerberoSec: Submit Message
    KerberoSec->>OTel: task.conversation_turn (user)
    
    KerberoSec->>KerberoSec: Process with AI
    KerberoSec->>OTel: task.tokens
    KerberoSec->>OTel: task.conversation_turn (assistant)
    
    KerberoSec->>KerberoSec: Use Tool
    KerberoSec->>OTel: task.tool_used
    
    User->>KerberoSec: Provide Feedback
    KerberoSec->>OTel: task.option_selected
    
    User->>KerberoSec: Complete Task
    KerberoSec->>OTel: task.completed
```
  ...
  "widgets": [
    {
      "definition": {
        ...
        "requests": [
          {
            "q": "sum:kerberosec.task.completed{*}.as_count()",
            "display_type": "bars"
          }
        ],
