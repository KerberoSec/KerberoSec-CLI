...
export async function writeLgWebhookConfig(webhookUrl: string, webhookToken: string): Promise<void> {
	...
	const kerberosecDir = path.join(documentsPath, "KerberoSec")
	const configPath = path.join(kerberosecDir, "webhook_config.json")

	await fs.mkdir(kerberosecDir, { recursive: true })
	await fs.writeFile(
		configPath,
		JSON.stringify(
			{
...
try {
    ...
    $configPath = Join-Path (Join-Path $documentsPath "KerberoSec") "webhook_config.json"
    ...
    $payload = @{
        ...
        data = @{
            ...
            kerberosec_version = if ($inputData) { $inputData.kerberosecVersion } else { $null }
...
async function readConfig() {
  const configPath = path.join(os.homedir(), "Documents", "KerberoSec", "webhook_config.json")
  try {
...
main((input) => ({
  ...
  data: {
    ...
    kerberosec_version: input.kerberosecVersion ?? null,
