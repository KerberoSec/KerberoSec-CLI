 ...
 *   KERBEROSEC_VCR           - "record" to record HTTP requests, "playback" to replay them
 *   KERBEROSEC_VCR_CASSETTE  - Path to the cassette file (default: ./vcr-cassette.json)
 *   KERBEROSEC_VCR_FILTER    - Substring to filter recorded/replayed request paths.
 ...
 *   KERBEROSEC_VCR_INCLUDE_REQUEST_BODY - "1" to save sanitized request bodies and
 *                         assert them during playback.
 *   KERBEROSEC_VCR_SSE_DELAY - Milliseconds between SSE chunks during playback (default: 100).
 *                         Set to 0 for instant delivery.
 *
 * Usage:
 *   # Record only inference requests
 *   KERBEROSEC_VCR=record KERBEROSEC_VCR_CASSETTE=./fixtures/my-test.json kerberosec task "hello"
 *
 *   # Replay: auth/S3/etc. requests go through normally, only inference is mocked
 *   KERBEROSEC_VCR=playback KERBEROSEC_VCR_CASSETTE=./fixtures/my-test.json kerberosec task "hello"
 *
 *   # Record everything (no filter)
 *   KERBEROSEC_VCR=record KERBEROSEC_VCR_FILTER="" KERBEROSEC_VCR_CASSETTE=./fixtures/all.json kerberosec task "hello"
 */
...
const PATH_NORMALIZATION_PATTERNS: { pattern: RegExp; replacement: string }[] =
	[
		...
		// Matches common KerberoSec ID formats: prefix + ULID/UUID-like suffix
		{
			pattern:
...
function getVcrConfig(vcrMode: string | undefined): VcrConfig | null {
	...
	if (!process.env.KERBEROSEC_VCR_CASSETTE) {
		process.stderr.write(
			"[VCR] No KERBEROSEC_VCR_CASSETTE: requests will not be recorded or played back.\n",
		);
		return null;
	}
	...
	if (vcrMode !== "record" && vcrMode !== "playback") {
		process.stderr.write(
			`[VCR] Invalid KERBEROSEC_VCR value: "${vcrMode}". Expected "record" or "playback".\n`,
		);
		process.exit(1);
	}

	const cassettePath = resolve(process.env.KERBEROSEC_VCR_CASSETTE);
	const filter = process.env.KERBEROSEC_VCR_FILTER ?? "";
	const includeRequestBody =
		process.env.KERBEROSEC_VCR_INCLUDE_REQUEST_BODY === "1" ||
		process.env.KERBEROSEC_VCR_INCLUDE_REQUEST_BODY === "true";
...
function startPlayingBackRequests(cassettePath: string, filter: string): void {
	...
	const sseDelayMs = Number.parseInt(
		process.env.KERBEROSEC_VCR_SSE_DELAY ?? "100",
		10,
	);
	...
	globalThis.fetch = Object.assign(
		async (
			...
			if (matchIndex >= 0) {
				...
				// real-time delivery (controlled by KERBEROSEC_VCR_SSE_DELAY).
 ...
 *
 * Does nothing if `KERBEROSEC_VCR` is not set.
 */
