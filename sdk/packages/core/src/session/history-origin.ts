import type { ClientContext } from "@kerberosec/shared";
import {
	SessionSource,
...
export function resolveClientSessionSource(
	...
	if (identity.includes("kerberosec-platform")) {
		return SessionSource.WEB;
	}
	if (
		identity.includes("kerberosec-cli") ||
		identity.includes("kerberosec-acp")
	) {
		return SessionSource.CLI;
	}
	if (
		identity.includes("kerberosec-sdk") ||
		identity.includes("kerberosec-core")
	) {
