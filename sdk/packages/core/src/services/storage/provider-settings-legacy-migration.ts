...
import { ReasoningLevelSchema } from "@kerberosec/shared";
import { resolveKerberoSecDataDir } from "@kerberosec/shared/storage";
import {
...
interface LegacyGlobalState {
	...
	planModeKerberoSecModelId?: string;
	actModeKerberoSecModelId?: string;
...
interface LegacySecrets {
	apiKey?: string;
	kerberosecApiKey?: string;
	"kerberosec:kerberosecAccountId"?: string;
	kerberosecAccountId?: string;
...
export type LegacyKerberoSecUserInfo = {
	idToken: string;
	expiresAt: number;
	refreshToken: string;
	userInfo: {
		...
		kerberosecBenchConsent: boolean;
		createdAt: string;
		updatedAt: string;
	};
	provider: string;
...
/**
 * Resolves legacy KerberoSec account auth data from the raw `kerberosec:kerberosecAccountId`
 ...
 * unparseable JSON.
 */
export function resolveLegacyKerberoSecAuth(
	...
	try {
		const data = JSON.parse(trimmed) as LegacyKerberoSecUserInfo;
		if (!data) {
			return undefined;
		}
		const expiresAt =
...
function resolveLegacyStorage(
	...
	const dataDir = options.dataDir ?? resolveKerberoSecDataDir();
...
function resolveModelForProvider(
	...
	const providerModelKeyById: Record<string, keyof LegacyGlobalState> = {
		...
		kerberosec: `${modePrefix}KerberoSecModelId` as keyof LegacyGlobalState,
...
function getDefaultModelForProvider(providerId: string): string | undefined {
	...
	// KerberoSec's generated block holds a few free models while its default,
...
/**
 * KerberoSec is a fixed-catalog provider: an unknown legacy model id (retired
 ...
 * runtime catalog (the collection model list, which `buildKerberoSecModels` in
 * @kerberosec/llms mirrors), folding alias spellings onto their canonical ids
 ...
 */
function resolveKnownKerberoSecModel(
	providerId: string,
	modelId: string | undefined,
): string | undefined {
	if (!modelId || providerId !== "kerberosec") {
		return modelId;
	}
	const catalogModels =
...
function buildLegacyProviderSettings(
	...
	const secretByProvider: Record<string, string | undefined> = {
		...
		kerberosec: legacySecrets.kerberosecApiKey,
	...
	if (providerId === "kerberosec") {
		try {
			const legacyAuthString = trimNonEmpty(
				legacySecrets["kerberosec:kerberosecAccountId"],
			);

			if (legacyAuthString) {
				providerSpecific.auth = {
					...(providerSpecific.auth ?? {}),
					...resolveLegacyKerberoSecAuth(legacyAuthString),
				};
			}
		} catch {
			// Failed to parse stored kerberosec auth data
		}
	}
	if (
...
function collectCandidateProviderIds(
	...
	if (trimNonEmpty(legacySecrets.kerberosecApiKey))
		candidates.add("kerberosec");
	const legacyKerberoSecAuth = resolveLegacyKerberoSecAuth(
		trimNonEmpty(legacySecrets["kerberosec:kerberosecAccountId"]),
	);
	if (
		legacyKerberoSecAuth?.accessToken ||
		legacyKerberoSecAuth?.refreshToken ||
		legacyKerberoSecAuth?.accountId
	) {
		candidates.add("kerberosec");
	}
