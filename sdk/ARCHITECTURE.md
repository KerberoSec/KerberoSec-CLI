# KerberoSec SDK Architecture

This document is the architecture source of truth for the KerberoSec SDK repository. It describes how the system is organized, how components interact, and the design principles that guide development decisions.

**Who should read this?**
- SDK contributors working across multiple packages
- Developers building integrations or host applications using `@kerberosec/core`
...
```mermaid
flowchart LR
  shared["@kerberosec/shared"]
  llms["@kerberosec/llms"]
  agents["@kerberosec/agents"]
  core["@kerberosec/core"]
  apps["Host Apps"]
...
- hub discovery, the detached hub daemon, and the `@kerberosec/core/hub/daemon-entry` subpath
- host-side hub client adapters (`NodeHubClient`, `HubSessionClient`, `HubUIClient`, `connectToHub`) exported from `@kerberosec/core/hub`
...
1. Host constructs a `RuntimeHost` through `@kerberosec/core`.
2. `@kerberosec/core` selects `LocalRuntimeHost` through `packages/core/src/runtime/host.ts`.
...
4. `@kerberosec/core` prepares a local bootstrap artifact from `localRuntime`, then builds the runtime from it.
5. `@kerberosec/core` creates an `Agent` from `@kerberosec/agents`.
6. `@kerberosec/agents` runs the loop using `@kerberosec/llms` handlers.
7. `@kerberosec/core` persists state, artifacts, and metadata.
...
KerberoSec's `attempt_completion`) is observed. A single teardown choke
...
1. Host constructs a `RuntimeHost` through `@kerberosec/core`.
2. `@kerberosec/core` selects `HubRuntimeHost` or `RemoteRuntimeHost` through `packages/core/src/runtime/host.ts`.
3. When no compatible local hub is already discovered, `@kerberosec/core` can spawn a detached hub daemon and reconnect through discovery.
...
5. The hub-hosted runtime executes the agent loop using `@kerberosec/agents` and `@kerberosec/llms`.
6. `@kerberosec/core` hub services broker sessions, events, approvals, schedules, and client-owned runtime capabilities such as session-local tool executors.
...
8. Hub client adapters exported from `@kerberosec/core/hub` (`NodeHubClient`, `HubSessionClient`, `HubUIClient`, `connectToHub`) translate command/reply and event streams into host-facing APIs.
...
1. `@kerberosec/llms` validates provider output once and creates canonical
...
2. Provider model tools are adapters, not raw AI SDK tools. An adapter owns its
   ...
   model-tool metadata.
3. `@kerberosec/agents` appends media events at their exact stream position in the
...
4. `@kerberosec/core` projects live media as `content_end(media)`. The hub publishes
   `assistant.media`, preserving the same media ID, and clients deduplicate live
   and hydrated content by that ID.
5. Web clients share `GeneratedMediaContent` from `@kerberosec/ui` for image, audio,
...
`~/.kerberosec/data/workspaces/chat`). The chat workspace is seeded with an
...
3. Hub-required flows such as `kerberosec hub`, schedules, connectors, and `--zen` may still call the explicit ensure path because those commands require a live hub before proceeding.
...
1. `@kerberosec/shared/db` owns the low-level SQLite connector store and the one-time legacy JSON import.
...
3. `@kerberosec/core` owns connector autostart persistence and reconnect orchestration. The detached hub daemon is the sole startup reconnect owner, preventing dashboard startup from racing it and launching duplicate processes.
...
2. `@kerberosec/shared/remote-config` caches the bundle when configured.
3. Shared remote-config materializes managed rules/workflows/skills under workspace-local `.kerberosec/<plugin>/`.
4. Shared remote-config derives generic OpenTelemetry config and session blob upload metadata from the bundle.
5. `@kerberosec/core` exposes the app-facing integration wrapper that applies extensions, telemetry, and session metadata to `StartSessionInput`.
6. `@kerberosec/core` consumes the prepared local overrides during local bootstrap.
...
- `KerberoSecCore` delegates uniformly to `RuntimeHost` and does not branch on local vs hub behavior
- transport-specific translation belongs inside concrete hosts, not in top-level orchestration
- `RuntimeHost` inputs stay transport-safe, while `KerberoSecCore.start(...)` is the app-facing facade that normalizes broad local config before delegation
...
- pending prompt list/update/delete are exposed through the grouped
  `KerberoSecCore.pendingPrompts` service. Usage summary lookup and active-session
  model switching are also service-style capabilities exposed through
  `KerberoSecCore` when the concrete transport implements them. These service APIs
...
`KerberoSecCore.create(...)` exposes a generic `prepare(input)` hook.
...
Cross-package logging uses a small injected interface exported from `@kerberosec/shared`:
...
- **`TelemetryLoggerSink` (`@kerberosec/core`)** — an **`ITelemetryAdapter`** that mirrors telemetry events and metrics into a `BasicLogger`. It is a telemetry sink, not a host logging implementation.
...
- `@kerberosec/agents` owns the generic turn-preparation seam:
...
- `@kerberosec/core` owns compaction policy:
...
`KERBEROSEC_PLUGIN_IDLE_TIMEOUT_MS`), and the next plugin call starts and
...
Do not move these concerns into `@kerberosec/agents`:
...
Do not make `@kerberosec/core` organization- or provider-specific.

If a capability is truly generic and app-facing, add a generic core seam. Reusable remote-config parsing, materialization, and upload primitives belong in `@kerberosec/shared/remote-config`.
...
- User-editable intent is represented as Markdown with YAML frontmatter in
  `~/.kerberosec/tasks/*.task.md` for global tasks and
  `<workspace>/.kerberosec/tasks/*.task.md` for workspace tasks. Operational fields
...
- KerberoSec Code projects the same Hub state into an Agenda section in the desktop
...
## File-Based And Event-Driven Automation (`KerberoSecCore` / `CronService`)

`@kerberosec/core` ships a file-based automation subsystem under
`packages/core/src/cron/`. It lets operators author recurring and one-off
tasks as Markdown files under global `~/.kerberosec/cron/` by default, and
event-driven tasks as `events/*.event.md` specs. All trigger kinds run
through the same durable queue and runtime handlers. `KerberoSecCore` exposes the
SDK-facing `kerberosec.automation.*` entry points; `CronService` is the internal
...
1. **Spec parser** (`cron/specs/cron-spec-parser.ts`): parses YAML frontmatter + body
   ...
   Types live in `@kerberosec/shared` under `src/cron/cron-spec-types.ts`
...
2. **Store** (`cron/store/sqlite-cron-store.ts`): owns `cron.db` at
   `resolveCronDbPath()` (default `.kerberosec/data/db/cron.db`). Schema is
...
3. **Reconciler** (`cron/specs/cron-reconciler.ts`): scans the configured cron specs
   directory (global `~/.kerberosec/cron/` by default, or workspace-scoped when
...
8. **Reports** (`cron/reports/cron-report-writer.ts`): writes
   `.kerberosec/cron/reports/<run-id>.md` with run frontmatter plus
...
9. **Service** (`cron/service/cron-service.ts`): orchestrates all of the above.
   `KerberoSecCore.create({ automation })` owns the SDK-facing lifecycle and exposes
   `kerberosec.automation.*` methods. Hub-side callers can submit normalized events
   through the `cron.event.ingest` command.

The detached hub daemon passes its workspace root as `cronOptions`, so
normal CLI/hub startup watches `${workspaceRoot}/.kerberosec/cron/` without a
...
- **`KerberoSecCore`** — `packages/core/src/index.ts` — the main SDK orchestrator
...
- `@kerberosec/shared` — shared types, contracts, and low-level utilities
- `@kerberosec/llms` — provider integrations and model manifests
- `@kerberosec/agents` — the agent loop and tool orchestration
- `@kerberosec/core` — the main SDK with session management, hub, and configuration
