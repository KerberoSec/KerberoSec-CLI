import { captureSdkError } from "@kerberosec/shared";
import {
...
import type { KerberoSecCoreOptions } from "../../kerberosec-core/types";
...
function resolveConfiguredBackendMode(
	...
	if (process.env.KERBEROSEC_VCR?.trim()) {
		return "local";
	}
	const raw = process.env.KERBEROSEC_SESSION_BACKEND_MODE?.trim().toLowerCase();
...
function createLocalBackend(options: KerberoSecCoreOptions): SessionBackend {
	try {
