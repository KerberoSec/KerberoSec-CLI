...
// unless the user opts in via `localStorage.setItem("kerberosec:debug-logs", "1")`.
...
function shouldLogVerboseCoreLogs(): boolean {
	if (verboseCoreLogs === undefined) {
		try {
			verboseCoreLogs =
				window.localStorage.getItem("kerberosec:debug-logs") === "1";
		} catch {
		...
		(sid: string, detail: string) => {
			...
			const content = [
				...
				looksCredentialRelated
					? "Check your model connection in Settings → Models (or sign in with KerberoSec), then try again."
					: "",
			]
				.filter(Boolean)
				.join(" ");
