...
import { KERBEROSEC_PASS_PROVIDER_ID } from "@/core/controller/models/handleKerberoSecPassProviderSelection"
import { KerberoSecAccountService } from "@/services/account/KerberoSecAccountService"
...
async function makeAuthenticatedRequest<T>(endpoint: string, organizationId: string): Promise<T> {
	...
	const url = new URL(apiEndpoint, KerberoSecEnv.config().apiBaseUrl).toString()
...
async function fetchRemoteConfigForOrganization(organizationId: string): Promise<RemoteConfig | undefined> {
	try {
		...
		const configData = await makeAuthenticatedRequest<{ value: string; enabled: boolean }>(
			KERBEROSEC_API_ENDPOINT.REMOTE_CONFIG,
			organizationId,
		)
...
async function fetchApiKeysForOrganization(organizationId: string): Promise<APIKeySettings> {
	try {
		...
		const response = await makeAuthenticatedRequest<{ providerApiKeys: string }>(KERBEROSEC_API_ENDPOINT.API_KEYS, organizationId)
...
async function discoverRemoteConfigOrg(): Promise<{ organizationId: string; discoveredValue?: string } | undefined> {
	const accountService = KerberoSecAccountService.getInstance()
...
function isKerberoSecPassSelected(controller: Controller): boolean {
...
async function ensureUserInOrgWithRemoteConfig(controller: Controller): Promise<RemoteConfig | undefined> {
	...
	if (isKerberoSecPassSelected(controller)) {
		clearRemoteConfig()
