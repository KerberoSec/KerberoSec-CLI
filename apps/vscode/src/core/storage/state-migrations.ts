...
export async function migrateCustomInstructionsToGlobalRules(context: vscode.ExtensionContext) {
	try {
		...
		if (customInstructions?.trim()) {
			Logger.log("Migrating custom instructions to global KerberoSec rules...")

			// Create global .kerberosecrules directory if it doesn't exist
			...
			Logger.log("Successfully migrated custom instructions to global KerberoSec rules")
		}
	} catch (error) {
...
export async function migrateWelcomeViewCompleted(context: vscode.ExtensionContext, dataDir?: string) {
	try {
		...
		if (welcomeViewCompleted === undefined) {
			...
			const kerberosecAccountId = await context.secrets.get("kerberosecAccountId")
			...
			// file-backed stores (~/.kerberosec/data/globalState.json + secrets.json), not in
...
export async function cleanupOldApiKey(context: vscode.ExtensionContext) {
	try {
		...
		await context.secrets.delete("kerberosecApiKey")
	} catch (error) {
		Logger.error("Failed to cleanup old kerberosecApiKey", error)
	}
}
