...
import { setSdkLogger } from "@kerberosec/core"
import { HostProvider } from "@/hosts/host-provider"
import { Logger } from "@/shared/services/Logger"
import type { StorageContext } from "@/shared/storage/storage-context"
import { clearOnboardingModelsCache } from "./core/controller/models/getKerberoSecOnboardingModels"
...
import { KerberoSecTempManager } from "./services/temp"
...
/**
 * Performs intialization for KerberoSec that is common to all platforms.
 *
 * @param context
 * @returns The webview provider
 * @throws KerberoSecConfigurationError if endpoints.json exists but is invalid
 */
export async function initialize(storageContext: StorageContext): Promise<WebviewProvider> {
	...
	// ProviderSettingsManager, RuntimeOAuthTokenManager, and KerberoSec auth
	// flow through Logger.debug → KerberoSec output channel.
	// These components operate before/outside of KerberoSecCore sessions, so the
	...
	// Initialize KerberoSecEndpoint configuration (reads bundled and ~/.kerberosec/endpoints.json if present)
	// This must be done before any other code that calls KerberoSecEnv.config()
	// Throws KerberoSecConfigurationError if config file exists but is invalid
	const { KerberoSecEndpoint } = await import("./config")
	await KerberoSecEndpoint.initialize(HostProvider.get().extensionFsPath)

	try {
		await StateManager.initialize(storageContext)
	} catch (error) {
		Logger.error("[KerberoSec] CRITICAL: Failed to initialize StateManager:", error)
	...
	if (!KerberoSecEndpoint.isSelfHosted()) {
	...
	KerberoSecTempManager.startPeriodicCleanup()
...
async function showVersionUpdateAnnouncement(stateManager: StateManager) {
	...
	const previousVersion = stateManager.getGlobalStateKey("kerberosecVersion")
	// Perform post-update actions if necessary
	try {
		if (!previousVersion || currentVersion !== previousVersion) {
			Logger.log(`KerberoSec version changed: ${previousVersion} -> ${currentVersion}. First run or update detected.`)
			...
			if (lastShownAnnouncementId !== latestAnnouncementId) {
				...
				const message = previousVersion
					? `KerberoSec has been updated to v${currentVersion}`
					: `Welcome to KerberoSec v${currentVersion}`
				HostProvider.window.showMessage({
			...
			stateManager.setGlobalState("kerberosecVersion", currentVersion)
		}
	} catch (error) {
 ...
 * If so, opens the KerberoSec sidebar and clears the state.
 */
async function checkWorktreeAutoOpen(stateManager: StateManager): Promise<void> {
	try {
		...
		if (arePathsEqual(currentPath, worktreeAutoOpenPath)) {
			...
			// Open the KerberoSec sidebar
			await HostProvider.workspace.openKerberoSecSidebarPanel({})
		}
	} catch (error) {
...
/**
 * Performs cleanup when KerberoSec is deactivated that is common to all platforms.
 */
export async function tearDown(): Promise<void> {
	try {
		...
		KerberoSecTempManager.stopPeriodicCleanup()
	} finally {
		try {
			await StateManager.get().flushPendingState()
		} catch (error) {
			Logger.error("[KerberoSec] Failed to flush pending state during teardown:", error)
		}
	}
}
