 ...
 * (~/.kerberosec/data/sessions/<id>/<id>.json, snake_case on disk). `provider` is
...
function seedSessionRecord(
	...
	const sessionDir = path.join(kerberosecDir, "data", "sessions", id)
...
e2e("History - hides cost estimates for subscription-billed tasks", async ({ app, page, helper, server: _server }) => {
	...
	const kerberosecDir = await app.evaluate(() => process.env.KERBEROSEC_DIR)
	expect(kerberosecDir).toBeTruthy()
	...
	seedSessionRecord(kerberosecDir as string, {
