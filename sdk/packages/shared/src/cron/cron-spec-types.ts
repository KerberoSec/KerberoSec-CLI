 ...
 * Split out from `@kerberosec/core` so the parser can live in `shared` where
 * multiple consumers (hub daemon, CLI tools, enterprise adapters) can validate
 * the same `.kerberosec/cron/` spec format without pulling in core's stateful
 * orchestration layer.
 */
 ...
 * - everything else under `.kerberosec/cron/*.md` -> one_off
 */
...
export interface CronSpecParseResult {
	...
	externalId: string;
	/** Normalized posix-style path relative to `.kerberosec/cron/`. */
	relativePath: string;
