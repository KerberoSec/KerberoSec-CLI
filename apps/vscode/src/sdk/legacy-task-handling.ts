import { type ContentBlock, formatDisplayUserInput, type MessageWithMetadata } from "@kerberosec/shared"
...
/**
 * Classic KerberoSec truncated long conversations by omitting an index range of
 ...
 * "prompt is too long" on every request (kerberosec/kerberosec#12996).
 */
...
export function mergeLegacyUiMessagesWithResumedSdkMessages(
	legacyUiMessages: KerberoSecMessage[],
	sdkKerberoSecMessages: KerberoSecMessage[],
): KerberoSecMessage[] {
	const warningIndex = sdkKerberoSecMessages.findIndex((message) => message.text?.includes(LEGACY_RESUME_MODEL_WARNING))
	if (warningIndex === -1) {
		return sdkKerberoSecMessages
	}

	const resumedMessages = sdkKerberoSecMessages.slice(warningIndex + 1)
