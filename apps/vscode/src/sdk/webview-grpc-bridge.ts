...
// module translates them to proto KerberoSecMessages and pushes them through
...
import type { KerberoSecMessage, ExtensionState } from "@shared/ExtensionMessage"
import type { KerberoSecMessage as ProtoKerberoSecMessage } from "@shared/proto/kerberosec/ui"
import { convertKerberoSecMessageToProto } from "@shared/proto-conversions/kerberosec-message"
 ...
 * 1. Translates them to KerberoSecMessage[] via the message translator
 * 2. Converts each KerberoSecMessage to proto format
...
export class WebviewGrpcBridge {
	...
	createListener(): SessionEventListener {
		return (messages: KerberoSecMessage[], event: CoreSessionEvent) => {
	 ...
	 */
	private handleSessionEvent(messages: KerberoSecMessage[], event: CoreSessionEvent): void {
	...
	/**
	 * Push a KerberoSecMessage to the webview via the subscribeToPartialMessage stream.
	 */
	private async pushPartialMessage(message: KerberoSecMessage): Promise<void> {
		try {
			const protoMessage: ProtoKerberoSecMessage = convertKerberoSecMessageToProto(message)
...
/**
 * Standalone function to push a KerberoSecMessage to the webview.
 * Useful for one-off messages outside the bridge's event loop.
 */
export async function pushMessageToWebview(message: KerberoSecMessage): Promise<void> {
	try {
		const protoMessage: ProtoKerberoSecMessage = convertKerberoSecMessageToProto(message)
		await sendPartialMessageEvent(protoMessage)
	} catch (error) {
