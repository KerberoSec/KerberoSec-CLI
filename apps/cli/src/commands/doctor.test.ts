...
const {
	...
	mockSpawnSync: vi.fn(),
	mockResolveKerberoSecDataDir: vi.fn(() => "/tmp/kerberosec-data"),
...
vi.mock("@kerberosec/core", () => ({
	resolveKerberoSecDataDir: mockResolveKerberoSecDataDir,
...
describe("runDoctorCommand", () => {
	...
	afterEach(() => {
		vi.clearAllMocks();
		mockResolveKerberoSecDataDir.mockReturnValue("/tmp/kerberosec-data");
	...
	it("sees the hub through the set-aside record during the shielded update window", async () => {
		...
		mockSpawnSync.mockImplementation((command: string, args?: string[]) => {
			...
			if (
				command === "pgrep" &&
				Array.isArray(args) &&
				args[2] === "--kerberosec-hub-daemon"
			) {
				return {
					status: 0,
					stdout: "50174 /usr/local/bin/kerberosec --kerberosec-hub-daemon\n",
				};
			}
			return { status: 1, stdout: "" };
		});
...
describe("createDoctorCommand log subcommand", () => {
	...
	it("opens the log file for doctor log", async () => {
		...
		mockResolveKerberoSecDataDir.mockReturnValue(dataDir);
...
describe("doctor supervision reporting", () => {
	...
	it("reports what the hub is supervising", async () => {
		...
		await expect(runDoctorJson()).resolves.toMatchObject({
			supervisedConnectors: [
				{ channel: "slack", instanceId: "kerberosec-slack", state: "backoff" },
			],
		});
	});
	...
	it("formats restart and failure state so a crash loop is visible", () => {
		expect(
			...
			"slack | instance=kerberosec-slack | state=failed | origin=adopted | pid=42 | restarts=5 | lastExit=1 | error=invalid token",
		);
	});
	...
	it("leaves out fields that do not apply to a healthy connector", () => {
		expect(
			...
			"telegram | instance=kerberosec_bot | state=running | origin=spawned | pid=7",
		);
	});
});
