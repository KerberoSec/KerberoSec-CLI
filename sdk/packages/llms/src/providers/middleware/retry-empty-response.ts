...
// backends (openrouter, kerberosec, generic OpenAI-compatible endpoints) do the
// same. In KerberoSec's runtime an empty assistant turn is a hard failure ("Model
...
// incompatible with KerberoSec, which streams via `doStream` and runs its own tool
