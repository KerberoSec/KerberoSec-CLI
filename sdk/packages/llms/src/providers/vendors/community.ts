...
import type { GatewayResolvedProviderConfig } from "@kerberosec/shared";
...
export async function createClaudeCodeProviderModule(
	...
	try {
		...
		throw new Error(
			"The Claude Code provider requires the optional 'ai-sdk-provider-claude-code' package. " +
				"Install it alongside @kerberosec/llms to use this provider.",
			{ cause: error },
		);
	}
	...
	// @kerberosec/core's buildGatewayProviderOptions). Anchor the spawned agent
	...
	// KerberoSec has no interactive permission prompt wired into the CLI session
...
export async function createSapAiCoreProviderModule(
	...
	const provider = createSAPAIProvider({
		...
		!Array.isArray(options.defaultSettings)
			...
			: {}),
		requestConfig: {
			headers: { "ai-client-type": "KerberoSec" },
			// Standard kerberosec axios settings mirroring `getAxiosSettings()`
			adapter: "fetch",
