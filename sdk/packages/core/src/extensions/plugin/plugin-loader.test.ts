...
describe("plugin-loader", () => {
	...
	beforeAll(async () => {
		...
		const sdkDir = join(dir, "node_modules", "@kerberosec", "shared");
		...
		await writeFile(
			...
			[
				"import { safeJsonStringify } from '@kerberosec/shared';",
				"import { resolveKerberoSecDataDir } from '@kerberosec/shared/storage';",
				"import YAML from 'yaml';",
				"export default {",
				"  name: typeof safeJsonStringify === 'function' ? YAML.stringify({ ok: !!resolveKerberoSecDataDir() }) : 'invalid',",
				"  manifest: { capabilities: ['tools'] },",
				"};",
			].join("\n"),
			"utf8",
		...
		await writeFile(
			...
			[
				"import { createConfiguredTelemetryHandle } from '@kerberosec/core/telemetry';",
				"export default {",
	...
	it("resolves standalone plugin dependencies from the npm wrapper path", async () => {
		...
		await mkdir(depDir, { recursive: true });
		await writeFile(join(wrapperBinDir, "kerberosec"), "#!/usr/bin/env node\n");
		await writeFile(
			join(depDir, "package.json"),
			JSON.stringify({
		...
		try {
			process.env.KERBEROSEC_WRAPPER_PATH = join(wrapperBinDir, "kerberosec");
