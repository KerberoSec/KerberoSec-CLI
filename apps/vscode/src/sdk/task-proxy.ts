...
import type { KerberoSecAskResponse } from "@shared/WebviewMessage"
...
export interface TaskProxy {
	...
	handleWebviewAskResponse: (askResponse: KerberoSecAskResponse, text?: string, images?: string[], files?: string[]) => Promise<void>
...
export interface MessageStateHandlerEvents {
	kerberosecMessagesChanged: [change: KerberoSecMessageChange]
}

/**
 * Change event for message updates.
 * Mirrors KerberoSecMessageChange from src/core/task/message-state.ts.
 */
interface KerberoSecMessageChange {
...
/**
 * Message state handler that accumulates KerberoSecMessages and emits change events.
 ...
 * getKerberoSecMessages() and event emitter interface that consumers expect.
 */
export class MessageStateHandler extends EventEmitter<MessageStateHandlerEvents> {
	private messages: KerberoSecMessage[] = []
	 ...
	 */
	addMessages(messages: KerberoSecMessage[]): void {
		for (const message of messages) {
			...
			if (existingIndex !== -1) {
				...
				this.emit("kerberosecMessagesChanged", {
					type: "update",
				...
				this.emit("kerberosecMessagesChanged", { type: "add", messages: this.messages, message })
			}
		}
	}

	/** Get all accumulated messages (returns a copy) */
	getKerberoSecMessages(): KerberoSecMessage[] {
	...
	replaceMessages(messages: KerberoSecMessage[]): void {
		this.messages = [...messages]
		this.emit("kerberosecMessagesChanged", { type: "set", messages: this.messages })
	}
}
...
interface TaskProxyState {
	askResponse?: KerberoSecAskResponse
