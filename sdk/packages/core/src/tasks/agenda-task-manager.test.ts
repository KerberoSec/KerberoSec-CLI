...
import type { AgendaTaskRecord } from "@kerberosec/shared";
...
function createHarness(
	...
	const root = mkdtempSync(join(tmpdir(), "kerberosec-agenda-manager-"));
	const events: string[] = [];
...
describe("AgendaTaskManager", () => {
	...
	it("rehydrates known workspaces and watches their task specs after restart", async () => {
		const root = mkdtempSync(join(tmpdir(), "kerberosec-agenda-workspace-"));
	...
	it("does not recreate a missing historical workspace during startup", async () => {
		const root = mkdtempSync(join(tmpdir(), "kerberosec-agenda-missing-root-"));
		const workspaceRoot = join(root, "workspace");
	...
	it.skipIf(process.platform === "win32")(
		...
		async () => {
			const root = mkdtempSync(
				join(tmpdir(), "kerberosec-agenda-source-isolation-"),
			);
			...
			const badSpecsDir = join(badWorkspace, ".kerberosec", "tasks");
			const badSpecsBackup = join(badWorkspace, ".kerberosec", "tasks-real");
	...
	it("recovers a crash during session startup back to approved", async () => {
		const root = mkdtempSync(join(tmpdir(), "kerberosec-agenda-recovery-"));
