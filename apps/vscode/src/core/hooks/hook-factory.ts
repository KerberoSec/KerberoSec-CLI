...
import { version as kerberosecVersion } from "../../../package.json"
...
} from "../../shared/proto/kerberosec/hooks"
 ...
 * provide--the other common parameters like kerberosecVersion and userId are handled by the
 * hook system.
 */
...
export abstract class HookRunner<Name extends HookName> {
	 ...
	 * - kerberosecVersion: Current KerberoSec extension version
	 ...
	 * - userId: KerberoSec user ID, machine ID, or generated UUID
	 *
	...
	protected async completeParams(params: NamedHookInput<Name>): Promise<HookInput> {
		...
		return {
			...
			workspaceRoots,
			userId: getDistinctId(), // Always available: KerberoSec User ID, machine ID, or generated UUID
			...params,
			model,
		}
	}
 ...
 *
 * Used in multi-root workspaces where both global hooks (from ~/Documents/KerberoSec/Hooks/)
 * and workspace-specific hooks (from each workspace's .kerberosecrules/hooks/) exist for the
 * same hook type.
 *
 * Behavior:
...
function isExpectedHookError(error: unknown): boolean {
	...
	// Note: This is expected because users may have hooks in .kerberosecrules that they don't want to execute
...
export class HookFactory {
	 ...
	 * hooks run for. Discovery additionally scans this root's .kerberosecrules/hooks
	...
	private sessionHooksDir(): string | undefined {
		...
		return root ? path.join(root, ".kerberosecrules", "hooks") : undefined
	}
	 ...
	 * Global hooks are located in paths containing "KerberoSec/Hooks" or "kerberosec/hooks".
	 */
	 ...
	 *
	 * - Global hooks (from ~/Documents/KerberoSec/Hooks/): run from the primary workspace root
	 * - Workspace hooks (from workspaceRoot/.kerberosecrules/hooks/): run from that specific workspace root
	 *
	...
	private determineHookCwd(scriptPath: string, hooksDirs: string[], workspaceRoots: string[]): string | undefined {
		...
		// Workspace hooks are at: workspaceRoot/.kerberosecrules/hooks/. Prefer the
	 ...
	 * Global hooks are located in ~/Documents/KerberoSec/Hooks/
	 * Workspace hooks are located in workspace .kerberosecrules/hooks/ directories
	 *
	...
	/**
	 * Finds the path to a hook in a .kerberosecrules hooks directory.
	 *
	 * @param hookName the name of the hook to search for, for example 'PreToolUse'
	 * @param hooksDir the .kerberosecrules directory path to search
