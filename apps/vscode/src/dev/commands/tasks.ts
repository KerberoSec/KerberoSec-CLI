...
export function registerTaskCommands(controller: Controller): vscode.Disposable[] {
	return [
		vscode.commands.registerCommand("kerberosec.dev.expireMcpOAuthTokens", async () => {
			try {
		...
		vscode.commands.registerCommand("kerberosec.dev.createTestTasks", async () => {
			const count = (
 ...
 */
function createRealisticMessageSequence(baseTimestamp: number, taskPrompt: string, taskIndex: number): KerberoSecMessage[] {
	...
	const messages: KerberoSecMessage[] = [
		// Initial task message - uses "say" with "text" which is the format used in KerberoSec.ts
		{
			ts: baseTimestamp,
			type: "say",
			say: "text",
