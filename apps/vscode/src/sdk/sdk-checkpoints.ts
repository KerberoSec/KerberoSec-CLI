...
export function isVisibleCheckpointUserMessage(message: KerberoSecMessage): boolean {
	return message.type === "say" && (message.say === "task" || message.say === "user_feedback")
}

export function isCheckpointAnswerMessage(messages: KerberoSecMessage[], index: number): boolean {
...
export function isCheckpointRunUserMessage(messages: KerberoSecMessage[], index: number): boolean {
	return isVisibleCheckpointUserMessage(messages[index]) && !isCheckpointAnswerMessage(messages, index)
}

export function getCheckpointRunCountForMessage(messages: KerberoSecMessage[], targetIndex: number): number | undefined {
...
export function findVisibleCheckpointUserMessageByRun(
	...
	runCount: number,
): { message: KerberoSecMessage; index: number } | undefined {
	let seenUsers = 0
