...
describe("root option help text", () => {
	const FAKE_HOME = "/home/kerberosec-help-test";
	const savedEnv: Record<string, string | undefined> = {};

	beforeAll(() => {
		// Pin the resolver inputs so the defaults below are the true defaults
		// (no KERBEROSEC_DIR/KERBEROSEC_DATA_DIR overrides, known home directory).
		for (const key of ["KERBEROSEC_DIR", "KERBEROSEC_DATA_DIR"]) {
	...
	it("reports the actual resolver defaults for --config and --data-dir", () => {
		...
		const configDefault = tildePath(resolveKerberoSecDir(), FAKE_HOME);
		const dataDirDefault = tildePath(resolveKerberoSecDataDir(), FAKE_HOME);

		// Sanity-check the resolvers themselves so the assertions below can't
		// silently drift along with a resolver regression.
		expect(configDefault).toBe("~/.kerberosec");
		expect(dataDirDefault).toBe("~/.kerberosec/data");
