...
import { DeleteSkillRequest, ToggleSkillRequest } from "@shared/proto/kerberosec/file"
import {
	MarketplaceCatalog,
	MarketplaceEntry,
...
} from "@shared/proto/kerberosec/marketplace"
...
const MARKETPLACE_CATALOG_URL = "https://kerberosec.github.io/marketplace/catalog.json"
...
function resolveKerberoSecHome(): string {
	return process.env.KERBEROSEC_DIR?.trim() || join(homedir(), ".kerberosec")
}
...
async function installSkillMarketplaceEntry(entry: MarketplaceEntry, args: string[]): Promise<MarketplaceInstallResult> {
	const command = "npx"
	const commandArgs = ["-y", "skills@latest", "add", ...args, "-g", "-a", "kerberosec", "-y"]
...
function isGlobalKerberoSecPath(filePath: string | undefined): boolean {
	if (!filePath || filePath.startsWith("remote:")) return false
	return [resolveKerberoSecHome(), join(homedir(), ".agents", "skills")].some((root) => isPathWithin(root, filePath))
}
		...
		for (const pluginPath of discoverPluginModulePaths(root)) {
			entries.push(
				MarketplaceLocalInstalledEntry.create({
					...
					path: pluginPath,
					source: isGlobalKerberoSecPath(pluginPath) ? "global" : "workspace",
	...
	const skillEntries = [
		...
		...refreshedSkills.localSkills.map((skill) =>
			MarketplaceLocalInstalledEntry.create({
				...
				description: skill.description,
				path: skill.path,
				source: isGlobalKerberoSecPath(skill.path) ? "global" : "workspace",
				enabled: skill.enabled,
			}),
		),
	]
