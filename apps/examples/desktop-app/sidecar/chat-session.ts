...
import {
	...
	type KerberoSecCore,
	type KerberoSecCoreStartConfig,
...
type SessionConnectionUpdate = Parameters<
	KerberoSecCore["updateSessionConnection"]
>[1];
...
export function buildSessionConnectionUpdate(
	...
	// shared @kerberosec/core builder.
...
function getSessionManager(ctx: SidecarContext): KerberoSecCore {
...
async function handleStart(
	...
	const startResult = await manager.start({
		...splitCoreSessionConfig(
			coreConfig as unknown as KerberoSecCoreStartConfig,
		),
	...
	const restarted = await manager.start({
		...splitCoreSessionConfig(
			buildCoreSessionConfig({
				...
				sessionId,
				systemPrompt,
			}) as unknown as KerberoSecCoreStartConfig,
		),
		source: SessionSource.DESKTOP,
		interactive: true,
...
async function handleSend(
	...
	try {
		...
		if (session?.attachedViaHub) {
			// Once KerberoSecCore sends a turn, its HubRuntimeHost owns the session
		...
		let result: Awaited<ReturnType<KerberoSecCore["send"]>>;
		try {
