...
Shared visual foundations and reusable React presentation primitives for KerberoSec
...
`npm view @kerberosec/ui version`; an `E404` means the first release is still pending.
...
Use `@kerberosec/ui@next` only for deliberate previews. Monorepo consumers use
`"@kerberosec/ui": "workspace:*"` instead.

## Entry points

| Import | Contents | Runtime requirement |
| --- | --- | --- |
| `@kerberosec/ui` | Button, icon-button, agent ask-question, approval-card, Aurora, hero-heading, prompt-queue, quick-action, search-combobox, and session-status React primitives | React 18.3 or 19 and Tailwind v4 |
| `@kerberosec/ui/components.css` | Styles, namespaced Tailwind mappings, and source registration for the root React primitives | Tailwind v4 and theme tokens |
| `@kerberosec/ui/theme/palette.css` | KerberoSec-owned light/dark solid and alpha color scales | CSS |
| `@kerberosec/ui/theme/tokens.css` | Light/dark custom properties only | CSS |
| `@kerberosec/ui/theme/scoped-tokens.css` | Light/dark custom properties scoped to `.kerberosec-ui-theme` | CSS |
| `@kerberosec/ui/theme/theme.css` | Tailwind v4 semantic mapping and dark variant | Tailwind v4 |
| `@kerberosec/ui/components/markdown.css` | Optional framework-neutral Markdown and Streamdown treatment | Theme tokens |
| `@kerberosec/ui/theme/base.css` | Optional document, Markdown, scrollbar, selection, and cursor styles | Tailwind v4 |
| `@kerberosec/ui/theme/index.css` | Complete theme: tokens, Tailwind mapping, and base styles | Tailwind v4 |
| `@kerberosec/ui/components/agent-chat` | Conversation, message, reasoning, action, and tool-activity React primitives | React 18.3 or 19 |
| `@kerberosec/ui/components/agent-chat.css` | Framework-neutral styles for the agent-chat primitives | Theme tokens |

`SessionStatus` uses semantic tone colors by default. Set
`--kerberosec-ui-session-status-color` on the component to override its dot color
...
```tsx
import { Button, IconButton } from "@kerberosec/ui";
...
`AgentWelcomeHero` renders the interactive KerberoSec bot and grid used on agent
...
@import "@kerberosec/ui/components/markdown.css";
```

```tsx
<section className="kerberosec-ui-theme">
	<div className="kerberosec-markdown">...</div>
</section>
```

Dark values activate when `.dark` is on the wrapper or an ancestor.

Embedded hosts should not import `@kerberosec/ui/theme/theme.css`; it intentionally
maps generic Tailwind names such as `bg-background` for KerberoSec-owned surfaces.
...
normal Tailwind utilities inherit KerberoSec defaults without custom adapters.

Theme authors work through three layers:

1. KerberoSec-owned 12-step solid and alpha palettes: Slate as `--neutral-*`,
...
license; `@kerberosec/ui` does not depend on Radix Colors at runtime.
...
For assistant Markdown, `@kerberosec/ui/components/markdown` exports the shared
Streamdown configuration — `markdownCodeHighlighter` (lazy Shiki with GitHub
light/dark themes) and `agentMarkdownControls` — and
`@kerberosec/ui/components/markdown.css` carries the matching chat styling (quiet
...
Storybook currently runs from a KerberoSec monorepo checkout. It is not hosted or
included in the npm package; deployment can be added once the catalog and
ownership model settle.

## Layering and compatibility

- Import the KerberoSec theme after Tailwind so its default typography values win.
