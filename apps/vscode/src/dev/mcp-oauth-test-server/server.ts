 ...
 * exercising and debugging KerberoSec's MCP OAuth flow locally, including failure
 * modes such as:
 *
 ...
 *                         for exercising KerberoSec's OAuth state-expiry window).
 ...
 *
 * Then add an MCP server to KerberoSec pointing at:
 *   http://127.0.0.1:7777/mcp   (type: streamableHttp)
 *
 * Click "Authenticate" in KerberoSec; a browser opens the /authorize consent page.
 */
...
interface TestServerOptions {
	 ...
	 * so you can add several distinct MCP servers to KerberoSec at once and exercise
...
class TestServer {
	...
	private async handleMcp(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
		...
		if (!hasBearer) {
			// This is what triggers KerberoSec's OAuth flow: 401 + WWW-Authenticate
		...
		// succeeds and KerberoSec shows the server as connected.
		const body = await this.readJsonBody(req)
...
/**
 * Build a paste-ready `mcpServers` fragment for kerberosec_mcp_settings.json from a
...
if (isMain) {
	...
	void (async () => {
		...
		console.log("\nPaste into ~/.kerberosec/data/settings/kerberosec_mcp_settings.json (merge under mcpServers):\n")
