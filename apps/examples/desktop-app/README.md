...
Tauri desktop shell + Bun sidecar backend + Next.js UI for running and inspecting KerberoSec chat sessions.
...
`KERBEROSEC_SIDECAR_SKIP_SHELL_PATH=1` to disable. Implementation and details:
[`sidecar/shell-path.ts`](./sidecar/shell-path.ts).

## Web Visual System

The framework-neutral color, typography, radius, and navigation contract lives
in the internal [`@kerberosec/ui`](../../../sdk/packages/ui/README.md) workspace
package. Other KerberoSec web surfaces can take only its tokens or opt into the
...
(`.kerberosec/skills/publish-desktop/SKILL.md`).
...
There is also a beta channel ("KerberoSec Beta", a separate app that installs
...
2. The desktop backend starts the Bun sidecar, which discovers or starts the
   canonical shared KerberoSec Hub and exposes one websocket transport (`/transport`)
...
- The view is wired to the same scheduler APIs used by `kerberosec schedule` through Tauri commands and `scripts/routine-schedules.ts`.
...
- Session artifacts are written under `~/.kerberosec/data/sessions/<sessionId>/` (or `KERBEROSEC_SESSION_DATA_DIR`).
...
`~/.kerberosec/data/logs/code.log` by default. Telemetry continues to honor the global
...
- `KERBEROSEC_LOG_ENABLED=0` disables file logging.
- `KERBEROSEC_LOG_LEVEL` sets the Pino level (for example, `debug` or `warn`).
- `KERBEROSEC_LOG_PATH` overrides the log destination.
- `KERBEROSEC_LOG_NAME` overrides the logger name.
...
enable the same console stream with `NEXT_PUBLIC_KERBEROSEC_DEBUG_LOGS=1` at build
time, or at runtime from DevTools with
`localStorage.setItem("kerberosec.debugLogs", "1")` followed by a reload. Diagnostic
...
- Speech input requires an enabled provider whose models.dev metadata identifies
  ...
  **Settings → Models → Voice input**. That selection is stored separately from
  ...
  `~/.kerberosec/data/settings/providers.json`; provider credentials remain in their
