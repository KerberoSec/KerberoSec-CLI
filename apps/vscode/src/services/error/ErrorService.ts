...
import { KerberoSecError } from "./KerberoSecError"
import { ErrorProviderFactory } from "./ErrorProviderFactory"
import { IErrorProvider } from "./providers/IErrorProvider"

/**
 * ErrorService handles error logging and tracking for the KerberoSec extension
...
export class ErrorService {
	...
	captureException(error: Error | KerberoSecError, properties?: Record<string, unknown>) {
		return this.provider.captureException(error, properties)
	}

	public logException(error: Error | KerberoSecError, properties?: Record<string, unknown>): void {
	...
	public toKerberoSecError(rawError: unknown, modelId?: string, providerId?: string): KerberoSecError {
		const transformed = KerberoSecError.transform(rawError, modelId, providerId)
