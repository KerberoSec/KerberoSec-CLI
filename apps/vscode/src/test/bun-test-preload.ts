...
// directly and stub `vscode`/`@kerberosec/core` per-file) and delete this preload's
...
//   @kerberosec/core  -> src/test/kerberosec-core-vitest-stub.ts (lightweight SDK stub)
//
// bun's runtime plugin `onResolve` hook does NOT intercept these (`vscode` is
// host/builtin-like and `@kerberosec/core` is a symlinked workspace package — both
...
// process. (Other workspace packages — @kerberosec/llms, @kerberosec/shared,
// @kerberosec/shared/storage — and the tsconfig `paths` aliases resolve on their own.)
//
// bun's ESM linker statically validates every named import against the names on
// the mock namespace. The stub only implements the @kerberosec/core exports these
// tests exercise, but other modules in the import graph statically import
// additional names (e.g. `prepareRemoteConfigCoreIntegration`, `KerberoSecCore`,
// `createMcpTools`); a missing name is a hard "Export named 'X' not found" link
// error. So we seed the mock namespace with every name the real @kerberosec/core
...
import * as realKerberoSecCore from "@kerberosec/core"
import * as LlmsModels from "@kerberosec/llms"
import * as kerberosecCoreStub from "./kerberosec-core-vitest-stub"
import * as vscodeStub from "./vscode-vitest-stub"

const kerberosecCoreNamespace: Record<string, unknown> = {}
for (const name of Object.keys(realKerberoSecCore)) {
	kerberosecCoreNamespace[name] = undefined
}
Object.assign(kerberosecCoreNamespace, kerberosecCoreStub)

bunBeforeEach(() => {
	kerberosecCoreStub.resetModelsFileState()
...
mock.module("@kerberosec/core", () => kerberosecCoreNamespace)
...
//                           (@kerberosec/core, vscode), calling bun's `import()` from
...
const actualNamespaceRegistry: Record<string, unknown> = {
	"@kerberosec/core": kerberosecCoreNamespace,
