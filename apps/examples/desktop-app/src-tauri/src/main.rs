...
const TRAY_ICON_ID: &str = "kerberosec-code";
...
// builds ("KerberoSec Beta") identify themselves in the tooltip too.
...
fn resolve_desktop_backend_binary_path(context: &AppContext) -> Option<PathBuf> {
    ...
    let explicit = std::env::var("KERBEROSEC_CODE_SIDECAR_BIN")
        .ok()
...
fn resolve_mcp_settings_path() -> Result<PathBuf, String> {
    if let Ok(value) = std::env::var("KERBEROSEC_MCP_SETTINGS_PATH") {
    ...
    Ok(PathBuf::from(home)
        ...
        .join("data")
        .join("settings")
        .join("kerberosec_mcp_settings.json"))
}
...
/// flows that need an update staged right now (e.g. the "KerberoSec Hub was
...
fn setup_tray_icon(app: &tauri::App) -> tauri::Result<()> {
    ...
    let menu = MenuBuilder::new(app)
        .text(
            ...
            // builds ("KerberoSec Beta") identify themselves in the tray too.
            format!(
                "{} v{}",
