...
async function performCommitMsgGeneration(controller: Controller, gitDiff: string, inputBox: GitRepositoryInputBox) {
	try {
		vscode.commands.executeCommand("setContext", "kerberosec.isGeneratingCommit", true)
		...
		vscode.commands.executeCommand("setContext", "kerberosec.isGeneratingCommit", false)
	}
}
