...
import { ModelCapabilitySchema } from "@kerberosec/shared"
...
const modelInfoKeysByProvider: Partial<Record<string, ModelInfoKeys>> = {
	...
	kerberosec: { plan: "planModeKerberoSecModelInfo", act: "actModeKerberoSecModelInfo" },
...
function writeStateFields(providerId: ProviderId, patch: ProviderConfigPatch): void {
	...
	if (provider === "kerberosec" && "auth" in patch) {
		writeStateKey("kerberosecApiKey", patch.auth?.accessToken)
		writeStateKey("kerberosecAccountId", patch.auth?.accountId)
	}
