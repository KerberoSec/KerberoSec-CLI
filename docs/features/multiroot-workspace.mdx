...
KerberoSec works with VSCode's multi-root workspaces, letting you manage multiple project folders or repositories in a single window. Whether you're working with a monorepo or separate Git repositories, KerberoSec can read files, write code, and run commands across all of them.

<Frame>
  <video
    src="https://storage.googleapis.com/kerberosec_public_images/multiworkspace.mp4"
    autoPlay
    muted
    loop
    playsInline
...
- **KerberoSec rules** only work in the primary workspace folder
...
KerberoSec can complete tasks that span multiple projects or repositories:
...
KerberoSec supports both patterns, as well as hybrid setups where some folders are Git repositories and others are not. The key difference: with multiple repositories, each folder has its own `.git` directory and KerberoSec tracks them independently.
...
When you open separate Git repositories in one workspace, KerberoSec treats each as an independent project with its own version control.

### What KerberoSec Tracks Per Repository

For each workspace folder, KerberoSec detects:
...
This means KerberoSec understands that your frontend and backend might be at different commits, on different branches, or even use different version control systems.

<Note>
While KerberoSec detects VCS information for all workspace folders, certain features only use the **primary workspace** (the first folder): [KerberoSec rules](/customization/kerberosec-rules), [skills](/customization/skills#triggering-skills-with-slash-commands), and [Git-related features](/core-workflows/working-with-files) like `@git` mentions.
</Note>
...
KerberoSec understands natural references to your workspaces:
...
- KerberoSec needs to resolve ambiguity
...
Each service has its own repository. KerberoSec can update the proto definitions and regenerate clients across all services.
...
The types repository defines interfaces used by both client and server. When you update a type, KerberoSec can fix both consumers.
...
Mix of repositories and plain folders. KerberoSec adapts to each folder's configuration.
...
[KerberoSec rules](/customization/kerberosec-rules) (`.kerberosecrules/` directory) only work in the **primary workspace** (the first folder in your workspace). Rules in other workspace folders are ignored.

**Workaround:** Place shared rules in the primary workspace, or use global rules (`~/Documents/KerberoSec/Rules/`) which apply everywhere.

### Checkpoints

[Checkpoints](/core-workflows/checkpoints) are disabled in multi-root workspace mode. KerberoSec displays a warning when this happens.
...
4. **Consider the primary workspace** for KerberoSec rules placement
...
- Use [Plan mode](/core-workflows/plan-and-act) to let KerberoSec understand structure first
