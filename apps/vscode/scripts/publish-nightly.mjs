 ...
 *    - Changes name to "kerberosec-nightly"
 *    - Changes displayName to "KerberoSec (Nightly)"
 ...
 *   `kerberosec-nightly` (this is what the scheduled daily nightly workflow
 *   uses). Pass --pre-release to instead publish to the pre-release
 *   channel of `kerberosec-nightly` (used for manual publishes from feature
...
const config = {
	...
	nightlyName: "kerberosec-nightly",
	originalName: "claude-dev",
	nightlyDisplayName: "KerberoSec (Nightly)",
	...
	get vsixPath() {
		return path.join(this.distDir, "kerberosec-nightly.vsix")
	},
	get nodeModulesPath() {
...
class NightlyPublisher {
	 ...
	 * changes package.json name from "claude-dev" to "kerberosec-nightly". If we don't
	 * align this link, vsce's dependency detection (`npm list --production`) fails
	 * with ELSPROBLEMS (missing kerberosec-nightly + extraneous claude-dev).
	 */
	 ...
	 */
	updatePackageJson() {
		// Replace any occurrences kerberosec. or claude-dev with nightly name
		const rawContent = fs.readFileSync(config.packageJsonPath, "utf-8")
		const content = rawContent.replaceAll("claude-dev", config.nightlyName).replaceAll('"kerberosec.', `"${config.nightlyName}.`)
	...
	packageExtension(isPreRelease = false) {
		...
		const args = [
			...
			// (the @kerberosec/* workspace symlinks point outside the package).
			"--no-dependencies",
...
Options:
  --pre-release    Publish to the pre-release channel of kerberosec-nightly.
