...
export function addRootOptions(cmd: Command): Command {
	return (
		cmd
			...
			.option("-P, --provider <id>", "Provider id (default: kerberosec)")
			...
			.option(
				"--config <path>",
				"Configuration directory (default: ~/.kerberosec)",
			)
			.option(
				"--data-dir <path>",
				"Use isolated local state at this directory path (default: ~/.kerberosec/data)",
			)
			.option(
				"--hooks-dir <path>",
				"Directory path to additional hooks for runtime hook injection (default: ~/.kerberosec/hooks)",
			)
			.option(
				"--worktree",
				"Auto-create a detached git worktree under ~/.kerberosec/worktrees/ and run the task there",
			)
