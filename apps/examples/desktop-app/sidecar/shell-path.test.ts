...
const MARKER_START = "__KERBEROSEC_SIDECAR_PATH_START__";
const MARKER_END = "__KERBEROSEC_SIDECAR_PATH_END__";
...
function writeFakeShell(
	...
	const dir = mkdtempSync(join(tmpdir(), "kerberosec-shell-path-"));
	tempDirs.push(dir);
...
describe("ensureLoginShellPath", () => {
	...
	it("skips when the escape hatch is set", async () => {
		const env: NodeJS.ProcessEnv = {
			PATH: "/usr/bin",
			KERBEROSEC_SIDECAR_SKIP_SHELL_PATH: "1",
		};
