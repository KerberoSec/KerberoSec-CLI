...
import type { HubSessionClient, HubSessionRow } from "@kerberosec/core";
import {
	ensureParentDir,
...
export const KERBEROSEC_CONNECTOR_DETACHED_CHILD_ENV =
	"KERBEROSEC_CONNECTOR_DETACHED_CHILD";
...
function buildDetachedConnectorEnv(
	...
	const childEnv = {
		...withResolvedKerberoSecBuildEnv(env),
		[childEnvKey]: "1",
		[KERBEROSEC_CONNECTOR_DETACHED_CHILD_ENV]: "1",
	};
	delete childEnv[KERBEROSEC_RUN_AS_HUB_DAEMON_ENV];
	return childEnv;
}
 ...
 *
 * Uses O_EXCL so two concurrent `kerberosec connect` launches cannot both observe
