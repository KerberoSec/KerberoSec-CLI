...
describe("SdkModeCoordinator", () => {
	...
	it("preserves composer content when the rebuild aborts on a kerberosec auth error", async () => {
		const activeSession = makeActiveSession()
		const task = makeTask("old-session", planMessages())
		const { coordinator, options, state } = makeCoordinator({
			activeSession,
		...
		expect(options.emitKerberoSecAuthError).toHaveBeenCalledOnce()
		expect(options.sessions.fireAndForgetSend).not.toHaveBeenCalled()
		expect(options.messages.appendAndEmit).not.toHaveBeenCalled()
	...
	it("emits an auth error and skips replacement when the target kerberosec provider has no token", async () => {
		const activeSession = makeActiveSession()
		const { coordinator, options, state } = makeCoordinator({
			activeSession,
			config: {
	...
	it("cancels and finalizes a running turn before rebuilding for mode change", async () => {
		...
		expect(options.messages.finalizeMessagesForSave).toHaveBeenCalledWith(task.messageStateHandler.getKerberoSecMessages())
...
function makeTask(taskId: string, messages: Array<Partial<KerberoSecMessage>> = []) {
	return {
		taskId,
		messageStateHandler: {
			getKerberoSecMessages: vi.fn(() => messages as KerberoSecMessage[]),
		},
	} as unknown as {
		taskId: string
		messageStateHandler: { getKerberoSecMessages: () => KerberoSecMessage[] }
	}
}

function planMessages(): Array<Partial<KerberoSecMessage>> {
