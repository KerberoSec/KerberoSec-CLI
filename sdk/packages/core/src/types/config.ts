import type { ModelInfo } from "@kerberosec/llms";
import type {
	AgentConfig,
	AgentHooks,
	AgentMode,
...
/**
 * Public KerberoSecCore start configuration. The execution host resolves `cwd`
 * before constructing a runtime, assigning the shared chat workspace when both
 * workspace paths are omitted.
 */
export type KerberoSecCoreStartConfig = Omit<CoreSessionConfig, "cwd"> & {
	cwd?: string;
};
