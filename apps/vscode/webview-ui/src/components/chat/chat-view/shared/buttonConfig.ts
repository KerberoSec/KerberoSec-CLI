import type { KerberoSecMessage, KerberoSecSayTool, TurnState } from "@shared/ExtensionMessage"
 ...
 */
export function getButtonConfig(message: KerberoSecMessage | undefined, _mode: Mode = "act"): ButtonConfig {
	if (!message) {
...
function isInertStatusMessage(message: KerberoSecMessage): boolean {
	if (message.type !== "say") {
		return false
	}
 ...
 */
export function getButtonConfigForMessages(messages: KerberoSecMessage[], mode: Mode = "act"): ButtonConfig {
...
export function buttonsForPhase(
	turnState: TurnState,
	anchoredMessage: KerberoSecMessage | undefined,
