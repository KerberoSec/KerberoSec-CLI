...
`,
		...
		const traversal = parseAgendaTaskSpec({
			specPath: join(WORKSPACE_ROOT, ".kerberosec", "tasks", "task.task.md"),
			scope: "workspace",
			workspaceRoot: WORKSPACE_ROOT,
			raw: `---
type: todo
...
describe("AgendaTaskSpecFileStore", () => {
	it("atomically writes and reads a canonical task spec", () => {
		const root = mkdtempSync(join(tmpdir(), "kerberosec-task-specs-"));
		roots.push(root);
		const store = new AgendaTaskSpecFileStore({
			scope: "workspace",
			workspaceRoot: root,
			taskSpecsDir: join(root, ".kerberosec", "tasks"),
		});
	...
	it("rejects a workspace task directory that escapes through a symlink", () => {
		...
		const outside = mkdtempSync(
			join(tmpdir(), "kerberosec-task-specs-outside-"),
		);
		roots.push(root, outside);
		mkdirSync(join(root, ".kerberosec"));
		symlinkSync(outside, join(root, ".kerberosec", "tasks"), "dir");
		const store = new AgendaTaskSpecFileStore({
			scope: "workspace",
			workspaceRoot: root,
		});
