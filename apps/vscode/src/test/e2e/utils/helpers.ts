...
import { KerberoSecApiServerMock } from "../fixtures/server"
...
export class E2ETestHelper {
	...
	public async signin(webview: Frame): Promise<void> {
		await webview.getByRole("button", { name: "Login to KerberoSec" }).click({ delay: 100 })

		// Verify start up page is no longer visible
		await expect(webview.getByRole("button", { name: "Login to KerberoSec" })).not.toBeVisible()
	...
	public static async openKerberoSecSidebar(page: Page): Promise<void> {
		await page.getByRole("tab", { name: /KerberoSec/ }).locator("a").click()
	}
...
/**
 * NOTE: Use the `e2e` test fixture for all E2E tests to test the KerberoSec extension.
 *
 * Extended Playwright test configuration for KerberoSec E2E testing.
 *
 * This test configuration provides a comprehensive setup for end-to-end testing of the KerberoSec VS Code extension,
 ...
 *
 * Fixtures provided:
 * - `server`: Shared KerberoSecApiServerMock instance for API mocking (reused across all tests)
 ...
 * - `page`: Playwright Page object representing the main VS Code window with KerberoSec sidebar opened
 * - `sidebar`: Playwright Frame object representing the KerberoSec extension's sidebar iframe
 *
 * @returns Extended test object with all fixtures available for E2E test scenarios:
 * - **server**: Automatically starts and manages a KerberoSecApiServerMock instance
 ...
 * - **page**: Configures the main VS Code window with notifications disabled and KerberoSec sidebar open
 * - **sidebar**: Provides access to the KerberoSec extension's sidebar frame
 *
 * @example
 * ```typescript
 ...
 * - Installs the KerberoSec extension in development mode
...
export const e2e = test
	.extend<{ server: KerberoSecApiServerMock | null }>({
		server: async ({}, use) => {
			// Start server if it doesn't exist
			if (!KerberoSecApiServerMock.globalSharedServer) {
				await KerberoSecApiServerMock.startGlobalServer()
			}
			await use(KerberoSecApiServerMock.globalSharedServer)
		},
	})
	...
	.extend<{ openVSCode: (workspacePath: string) => Promise<ElectronApplication> }>({
		openVSCode: async ({ userDataDir, channel }, use, testInfo) => {
			...
			await use(async (workspacePath: string) => {
				// Create isolated KerberoSec data directory for this test
				const kerberosecTestDir = mkdtempSync(path.join(os.tmpdir(), "kerberosec-e2e-"))
				const kerberosecDataDir = path.join(kerberosecTestDir, "data")
				...
				const app = await _electron.launch({
					...
					env: {
						...
						TEMP_PROFILE: "true",
						E2E_TEST: "true",
						KERBEROSEC_ENVIRONMENT: "local",
						// Prevent OAuth E2E from launching a browser that can outlive VS Code on Linux.
						KERBEROSEC_CAPTURE_BROWSER: "true",
						KERBEROSEC_DIR: kerberosecTestDir, // Isolate test data from user's ~/.kerberosec
						KERBEROSEC_DATA_DIR: kerberosecDataDir, // Keep SDK/shared storage off the user's real KerberoSec data dir
	...
	.extend<{ app: ElectronApplication; kerberosecTestDir: string }>({
		app: async ({ openVSCode, userDataDir, extensionsDir, workspaceType, workspaceDir, multiRootWorkspaceDir }, use) => {
			...
			try {
				...
				// Clean up the isolated KerberoSec data directory
				// Find all temp directories matching our pattern
				const tmpDir = os.tmpdir()
				try {
					const entries = readdirSync(tmpDir)
					for (const entry of entries) {
						if (entry.startsWith("kerberosec-e2e-")) {
	...
	.extend<{ sidebar: Frame }>({
		sidebar: async ({ page, helper, server }, use) => {
			await E2ETestHelper.openKerberoSecSidebar(page)
