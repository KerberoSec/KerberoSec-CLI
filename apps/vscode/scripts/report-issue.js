...
const getKerberoSecVersion = () => {
	try {
		const extensions = execSync("code --list-extensions --show-versions").toString()
		const kerberosecMatch = extensions.match(/claude-dev@(\d+\.\d+\.\d+)/)
		return kerberosecMatch ? kerberosecMatch[1] : "Not installed"
	} catch (_err) {
...
const collectSystemInfo = () => {
	...
	return {
		...
		kerberosecVersion: getKerberoSecVersion(),
	}
}
...
const createIssueUrl = (systemInfo, issueTitle) => {
	return (
		`https://github.com/kerberosec/kerberosec/issues/new?template=bug_report.yml` +
		`&title=${issueTitle}` +
		`&operating-system=${systemInfo.os}` +
		`&kerberosec-version=${systemInfo.kerberosecVersion}` +
		`&system-info=${
