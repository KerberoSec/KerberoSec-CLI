...
beforeEach(() => {
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "kerberosec-legacy-state-"))
})
...
describe("resolveDataDir", () => {
	...
	it("falls back to KERBEROSEC_DATA_DIR env", () => {
		const original = process.env.KERBEROSEC_DATA_DIR
		process.env.KERBEROSEC_DATA_DIR = "/env/data"
		try {
			expect(resolveDataDir()).toBe("/env/data")
		} finally {
			process.env.KERBEROSEC_DATA_DIR = original
		}
	})

	it("trims a whitespace-padded KERBEROSEC_DATA_DIR like createStorageContext", () => {
		const original = process.env.KERBEROSEC_DATA_DIR
		process.env.KERBEROSEC_DATA_DIR = "  /env/data  "
		try {
			expect(resolveDataDir()).toBe("/env/data")
	...
	it("falls back to KERBEROSEC_DIR/data", () => {
		const originalData = process.env.KERBEROSEC_DATA_DIR
		const originalDir = process.env.KERBEROSEC_DIR
		delete process.env.KERBEROSEC_DATA_DIR
		process.env.KERBEROSEC_DIR = "/kerberosec"
		try {
			// path.join keeps the platform separator (\ on Windows), so compare
			// against a joined path rather than a hardcoded POSIX string.
			expect(resolveDataDir()).toBe(path.join("/kerberosec", "data"))
		} finally {
			process.env.KERBEROSEC_DATA_DIR = originalData
			process.env.KERBEROSEC_DIR = originalDir
		}
	})

	it("falls back to ~/.kerberosec/data", () => {
		const originalData = process.env.KERBEROSEC_DATA_DIR
		const originalDir = process.env.KERBEROSEC_DIR
		delete process.env.KERBEROSEC_DATA_DIR
		delete process.env.KERBEROSEC_DIR
		try {
			expect(resolveDataDir()).toBe(path.join(os.homedir(), ".kerberosec", "data"))
		} finally {
			process.env.KERBEROSEC_DATA_DIR = originalData
			process.env.KERBEROSEC_DIR = originalDir
		}
...
describe("readMcpSettings", () => {
	...
	it("reads kerberosec_mcp_settings.json from settings/ subdirectory", () => {
		...
		writeJson(path.join(tempDir, "settings", "kerberosec_mcp_settings.json"), settings)
	...
	it("returns empty mcpServers for corrupt JSON", () => {
		const filePath = path.join(tempDir, "settings", "kerberosec_mcp_settings.json")
		fs.mkdirSync(path.dirname(filePath), { recursive: true })
...
describe("readAllLegacyState", () => {
	...
	it("reads all state files at once", () => {
		...
		writeJson(path.join(tempDir, "settings", "kerberosec_mcp_settings.json"), {
			mcpServers: {
