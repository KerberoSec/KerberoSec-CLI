import type { AgentEvent, CoreSessionEvent } from "@kerberosec/core"
import { refreshKerberoSecRecommendedModels } from "@/core/controller/models/refreshKerberoSecRecommendedModels"
import type { StateManager } from "@/core/storage/StateManager"
import { KERBEROSEC_RECOMMENDED_MODELS_FALLBACK } from "@/shared/kerberosec/recommended-models"
import type { KerberoSecApiReqInfo, TurnPhase } from "@/shared/ExtensionMessage"
...
export interface SdkSessionEventCoordinatorOptions {
	...
	isKerberoSecFreeModel?: () => Promise<boolean>
	/**
...
export class SdkSessionEventCoordinator {
	...
	async handleSessionEvent(event: CoreSessionEvent): Promise<void> {
		...
		const zeroCostPromise = this.zeroCostForFreeKerberoSecModel(result)
		if (zeroCostPromise) {
	...
	private zeroCostForFreeKerberoSecModel(result: TranslationResult): Promise<void> | undefined {
		...
		const hasMessageCost = result.messages.some((message) => {
			...
			try {
				const info = JSON.parse(message.text) as KerberoSecApiReqInfo
		...
		return (async () => {
			if (!(await this.isCurrentKerberoSecModelFree())) {
				return
			}
			...
			result.messages = result.messages.map((message) => {
				...
				try {
					...
					return {
						...message,
						text: JSON.stringify({ ...info, cost: 0 } satisfies KerberoSecApiReqInfo),
					}
				} catch {
					return message
				}
	...
	private async isCurrentKerberoSecModelFree(): Promise<boolean> {
		if (this.options.isKerberoSecFreeModel) {
			return this.options.isKerberoSecFreeModel()
		}
		...
		try {
			...
			if (!isKerberoSecManagedProvider(provider)) {
				return false
			}

			const modelId = this.getCurrentKerberoSecModelId()
			if (!modelId) {
				return false
			}
			...
			const resolvedFreeIds =
				freeIds.length > 0 ? freeIds : KERBEROSEC_RECOMMENDED_MODELS_FALLBACK.free.map((model) => normalizeModelId(model.id))
			return resolvedFreeIds.includes(normalizedModelId)
		} catch (error) {
			Logger.error("[SdkController] Failed to check KerberoSec free model list:", error)
			const modelId = this.getCurrentKerberoSecModelId()
			if (!modelId) {
				return false
			}
			const fallbackFreeIds = KERBEROSEC_RECOMMENDED_MODELS_FALLBACK.free.map((model) => normalizeModelId(model.id))
			return fallbackFreeIds.includes(normalizeModelId(modelId))
		}
	}

	private getCurrentKerberoSecModelId(): string | undefined {
		const stateManager = this.options.stateManager
		if (!stateManager) {
			return undefined
		}
		...
		const mode = stateManager.getGlobalSettingsKey("mode") === "plan" ? "plan" : "act"
		const provider = mode === "plan" ? apiConfig.planModeApiProvider : apiConfig.actModeApiProvider
		if (provider === "kerberosec-pass") {
			return mode === "plan" ? apiConfig.planModeKerberoSecPassModelId : apiConfig.actModeKerberoSecPassModelId
		}
		return mode === "plan" ? apiConfig.planModeKerberoSecModelId : apiConfig.actModeKerberoSecModelId
	}
