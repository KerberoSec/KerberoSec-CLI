...
beforeEach(() => {
	...
	invoke.mockReset();
	invoke.mockRejectedValue(new Error("No KerberoSec account auth token found"));
...
describe("AgentSidebar session organization", () => {
	...
	it("builds the hover overview with branch and secondary metadata last", () => {
		...
		expect(getSessionOverviewItems(thread)).toEqual([
			["Workspace", "kerberosec", "/projects/kerberosec"],
		...
		expect(
			getSessionOverviewItems(makeThread("kerberosec", 5)),
		).not.toContainEqual(["Branch", expect.anything()]);
		expect(
	...
	it("shows the signed-in account and active organization in the footer", async () => {
		...
		await vi.waitFor(() => {
			...
			expect(container.textContent).toContain("KerberoSec Bot Inc");
		});
		expect(container.textContent).not.toContain("KerberoSec Desktop");
		...
		const organizationName = [
			...(accountButton?.querySelectorAll("span") ?? []),
		].find((element) => element.textContent === "KerberoSec Bot Inc");
	...
	it("shows the desktop app version and connected Hub when the logo is hovered", async () => {
		...
		await vi.waitFor(() => {
			...
			expect(document.body.textContent).toContain("KerberoSec Hub @25463");
	...
	it("hosts back and forward navigation in the draggable sidebar title bar", async () => {
		...
		expect(titleBar?.textContent).not.toContain("KerberoSec");
	...
	it("stacks New, Schedule, and Customize as full-width rows below the logo", async () => {
		...
		const logo = container.querySelector('[aria-label="KerberoSec home"]');
	...
	it("uses only the KerberoSec logo for home in the collapsed sidebar", async () => {
		await act(async () => {
			root.render(
				<AccountProvider>
		...
		expect(
			container.querySelector('[aria-label="KerberoSec home"]'),
		).not.toBeNull();
		expect(
