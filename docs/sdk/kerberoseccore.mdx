---
title: "KerberoSecCore"
sidebarTitle: "KerberoSecCore"
description: "The full KerberoSec harness: built-in tools, sessions, approvals, scheduling, and hub support."
---

`KerberoSecCore` is the full KerberoSec harness as a programmable runtime. It gives you everything KerberoSec ships out of the box: built-in tools for files, shell, search, and web; session persistence and message history; tool approval callbacks; local, hub, and remote backends; scheduling and automation APIs; and plugin support.

Under the hood, `KerberoSecCore` uses the `Agent` class from `@kerberosec/agents`. You can use `Agent` directly if you want to skip the harness and wire everything yourself: your own tools, your own persistence, your own lifecycle.

## When to use which

| Need | Use |
|------|-----|
| Sessions, persistence, message history | `KerberoSecCore` |
| Built-in tools for files, shell, search, web fetch | `KerberoSecCore` |
| Hub/remote runtime | `KerberoSecCore` |
| Scheduling or event automation | `KerberoSecCore` |
| Multi-agent teams | `KerberoSecCore` |
...
`KerberoSecCore` wraps runtime execution with application features:
...
| Method | Purpose |
|--------|---------|
| `KerberoSecCore.create(options)` | Create the runtime |
...
See [KerberoSecCore reference](/sdk/reference/kerberosec-core) for exact signatures.
...
`KerberoSecCore` stores session manifests and messages as files. A session result includes:
...
`Agent` (also exported as `AgentRuntime`) is the stateless primitive that `KerberoSecCore` builds on. Use it directly when you want full control or don't need the harness.
