...
describe("plugin-config-loader", () => {
	...
	it("discovers plugin modules recursively", async () => {
		...
		try {
			...
			await writeFile(
				join(root, ".a.js.kerberosec-plugin.js"),
				"export default {}",
				"utf8",
			);
	...
	it("includes shared search-path plugins", async () => {
		...
		try {
			process.env.HOME = home;
			setHomeDir(home);
			const workspacePlugins = join(workspace, ".kerberosec", "plugins");
			const userPlugins = join(home, ".kerberosec", "plugins");
			const documentsPlugins = join(home, "Documents", "KerberoSec", "Plugins");
