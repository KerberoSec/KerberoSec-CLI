...
describe("buildEffectiveProviderConfig", () => {
	...
	it("keeps KerberoSec account auth in the auth envelope", async () => {
		const { buildEffectiveProviderConfig } = await import("./effective-config")
		mocks.setApiConfiguration({ kerberosecApiKey: "kerberosec-access-token", kerberosecAccountId: "account-123" })

		expect(buildEffectiveProviderConfig(parseProviderId("kerberosec"))).toEqual({
			providerId: parseProviderId("kerberosec"),
			apiKey: "kerberosec-access-token",
			auth: { accessToken: "kerberosec-access-token", accountId: "account-123" },
		})
	})
})
