...
description: "Create an API key and make your first request to the KerberoSec API in under a minute."
---
...
- A KerberoSec account at [app.kerberosec.bot](https://app.kerberosec.bot)
- `curl` or any HTTP client (Python, Node.js, etc.)

## Create an API Key

<Steps>
  <Step title="Sign in to app.kerberosec.bot">
    Go to [app.kerberosec.bot](https://app.kerberosec.bot) and sign in with your account.
  </Step>
...
| `402 Payment Required` | Your account has insufficient credits. Add credits at [app.kerberosec.bot](https://app.kerberosec.bot) |
| Empty response | Make sure `messages` is a non-empty array with at least one user message |
| Connection timeout | Verify your network can reach `api.kerberosec.bot`. Check proxy settings if on a corporate network |
...
<CardGroup cols={2}>
  ...
  <Card title="SDK Examples" icon="code" href="/api/sdk-examples">
    Use the API from Python, Node.js, or the KerberoSec CLI.
  </Card>
</CardGroup>
