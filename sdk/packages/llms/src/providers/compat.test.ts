...
import { KerberoSecNotSubscribedError } from "./errors";
...
describe("createGatewayApiHandler.getMessages", () => {
	...
	it("normalizes mixed legacy blocks and structured tool results", () => {
		...
		// image-extraction; this layer only translates KerberoSec's `Message[]`
...
describe("createGatewayApiHandler.createMessage", () => {
		...
		for await (const _chunk of handler.createMessage("", [
			{ role: "user", content: "hello" },
			{
				...
				content: [
					...
					{ type: "text", text: "Hello from KerberoSec" },
				],
			},
			{
				role: "assistant",
	...
	it("throws KerberoSecNotSubscribedError for KerberoSecPass required-plan 403 responses", async () => {
		streamTextSpy.mockReturnValue({
			fullStream: (async function* () {
				yield { type: "finish", finishReason: "stop" };
			})(),
