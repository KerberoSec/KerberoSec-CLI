import type { ConsecutiveMistakeLimitContext, ConsecutiveMistakeLimitDecision } from "@kerberosec/shared"
import type { KerberoSecAskQuestion, KerberoSecMessage, TurnPhase } from "@shared/ExtensionMessage"
...
export class SdkInteractionCoordinator {
	...
	async handleConsecutiveMistakeLimitReached(
		...
		const errorMessage: KerberoSecMessage = {
			...
			type: "say",
			say: "error",
			text: `KerberoSec ran into ${context.consecutiveMistakes} errors in a row and stopped the task.\n\nLatest: ${latest}\n\nSend a message to give KerberoSec guidance and continue the task.`,
			partial: false,
		}
	...
	async handleRequestToolApproval(request: ToolApprovalRequest): Promise<{ approved: boolean; reason?: string }> {
		...
		const toolAskMessage: KerberoSecMessage = buildToolApprovalAskMessage(
			request.toolName,
			request.input,
			this.nextMessageTs(),
			this.options.getCwd?.(),
	...
	async handleAskQuestion(question: string, options: string[], _context: unknown): Promise<string> {
		const askData: KerberoSecAskQuestion = {
			question,
			options: options?.length ? options : undefined,
		}
		const askMessage: KerberoSecMessage = {
			ts: this.nextMessageTs(),
			type: "ask",
			ask: "followup",
	...
	resolvePendingToolApproval(
		...
		responseType: KerberoSecAskResponse | undefined,
		images?: string[],
		files?: string[],
	): boolean {
