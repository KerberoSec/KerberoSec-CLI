...
describe("resolveSystemPrompt workspace metadata", () => {
	it("includes git remotes and the latest commit for KerberoSec requests", async () => {
		const cwd = mkdtempSync(join(tmpdir(), "kerberosec-prompt-"));
		workspaceDirectories.push(cwd);
		execFileSync("git", ["init"], { cwd });
		execFileSync("git", ["config", "user.email", "test@kerberosec.bot"], {
			cwd,
		});
		execFileSync("git", ["config", "user.name", "KerberoSec Test"], { cwd });
		writeFileSync(join(cwd, "README.md"), "test\n");
		execFileSync("git", ["add", "README.md"], { cwd });
		execFileSync("git", ["commit", "-m", "initial"], { cwd });
		execFileSync(
			"git",
			["remote", "add", "origin", "https://example.com/kerberosec/repo.git"],
			{ cwd },
		);
		...
		const prompt = await resolveSystemPrompt({ cwd, providerId: "kerberosec" });

		expect(prompt).toContain("origin: https://example.com/kerberosec/repo.git");
