 ...
 */
export function isLowStakesTool(message: KerberoSecMessage): boolean {
 ...
 */
export function isToolGroup(item: KerberoSecMessage | KerberoSecMessage[]): item is KerberoSecMessage[] & { _isToolGroup: true } {
	return Array.isArray(item) && (item as KerberoSecMessage[] & { _isToolGroup?: boolean })._isToolGroup === true
}

function isDuplicateAskOptionEcho(message: KerberoSecMessage, previousMessage: KerberoSecMessage | undefined): boolean {
	if (
		message.type !== "say" ||
	...
	try {
		const parsed = JSON.parse(previousMessage.text || "{}") as KerberoSecAskQuestion | KerberoSecPlanModeResponse
...
function isVisibleCheckpointUserMessage(message: KerberoSecMessage): boolean {
	return message.type === "say" && (message.say === "task" || message.say === "user_feedback")
}

function isCheckpointAnswerMessage(messages: KerberoSecMessage[], index: number): boolean {
...
export function canRestoreWorkspaceFromMessage(messages: KerberoSecMessage[], messageTs: number | undefined): boolean {
 ...
 */
export function filterVisibleMessages(messages: KerberoSecMessage[]): KerberoSecMessage[] {
	return messages.filter((message, index, arr) => {
		...
		switch (message.ask) {
			case "completion_result":
				// don't show a chat row for a completion_result ask without text. This specific type of message only occurs if kerberosec wants to execute a command as part of its completion result, in which case we interject the completion_result tool with the execute_command tool.
				if (message.text === "") {
					return false
				}
				break
		...
		switch (message.say) {
			...
			case "text":
				// Sometimes kerberosec returns an empty text message, we don't want to render these. (We also use a say text for user messages, so in case they just sent images we still render that)
 ...
 */
function isBrowserSessionMessage(message: KerberoSecMessage): boolean {
	if (message.type === "ask") {
 ...
 */
export function groupMessages(visibleMessages: KerberoSecMessage[]): (KerberoSecMessage | KerberoSecMessage[])[] {
	const result: (KerberoSecMessage | KerberoSecMessage[])[] = []
	let currentGroup: KerberoSecMessage[] = []
		...
		if (message.ask === "browser_action_launch" || message.say === "browser_action_launch") {
			...
			if (isBrowserSessionMessage(message)) {
				...
				if (message.say === "browser_action") {
					const browserAction = JSON.parse(message.text || "{}") as KerberoSecSayBrowserAction
 ...
 */
export function getToolsNotInCurrentActivities(toolGroupMessages: KerberoSecMessage[], allMessages: KerberoSecMessage[]): KerberoSecMessage[] {
	...
	let mostRecentApiReq: KerberoSecMessage | null = null
 ...
 *
 * Note: this operates on a flat `KerberoSecMessage[]` (e.g. `modifiedMessages`) rather than
 ...
 */
export function isApiReqAbsorbable(apiReqTs: number, allMessages: KerberoSecMessage[]): boolean {
	const apiReqIndex = allMessages.findIndex((m) => m.ts === apiReqTs && m.say === "api_req_started")
	if (apiReqIndex === -1) {
		return false
	}
 ...
 */
function isApiReqFollowedOnlyByLowStakesTools(index: number, messages: (KerberoSecMessage | KerberoSecMessage[])[]): boolean {
	let hasLowStakesTool = false
	let hasReasoning = false
 ...
 */
export function groupLowStakesTools(groupedMessages: (KerberoSecMessage | KerberoSecMessage[])[]): (KerberoSecMessage | KerberoSecMessage[])[] {
	const result: (KerberoSecMessage | KerberoSecMessage[])[] = []
	let toolGroup: KerberoSecMessage[] = []
	let pendingReasoning: KerberoSecMessage[] = []
	let pendingApiReq: KerberoSecMessage[] = []
	let hasTools = false
	const pendingTools: KerberoSecMessage[] = []
	...
	const commitToolGroup = () => {
		if (toolGroup.length > 0 && hasTools) {
			const group = toolGroup as KerberoSecMessage[] & { _isToolGroup: boolean }
