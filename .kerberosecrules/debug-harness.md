...
The debugee runs with `KERBEROSEC_DIR=~/.kerberosec2` by default, separate from your real `~/.kerberosec`.
This prevents the debugee's logout from logging out the debugger, and vice versa.
Override with `--kerberosec-dir /tmp/test-dir`. Check with `status()` → `kerberosecDir`.

## Browser Capture & OAuth

The debugee runs with `KERBEROSEC_CAPTURE_BROWSER=1`, which intercepts `openExternal()` in
`src/utils/env.ts`. URLs are captured instead of opening a real browser:

- Logged to `$KERBEROSEC_DIR/data/debug-captured-urls.jsonl`
...
For **KerberoSec OAuth** (SDK local callback): The SDK starts a local HTTP server, the auth URL
...
when `KERBEROSEC_CAPTURE_BROWSER` is set (the harness always sets it; never ships in prod).
...
| Command | View |
|---------|------|
| `kerberosec.accountButtonClicked` | Account / sign-in |
| `kerberosec.historyButtonClicked` | Task history |
| `kerberosec.settingsButtonClicked` | Settings |
| `kerberosec.mcpButtonClicked` | MCP servers |
| `kerberosec.plusButtonClicked` | New task (chat) |
| `kerberosec.worktreesButtonClicked` | Worktrees |
...
- **`ui.screenshot`** — screenshot to `/tmp/kerberosec-debug/`; returns `{path}` — **use `read_file` on the path to examine, do NOT `open` the file** (Preview.app covers the VSCode window)
...
- **⚠️ Dismiss promotional overlays FIRST**: On fresh launches, full-screen promo overlays block the sidebar. **Dismiss immediately after `ui.open_sidebar`**, before any other interaction or screenshot. May need to run twice:
  ...
  curl localhost:19229/api -d '{"method": "web.evaluate", "params": {"expression": "document.querySelectorAll(\".sr-only\").forEach(el => el.parentElement?.click())"}}'
  ```
- **Screenshots — don't open the file**: `ui.screenshot` and `ui.sidebar_screenshot` save PNGs to `/tmp/kerberosec-debug/` and return the `{path}`. Use `read_file` on that path to examine screenshots. Running `open <path>` launches Preview.app on macOS which covers the VSCode window.
