...
import { KerberoSecStorage } from "./KerberoSecStorage"
import type { BlobStoreSettings } from "./types"

export type { BlobStoreSettings } from "./types"

/**
 * S3/R2/Azure blob storage implementation of KerberoSecStorage.
 * Uses AWS S3, Cloudflare R2, or Azure Blob Storage as the backend storage.
 */
class KerberoSecBlobStorage extends KerberoSecStorage {
	override name = "KerberoSecBlobStorage"

	private static store: KerberoSecBlobStorage | null = null
	static get instance(): KerberoSecBlobStorage {
		if (!KerberoSecBlobStorage.store) {
			KerberoSecBlobStorage.store = new KerberoSecBlobStorage()
		}
		return KerberoSecBlobStorage.store
	}
	...
	public init(settings?: BlobStoreSettings) {
		...
		try {
			if (!KerberoSecBlobStorage.isConfigured(settings)) {
			...
			if (adapter) {
				...
				Logger.log(`[KerberoSecBlobStorage] Adapter created for ${settings.adapterType}`)
			}
		} catch (error) {
			// Log but don't throw - allow startup to continue
			Logger.error("[KerberoSecBlobStorage] initialization failed:", error)
		}
	}
 ...
 */
export const blobStorage = KerberoSecBlobStorage.instance
