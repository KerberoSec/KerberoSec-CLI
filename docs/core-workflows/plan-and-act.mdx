...
description: "Think first, then build. KerberoSec's dual-mode system for structured development."
---
...
Plan mode is where you and KerberoSec figure out what you're building and how. In this mode, KerberoSec can read your codebase, run searches, and discuss strategy, but cannot modify any files or execute commands.
...
Once you have a plan, switch to Act mode. KerberoSec retains the full context from your planning session and can now modify files, run commands, and execute your strategy.

The conversation history carries over when you switch modes. KerberoSec remembers everything you discussed in Plan mode, so you don't need to repeat yourself. This makes the transition seamless.

<Note>
	While you can start directly in Act mode, planning first is highly recommended. The planning phase intentionally builds context that KerberoSec needs to implement changes effectively. Without it, KerberoSec may lack the understanding required to make the right decisions.
</Note>

## Typical Workflow

1. Start in Plan mode and describe what you want to build
2. Let KerberoSec explore relevant files and understand the codebase
3. Discuss the approach, considering edge cases and potential issues
4. When confident in the plan, switch to Act mode
5. KerberoSec implements the solution based on your planning session
...
For complex tasks that need thorough analysis, use the `/deep-planning` slash command. This triggers an extended planning session where KerberoSec:
...
For complex features that span multiple files, require architectural decisions, or will take multiple sessions to complete, use the `/deep-planning` slash command. This creates a detailed implementation plan that KerberoSec can reference throughout the work.
...
- Have KerberoSec write a markdown file summarizing the plan for future reference
- Use [file mentions](/core-workflows/working-with-files) to point KerberoSec at relevant files during planning
...
- For large tasks, ask KerberoSec to create a todo list during planning that you can track in Act mode
