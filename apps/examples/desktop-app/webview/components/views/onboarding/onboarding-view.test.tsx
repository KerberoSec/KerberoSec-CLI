...
describe("sortProvidersForApiKeySetup", () => {
	it("drops OAuth-managed providers and ranks popular ones first", () => {
		const sorted = sortProvidersForApiKeySetup([
			...
			makeProvider({ id: "kerberosec", name: "KerberoSec" }),
...
describe("OnboardingView", () => {
	...
	it("walks from welcome to the connect step", async () => {
		...
		// KerberoSec is selected by default. The inactive API-key card is inert so its
		// controls cannot receive pointer or keyboard input through the overlay.
		expect(container.textContent).toContain("Set up KerberoSec");
		const kerberosecOption = container.querySelector(
			'[data-onboarding-option="kerberosec"]',
		);
		...
		expect(kerberosecOption?.getAttribute("data-selected")).toBe("true");
	...
	it("moves the accent selected state to the chosen setup option", async () => {
		...
		expect(kerberosecOption?.getAttribute("data-selected")).toBe("false");
		...
		const kerberosecCardAction = container.querySelector<HTMLButtonElement>(
			'button[aria-label="Sign in with KerberoSec"]',
		);
		expect(kerberosecCardAction).not.toBeNull();
		await act(async () => {
			kerberosecCardAction?.click();
		});
		// Switching back performs the inverse transition and restores focus to
		// the primary KerberoSec action.
		expect(kerberosecOption?.getAttribute("data-selected")).toBe("true");
		expect(apiKeyOption?.getAttribute("data-selected")).toBe("false");
	...
	it("keeps the KerberoSec API key form chevron static while toggling the panel", async () => {
		await render();
		await act(async () => {
			buttonByText("Get started").click();
		});
		...
		const trigger = buttonByText("Use a KerberoSec API key");
		const chevron = trigger.querySelector("svg");
		const chevronClassName = chevron?.getAttribute("class");
		const panel = container.querySelector("#onboarding-kerberosec-key-form");

		expect(trigger.getAttribute("aria-controls")).toBe(
			"onboarding-kerberosec-key-form",
		);
	...
	it("records KerberoSec as the provider when a signed-in user continues", async () => {
		...
		// Connecting a KerberoSec account routes through the GitHub integration step.
	...
	it("connects with a KerberoSec API key when OAuth sign-in is not used", async () => {
		const onComplete = await render();
		await act(async () => {
			buttonByText("Get started").click();
		});
		...
		await act(async () => {
			buttonByText("Use a KerberoSec API key").click();
		});
		const keyInput = container.querySelector<HTMLInputElement>(
			'input[aria-label="KerberoSec API key"]',
		);
		...
		invoke.mockImplementation(async (command: string) => {
			if (command === "save_provider_settings") {
				return { providerId: "kerberosec", enabled: true };
			}
			if (command === "kerberosec_account") {
				return { email: "dev@example.com", displayName: "Dev" };
			}
		...
		await act(async () => {
			...
			setter?.call(keyInput, "kerberosec_key_123");
	...
	it("rejects an invalid KerberoSec API key and rolls back the saved key", async () => {
		await render();
		await act(async () => {
			buttonByText("Get started").click();
		});
		...
		invoke.mockImplementation(
			async (command: string, args?: Record<string, unknown>) => {
				...
				if (command === "kerberosec_account") {
					throw new Error("KerberoSec account request failed with status 401");
				}
				return {};
			},
		);
	...
	it("keeps KerberoSec sign-in available while API-key setup is expanded", async () => {
		const onComplete = await render();
		await act(async () => {
			buttonByText("Get started").click();
		});
		...
		// still switch back and finish through the KerberoSec OAuth path.
		await act(async () => {
			container
		...
		invoke.mockImplementation(async (command: string) => {
			if (command === "run_provider_oauth_login") {
				return { provider: "kerberosec", accessToken: "token" };
			}
			if (command === "kerberosec_account") {
				throw new Error("No KerberoSec account auth token found");
			}
