...
Guidelines for deploying KerberoSec SDK agents in production environments.
...
For KerberoSecCore, check `finishReason`:
...
```typescript
import type { BasicLogger } from "@kerberosec/sdk"
...
process.on("SIGTERM", async () => {
  await kerberosec.dispose("SIGTERM")
  process.exit(0)
})
```
...
- `../kerberoseccore/REFERENCE.md` - KerberoSecCore overview
