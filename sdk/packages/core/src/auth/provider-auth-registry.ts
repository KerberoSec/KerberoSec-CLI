...
import {
	...
	type KerberoSecOAuthCredentials,
...
function stripKerberoSecApiKeyPrefix(accessToken: string): string {
	const token = accessToken.trim();
	return token.toLowerCase().startsWith(WORKOS_TOKEN_PREFIX)
...
function createKerberoSecAuthHandler(input: {
	providerId: string;
	storageProviderId?: string;
}): ProviderAuthHandler {
	return createOAuthHandler({
		...
		formatAccessToken: formatKerberoSecApiKey,
		normalizeStoredAccessToken: stripKerberoSecApiKeyPrefix,
		...
		refresh: ({ settings, credentials, forceRefresh, telemetry }) =>
			getValidKerberoSecCredentials(
				credentials as KerberoSecOAuthCredentials,
				{
					apiBaseUrl:
						settings.baseUrl?.trim() ||
						getKerberoSecEnvironmentConfig().apiBaseUrl,
...
const providerAuthHandlers = [
	createKerberoSecAuthHandler({ providerId: "kerberosec" }),
	createKerberoSecAuthHandler({
		providerId: "kerberosec-pass",
		storageProviderId: "kerberosec",
	}),
	createOAuthHandler({
		providerId: "oca",
		login: ({ settings, callbacks, telemetry }) =>
