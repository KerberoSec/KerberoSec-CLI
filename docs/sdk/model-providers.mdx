...
The `@kerberosec/llms` package provides provider registries, model catalogs, handlers, and a gateway. `@kerberosec/agents` and `@kerberosec/core` use this layer internally.
...
```typescript
import { DefaultGateway } from "@kerberosec/llms"
