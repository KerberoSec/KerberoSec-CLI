...
import {
	...
	resolveLocalKerberoSecAuthToken,
...
describe("addLocalProvider – model ID parsing via modelsSourceUrl", () => {
	...
	it("uses only live KerberoSecPass models when live models are found", async () => {
		const fetchMock = vi.fn(async (url: string) => {
			...
			return new Response(
				JSON.stringify({
					kerberosecPass: [
						{
							id: "kerberosec-pass/live-pass-model",
							name: "vendor/live-pass-model",
						},
					],
					free: [{ id: "kerberosec-free/live-free-model" }],
				}),
				{
					status: 200,
					headers: { "content-type": "application/json" },
		...
		const { models } = await getLocalProviderModels("kerberosec-pass");
		...
		expect(models.map((model) => model.id)).toEqual(
			expect.arrayContaining([
				"kerberosec-pass/live-pass-model",
				"kerberosec-free/live-free-model",
			]),
		);
		expect(
			models.find((model) => model.id === "kerberosec-pass/live-pass-model"),
		).toMatchObject({
			id: "kerberosec-pass/live-pass-model",
			name: "Live Pass Model",
			supportsReasoning: true,
		...
		expect(
			models.find((model) => model.id === "kerberosec-free/live-free-model"),
		).toMatchObject({
			id: "kerberosec-free/live-free-model",
			name: "Live Free Model (free)",
			supportsReasoning: true,
		});
	});

	it("falls back to generated KerberoSecPass models when no live KerberoSecPass models are found", async () => {
		const fetchMock = vi.fn(async (url: string) => {
			if (url === "https://models.dev/api.json") {
				return new Response(
					JSON.stringify({
			...
			return new Response(JSON.stringify({ kerberosecPass: [] }), {
				status: 200,
				headers: { "content-type": "application/json" },
			});
		});
...
describe("models.json model overlays", () => {
	it("loads model-only entries for built-in providers", async () => {
		...
		try {
			...
			const { models } = await getLocalProviderModels("kerberosec");
			expect(
...
describe("listLocalProviders", () => {
	...
	it("hides KerberoSecPass when the KerberoSecPass feature flag is disabled", async () => {
		const { providers } = await listLocalProviders(manager, {
			isKerberoSecPassEnabled: false,
		});

		expect(providers.map((p) => p.id)).not.toContain("kerberosec-pass");
	});

	it("includes KerberoSecPass when the KerberoSecPass feature flag is enabled", async () => {
		const { providers } = await listLocalProviders(manager, {
			isKerberoSecPassEnabled: true,
		});

		expect(providers.map((p) => p.id)).toContain("kerberosec-pass");
	});
	...
	it("stamps featured tiers from the bundled fallback without a feed fetch", async () => {
		...
		const { providers } = await listLocalProviders(manager);
		const modelList =
			providers.find((provider) => provider.id === "kerberosec")?.modelList ??
			[];
		const stampedIds = modelList
			.filter((model) => model.featured?.tier === "recommended")
			.map((model) => model.id);
		const expectedIds = FALLBACK_KERBEROSEC_RECOMMENDED_MODELS.recommended
			.map((model) => model.id)
	...
	it("stamps featured tiers from the cached live feed once warmed", async () => {
		const kerberosecModelIds = Object.keys(
			await LlmsModels.getModelsForProvider("kerberosec"),
		);
		const [recommendedId, freeId] = kerberosecModelIds;
		await getCachedKerberoSecRecommendedModels({
	...
	it("marks alias providers enabled without copying shared OAuth credentials", async () => {
		...
		markLocalProviderEnabled(manager, "kerberosec-pass", {
			tokenSource: "oauth",
		});

		const state = manager.read();
		expect(state.providers["kerberosec-pass"]?.settings).toEqual({
			provider: "kerberosec-pass",
		});
		expect(state.providers["kerberosec-pass"]?.tokenSource).toBe("oauth");
	});

	it("resolves shared OAuth metadata for KerberoSecPass catalog entries", async () => {
		manager.saveProviderSettings(
			{
				provider: "kerberosec",
				auth: {
		...
		const kerberosecPass = providers.find(
			(provider) => provider.id === "kerberosec-pass",
		);

		expect(kerberosecPass).toMatchObject({
			enabled: true,
	...
	it("uses KerberoSec-specific Z.ai aliases in the built-in model list", async () => {
		manager.saveProviderSettings(
			{
				provider: "kerberosec",
				apiKey: "test-key",
		...
		const { providers } = await listLocalProviders(manager);
		const kerberosec = providers.find(
			(provider) => provider.id === "kerberosec",
		);
		const openrouter = providers.find(
			(provider) => provider.id === "openrouter",
		);
		const kerberosecModelIds = new Set(
			kerberosec?.modelList?.map((model) => model.id) ?? [],
		);
		const openrouterModelIds = new Set(
			openrouter?.modelList?.map((model) => model.id) ?? [],
		);

		expect(kerberosec?.modelList?.length).toBeGreaterThan(0);
		expect(kerberosecModelIds).toContain("zai/glm-5.2");
		expect(kerberosecModelIds).not.toContain("z-ai/glm-5.2");
...
describe("normalizeOAuthProvider", () => {
	it("normalizes 'kerberosec' to 'kerberosec'", () => {
		expect(normalizeOAuthProvider("kerberosec")).toBe("kerberosec");
		expect(normalizeOAuthProvider("  KERBEROSEC  ")).toBe("kerberosec");
	});
...
describe("resolveLocalKerberoSecAuthToken", () => {
	it("returns undefined when settings is undefined", () => {
		expect(resolveLocalKerberoSecAuthToken(undefined)).toBeUndefined();
	});

	it("returns accessToken when present", () => {
		expect(
			resolveLocalKerberoSecAuthToken({
				provider: "kerberosec" as never,
	...
	it("returns undefined when both fields are absent", () => {
		expect(
			resolveLocalKerberoSecAuthToken({ provider: "kerberosec" as never }),
		).toBeUndefined();
	});
});
