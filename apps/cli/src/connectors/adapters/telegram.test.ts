...
import type { ConnectTelegramOptions } from "@kerberosec/shared";
...
const originalKerberoSecDataDir = process.env.KERBEROSEC_DATA_DIR;
...
function useTempKerberoSecDataDir(): string {
	const dataDir = mkdtempSync(join(tmpdir(), "kerberosec-telegram-test-"));
...
describe("telegramConnector", () => {
	...
	it("rejects mixing --allowed-user-id with the hook command env var", () => {
		const originalHookCommand = process.env.KERBEROSEC_CONNECT_HOOK_COMMAND;
		process.env.KERBEROSEC_CONNECT_HOOK_COMMAND = "echo noop";
		try {
			expect(() =>
				parseTelegramArgs([
					"--bot-token",
			...
			if (originalHookCommand === undefined) {
				delete process.env.KERBEROSEC_CONNECT_HOOK_COMMAND;
			} else {
				process.env.KERBEROSEC_CONNECT_HOOK_COMMAND = originalHookCommand;
			}
		}
	});
	...
	it("validates a token before reporting its connector as already running", async () => {
		const dataDir = useTempKerberoSecDataDir();
...
describe("telegram slash command delivery", () => {
	it("receives bot_command updates intercepted by the telegram chat library", async () => {
		...
		const dir = mkdtempSync(join(tmpdir(), "kerberosec-telegram-slash-"));
