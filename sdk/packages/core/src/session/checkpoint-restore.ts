...
export async function beginWorktreeRestoreTransaction(
	...
	const privateRef = `refs/kerberosec/restore-transactions/${transactionId}`;
	...
	await execFile(
		...
		[
			...
			"--include-untracked",
			"--message",
			`kerberosec restore transaction ${transactionId}`,
		],
		{ windowsHide: true },
	);
...
async function resolveCheckpointKind(
	...
	// Checkpoints created before `kind` was persisted used KerberoSec's stash
