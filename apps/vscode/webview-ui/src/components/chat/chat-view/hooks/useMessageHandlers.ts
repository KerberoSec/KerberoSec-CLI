...
import { AskResponseRequest, NewTaskRequest } from "@shared/proto/kerberosec/task"
import { IntentEvent } from "@shared/proto/kerberosec/ui"
 ...
 */
export function useMessageHandlers(messages: KerberoSecMessage[], chatState: ChatState): MessageHandlers {
	...
	const handleSendMessage = useCallback(
		async (text: string, images: string[], files: string[]) => {
			...
			// compacting the context window (KERBEROSEC-2503). `/newtask` aliases
			...
			if (hasContent) {
				...
				const beginPendingResponse = (pendingMessage?: KerberoSecMessage) => {
				...
				if (messages.length === 0) {
					...
					if (kerberosecAsk === "resume_task" || kerberosecAsk === "resume_completed_task") {
						...
						switch (kerberosecAsk) {
							...
							case "report_bug": {
								...
								const showPendingMessage = kerberosecAsk !== "followup" && turnState?.phase !== "streaming"
					...
					// No kerberosecAsk set, but there is an existing conversation. Route this to the
					...
					//      does not emit a trailing ask:"completion_result", so kerberosecAsk is
