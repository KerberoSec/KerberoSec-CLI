...
describe("CLI distribution package shape", () => {
	it("rejects direct source package publishing by default", () => {
		...
		expect(
			shouldAllowDirectPublish({ KERBEROSEC_ALLOW_DIRECT_PUBLISH: "1" }),
		).toBe(true);
	...
	it("packs the generated npm wrapper package", async () => {
		const packageDir = await mkdtemp(join(tmpdir(), "kerberosec-cli-pack-"));
		try {
				...
				`${JSON.stringify(
					{
						...
						optionalDependencies: {
							"@kerberosec/cli-linux-x64": "1.2.3",
						},
					},
					null,
					2,
			...
			await writeFile(
				join(packageDir, "bin", "kerberosec"),
				[
					"#!/usr/bin/env node",
					'console.log("kerberosec wrapper smoke test");',
					"",
				].join("\n"),
			);
			await chmod(join(packageDir, "bin", "kerberosec"), 0o755);
			await writeFile(
