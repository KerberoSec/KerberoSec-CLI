...
import { setHomeDir } from "@kerberosec/shared/storage";
...
describe("DefaultRuntimeBuilder", () => {
	...
	const previousGlobalSettingsPath =
	...
	it("derives enabled provider tools without registering a local executor", async () => {
		const settingsRoot = mkdtempSync(join(tmpdir(), "kerberosec-model-tools-"));
	...
	it("loads configured agent files as named subagent tools", async () => {
		const tempHome = mkdtempSync(join(tmpdir(), "kerberosec-agent-home-"));
		const workspaceRoot = mkdtempSync(
			join(tmpdir(), "kerberosec-agent-workspace-"),
		);
		tempDirs.push(tempHome, workspaceRoot);
		setHomeDir(tempHome);

		const globalAgentsDir = join(tempHome, ".kerberosec", "agents");
...
You are a code reviewer.`,
	...
	it("does not register root skills when only configured agents declare skills", async () => {
		...
		const agentsDir = join(workspaceRoot, ".kerberosec", "agents");
...
Use the review guidance.`,
	...
	it("includes MCP tools from configured servers", async () => {
		...
		const settingsPath = join(tempRoot, "kerberosec_mcp_settings.json");
...
process.stdin.on("data", (chunk) => {
  ...
  process.stdout.write("{]\\n");
	...
	it("includes skills tool when workspace skills are available", async () => {
		...
		const skillDir = join(cwd, ".kerberosec", "skills", "commit");
		mkdirSync(skillDir, { recursive: true });
		writeFileSync(
			join(skillDir, "SKILL.md"),
			`---
...
Use conventional commits.`,
	...
	it("includes skills bundled in discovered plugin packages", async () => {
		...
		setHomeDir(cwd);
		const pluginDir = join(cwd, ".kerberosec", "plugins", "review-plugin");
...
Use conventional commits.`,
	...
	it("marks configured but disabled skills in executor metadata", async () => {
		...
		const enabledDir = join(cwd, ".kerberosec", "skills", "commit");
		const disabledDir = join(cwd, ".kerberosec", "skills", "review");
...
Disabled skill.`,
	...
	it("scopes skills tool to session-configured skills", async () => {
		...
		const commitDir = join(cwd, ".kerberosec", "skills", "commit");
		const reviewDir = join(cwd, ".kerberosec", "skills", "review");
...
Review skill.`,
	...
	it("does not register the skills tool when all configured skills are disabled", async () => {
		...
		const skillDir = join(cwd, ".kerberosec", "skills", "review");
		mkdirSync(skillDir, { recursive: true });
		writeFileSync(
			join(skillDir, "SKILL.md"),
			`---
...
Review skill.`,
		...
		const userInstructionService = createUserInstructionConfigService({
			skills: { directories: [join(cwd, ".kerberosec", "skills")] },
			rules: { directories: [] },
