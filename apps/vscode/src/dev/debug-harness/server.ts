 ...
 *
 * Launches VSCode with the KerberoSec extension in debug mode and provides
 * an HTTP API for:
...
const SCREENSHOT_DIR = path.join(os.tmpdir(), "kerberosec-debug")
const DEFAULT_WORKSPACE = path.join(os.tmpdir(), "kerberosec-debug-workspace")
const DEFAULT_KERBEROSEC_DIR = path.join(os.homedir(), ".kerberosec2") // Separate profile from user's ~/.kerberosec
...
const KERBEROSEC_DIR_ARG = getArg("--kerberosec-dir") // Override the isolated KERBEROSEC_DIR
...
class DebugHarness {
	...
	private kerberosecDir: string = DEFAULT_KERBEROSEC_DIR // The KERBEROSEC_DIR used for the debugee
	...
	async launch(opts: { workspace?: string; skipBuild?: boolean } = {}): Promise<any> {
		...
		// Resolve the KERBEROSEC_DIR for the debugee (separate from debugger's ~/.kerberosec)
		this.kerberosecDir = KERBEROSEC_DIR_ARG || DEFAULT_KERBEROSEC_DIR
		fs.mkdirSync(this.kerberosecDir, { recursive: true })
		fs.mkdirSync(path.join(this.kerberosecDir, "data"), { recursive: true })
		log(`Debugee KERBEROSEC_DIR: ${this.kerberosecDir}`)

		// Clear any previously captured URLs
		this.capturedUrls = []
		// Also clear the captured URLs file on disk
		const captureFile = path.join(this.kerberosecDir, "data", "debug-captured-urls.jsonl")
		try {
			fs.unlinkSync(captureFile)
		} catch {}

		// Create temp user data dir to avoid interfering with real VSCode profile
		const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "kerberosec-debug-profile-"))
		...
		try {
			this.app = await _electron.launch({
				...
				env: {
					...
					KERBEROSEC_ENVIRONMENT: "production",
					// ── Data isolation: use separate profile from user's ~/.kerberosec ──
					KERBEROSEC_DIR: this.kerberosecDir,
					// ── Browser capture: intercept openExternal() for OAuth testing unless explicitly disabled ──
					KERBEROSEC_CAPTURE_BROWSER: BROWSER_CAPTURE ? "1" : "0",
					KERBEROSEC_DEBUG_HARNESS_PORT: String(PORT),
				},
		...
		return {
			...
			kerberosecDir: this.kerberosecDir,
		...
		while (Date.now() - start < 30000) {
			for (const frame of this.page.frames()) {
				...
				try {
					...
					if (title.startsWith("KerberoSec")) {
	 ...
	 * The vsCodeApi is exposed on window.__kerberosecVsCodeApi by platform.config.ts.
	...
	async webPostMessage(params: { message: any }): Promise<any> {
		...
		try {
			const result = await sidebar.evaluate((msg: any) => {
				const api = (window as any).__kerberosecVsCodeApi
				if (!api) {
					throw new Error(
						"window.__kerberosecVsCodeApi not found. " +
	...
	async uiOpenSidebar(): Promise<any> {
		if (!this.page) throw new Error("VSCode not running")
		try {
			await this.page.getByRole("tab", { name: /KerberoSec/ }).locator("a").click()
		} catch {
			// Activity bar might need a different approach
			await this.page.keyboard.press("Meta+Shift+p")
			await sleep(300)
			await this.page.keyboard.type("KerberoSec: Focus on KerberoSec View")
			await sleep(200)
	...
	async uiSendMessage(params: { text: string; images?: string[]; files?: string[]; responseType?: string }): Promise<any> {
		...
		return sidebar.evaluate(
			({
				...
				const api = (window as any).__kerberosecVsCodeApi
				if (!api) {
					throw new Error("window.__kerberosecVsCodeApi not found")
				}
				...
				if (responseType) {
					...
					api.postMessage({
						type: "grpc_request",
						grpc_request: {
							service: "kerberosec.TaskService",
							method: "askResponse",
	 ...
	 * KERBEROSEC_CAPTURE_BROWSER). Each entry has a timestamp and URL.
	...
	oauthReadStoredToken(): any {
		const secretsFile = path.join(this.kerberosecDir, "data", "secrets.json")
		if (!fs.existsSync(secretsFile)) {
			return { found: false, path: secretsFile }
		}
		try {
			const data = JSON.parse(fs.readFileSync(secretsFile, "utf-8"))
			// Extract the KerberoSec account ID and related auth info
			const accountId = data["kerberosec:kerberosecAccountId"]
	 ...
	 *
	 * For KerberoSec OAuth: the SDK's local callback server captures the code
	 ...
	 *   provider - Provider name for /auth path (e.g., "kerberosec", "oca")
		...
		try {
			...
			const result = await this.extCdp.send("Runtime.evaluate", {
				expression: `
					(() => {
						...
						// ui.command_palette with "KerberoSec: Handle URI" or similar.
			...
			return {
				...
				note:
					...
					"1. For KerberoSec OAuth (SDK local callback): the SDK captures the code " +
	...
	/**
	 * Read the captured URLs JSONL file from the debugee's KERBEROSEC_DIR.
