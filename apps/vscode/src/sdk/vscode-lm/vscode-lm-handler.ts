// KerberoSec SDK `ApiHandler` (from `@kerberosec/llms`) backed by the VS Code Language
...
import type { ApiHandler, ApiStreamChunk, HandlerModelInfo, Message, ProviderConfig, ToolDefinition } from "@kerberosec/llms"
...
export class VsCodeLmHandler implements ApiHandler {
	...
	private async getClient(): Promise<vscode.LanguageModelChat> {
		...
		try {
			...
			throw new Error(`KerberoSec <Language Model API>: Failed to select model: ${message}`)
		}
	}
	...
	async *createMessage(systemPrompt: string, messages: Message[], tools?: ToolDefinition[]): AsyncGenerator<ApiStreamChunk> {
		...
		try {
			const requestOptions: vscode.LanguageModelChatRequestOptions = {
				justification: `KerberoSec would like to use '${client.name}' from '${client.vendor}', Click 'Allow' to proceed.`,
			}
			...
			if (error instanceof vscode.CancellationError) {
				throw new Error("KerberoSec <Language Model API>: Request cancelled by user")
			}
			if (error instanceof Error) {
				Logger.error("KerberoSec <Language Model API>: Stream error:", error)
				throw error
			}
			throw new Error(`KerberoSec <Language Model API>: Response stream error: ${String(error)}`)
		} finally {
			this.ensureCleanState()
		}
	}
