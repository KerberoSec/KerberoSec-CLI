...
/**
 * Interactive Playwright launcher for the KerberoSec VS Code extension.
 *
 * Overview:
 *  - Starts the mock KerberoSec API server (from the e2e test fixtures).
 *  - Downloads a stable build of VS Code (via @vscode/test-electron).
 *  - Creates a temporary VS Code user profile directory.
 *  - Installs and links the KerberoSec extension (from dist/e2e.vsix and the dev path).
 *  - Opens a test workspace and automatically reveals the KerberoSec sidebar.
 ...
 *        bun run test:e2e:ui
 *
 *   3. VS Code will launch with the KerberoSec extension loaded and gRPC recording enabled.
 *
...
import { KerberoSecApiServerMock } from "../src/test/e2e/fixtures/server"
...
async function main() {
	...
	console.log("VSCode with KerberoSec extension is now running!")
