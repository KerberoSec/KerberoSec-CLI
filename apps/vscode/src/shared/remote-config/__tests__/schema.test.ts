...
describe("Remote Config Schema", () => {
	...
	describe("KerberoSecSettingsSchema", () => {
		it("should accept valid KerberoSec provider settings", () => {
			const validSettings = {
				models: [{ id: "claude-3-5-sonnet-20241022" }, { id: "claude-3-5-haiku-20241022" }],
			}
			const result = KerberoSecSettingsSchema.parse(validSettings)
			expect(result).to.deep.equal(validSettings)
		})

		it("should accept empty settings object", () => {
			const result = KerberoSecSettingsSchema.parse({})
			expect(result.models).to.be.undefined
		})
		...
		it("should accept models with only id field", () => {
			...
			expect(() => KerberoSecSettingsSchema.parse(settings)).to.not.throw()
		})

		it("should reject models with missing id field", () => {
			expect(() =>
				KerberoSecSettingsSchema.parse({
					models: [{}],
				}),
			).to.throw()
		})
	...
	describe("RemoteConfigSchema", () => {
		...
		it("should handle complete config with all fields", () => {
			const config = {
				...
				providerSettings: {
					...
					Anthropic: {
						...
						baseUrl: "https://example.kerberosec.bot",
					},
				},
				enterpriseTelemetry: {
					promptUploading: {
			...
			// Verify KerberoSec settings
			expect(result.providerSettings?.KerberoSec?.models).to.have.lengthOf(2)
			expect(result.providerSettings?.KerberoSec?.models?.[0].id).to.equal("claude-3-5-sonnet-20241022")
			expect(result.providerSettings?.KerberoSec?.models?.[1].id).to.equal("claude-3-5-haiku-20241022")
			...
			expect(result.providerSettings?.Anthropic?.baseUrl).to.equal("https://example.kerberosec.bot")
