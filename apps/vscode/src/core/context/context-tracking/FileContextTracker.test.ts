...
describe("FileContextTracker", () => {
	...
	it("should add a record when a file is read by a tool", async () => {
		...
		expect(fileEntry.kerberosec_read_date).to.be.a("number")
		expect(fileEntry.kerberosec_edit_date).to.be.null
	})

	it("should add a record when a file is edited by KerberoSec", async () => {
		await tracker.trackFileContext(filePath, "kerberosec_edited")
		...
		expect(activeEntry.record_source).to.equal("kerberosec_edited")
		expect(activeEntry.kerberosec_read_date).to.be.a("number")
		expect(activeEntry.kerberosec_edit_date).to.be.a("number")
	})
	...
	it("should mark existing entries as stale when adding a new entry for the same file", async () => {
		...
		mockTaskMetadata.files_in_context = [
			{
				...
				kerberosec_read_date: Date.now() - 1000, // 1 second ago
				kerberosec_edit_date: null,
				user_edit_date: null,
			},
		]
		...
		expect(newEntry.record_source).to.equal("kerberosec_edited")
	})
	...
	it("should not track KerberoSec edits as user edits", async () => {
		// First track the file to set up the watcher
		await tracker.trackFileContext(filePath, "read_tool")

		// Mark the file as edited by KerberoSec
		tracker.markFileAsEditedByKerberoSec(filePath)
