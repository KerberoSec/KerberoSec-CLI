import { VERCEL_OPENROUTER_MODEL_ID_ALIAS_RULES } from "@kerberosec/llms";
import {
...
export interface KerberoSecRecommendedModel {
	id: string;
...
export interface FetchKerberoSecRecommendedModelsOptions {
	baseUrl?: string;
...
export const FALLBACK_KERBEROSEC_RECOMMENDED_MODELS: KerberoSecRecommendedModelsData =
	{
		recommended: [
			{
...
function cloneRecommendedModels(
	...
	return {
		...
		kerberosecPass: data.kerberosecPass.map((model) => ({
			...model,
			tags: [...model.tags],
		})),
	};
...
function normalizeModel(raw: unknown): KerberoSecRecommendedModel | null {
...
function normalizeResponse(
	raw: unknown,
): KerberoSecRecommendedModelsData | null {
	if (!raw || typeof raw !== "object") return null;
	const data = raw as Record<string, unknown>;
	...
	const free = freeRaw
		.map(normalizeModel)
		.filter((model): model is KerberoSecRecommendedModel => model !== null);
	const kerberosecPass = kerberosecPassRaw
		.map(normalizeModel)
		.filter((model): model is KerberoSecRecommendedModel => model !== null);
	if (
		recommended.length === 0 &&
		free.length === 0 &&
		kerberosecPass.length === 0
	) {
		return null;
	}
...
function getConfiguredApiBaseUrl(
	options: FetchKerberoSecRecommendedModelsOptions,
): string {
	const explicitBaseUrl = options.baseUrl?.trim();
	if (explicitBaseUrl) return explicitBaseUrl;

	const fallbackBaseUrl = getKerberoSecEnvironmentConfig().apiBaseUrl;
	try {
		const manager =
...
function resolveEntryDisplayName(
	entry: KerberoSecRecommendedModel,
...
async function resolveDisplayNames(
	...
	catalogLoader: () => Promise<ModelsCatalog>,
	timeoutMs: number,
): Promise<KerberoSecRecommendedModelsData> {
	const catalog = await loadCatalogWithTimeout(catalogLoader, timeoutMs);
	const withNames = (
		models: KerberoSecRecommendedModel[],
		catalogs: Array<Record<string, ModelInfo> | undefined>,
	) =>
		models.map((model) => ({
			...model,
	...
	return {
		...
		free: withNames(data.free, [catalog?.kerberosec, catalog?.openrouter]),
		kerberosecPass: withNames(data.kerberosecPass, [
			catalog?.["kerberosec-pass"],
		]),
	};
}

export async function fetchKerberoSecRecommendedModels(
	options: FetchKerberoSecRecommendedModelsOptions = {},
): Promise<KerberoSecRecommendedModelsData> {
	...
	// FALLBACK_KERBEROSEC_RECOMMENDED_MODELS (e.g. to avoid caching a transient
	// failure).
	return cloneRecommendedModels(FALLBACK_KERBEROSEC_RECOMMENDED_MODELS);
}
...
let feedInFlight: Promise<KerberoSecRecommendedModelsData> | null = null;
let feedGeneration = 0;

/**
 * `fetchKerberoSecRecommendedModels` with a shared in-memory cache, for callers on
 ...
 * TTL.
 */
export async function getCachedKerberoSecRecommendedModels(
	options: FetchKerberoSecRecommendedModelsOptions = {},
): Promise<KerberoSecRecommendedModelsData> {
	...
	const request = fetchKerberoSecRecommendedModels(options)
		.then((data) => {
 ...
 * async `getCachedKerberoSecRecommendedModels` path refreshes the cache, after
 * which peeks serve live data for the TTL.
 */
export function peekKerberoSecRecommendedModels(): KerberoSecRecommendedModelsData {
	if (feedCache && feedCache.expiresAt > Date.now()) {
		return cloneRecommendedModels(feedCache.data);
	}
	return cloneRecommendedModels(FALLBACK_KERBEROSEC_RECOMMENDED_MODELS);
...
> = {
	...
	// KerberoSecPass surfaces its subscription models plus the free models (both
	// ride the same KerberoSec API; free models bill $0 outside the quota).
	"kerberosec-pass": [
		{ tier: "subscribed", select: (data) => data.kerberosecPass },
		{ tier: "free", select: (data) => data.free },
	],
};
 ...
 * vs `kerberosec-free/kat-coder-pro`), most visibly when the bundled fallback
 ...
 */
export function applyKerberoSecFeaturedModels(
	providerId: string,
	...
	// `kerberosec-free/x` from the free overlay) would render the model twice
	// inside its tier.
