...
export const KERBEROSEC_PLUGIN_IDLE_TIMEOUT_MS_ENV =
	"KERBEROSEC_PLUGIN_IDLE_TIMEOUT_MS";
...
export interface PluginSandboxOptions extends PluginTargeting {
	 ...
	 * to the `KERBEROSEC_PLUGIN_IMPORT_TIMEOUT_MS` env var when this option is not
...
function getPlatformPackageName(): string {
	...
	return `@kerberosec/cli-${platform}-${process.arch}`;
}
...
> {
	...
	const importTimeoutMs = withTimeoutFallback(
		...
		4000,
		"KERBEROSEC_PLUGIN_IMPORT_TIMEOUT_MS",
	);
	...
	const extensions: NonNullable<AgentConfig["extensions"]> = descriptors.map(
		(descriptor) => {
			const extension: SandboxedAgentExtension = {
				...
				__kerberosecPluginPath: descriptor.pluginPath,
