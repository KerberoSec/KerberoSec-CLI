...
const sourcemap = Bun.env.KERBEROSEC_SOURCEMAPS === "1" ? "linked" : "none";
const rootDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(rootDir, "../../");
const hubWebviewSourcePath = join(repoRoot, "apps/kerberosec-hub/src/webview");
const hubWebviewDistPath = join(repoRoot, "apps/kerberosec-hub/dist/webview");
const hubWebviewIndexPath = join(hubWebviewDistPath, "index.html");
const cliHubWebviewDistPath = join(rootDir, "dist/kerberosec-hub/webview");
...
if (shouldBuildHubWebview()) {
	...
	await $`bun -F @kerberosec/kerberosec-hub build:webview`.cwd(repoRoot);
}
...
const result = await Bun.build({
	...
	env: "OTEL_*",
	banner:
		'import { createRequire as __kerberosecCreateRequire } from "node:module"; const require = __kerberosecCreateRequire(import.meta.url);',
});
