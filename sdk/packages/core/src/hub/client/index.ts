...
const HUB_AUTH_PROTOCOL_PREFIX = "kerberosec-hub-auth.";
	...
	async connect(): Promise<void> {
		...
		socket.addEventListener("message", (data: unknown) => {
			try {
				...
				// Ignore malformed or non-JSON transport frames
			}
		});
		socket.addEventListener("close", (event: unknown) => {
					...
					: setTimeout(() => {
							...
							reject(
								new HubCommandError(
									command,
									"hub_command_timeout",
									`Hub command ${command} timed out after ${effectiveTimeoutMs}ms (hub=${this.currentUrl}, requestId=${requestId}, clientId=${this.clientId}). Check hub-daemon.log for matching command.start/command.slow entries, or run 'kerberosec doctor fix' to restart the hub.`,
								),
							);
						}, effectiveTimeoutMs);
