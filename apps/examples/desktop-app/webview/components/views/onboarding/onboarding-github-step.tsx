...
import { GitHubIcon } from "@kerberosec/ui";
...
import {
	...
	type KerberoSecGitHubRepository,
	listKerberoSecGitHubRepositories,
	listKerberoSecIntegrations,
} from "@/lib/kerberosec-integrations";
...
export function GitHubConnectStep({ onContinue }: { onContinue: () => void }) {
	...
	const [repos, setRepos] = useState<KerberoSecGitHubRepository[] | null>(null);
	...
	useEffect(() => {
		...
		void (async () => {
			try {
				const result = await listKerberoSecIntegrations();
				if (cancelled) {
					return;
				}
				if (
		...
		async function poll() {
			try {
				...
				if (result.status === "not-authenticated") {
					...
					stop();
					setConnectError(
						"Your KerberoSec account session ended. Sign in again to connect GitHub.",
					);
					setPhase("connect");
					return;
				}
	...
	return (
		<>
			...
			<p className="mt-2 text-sm text-muted-foreground">
				Grant KerberoSec access to your GitHub repositories to supercharge it
				with real-world context. You can always do this later from your
				dashboard.
			</p>
				...
				{phase === "waiting" ? (
					<div className="mt-3 flex flex-wrap items-center gap-3">
						<p className="inline-flex items-center gap-2 text-sm text-muted-foreground">
							...
							Finish installing the KerberoSec GitHub App in your browser...
						</p>
						<button
