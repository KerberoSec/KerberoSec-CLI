...
const DEPRECATED_CONFIG_DIR = ".kerberosecrules";
const KERBEROSEC_CONFIG_DIR = ".kerberosec";
/**
 ...
 * - KerberoSec plugin  -> a JS/TS module, discovered under `.kerberosec/plugins`
 *
 * KerberoSec plugin discovery treats it as a hard stop: a directory carrying this
 * file is never scanned for KerberoSec plugin modules, so a plugin authored for
 * another client and dropped into a KerberoSec plugin root cannot have its skill
 * scripts or another vendor's extension directory imported as KerberoSec code.
 */
 ...
 * Lives under the kerberosec data dir (not `os.tmpdir()`) so OS temp reapers never
...
export const KERBEROSEC_MCP_SETTINGS_FILE_NAME = "kerberosec_mcp_settings.json";
export const KERBEROSEC_CONNECTOR_SETTINGS_FILE_NAME = "settings.json";
...
let KERBEROSEC_DIR: string | undefined;
let KERBEROSEC_DIR_SET_EXPLICITLY = false;

export function setKerberoSecDir(dir: string): void {
	const trimmed = dir.trim();
	if (!trimmed) {
		return;
	}
	KERBEROSEC_DIR = trimmed;
	KERBEROSEC_DIR_SET_EXPLICITLY = true;
}

export function setKerberoSecDirIfUnset(dir: string): void {
	if (KERBEROSEC_DIR_SET_EXPLICITLY) {
		return;
	}
	const trimmed = dir.trim();
	if (!trimmed) {
...
export function resolveKerberoSecDir(): string {
	if (KERBEROSEC_DIR) {
		return KERBEROSEC_DIR;
	}
	const envDir = process.env.KERBEROSEC_DIR?.trim();
	if (envDir) {
		return envDir;
	}
	return join(HOME_DIR, ".kerberosec");
}

export function resolveDocumentsKerberoSecDirectoryPath(): string {
	return join(HOME_DIR, "Documents", "KerberoSec");
}
...
export function resolveDocumentsExtensionPath(
	...
	return join(resolveDocumentsKerberoSecDirectoryPath(), name);
}

export function resolveKerberoSecDataDir(): string {
	const explicitDir = process.env.KERBEROSEC_DATA_DIR?.trim();
	if (explicitDir) {
		return explicitDir;
	}
	return join(resolveKerberoSecDir(), "data");
}

export function resolveSessionDataDir(): string {
	const explicitDir = process.env.KERBEROSEC_SESSION_DATA_DIR?.trim();
	if (explicitDir) {
		return explicitDir;
	}
	return join(resolveKerberoSecDataDir(), "sessions");
}

export function resolveTeamDataDir(): string {
	const explicitDir = process.env.KERBEROSEC_TEAM_DATA_DIR?.trim();
	if (explicitDir) {
		return explicitDir;
	}
	return join(resolveKerberoSecDataDir(), "teams");
}

export function resolveConnectorDataDir(): string {
	const explicitDir = process.env.KERBEROSEC_CONNECTOR_DATA_DIR?.trim();
	if (explicitDir) {
		return explicitDir;
	}
	return join(resolveKerberoSecDataDir(), "connectors");
}
...
export function resolveConnectorSettingsPath(): string {
	const explicitPath = process.env.KERBEROSEC_CONNECTOR_SETTINGS_PATH?.trim();
	if (explicitPath) {
		return explicitPath;
	}
	return join(
...
export function resolveDbDataDir(): string {
	const explicitDir = process.env.KERBEROSEC_DB_DATA_DIR?.trim();
	if (explicitDir) {
		return explicitDir;
	}
	return join(resolveKerberoSecDataDir(), "db");
}
...
export function resolveConnectorsDbPath(): string {
	const explicitPath = process.env.KERBEROSEC_CONNECTORS_DB_PATH?.trim();
	if (explicitPath) {
		return explicitPath;
	}
...
export function resolveCronDbPath(): string {
	const explicitPath = process.env.KERBEROSEC_CRON_DB_PATH?.trim();
	if (explicitPath) {
		return explicitPath;
	}
...
export function resolveTasksDbPath(): string {
	const explicitPath = process.env.KERBEROSEC_TASKS_DB_PATH?.trim();
	if (explicitPath) {
		return explicitPath;
	}
...
/**
 * Home workspace for agent-created schedules: `~/.kerberosec/schedules/`.
...
export function resolveAgentSchedulesDir(): string {
	return join(resolveKerberoSecDir(), "schedules");
}

/** Global file-backed agenda tasks: `~/.kerberosec/tasks/`. */
export function resolveGlobalTaskSpecsDir(): string {
	return join(resolveKerberoSecDir(), "tasks");
}

/** Workspace file-backed agenda tasks: `<workspace>/.kerberosec/tasks/`. */
export function resolveWorkspaceTaskSpecsDir(workspaceRoot: string): string {
	...
	return join(normalized, ".kerberosec", "tasks");
}
...
export interface ResolveCronSpecsDirOptions {
	 ...
	 */
	cronSpecsDir?: string;
	/** Defaults to `global`, i.e. `~/.kerberosec/cron`. */
...
export function resolveProviderSettingsPath(): string {
	const explicitPath = process.env.KERBEROSEC_PROVIDER_SETTINGS_PATH?.trim();
	if (explicitPath) {
		return explicitPath;
	}
	return join(resolveKerberoSecDataDir(), "settings", "providers.json");
}

export function resolveGlobalSettingsPath(): string {
	const explicitPath = process.env.KERBEROSEC_GLOBAL_SETTINGS_PATH?.trim();
	if (explicitPath) {
		return explicitPath;
	}
	return join(resolveKerberoSecDataDir(), "settings", "global-settings.json");
}

export function resolveMcpSettingsPath(): string {
	const explicitPath = process.env.KERBEROSEC_MCP_SETTINGS_PATH?.trim();
	if (explicitPath) {
		return explicitPath;
	}
	return join(
...
export function resolveAgentsConfigDirPath(): string {
	return join(resolveKerberoSecDir(), AGENT_CONFIG_DIRECTORY_NAME);
}
...
export function resolveAgentConfigSearchPaths(
	...
	return dedupePaths([
		workspacePath
			? join(workspacePath, KERBEROSEC_CONFIG_DIR, AGENT_CONFIG_DIRECTORY_NAME)
			: "",
...
export function resolveHooksConfigSearchPaths(
	...
	const hooks = [
		...
		join(resolveKerberoSecDir(), HOOKS_CONFIG_DIRECTORY_NAME),
	];
	if (workspacePath) {
		hooks.push(
			join(workspacePath, DEPRECATED_CONFIG_DIR, HOOKS_CONFIG_DIRECTORY_NAME),
			join(workspacePath, KERBEROSEC_CONFIG_DIR, HOOKS_CONFIG_DIRECTORY_NAME),
		);
	}
...
export function resolveSkillsConfigSearchPaths(
	...
	return dedupePaths([
		...
		join(resolveKerberoSecDir(), SKILLS_CONFIG_DIRECTORY_NAME),
		join(
			HOME_DIR,
			LEGACY_AGENT_SKILLS_CONFIG_DIR,
...
export function resolveRulesConfigSearchPaths(
	...
	const wsPaths = workspacePath
		? [
				...
				join(workspacePath, KERBEROSEC_CONFIG_DIR, RULES_CONFIG_DIRECTORY_NAME),
			]
		: [];
	...
	return dedupePaths([
		...
		join(resolveKerberoSecDir(), RULES_CONFIG_DIRECTORY_NAME),
		...
		// global rules at ~/KerberoSec/Rules instead of ~/Documents/KerberoSec/Rules
		// (kerberosec/kerberosec#13542).
		join(HOME_DIR, "KerberoSec", "Rules"),
...
export function resolveWorkflowsConfigSearchPaths(
	...
	return dedupePaths([
		workspacePath
			? join(workspacePath, ".kerberosecrules", WORKFLOWS_CONFIG_DIRECTORY_NAME)
			: "",
		resolveDocumentsExtensionPath("Workflows"),
		join(resolveKerberoSecDir(), WORKFLOWS_CONFIG_DIRECTORY_NAME),
		workspacePath
			? join(workspacePath, ".kerberosec", WORKFLOWS_CONFIG_DIRECTORY_NAME)
			: "",
	]);
}
...
export function resolvePluginConfigSearchPaths(
	...
	return dedupePaths([
		workspacePath
			? join(workspacePath, ".kerberosec", PLUGINS_DIRECTORY_NAME)
			: "",
		join(resolveKerberoSecDir(), PLUGINS_DIRECTORY_NAME),
...
/**
 * Never descended during KerberoSec plugin discovery. A dependency tree is never a
 ...
 * and must not be scanned for KerberoSec plugin modules.
 */
...
function readPluginPackageManifest(
	...
	try {
		const packageJson = JSON.parse(readFileSync(packageJsonPath, "utf8")) as {
			kerberosec?: PluginPackageManifest;
		};
		if (!packageJson.kerberosec || typeof packageJson.kerberosec !== "object") {
			return null;
		}
		return packageJson.kerberosec;
	} catch {
		return null;
	}
}
...
export function resolvePluginModuleEntries(
	...
	// An Agent Plugin is not a KerberoSec plugin. Claim nothing from it here so an
 ...
 * (e.g. `~/.kerberosec/plugins/_installed/<id>/package/index.ts`) are named after
	...
	while (stack.length > 0) {
		...
		for (const entry of entries) {
			...
			if (entry.isDirectory()) {
				...
				// its vendored dependencies as though each were a KerberoSec plugin —
...
export function ensureHookLogDir(filePath?: string): string {
	...
	const dir = join(resolveKerberoSecDataDir(), "logs");
	if (!existsSync(dir)) {
