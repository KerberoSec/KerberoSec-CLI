...
describe("sidebar session organization", () => {
	...
	it("uses the repository directory instead of the full workspace path", () => {
		expect(workspaceDisplayName("/Users/saoud/code/kerberosec/")).toBe(
			"kerberosec",
		);
		expect(workspaceDisplayName("C:\\Users\\saoud\\code\\kerberosec\\")).toBe(
			"kerberosec",
		);
	});

	it("labels chat workspace groups as Chat", () => {
		const path = "/home/host/.kerberosec/data/workspaces/chat";
