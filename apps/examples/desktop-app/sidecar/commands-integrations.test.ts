...
describe("listKerberoSecIntegrations", () => {
	it("lists integrations through the envelope with a bearer token", async () => {
		...
		const result = await listKerberoSecIntegrations(requestOptions(fetchImpl));
...
describe("listKerberoSecGitHubRepositories", () => {
	it("lists GitHub repositories from the repositories endpoint", async () => {
		const fetchImpl = vi.fn().mockResolvedValue(
			jsonResponse({
				success: true,
				data: [{ full_name: "kerberosec/kerberosec" }],
			}),
		);

		const result = await listKerberoSecGitHubRepositories(
			requestOptions(fetchImpl),
		);

		expect(result).toEqual([{ full_name: "kerberosec/kerberosec" }]);
	...
	it("surfaces the API envelope error message on failures", async () => {
		...
		await expect(
			listKerberoSecIntegrations(requestOptions(fetchImpl)),
...
describe("resolveGitHubInstallUrl", () => {
	it("resolves the GitHub install URL from the redirect location", async () => {
		const fetchImpl = vi.fn().mockResolvedValue(
			new Response(null, {
				...
				headers: {
					location:
						"https://github.com/apps/kerberosec/installations/new?state=abc",
				},
			}),
		);
		...
		expect(result).toEqual({
			url: "https://github.com/apps/kerberosec/installations/new?state=abc",
		});
		const [url, init] = fetchImpl.mock.calls[0] as [URL, RequestInit];
		expect(url.origin + url.pathname).toBe(
			"https://api.example.com/api/v1/integrations/github/install",
		);
		// The post-install browser hop must land on the KerberoSec dashboard.
	...
	it("resolves a relative redirect location against the request URL", async () => {
		const fetchImpl = vi.fn().mockResolvedValue(
			new Response(null, {
				status: 302,
				headers: { location: "//github.com/apps/kerberosec/installations/new" },
			}),
		);
		...
		expect(result).toEqual({
			url: "https://github.com/apps/kerberosec/installations/new",
		});
	});

	it.each([
		["https://evil.example/apps/kerberosec", "evil.example"],
		[
			"https://github.com.evil.example/apps/kerberosec",
			"github.com.evil.example",
		],
		// Subdomains are not part of the install flow, so they are not allowed
		// either -- the host must be exactly github.com.
		["https://gist.github.com/apps/kerberosec", "gist.github.com"],
	...
	it("rejects a redirect that does not use https", async () => {
		const fetchImpl = vi.fn().mockResolvedValue(
			new Response(null, {
				status: 302,
				headers: { location: "http://github.com/apps/kerberosec" },
			}),
		);
...
describe("kerberosec_integrations command auth states", () => {
	it("returns a typed not-authenticated result when signed out, without calling the API", async () => {
		...
		const result = await handleCommand(ctx, "kerberosec_integrations", {
			operation: "list",
		});
	...
	it("calls the KerberoSec API with the resolved fresh token when signed in", async () => {
	...
	it("rejects unknown operations", async () => {
		...
		const { handleCommand } = await import("./commands");
		await expect(
			handleCommand(ctx, "kerberosec_integrations", {
				operation: "dropIntegrations",
			}),
		).rejects.toThrow("Unsupported KerberoSec integrations operation");
		expect(fetchMock).not.toHaveBeenCalled();
	});
});
