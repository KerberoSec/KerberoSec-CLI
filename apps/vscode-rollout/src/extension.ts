...
async function activateBundle(
	...
	try {
		...
		// (built with KERBEROSEC_ROLLOUT_VARIANT). On fallback this runs in the legacy
		...
		console.error(
			"[kerberosec-rollout] next bundle failed to activate, falling back to legacy:",
			error,
		);
...
function showNightlyBundleIndicator(
	...
	try {
		...
		item.text = bundle === "next" ? "KerberoSec: Next" : "KerberoSec: Legacy";
		const detail = fallbackFrom
			...
			: meta.override
				...
				: "rollout assignment";
		item.tooltip = `KerberoSec nightly A/B rollout: running the ${bundle === "next" ? "next (SDK)" : "legacy"} bundle (${detail}).`;
		item.show();
		context.subscriptions.push(item);
	} catch (error) {
		console.warn("[kerberosec-rollout] could not show bundle indicator:", error);
	}
}
