...
describe("@kerberosec/ui agent chat primitives", () => {
	...
	it("marks message roles without requiring a runtime message schema", async () => {
		await render(
			<Message from="assistant">
				<MessageContent>Hello from KerberoSec</MessageContent>
			</Message>,
		);

		const message = container.querySelector(".kerberosec-chat-message");
		expect(message?.getAttribute("data-role")).toBe("assistant");
		expect(message?.textContent).toContain("Hello from KerberoSec");
	});
	...
	it("renders non-expandable tool activity as static content", async () => {
		...
		const summary = container.querySelector(".kerberosec-chat-tool-trigger");
	...
	it("swaps the icon for the spinner while the tool is in flight", async () => {
		...
		expect(
			container.querySelector(".kerberosec-chat-tool-progress"),
		).not.toBeNull();
	...
	it("toggles expandable tool details", async () => {
		...
		expect(panel?.getAttribute("inert")).toBe("");
		expect(
			container.querySelector(".kerberosec-chat-disclosure-icon"),
		).not.toBeNull();
	...
	it("labels the thinking row by streaming state and duration", async () => {
		...
		expect(
			trigger.querySelector(".kerberosec-chat-streaming-title"),
		).not.toBeNull();
		...
		expect(
			panel?.querySelector(
				".kerberosec-chat-thinking-content.kerberosec-chat-panel-rail",
			),
		).not.toBeNull();
	});
	...
	it("toggles the collapsed work summary open and closed", async () => {
		...
		const content = panel?.querySelector(".kerberosec-chat-work-content");
