...
// cancelTask, …) to the KerberoSec SDK (@kerberosec/core) and bridges SDK events to
...
import { formatDisplayUserInput, type RemoteConfig, type RemoteConfigBundle } from "@kerberosec/shared"
import type { ApiConfiguration } from "@shared/api"
import type { ChatContent } from "@shared/ChatContent"
import { KERBEROSEC_ACCOUNT_AUTH_ERROR_MESSAGE } from "@shared/KerberoSecAccount"
import { mentionRegexGlobal } from "@shared/context-mentions"
import type { KerberoSecApiReqInfo, KerberoSecMessage, ExtensionState } from "@shared/ExtensionMessage"
import type { HistoryItem } from "@shared/HistoryItem"
import { DeleteAllTaskHistoryCount, type GetTaskHistoryRequest, TaskHistoryArray, TaskResponse } from "@shared/proto/kerberosec/task"
import type { Settings } from "@shared/storage/state-keys"
import type { Mode } from "@shared/storage/types"
import type { TelemetrySetting } from "@shared/TelemetrySetting"
import type { KerberoSecCheckpointRestore } from "@shared/WebviewMessage"
...
import { KerberoSecError } from "@/services/error/KerberoSecError"
import { McpHub } from "@/services/mcp/McpHub"
import { telemetryService } from "@/services/telemetry"
import type { KerberoSecExtensionContext } from "@/shared/kerberosec"
import { toLegacyApiProvider } from "@/shared/model-catalog/provider-helpers"
import { ShowMessageRequest, ShowMessageType } from "@/shared/proto/host/window"
import { Logger } from "@/shared/services/Logger"
import { isKerberoSecManagedProvider } from "@/shared/utils/kerberosec"
import { arePathsEqual, getDesktopDir } from "@/utils/path"
import { KerberoSecAccountService } from "./account-service"
import { AuthService, LogoutReason } from "./auth-service"
import { BUILTIN_SLASH_COMMANDS } from "./builtin-slash-commands"
import { buildStartSessionInput, createHistoryItemFromSession } from "./kerberosec-session-factory"
...
function usesKerberoSecAccountAuth(providerId: string): boolean {
	return getProviderAuthStorageId(providerId) === "kerberosec"
}
...
export class Controller {
	...
	mcpHub: McpHub
	accountService: KerberoSecAccountService
	...
	private pendingKerberoSecAuthRetryPrompt?: string
	...
	// materialized by remote config under `.kerberosec/remote-config/`. Used to expand
	...
	constructor(readonly context: KerberoSecExtensionContext) {
		...
		// IMPORTANT: Use ~/.kerberosec/data/settings/ for the settings directory,
		...
		// settings file lives at ~/.kerberosec/data/settings/kerberosec_mcp_settings.json
		...
		this.accountService = KerberoSecAccountService.getInstance()
		...
		this.messageTranslatorState = new MessageTranslatorState(
			...
			// retired kerberosec-free/ models (the error payload itself never names one).
		...
		this.sessions = new SdkSessionLifecycle({
			...
			onSendError: async (error, sessionId) => {
				...
				const isKerberoSecAuthError =
					isKerberoSecManagedProvider(providerId) &&
					(errorMessage.includes(KERBEROSEC_ACCOUNT_AUTH_ERROR_MESSAGE) ||
						errorMessage.toLowerCase().includes("missing api key") ||
						errorMessage.toLowerCase().includes("unauthorized"))

				if (isKerberoSecAuthError) {
					...
					this.emitKerberoSecAuthError()
				} else if (isKerberoSecManagedProvider(providerId) && this.isKerberoSecBalanceError(errorMessage)) {
					this.captureProviderFailure({
						sessionId,
						error,
						providerId,
					...
					this.emitKerberoSecBalanceError(errorMessage)
				} else {
					this.captureProviderFailure({
						sessionId,
						error,
		...
		this.mode = new SdkModeCoordinator({
			...
			emitKerberoSecAuthError: () => this.emitKerberoSecAuthErrorWithTelemetry(),
		...
		this.followups = new SdkFollowupCoordinator({
			...
			buildStartSessionInput,
			resolveContextMentions: (text) => this.resolveContextMentions(text),
			isKerberoSecManagedProviderActive: () => this.isKerberoSecManagedProviderActive(),
			emitKerberoSecAuthError: () => this.emitKerberoSecAuthErrorWithTelemetry(),
			resetMessageTranslator: () => this.resetMessageTranslatorAndFence(),
			postStateToWebview: () => this.postStateToWebview(),
			onResumeFailed: () => {
		...
		this.taskStart = new SdkTaskStartCoordinator({
			...
			clearTask: async () => {
				this.pendingKerberoSecAuthRetryPrompt = undefined
			...
			resolveContextMentions: (text) => this.resolveContextMentions(text),
			isKerberoSecManagedProviderActive: () => this.isKerberoSecManagedProviderActive(),
			emitKerberoSecAuthError: (task) => this.emitKerberoSecAuthErrorWithTelemetry(task),
	...
	private async performRemoteConfigRefresh(isCurrent: () => boolean): Promise<boolean> {
		...
		// `.kerberosec/remote-config/`. Refresh the watcher so slash-command expansion
	 ...
	 * (`.kerberosecrules/workflows`, `.kerberosec/workflows`, …) and remote-config files
	 * materialized under `<root>/.kerberosec/remote-config/{workflows,skills,rules}`.
	 *
	 ...
	 * workspace (`~/.kerberosec/data/workspaces/chat`, seeded with an AGENTS.md
	...
	/**
	 * Subscribe to session events translated to KerberoSecMessages.
	...
	/**
	 * Check if the active API provider uses KerberoSec account auth for the current mode.
	 */
	private isKerberoSecManagedProviderActive(): boolean {
		return isKerberoSecManagedProvider(this.getActiveProviderId())
	}
	...
	private captureProviderFailure(event: ProviderFailureTelemetry): void {
		...
		const kerberosecError = KerberoSecError.transform(event.error, model, provider)

		telemetryService.captureProviderApiError({
			ulid,
			model,
			provider,
			errorMessage: kerberosecError.message || String(event.error),
			errorStatus: kerberosecError.status,
			requestId: kerberosecError.requestId,
	...
	private emitKerberoSecAuthErrorWithTelemetry(task?: string, sessionId?: string): void {
		this.emitKerberoSecAuthError(task)
		this.captureProviderFailure({
			sessionId: sessionId ?? this.task?.taskId,
			error: KERBEROSEC_ACCOUNT_AUTH_ERROR_MESSAGE,
	...
	/**
	 * Emit a proper auth error for the 'kerberosec' provider when the user is not
	 * logged in. The message sequence drives ErrorRow to render the
	 * "Sign in to KerberoSec" button.
	 *
	 * Message sequence:
	 *   1. say:'task'           – the user's message text
	 *   2. say:'api_req_started' – opens the API request row
	 *   3. ask:'api_req_failed'  – KerberoSecError JSON → ErrorRow renders auth UI
	 */
	private emitKerberoSecAuthError(task?: string): void {
		const ts = Date.now()
		this.pendingKerberoSecAuthRetryPrompt = task
		...
		const kerberosecError = new KerberoSecError(
			{ message: KERBEROSEC_ACCOUNT_AUTH_ERROR_MESSAGE, status: 401 },
			undefined, // modelId
			"kerberosec",
		)
		const serializedError = kerberosecError.serialize()
	 ...
	 * by reshaping it into KerberoSecError format and inspecting the result.
	 */
	private isKerberoSecBalanceError(errorMessage: string): boolean {
		try {
	...
	/**
	 * Emit a balance error for the 'kerberosec' provider when the user has insufficient
	 * credits. Produces the same message sequence as emitKerberoSecAuthError so the
	 * webview renders the "Buy Credits" button via CreditLimitError.
	 *
	 * Message sequence:
	 *   1. say:'api_req_started' – streamingFailedMessage holds the KerberoSecError JSON
	 *   2. ask:'api_req_failed'  – KerberoSecError JSON → ErrorRow renders balance UI
	 */
	private emitKerberoSecBalanceError(rawErrorMessage: string): void {
		const ts = Date.now()
		...
		// KerberoSecError-serialized JSON that the webview's ErrorRow expects.
	...
	async askResponse(prompt?: string, images?: string[], files?: string[]): Promise<void> {
		if (this.pendingKerberoSecAuthRetryPrompt !== undefined && this.task?.taskState?.askResponse === "yesButtonClicked") {
			const retryPrompt = this.pendingKerberoSecAuthRetryPrompt
			this.pendingKerberoSecAuthRetryPrompt = undefined
	...
	async editMessageAndRegenerate(input: {
		...
		const kerberosecMessages = currentTask.messageStateHandler.getKerberoSecMessages()
		const targetIndex = kerberosecMessages.findIndex((message) => message.ts === input.messageTs)
		if (targetIndex === -1) {
			throw new Error("Message to edit was not found")
		}
		const targetMessage = kerberosecMessages[targetIndex]
		if (targetMessage.type !== "say" || (targetMessage.say !== "task" && targetMessage.say !== "user_feedback")) {
			throw new Error("Only user messages can be edited")
		}

		const userOrdinal = kerberosecMessages
			.slice(0, targetIndex + 1)
			.filter((message) => message.type === "say" && (message.say === "task" || message.say === "user_feedback")).length
		const canRestoreWorkspace = getCheckpointRunCountForMessage(kerberosecMessages, targetIndex) !== undefined
		...
		try {
			...
			const historyTitle =
				userOrdinal === 1
					? editedText
					: extractSdkUserText(firstUserMessage ?? {}) || kerberosecMessages[0]?.text || editedText
			...
			if (usesKerberoSecAccountAuth(config.providerId) && !config.apiKey) {
				this.emitKerberoSecAuthErrorWithTelemetry(editedText)
				return
			}
			...
			const visibleMessages = kerberosecMessages.slice(0, targetIndex)
	...
	async restoreCheckpoint(input: { checkpointRunCount: number; restoreType: KerberoSecCheckpointRestore }): Promise<void> {
		...
		const currentMessages = currentTask.messageStateHandler.getKerberoSecMessages()
		...
		if (config && usesKerberoSecAccountAuth(config.providerId) && !config.apiKey) {
			this.emitKerberoSecAuthErrorWithTelemetry(restoredText)
			return
		}
	 ...
	 * receives a state with no currentTaskItem/kerberosecMessages and flashes back
	...
	async handleSignOut(): Promise<void> {
		...
		await this.taskControl.cancelKerberoSecTaskOnSignOut(isKerberoSecManagedProvider(sessionProviderId))
	...
	async handleAuthCallback(customToken: string, provider: string | null = null): Promise<void> {
		await this.authService.handleAuthCallback(customToken, provider ?? "kerberosec")
