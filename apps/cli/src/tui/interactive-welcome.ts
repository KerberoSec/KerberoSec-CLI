...
export async function resolveKerberoSecWelcomeLine(input: {
	config: Config;
	kerberosecApiBaseUrl?: string;
	kerberosecProviderSettings?: ProviderSettings;
}): Promise<string | undefined> {
	if (input.config.providerId !== "kerberosec") {
		return undefined;
	}
	try {
		const snapshot = await loadKerberoSecAccountSnapshot(input);
		const parts = [
			snapshot.user.email,
			`Credits: ${formatKerberoSecCredits(snapshot.displayedBalance)}`,
		];
