#!/usr/bin/env npx tsx
/**
 * kerberosec-bench Runner
 *
 * Runs real-world tasks from kerberosec-bench using Harbor framework.
 ...
 *
 * Usage:
 *   npx tsx evals/e2e/run-kerberosec-bench.ts [options]
 *
 * Options:
...
function getTaskList(kerberosecBenchDir: string, filter?: string): string[] {
	const tasksDir = path.join(kerberosecBenchDir, "tasks")
...
function runHarborTask(kerberosecBenchDir: string, taskId: string, options: RunOptions): TaskResult {
	...
	const harborArgs = ["run", "-p", `tasks/${taskId}`, "-a", "kerberosec-cli", "-m", harborModel, "--env", options.env]

	console.log(`  Running: harbor ${harborArgs.join(" ")}`)

	try {
		const result = spawnSync("harbor", harborArgs, {
			cwd: kerberosecBenchDir,
			env: harborEnv,
		...
		const jobsDir = path.join(kerberosecBenchDir, "jobs")
...
async function main() {
	...
	// Find kerberosec-bench directory
	const kerberosecBenchDir = path.join(__dirname, "..", "kerberosec-bench")
	if (!fs.existsSync(kerberosecBenchDir)) {
		console.error(`kerberosec-bench not found at: ${kerberosecBenchDir}`)
		console.error("Ensure the submodule is initialized: git submodule update --init")
		process.exit(1)
	}

	// Get task list
	const tasks = getTaskList(kerberosecBenchDir, options.tasks)
	if (tasks.length === 0) {
		console.error("No tasks found matching filter:", options.tasks)
		process.exit(1)
	}

	console.log(`kerberosec-bench E2E Runner`)
	console.log(`======================`)
	...
	for (const taskId of tasks) {
		...
		for (let trial = 0; trial < options.trials; trial++) {
			...
			const result = runHarborTask(kerberosecBenchDir, taskId, options)
			results.push(result)
