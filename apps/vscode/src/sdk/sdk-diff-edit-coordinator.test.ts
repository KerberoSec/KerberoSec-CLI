import type { AgentToolContext, ApplyPatchExecutor, EditFileInput, EditorExecutor } from "@kerberosec/core"
...
describe("computeNewEditorContent", () => {
	...
	// the executor still applies it (github.com/kerberosec/kerberosec/issues/13296).
...
describe("SdkDiffEditCoordinator", () => {
	...
	it("opens a populated preview before approval for an existing-file edit", async () => {
		...
		expect(previews[0].opened).toMatchObject({
			...
			title: "a.ts: Original ↔ KerberoSec's Changes (Preview)",
		})
	})
