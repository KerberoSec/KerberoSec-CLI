...
describe("SdkTaskStartCoordinator", () => {
	...
	it("emits a KerberoSec auth error instead of starting when the kerberosec provider has no token", async () => {
		const { coordinator, options } = makeCoordinator({ config: { providerId: "kerberosec", modelId: "model", apiKey: "" } })

		const sessionId = await coordinator.initTask("needs auth")

		expect(sessionId).toBeUndefined()
		expect(options.emitKerberoSecAuthError).toHaveBeenCalledWith("needs auth")
		expect(options.captureProviderApiError).not.toHaveBeenCalled()
		expect(options.sessions.startNewSession).not.toHaveBeenCalled()
	})

	it("emits a KerberoSec auth error instead of starting when KerberoSecPass has no token", async () => {
		const { coordinator, options } = makeCoordinator({ config: { providerId: "kerberosec-pass", modelId: "model", apiKey: "" } })

		const sessionId = await coordinator.initTask("needs kerberosecpass auth")

		expect(sessionId).toBeUndefined()
		expect(options.emitKerberoSecAuthError).toHaveBeenCalledWith("needs kerberosecpass auth")
		expect(options.captureProviderApiError).not.toHaveBeenCalled()
		expect(options.sessions.startNewSession).not.toHaveBeenCalled()
	})
	...
	it("emits a plain chat error when session start fails (e.g. provider misconfigured)", async () => {
		...
		expect(sessionId).toBeUndefined()
		expect(options.emitKerberoSecAuthError).not.toHaveBeenCalled()
	...
	it("emits KerberoSec auth errors when reinitialization fails due auth", async () => {
		const { coordinator, options } = makeCoordinator()
		options.sessionConfigBuilder.build.mockRejectedValue(new Error("missing api key"))
		options.isKerberoSecManagedProviderActive.mockReturnValue(true)

		await coordinator.reinitExistingTaskFromId("task-1")

		expect(options.emitKerberoSecAuthError).toHaveBeenCalledWith()
...
function makeCoordinator(input: Partial<MakeCoordinatorInput> = {}) {
	...
	const options = {
		...
		isKerberoSecManagedProviderActive: vi.fn(() => false),
		emitKerberoSecAuthError: vi.fn(),
		...
		isKerberoSecManagedProviderActive: ReturnType<typeof vi.fn>
		emitKerberoSecAuthError: ReturnType<typeof vi.fn>
