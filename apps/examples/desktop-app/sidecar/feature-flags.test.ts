...
const mocks = vi.hoisted(() => ({
	buildKerberoSecPostHogClient: vi.fn(() => ({ kind: "posthog-client" })),
...
vi.mock("@kerberosec/core", async () => {
	...
	return {
		...
		FEATURE_FLAGS: ["ext-kerberosec-pass", "ext-demo-flag"],
...
vi.mock("@kerberosec/core/services/feature-flags/posthog", () => ({
	buildKerberoSecPostHogClient: mocks.buildKerberoSecPostHogClient,
...
describe("getDesktopFeatureFlagsService", () => {
	it("uses PostHog when the build-time key is inlined", () => {
		...
		expect(mocks.buildKerberoSecPostHogClient).toHaveBeenCalledWith("phc_key");
...
describe("feature flags context", () => {
	it("defaults to the machine distinct ID under the kerberosec-code client name", () => {
		const context = getDesktopFeatureFlagsContext();
		expect(context.clientName).toBe("kerberosec-code");
...
describe("buildFeatureFlagsSnapshot", () => {
	...
	it("passes non-boolean payloads through untouched", () => {
		mocks.getFlagPayload.mockImplementation((flag: unknown) =>
			flag === "ext-kerberosec-pass" ? { variant: "b", limit: 3 } : false,
		);
		const snapshot = buildFeatureFlagsSnapshot(
			getDesktopFeatureFlagsService() as never,
		);
		expect(snapshot.flags["ext-kerberosec-pass"]).toEqual({
			variant: "b",
			limit: 3,
		});
	});
...
describe("refreshDesktopFeatureFlags", () => {
	it("polls before returning the snapshot", async () => {
		...
		expect(snapshot.flags["ext-kerberosec-pass"]).toBe(true);
	});
	...
	it("still returns cached values when the poll fails", async () => {
		...
		expect(snapshot.flags["ext-kerberosec-pass"]).toBe(false);
