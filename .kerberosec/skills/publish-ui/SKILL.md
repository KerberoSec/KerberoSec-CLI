---
name: publish-ui
description: Prepare, validate, and publish standalone @kerberosec/ui npm releases. Use when bumping the UI package version, publishing latest or next through ui-publish.yml, checking UI release readiness, or completing the one-time npm trusted-publishing bootstrap.
---

# Publish UI

Release `@kerberosec/ui` independently from the KerberoSec SDK runtime packages.
...
npm view @kerberosec/ui dist-tags versions --json
...
```sh
bun install --filter @kerberosec/ui --filter @kerberosec/code --frozen-lockfile
bun -F @kerberosec/ui typecheck
bun -F @kerberosec/ui test
bun -F @kerberosec/ui test:package
bun -F @kerberosec/ui build-storybook
bun -F @kerberosec/code test:chat-ui
```
...
Use this only while `npm view @kerberosec/ui` returns `E404`. npm requires the
...
1. Merge the package and `ui-publish.yml` to `main`. Start from a clean,
   ...
   access to the `@kerberosec` npm organization. The `npm trust` command in step 4
...
npm --version
npm whoami
npm view @kerberosec/ui version
```
...
```sh
npm trust github @kerberosec/ui \
