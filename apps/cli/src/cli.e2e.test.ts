...
describe("cli e2e", () => {
	...
	const createIsolatedEnv = (
		...
		return {
			...process.env,
			HOME: homeDir,
			KERBEROSEC_DIR: path.join(homeDir, ".kerberosec"),
			KERBEROSEC_DATA_DIR: dataDir,
			KERBEROSEC_DB_DATA_DIR: path.join(dataDir, "db"),
			KERBEROSEC_SESSION_DATA_DIR: sessionDir,
			KERBEROSEC_TEAM_DATA_DIR: teamDir,
			KERBEROSEC_SESSION_BACKEND_MODE: "local",
	...
	it("lists enabled workflows in text mode", () => {
		...
		const workflowsDir = path.join(workspace, ".kerberosecrules", "workflows");
...
Review checklist.`,
	...
	it("includes Documents/KerberoSec workflows", () => {
		const homeDir = mkdtempSync(path.join(os.tmpdir(), "cli-e2e-home-"));
...
Release from docs path.`,
	...
	it("lists enabled rules", () => {
		...
		tempDirs.push(workspace);
		const rulesDir = path.join(workspace, ".kerberosecrules");
...
Create a concise commit message.`,
	...
	it("includes Documents/KerberoSec rules and skills", () => {
		const homeDir = mkdtempSync(path.join(os.tmpdir(), "cli-e2e-home-"));
		const workspace = mkdtempSync(path.join(os.tmpdir(), "cli-e2e-workspace-"));
		tempDirs.push(homeDir, workspace);
		const docsRulesDir = path.join(homeDir, "Documents", "KerberoSec", "Rules");
...
Skill from docs path.`,
	...
	it("lists configured agents with source paths", () => {
		...
		const workspace = mkdtempSync(path.join(os.tmpdir(), "cli-e2e-workspace-"));
		tempDirs.push(homeDir, workspace);
		const globalAgentsDir = path.join(homeDir, ".kerberosec", "agents");
		const workspaceAgentsDir = path.join(workspace, ".kerberosec", "agents");
...
Break work into clear steps.`,
	...
	it("lists discovered plugins", () => {
		...
		const workspacePluginsDir = path.join(workspace, ".kerberosec", "plugins");
		const userPluginsDir = path.join(homeDir, ".kerberosec", "plugins");
		...
		expect(
			parsed.some((plugin) =>
				plugin.path.endsWith(
					path.join(".kerberosec", "plugins", "workspace-plugin.ts"),
				),
			),
		).toBe(true);
		expect(
			parsed.some((plugin) =>
				plugin.path.endsWith(
					path.join(".kerberosec", "plugins", "user-plugin.js"),
				),
			),
		).toBe(true);
		expect(
			parsed.some((plugin) =>
				plugin.path.endsWith(
					path.join("Documents", "KerberoSec", "Plugins", "docs-plugin.ts"),
				),
			),
		).toBe(true);
	});
	...
	it("lists configured mcp servers", () => {
		...
		const settingsPath = path.join(tempRoot, "kerberosec_mcp_settings.json");
		writeFileSync(
			settingsPath,
			JSON.stringify(
				{
		...
		const textResult = runCli(["config", "mcp"], {
			env: {
				...createIsolatedEnv(),
				KERBEROSEC_MCP_SETTINGS_PATH: settingsPath,
			},
		});
		expect(textResult.status).toBe(0);
	...
	it("routes mcp install and requires a TTY for the prefilled wizard", () => {
		...
		expect(result.status).toBe(1);
		expect(asText(result.stderr)).toContain(
			"kerberosec mcp install opens the MCP wizard and requires a TTY.",
		);
	});
	...
	it("lists available tools", () => {
		...
		const textResult = runCli(["config", "tools"], {
			...
			env: {
				...
				HOME: homeDir,
				KERBEROSEC_DATA_DIR: dataDir,
				KERBEROSEC_GLOBAL_SETTINGS_PATH: globalSettingsPath,
			},
		});
		expect(textResult.status).toBe(0);
	...
	it("accepts valid hook payloads and writes audit log", () => {
		...
		const result = runCli(["hook"], {
			env: {
				...
				HOME: homeDir,
				KERBEROSEC_SESSION_DATA_DIR: sessionDir,
				KERBEROSEC_HOOKS_LOG_PATH: hookPath,
			},
