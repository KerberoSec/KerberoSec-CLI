...
description: "Learn how to configure KerberoSec with various AI model providers that offer OpenAI-compatible APIs."
---

KerberoSec supports a wide range of AI model providers that offer APIs compatible with the OpenAI API standard. This allows you to use models from providers _other than_ OpenAI, while still utilizing a familiar API interface. This includes providers such as:
...
The key to using an OpenAI-compatible provider with KerberoSec is to configure these main settings:
...
You'll find these settings in the KerberoSec settings panel (click the ⚙️ icon):
...
### v0 (Vercel SDK) in KerberoSec:

-   For developers working with v0, their [AI SDK documentation](https://vercel.com/docs/v0/kerberosec) provides valuable insights and examples for integrating various models, many of which are OpenAI-compatible. This can be a helpful resource for understanding how to structure calls and manage configurations when using KerberoSec with services deployed on or integrated with Vercel.

-   v0 can be used in KerberoSec with the OpenAI Compatible provider.
...
By using an OpenAI-compatible provider, you can leverage the flexibility of KerberoSec with a wider array of AI models. Remember to always consult your provider's documentation for the most accurate and up-to-date information.
