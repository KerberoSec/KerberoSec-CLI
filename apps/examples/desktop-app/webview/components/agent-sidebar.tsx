...
export function AgentSidebar({
	...
	const accountName = username?.trim() || "KerberoSec Desktop";
			...
			setHubStatus({
				...
				error:
					error instanceof Error
						? error.message
						: "Unable to read KerberoSec Hub status.",
				url: null,
			});
		}
	}, []);
						...
						>
							<HoverCardTrigger asChild>
								<button
									...
									type="button"
								>
									<KerberoSecLogo className="size-5" />
								</button>
							</HoverCardTrigger>
							<HoverCardContent
								align="start"
								...
								<div className="mt-3 border-border border-t pt-3">
									<div className="flex items-center gap-2 text-xs">
										...
										<span className="font-medium">
											KerberoSec Hub @
											{hubPort(hubStatus?.url ?? null) ?? "unknown"}
										</span>
									</div>
									{hubStatus && !hubStatus.connected && (
										<p className="mt-1 text-[11px] text-destructive">
											{hubStatus.error ?? "KerberoSec Hub is not connected."}
										</p>
									)}
								</div>
							</HoverCardContent>
				...
				{!isCollapsed ? (
					...
					>
						...
						<Button
							...
							title="Customize KerberoSec with plugins, rules, and more"
							type="button"
							variant="sidebarItem"
						>
