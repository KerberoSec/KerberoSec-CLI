...
function say(ts: number, sayType: KerberoSecMessage["say"], partial?: boolean, text = ""): KerberoSecMessage {
...
function inputsFor(messages: KerberoSecMessage[], turnState: TurnState | undefined): ThinkingLoaderInputs {
	return {
		turnState,
...
describe("computeIsWaitingForResponse (legacy path)", () => {
	it("does not wait when the last raw message is an ask", () => {
		const ask: KerberoSecMessage = { ts: 1, type: "ask", ask: "followup", text: "?", partial: false }
...
describe("useThinkingLoaderRow anti-flash debounce", () => {
	...
	it("keeps an already-visible loader shown when a tool group tail finalizes", () => {
		const toolAsk = (partial: boolean): KerberoSecMessage => ({
			ts: 2,
			type: "ask",
			ask: "tool",
		...
		const toolGroupInputs = (tail: KerberoSecMessage): ThinkingLoaderInputs => {
			const group = Object.assign([tail], { _isToolGroup: true }) as KerberoSecMessage[]
			return {
				turnState: streaming(),
				lastRawMessage: tail,
