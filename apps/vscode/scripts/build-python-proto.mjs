 ...
 * Returns array of:
 * { serviceName: string, serviceKey: string, protoPackage: "kerberosec"|"host", moduleBase: string }
 */
async function parseServicesWithFiles(protoDir, protoFiles) {
	...
	for (const relPath of protoFiles) {
		...
		const pkg = relPath.startsWith("host/") ? "host" : "kerberosec"
...
async function generateKerberoSecClientPy(outDir, services) {
...
class KerberoSecClient:
    """
    Unified Python client analogous to src/generated/grpc-go/client/KerberoSecClient.

    Usage:
        client = KerberoSecClient("localhost:17611")
        client.connect()
...
`
	...
	await fs.writeFile(path.join(clientDir, "kerberosec_client.py"), content)
}
...
async function generatePythonClient(protoDir, pyOutDir, clientDir, protoFiles) {
	...
	// kerberosec_client.py (unified that composes service wrappers)
	await generateKerberoSecClientPy(clientDir, services)
}
...
[project]
name = "kerberosec-grpc-python"
version = "0.1.0"
description = "Generated Python gRPC stubs and client wrappers for KerberoSec protos"
