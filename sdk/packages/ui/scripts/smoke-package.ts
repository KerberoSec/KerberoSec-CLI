...
import { Conversation, Message } from "@kerberosec/ui/components/agent-chat";
import { ToolFileDiff } from "@kerberosec/ui/components/agent-chat/tool-diff";
import { buildToolSummary } from "@kerberosec/ui/components/agent-chat/tool-summary";

for (const specifier of [
	"@kerberosec/ui/components.css",
	"@kerberosec/ui/components/markdown.css",
	"@kerberosec/ui/theme/palette.css",
	"@kerberosec/ui/theme/scoped-tokens.css",
]) {
...
const packageJsonUrl = import.meta.resolve("@kerberosec/ui/package.json");
...
const css = import.meta.resolve("@kerberosec/ui/components/agent-chat.css");
const tokens = import.meta.resolve("@kerberosec/ui/theme/tokens.css");
...
function createConsumer(root: string): void {
	...
	writeFileSync(
		...
		`${JSON.stringify({ name: "kerberosec-ui-smoke", private: true, type: "module" }, null, 2)}\n`,
	);
}
...
async function verifyTailwindContract(
	...
	for (const candidate of [
		"bg-kerberosec-ui-background/95",
		"border-kerberosec-ui-border/60",
		"text-kerberosec-ui-muted-foreground",
		"bg-kerberosec-ui-primary/10",
		"max-h-64",
		"leading-none",
		"max-h-44",
		"not-last:border-b",
...
const temporaryRoot = mkdtempSync(join(tmpdir(), "kerberosec-ui-package-"));
