...
import type { KerberoSecCoreListHistoryOptions, SessionHistoryRecord } from "@kerberosec/core"
import type { MessageWithMetadata as SdkMessage } from "@kerberosec/llms"
import { formatDisplayUserInput, parseUserInputMode } from "@kerberosec/shared"
import { resolveSessionDataDir } from "@kerberosec/shared/storage"
...
export interface SdkTaskHistoryOptions {
	 ...
	 * state/taskHistory.json and tasks/<id>/ instead of ~/.kerberosec/data.
	 */
...
type SdkTaskHistoryListOptions = KerberoSecCoreListHistoryOptions & {
	offset?: number
}
...
export class SdkTaskHistory {
	...
	async listHistory(options: SdkTaskHistoryListOptions = {}): Promise<SessionHistoryRecord[]> {
		...
		const hostOptions: KerberoSecCoreListHistoryOptions = { ...options }
	...
	async getKerberoSecMessages(taskId: string): Promise<KerberoSecMessage[]> {
		...
		const kerberosecMessages = sdkMessagesToKerberoSecMessages(
		...
		if (sdkRecord && legacyTask) {
			return mergeLegacyUiMessagesWithResumedSdkMessages(readUiMessages(taskId, legacyTask.dataDir), kerberosecMessages)
		}
