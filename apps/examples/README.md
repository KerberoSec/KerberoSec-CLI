...
Learn how to build with the KerberoSec SDK through working examples, ordered from simple to complex.
...
bun install
bun run build:sdk
export KERBEROSEC_API_KEY="kerberosec_..."
bun dev
```
...
| [kerberosec-core-cli-agent](./kerberosec-core-cli-agent) | Interactive terminal chat powered by KerberoSecCore. | `KerberoSecCore.create()`, `kerberosec.start()`, `kerberosec.send()`, built-in tools, streaming |
...
```bash
npm add @kerberosec/sdk
```

`@kerberosec/sdk` re-exports everything from `@kerberosec/core`. You only need `@kerberosec/agents` or `@kerberosec/llms` if you want lower-level control over the agent runtime or model gateway directly.

## Learn more

- [SDK package docs](../../sdk/packages/README.md)
- [Architecture guide](../../ARCHITECTURE.md)
- [Plugin examples](../../examples/plugins) - extend the KerberoSec SDK and CLI with custom tools and event hooks
- [Hook examples](../../examples/hooks) - lifecycle hooks for logging, blocking, and injection for KerberoSec SDK and CLI
