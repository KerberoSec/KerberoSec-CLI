...
- Provider model lists now carry featured tiers (Recommended/Free, and Subscribed/Free for KerberoSecPass) plus model descriptions, served from the SDK with a cached recommended-models feed and a bundled offline fallback, so clients no longer join the feed themselves
...
- AI SDK telemetry spans now carry user, session, conversation, run, provider, and model context under a stable `kerberosec-sdk` service name
...
- Added a dedicated KerberoSec provider for the KerberoSec gateway, replacing the generic OpenAI-compatible path. Extended thinking budgets and other gateway options now reach the wire for both `kerberosec` and `kerberosec-pass`, which had silently stopped applying to `kerberosec-pass`
- Fixed two KerberoSec installations on different builds shutting each other's Hub daemon down in a loop, killing every live session with an abnormal socket close. Build identity is now compared through a total order, so at most one side of a pair can ever decide to retire the other
...
- `kerberosec doctor fix` now reports honestly: processes that survived a kill are separated from ones that appeared while the fix ran, a live parent respawning a daemon is named, and a startup lock held by a running process is reported as held rather than leaked
...
- Involuntary KerberoSec logouts (a rejected refresh token) are now reported instead of credentials being cleared silently; a transient network failure refreshing the stored session on startup no longer books as a logout
...
- Refreshed the model catalog from models.dev, and surfaced `meta/muse-spark-1.2-contributor` for the KerberoSec provider
...
- Checkpoint restore is now a true workspace rewind: files KerberoSec created during a task are captured in the snapshot and restored to their checkpoint-time content, and files created after the checkpoint are removed. `.gitignore`d paths (build output, `node_modules`, `.env`) are left untouched, and a pre-restore recovery snapshot can roll the whole operation back. Checkpoints taken before this change keep the old conservative behavior of never touching untracked files
- Migrated users whose stored KerberoSec model id isn't in the runtime catalog now fall back to the default KerberoSec model instead of carrying an unknown id into every inference request
...
- `fetchKerberoSecRecommendedModels` now returns display-ready model names, resolved through the model catalog (including Vercel and OpenRouter id aliases) under a single shared timeout budget, so hosts no longer have to map ids to names themselves
- KerberoSec free models now resolve their OpenRouter display names in the catalog
...
- The per-server `timeout` in `kerberosec_mcp_settings.json` is now honored by the SDK's MCP clients for `initialize`, `tools/list`, and `tools/call` instead of hardcoded 1.5s and 5s limits — it defaults to 60 seconds and is clamped to 1–3600 seconds
...
- Added `KerberoSecCore.readLiveMessages` for reading a resident session's in-memory transcript, so a plan/act rebuild during an in-flight turn no longer starts from an empty history
...
- A legacy single-file `.kerberosecrules` no longer aborts the config scan
- Telemetry events now carry `device_id`
- A malformed OTEL header entry no longer discards the valid ones

## 0.0.66

- Support for free KerberoSec models (`kerberosec-free`): free models are labeled "(free)", priced at zero, and hitting the free tier now raises a dedicated limit error that includes the reset time
...
- `@kerberosec/ui`: host-safe theme contract and Markdown exports
...
- Added Kimi K3 to the bundled KerberoSecPass model fallback
...
- New `@kerberosec/ui` agent chat components with Storybook and npm packaging
- Updated the bundled model catalog
...
- Added a shared `@kerberosec/ui` theme package
...
- Added the KerberoSecPass usage-limit error so limit-reached responses are surfaced clearly
...
- Added telemetry for auth-refresh outcomes and KerberoSec credential lifecycle debug logging

## 0.0.59

- You can now select KerberoSec free models on the KerberoSecPass provider
- The SDK now recognizes KerberoSecPass rate-limit responses and surfaces them as a typed `KerberoSecPassLimitError` (with `isKerberoSecPassLimitMessage` / `extractKerberoSecPassLimitMessage` helpers)
- Removed references to the retired KerberoSecPass GLM 5.1 model
...
- KerberoSec provider requests now send versioned KerberoSec client-identity headers
...
- Added a shared prompt-format helper in `@kerberosec/shared` and simplified runtime host support
...
- Add KerberoSecPass subscription support to the account service

## 0.0.53

- Show when request cost is covered by the user's KerberoSec subscription
- List KerberoSecPass features in the not-subscribed message
...
- Updated the KerberoSecPass model list live, restored KerberoSecPass models in onboarding, fixed KerberoSecPass error mapping, and scoped the KerberoSecPass URL to the CLI
...
- Fixed Z.ai model metadata not resolving correctly when using Z.ai models through the KerberoSec provider; aliases now map to the right model metadata and user overrides are preserved

## 0.0.50

- Truncate every tool result by default (including MCP and custom tool output), with tightened `MessageBuilder` limits and tunable `KERBEROSEC_MESSAGE_BUILDER_*` env overrides, to keep provider requests within budget
...
- Add KerberoSecPass provider support (built-in provider, error handling, format compatibility)
...
- Reverted KerberoSecPass recommended-models support, removing the `kerberosecPass` field from the recommended models data

## 0.0.48

- Added KerberoSecPass support and KerberoSecPass models
...
- Enforced a production singleton KerberoSec Hub so only one hub daemon runs, and a stale hub is respawned after an upgrade
...
- Added the KerberoSec Hub web app for managing and monitoring agent sessions
...
- Added a KERBEROSEC_PLUGIN_IMPORT_TIMEOUT_MS environment override for plugin import timeouts
...
- Improved KerberoSec provider migration
