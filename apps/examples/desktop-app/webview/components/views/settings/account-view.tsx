...
function normalizeAccountViewError(error: unknown): Error {
	...
	if (message.includes("unsupported desktop command: kerberosec_account")) {
		return new Error(
...
function isAccountAuthError(message: string): boolean {
	...
	return (
		normalized.includes("no kerberosec account auth token found") ||
...
async function fetchAccountUser(): Promise<KerberoSecAccountUser> {
	return await desktopClient.invoke<KerberoSecAccountUser>(
		"kerberosec_account",
		{
...
async function fetchAccountBalance(): Promise<KerberoSecAccountBalance> {
	return await desktopClient.invoke<KerberoSecAccountBalance>(
		"kerberosec_account",
		{
			action: "kerberosecAccount",
...
async function fetchAccountOrganizations(): Promise<
	KerberoSecAccountOrganization[]
> {
	return await desktopClient.invoke<KerberoSecAccountOrganization[]>(
		"kerberosec_account",
		{
			action: "kerberosecAccount",
...
async function fetchOrganizationBalance(
	...
	return await desktopClient.invoke<KerberoSecAccountOrganizationBalance>(
		"kerberosec_account",
		{
			action: "kerberosecAccount",
...
async function fetchUsageTransactions(): Promise<
	KerberoSecAccountUsageTransaction[]
> {
	return await desktopClient.invoke<KerberoSecAccountUsageTransaction[]>(
		"kerberosec_account",
		{
			action: "kerberosecAccount",
...
async function fetchPaymentTransactions(): Promise<
	KerberoSecAccountPaymentTransaction[]
> {
	return await desktopClient.invoke<KerberoSecAccountPaymentTransaction[]>(
		"kerberosec_account",
		{
			action: "kerberosecAccount",
...
async function switchActiveAccount(
	...
	await desktopClient.invoke("kerberosec_account", {
		action: "kerberosecAccount",
...
export function AccountView() {
	...
	// Overview data
	const [user, setUser] = useState<KerberoSecAccountUser | null>(null);
	const [balance, setBalance] = useState<KerberoSecAccountBalance | null>(null);
	const [organizationBalance, setOrganizationBalance] =
		useState<KerberoSecAccountOrganizationBalance | null>(null);
	...
	const loadOverview = useCallback(async () => {
		...
		try {
			...
			if (isKerberoSecAccountNotAuthenticatedResult(userData)) {
				resetAccountData();
				setSignedOut(true);
				return;
			}
			...
			if (
				isKerberoSecAccountNotAuthenticatedResult(balanceData) ||
				isKerberoSecAccountNotAuthenticatedResult(orgsData)
			) {
				resetAccountData();
				setSignedOut(true);
				return;
			...
			const organizationBalanceData = nextActiveOrganization
				...
				: null;
			if (isKerberoSecAccountNotAuthenticatedResult(organizationBalanceData)) {
				resetAccountData();
				setSignedOut(true);
				return;
			}
	...
	const loadUsage = useCallback(async () => {
		...
		try {
			...
			if (isKerberoSecAccountNotAuthenticatedResult(data)) {
				resetAccountData();
				setSignedOut(true);
				setActiveTab("overview");
				return;
			...
			<div className="mx-auto flex max-w-xl flex-col items-center gap-4 py-8 text-center">
				...
				<div>
					...
					<p className="mt-2 text-sm text-muted-foreground">
						Connect your KerberoSec account to review credits, usage, billing,
