...
import { formatDisplayUserInput, truncateStr } from "@kerberosec/shared";
...
export function formatCheckpointDetail(
	...
	const latestRun =
		typeof latest.runCount === "number" && Number.isFinite(latest.runCount)
			? ` run ${latest.runCount}`
			: "";
	return `Checkpoint ${shortRef}${latestRun} created ${created}. ${count} total. Restore with: kerberosec checkpoint restore latest --session-id ${row.sessionId}`;
}
